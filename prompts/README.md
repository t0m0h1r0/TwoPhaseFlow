...
# Prompt System Architecture — v8.0.0-candidate "Lean Kernel + Skills"
# Generated by EnvMetaBootstrapper v8.0.0-candidate | Target: Codex | Date: 2026-05-03
...
```
prompts/meta/
  kernel-constitution.md  — φ1–φ7, A1–A11, LA-1–LA-6, <immutable_zone> axioms
  ...
  kernel-ops.md           — Canonical ops + HAND/GIT/LOCK/OP-CONDENSE/METRIC/TOOL-TRUST shorthand
  kernel-workflow.md      — P-E-V-A loop, DYNAMIC-REPLANNING, PROTO-DEBATE
  kernel-deploy.md        — EnvMetaBootstrapper v8.0.0-candidate generation spec
  kernel-project.md       — Project-specific rules (PR-1–PR-6) [swap to retarget]
  kernel-antipatterns.md  — AP-01–AP-15 compact (detect/mitigate/severity/inject)
```
...
prompts/skills/           ← JIT Skill Capsules loaded by trigger, not embedded

  _base.yaml               — Universal agent foundation (v8.0.0-candidate feature flags)

  # Management (M-Domain)
  ResearchArchitect.md      — Protocol Enforcer / Router / OP-CONDENSE / effort policy
  ...
  EvidenceAnalyst.md         — Result Evidence Analyst

  # Paper (A-Domain)
  PaperWorkflowCoordinator.md — Paper Pipeline Orchestrator
  PaperWriter.md              — Academic Editor
  PresentationWriter.md       — Presentation Deck Specialist
  ...
  PromptAuditor.md           — Q3 13-item + telemetry + Skill Capsule Auditor
...
| AP-13 | Rule Bloat Regression | HIGH | PromptArchitect, PromptAuditor, ResearchArchitect |
| AP-14 | Delegation Overhead | HIGH | ResearchArchitect, TaskPlanner, CodeWorkflowCoordinator |
| AP-15 | Tool Trust Confusion | CRITICAL | tool-using agents |
...
Shows relationships and task handoffs across all 25 agents and 7 domains.
...
flowchart TD
    ...
    ER -->|"raw output"| SA
    SA -->|"HAND-02"| CWC
    SA -->|"analysis package"| EA
    EA -->|"HAND-02: evidence note"| CWC
    ...
    class TP,DA,DIA,THA,CA,CC,TR,ER,SA,EA,PW,PRES,PC,PR,KA,LIB,TM specialist
    class CSA auditNode
    class USER userNode
```
...
| E | EvidenceAnalyst | 2 | Result Evidence Analyst |
| A | PaperWorkflowCoordinator | 3 | Paper Pipeline Orchestrator |
| A | PaperWriter | 2 | Academic Editor |
| A | PresentationWriter | 2 | Presentation Deck Specialist |
...
- **To sync upstream kernel:** `make sync-research-agent` (default remote: `git@github.com:t0m0h1r0/research-agent.git`; preserves `prompts/meta/kernel-project.md`)
