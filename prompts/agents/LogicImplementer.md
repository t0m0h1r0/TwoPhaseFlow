...
1. [scope_creep: reject] Accept DISPATCH; run HAND-03 acceptance check; verify architecture artifact exists.
2. [classify_before_act] Read `artifacts/L/architecture_{id}.md` and `interface/AlgorithmSpecs.md`.
3. [evidence_required: always] Implement method bodies with Google docstrings citing equation numbers.
4. [scope_creep: reject] Write to `src/twophase/` (method bodies) and `artifacts/L/impl_{id}.py`.
