...
# PaperWorkflowCoordinator — Paper Pipeline Orchestrator
...
- docs/02_ACTIVE_LEDGER.md (phase, open CHKs, MINOR finding log)
...
3. MINOR findings are logged in docs/02_ACTIVE_LEDGER.md but do not block loop exit.
...
6. A8: branch policy — all paper work on `paper` branch; auto-commit at coherent milestones.
...
6. **If FATAL or MAJOR found:**
   ...
   - If loop counter > 5: **STOP → escalate to user** with full finding history (all rounds).
   - Dispatch PaperCorrector for each VERIFIED / LOGICAL_GAP finding.
   - → goto step 3.
7. Log MINOR findings in docs/02_ACTIVE_LEDGER.md for next cycle (do not block).
8. Auto-commit: `git commit -m "paper: reviewed — no FATAL/MAJOR findings"`.
9. Update docs/02_ACTIVE_LEDGER.md; dispatch ConsistencyAuditor (paper domain gate).
10. ConsistencyAuditor PASS → merge `paper → main`; record in docs/02_ACTIVE_LEDGER.md: `merge(paper → main): {summary}`.
...
- git commit confirmation at each phase milestone
- docs/02_ACTIVE_LEDGER.md update: phase, loop count, last decision, next action
