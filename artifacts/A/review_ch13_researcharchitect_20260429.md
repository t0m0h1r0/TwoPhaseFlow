...
- **根拠**: V10 再実行 (本査読 P3, 2026-04-29) — Zalesak α=1, N=128 → drift = **1.535%**;
  Zalesak α=2, N=128 → drift = **0.030%**．独立計算で 13e Tab.V10_zalesak と完全一致．
...
- **根拠**: V10 再実行 (本査読 P3, 2026-04-29) — Single vortex N=96, α=2 →
  volume_drift = **0.979%** ($V_T/V_0=0.9902$, L¹ reversal $2.42\times 10^{-3}$)．
  独立計算で 13e Tab.V10_single_vortex と完全一致．`area_drift = 0.0` (psi 領域面積) と
  `volume_drift = 0.979%` (psi 体積) は異なる指標であり，13e の `mass drift` 列は後者．
  13f master table が `volume drift = 0.000%` と記載するのは前者を流用したことに起因し，
  サブファイルの「mass drift」と整合させると誤値．
...
- V10 (`make cycle EXP=experiment/ch13/exp_V10_cls_advection_nonuniform.py`) — 完了．
  Zalesak: α=1, N=128 volume_drift = **1.535%** ($V_T/V_0=1.0153$); α=2, N=128 = **0.030%** ($V_T/V_0=1.0003$)．
  Single vortex: N=96, α=2 volume_drift = **0.979%** ($V_T/V_0=0.9902$), L¹ reversal = $2.42\times10^{-3}$．
  → 13e Tab.V10_zalesak / Tab.V10_single_vortex と全数一致．
  13f master table の `0.107%`/`1.519%`/`0.000%` はすべて誤り．
