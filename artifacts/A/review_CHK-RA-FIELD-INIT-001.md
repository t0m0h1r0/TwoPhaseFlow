...
- `bubble` is a semantic alias for a gas-filled circle, so common rising-bubble and multi-bubble YAMLs do not need to repeat `interior_phase: gas`.
...
- Remote full initial-condition pytest PASS: `twophase/tests/test_initial_conditions.py` (`41 passed`).
