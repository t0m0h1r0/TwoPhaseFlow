...
def test_same_operator_defect_correction_collapses_to_one_tight_solve():
    grid = object()
    ...
    assert base.seen_tolerances == [1.0e-8]
    assert base.tol == 1.0e-6
    assert operator.apply_calls == 0
    assert solver.last_residual_history == []
    assert solver.last_stalled is False
    assert solver.last_diagnostics["ppe_interface_coupling_affine_jump"] == 1.0
    np.testing.assert_allclose(pressure.ravel()[0], 0.0)
