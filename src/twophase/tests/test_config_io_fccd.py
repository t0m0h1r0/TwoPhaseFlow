...
def test_ch14_capillary_yaml_loads_execution_stack():
    ...
    assert cfg.run.T_final == pytest.approx(0.008899695230)
    assert cfg.run.snap_times == pytest.approx(
        (0.0, 0.002224923808, 0.004449847615, 0.006674771423, 0.008899695230)
    )
...
def test_ch14_canonical_yamls_share_base_numerical_stack():
    ...
    for path in sorted(p for p in config_dir.glob("*.yaml") if not p.name.startswith("_")):
        ...
        assert cfg.run.capillary_force_source == "bundle_virtual_work", path.name
