...
def test_ao_face_bridge_converts_integrated_face_cochain_on_nonuniform_grid():
    ...
    x_velocity = 2.0
    y_velocity = -3.0
    x_acceleration_cochain = x_velocity * dy.reshape((1, 3)) * np.ones((3, 3))
    y_acceleration_cochain = y_velocity * dx.reshape((2, 1)) * np.ones((2, 4))

    projected = _geometric_to_projection_face_pair_2d(
        xp=np,
        grid=grid,
        face_pair=[x_acceleration_cochain, y_acceleration_cochain],
...
def test_ao_face_bridge_uses_periodic_seam_metric_weights():
    ...
    y_acceleration = y_velocity * dx.reshape((2, 1))
