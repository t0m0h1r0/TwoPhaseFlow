...
def test_second_derivative_matches_differentiate_uniform(backend):
    """d2-only path must match the full CCD differentiate() result."""
    N = 32
    grid = make_grid_2d(N, backend)
    ccd = CCDSolver(grid, backend)
    x = np.linspace(0.0, 1.0, N + 1)
    ...
    _, d2_full = ccd.differentiate(f, axis=0)
    d2_only = ccd.second_derivative(f, axis=0)

    assert np.allclose(np.asarray(d2_only), np.asarray(d2_full))
...
def test_second_derivative_matches_differentiate_nonuniform(backend):
    """d2-only path must stay consistent after non-uniform metric mapping."""
    grid = _make_nonuniform_grid(32, 2.0, backend)
    ccd = CCDSolver(grid, backend, bc_type="wall")
    X, Y = np.meshgrid(grid.coords[0], grid.coords[1], indexing="ij")
    f = np.sin(np.pi * X) * np.cos(np.pi * Y)
