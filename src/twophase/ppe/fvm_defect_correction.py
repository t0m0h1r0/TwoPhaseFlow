"""Defect-correction wrapper for the FVM pressure Poisson equation."""
...
class PPESolverFVMDefectCorrection(IPPESolver):
    """Outer residual-correction loop using an explicit base PPE solver.

    The target operator is the same matrix-free FVM operator used by the
    projection.  ``base_solver`` supplies each inner correction solve, making
    the dependency explicit in configuration instead of hiding it in the class.
    """
    ...
    def __init__(
        ...
        operator: PPESolverFVMMatrixFree,
        *,
