...
係数を\textbf{中心対称}（$\alpha_1^+ = \alpha_1^-$：対称；$b_2^+ = -b_2^-$：反対称）
...
これにより，界面近傍では CCD 微分の設計精度を保護しつつ，
バルク領域では $\Ord{\varepsilon_d h^2}$ の制御された追加誤差と引き換えに
高周波応答を抑える．
フィルタ適用後の実効精度は §~\ref{sec:dccd_accuracy_impact} で明示的に評価する．
...
\medskip
\noindent\textbf{CLS 界面指示子との接続．}

第~\ref{sec:levelset}章（式~\eqref{eq:delta_eps_def}，§~\ref{sec:levelset_mapping}）で導出したように，
CLS 変数 $\psi = H_\varepsilon(\phi)$ の微分は正則化デルタ関数 $\delta_\varepsilon = \psi(1-\psi)/\varepsilon$ を与える．
その正規化版
\begin{equation}
  \Theta(\psi) \;\equiv\; 4\psi(1-\psi) \;\in\; [0, 1]
  \label{eq:interface_indicator}
\end{equation}
は界面（$\psi = 1/2$）でピーク値 1，バルク（$\psi \to 0$ または $1$）でゼロをとる
\textbf{界面指示子}である（導出詳細は §~\ref{sec:levelset_mapping} 参照）．

\medskip
\noindent\textbf{スイッチ関数の定義．}

フィルタは界面指示子の\textbf{補集合}（界面 $\to 0$，バルク $\to 1$）として定義する．
代数的に $1 - \Theta(\psi) = 1 - 4\psi(1-\psi) = (1-2\psi)^2 = (2\psi-1)^2$ であるから，
...
\end{equation}
あるいは，$\phi$ の観点から（式~\eqref{eq:logit_inverse} を代入して整理）：
\begin{equation}
...
散逸強度 $\varepsilon_{d,\max}$ の具体的な設計値（スペクトル精度要求と高周波抑制要求の同時達成）
および CLS 変数 $\psi$ に基づく点別適応制御アルゴリズムは，
アルゴリズム統合の文脈で §~\ref{sec:dccd_params} にて詳述する．

% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{設計の一貫性：CCD の \texorpdfstring{$\Ord{h^6}$}{O(h\textasciicircum 6)} 精度に対する影響の定量化}
...
\begin{equation}
  ...
  \approx \varepsilon_d^{(i)} \cdot h^2 \cdot (f')''_i + \Ord{h^4}
  = \varepsilon_d^{(i)}\,h^2\,f'''_i + \Ord{h^4}
...
$\psi$ の更新（CLS 移流・再初期化ステップ，§~\ref{sec:algorithm}）が
...
圧力場へのフィルタ適用の可否については，Balanced-Force 離散化の文脈で
§~\ref{sec:dccd_pressure_nofilt} にて論じる．

\bigskip\noindent
以上で，CCD の定式化（§~\ref{sec:CCD}）と散逸的フィルタ（DCCD，本節）が確立された．
次章では，CCD 空間演算子を時間軸上で整合的に前進させる時間積分フレームワークを設計する．
