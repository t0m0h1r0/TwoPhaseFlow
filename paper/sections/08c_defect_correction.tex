...
§~\ref{sec:ccd_poisson_matrix}（将来の分相 PPE 解法向け）の CCD 演算子 $L_H$ を
効率的に求解する枠組みを示す．変密度界面での DC 収束条件の詳細は
§~\ref{sec:ppe_discretization_choice} を参照；
現行の一括解法では反復なし（$k=1$，FD 直接法）を採用する．
...
\textbf{欠陥補正法（CCD-PPE 向け，将来実装）} & $\boldsymbol{\Ord{h^6}}$ & \textbf{不要} & \textbf{—} & $\boldsymbol{\mathcal{O}(N^2)}\times k$\textbf{反復} \\
\bottomrule
\end{tabular}
