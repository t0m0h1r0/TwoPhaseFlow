...
\begin{align}
  ...
  \text{$y$ スウィープ：}\quad
    ...
    \label{eq:dc_sweep_y}
\end{align}
この2段スイープは近似分解（ADI 型分裂）であり，分裂誤差 $\Ord{\Delta\tau^2}\,L_{L,x}L_{L,y}$ が生じる．
この誤差は外側の欠陥修正反復により各スイープで吸収されるため，収束解には影響しない．
...
\noindent\textbf{注（緩和係数 $\omega$ と仮想時間刻み $\Delta\tau$ の等価性）：}%
\label{sec:dc_terminology}%
式~\eqref{eq:dc_three_step} の $\omega$ と式~\eqref{eq:dc_sweep_x} の $\Delta\tau$ は
$[\frac{1}{\Delta\tau}\mathbf{I} - L_L]\,\delta p = d$ を整理すると
$\delta p = \Delta\tau[\mathbf{I} - \Delta\tau L_L]^{-1}d$，すなわち $\omega = \Delta\tau$ の関係にある
（厳密な導出は付録~\ref{app:dc_convergence_theory}）．
ここで $\omega$ および $\Delta\tau$ はいずれも無次元化された量であり，次元的整合性は保たれている．
...
LGMRES + CCD Kronecker & $\Ord{h^6}$ & 必要 & $\mathcal{O}(N^2)$ & $\mathcal{O}(N^2)\times k$反復 \\
...
\smallskip
\noindent 注：$k$ = 収束までの反復回数（実測で $k=3$--$5$，§~\ref{sec:dc_accuracy}）．
直接 LU の行列記憶量は2次元スパース LU 因子の fill-in による（$\mathcal{O}(N^{2.5})$）．
欠陥補正法は行列構築・保持が不要（matrix-free）であり，
