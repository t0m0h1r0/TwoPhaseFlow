...
\paragraph{C/RC 補正の効果}

§~\ref{sec:rc_high_order} の C/RC（CCD-enhanced Rhie--Chow）補正を
多ステップ静止液滴（400 ステップ，$\rho_l/\rho_g = 2$，$\mathit{We} = 10$，
Rhie--Chow Balanced-Force 拡張有効）に適用した結果を示す．

\begin{table}[htbp]
\centering
\caption{C/RC 補正の寄生流れへの効果（400 ステップ後）}
\label{tab:rc_crc_droplet}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{clccr}
\toprule
...
32 & C/RC $\Ord{h^4}$         & $2.49 \times 10^{-2}$ & $0.42\%$ & $\mathbf{-0.8\%}$ \\
\midrule
64 & 標準 $\Ord{h^2}$       & $5.49 \times 10^{-4}$ & $1.23\%$ & --- \\
64 & C/RC $\Ord{h^4}$         & $4.24 \times 10^{-4}$ & $1.23\%$ & $\mathbf{-23\%}$ \\
\bottomrule
\end{tabular}
\end{table}

$N = 64$ では C/RC 補正により寄生流れ振幅が 23\% 低減し，
$\Delta p$ 精度は同等に維持される．
