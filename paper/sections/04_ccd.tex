...
\begin{itemize}
  ...
  \item \textbf{Equation-II（$\fpp{0}$）}：$\Ord{h^2}$ 精度の片側スキーム（式~\eqref{eq:bcII_left}）．
        全体 L$^2$ 誤差への影響は境界2点分（$2/N$ の重み）のみであり，$\Ord{h^3}$ に抑えられる（式~\eqref{eq:bcII_left}の精度検証参照）．
\end{itemize}
...
$\mathbf{C}_{N-1}$ は右境界行（$i=N-1$）の左結合ブロック（$\mathbf{A}_R$ の境界修正版）である．

\begin{tcolorbox}[defbox, title={内点ブロック行列の数値係数（係数 \eqref{eq:coef_CCD} 代入後）}]
\label{defbox:ccd_numeric_blocks}
式 \eqref{eq:coef_CCD} の値 $\alpha_1=7/16,\;b_1=1/16,\;\beta_2=-1/8,\;a_2=3,\;b_2=-9/8$ を代入すると：
\[
  \mathbf{A}_L =
  \begin{bmatrix}
