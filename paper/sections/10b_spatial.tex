...
$N = 16$--$256$ で格子を精緻化した結果を表~\ref{tab:ccd_periodic} に示す．
1 階微分の $L_2$ 誤差は $N = 16 \to 128$ の範囲で $\Ord{h^6}$ の収束を示した
...
伝達関数は $e^{i\xi x}$ に対する CCD の応答比として定義し，
...
Nyquist モード（チェッカーボード振動）が完全に抑制される
（§\ref{sec:checkerboard_solution} の理論と一致）．
$\varepsilon_d = 0.05$（移流ステップ用）では $H(\pi) = 0.80$ であり，
高波数成分を 20\% 減衰させつつ低波数の移流精度を保持する．
$N = 128$ 格子上でチェッカーボードモード $(-1)^{i+j}$ に
$\varepsilon_d = 0.25$ を適用し，フィルタ後の振幅がゼロとなることを確認した．
...
経路 C は全格子で収束しなかった（表~\ref{tab:curvature_sinusoidal}）．

円形界面（$R = 0.25$，$\kappa_\mathrm{exact} = 4$）では，
経路 B の曲率収束は CCD・CD2 ともに $\Ord{h^1}$ にとどまり，
ロジット逆変換の界面厚み $\varepsilon = \Ord{h}$ が精度の上界を規定した．
