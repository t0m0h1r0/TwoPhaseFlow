...
%
% §7.5  CLS の A→F 6 段階アルゴリズム（SP-L §5–§6）
%        §7.1 (CLS 移流) / §7.4 (再初期化) を統合し,
%        1 物理時間ステップ内での順序を完結させる．
%
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{CLS の A$\to$F 6 段階アルゴリズム}
\label{sec:cls_stages}
% ═══════════════════════════════════════════════════════════════════════════════

SP-L \cite{SPL} §5--§6 は，CLS \cite{OlssonKreiss2005} を 1 物理時間ステップ内の
6 段階に分解し，
...
% -------------------------------------------------------------------------------
\subsubsection{3 責務原則と $\psi\leftrightarrow\phi$ 対応}
...
Stage & 操作 & 要件 \\
\hline
A & TVD-RK3 保存形移流 & WENO5 面フラックス，発散フリー $\mathbf{u}^n$ \\
...
\end{align}
$\mathcal{L}\psi = -\nabla\cdot(\mathbf{u}\psi)$ は WENO5 面フラックス．
CCD は $\psi$ に適用禁止（\S\ref{sec:scheme_per_variable}）．
...
% -------------------------------------------------------------------------------
\subsubsection{$\psi(1-\psi)$ 質量補正（界面局在）}
...
\begin{itemize}
  \item \textbf{$\psi$ に CCD / FCCD を適用}→急 tanh 勾配増幅→NaN．
        必ず WENO5 面フラックス．
...
\noindent検証：バルクの $\psi\in\{0,1\}$ 位置で
Stage F 補正が恒等的にゼロになることを毎ステップ監視
（CHK-160 回帰テスト）．
