...
%
% §6.5  CLS の A→F 6 段階アルゴリズム
%        §6.1 (CLS 移流) / §6.4 (再初期化) を統合し,
...
ch13 production stack では，Stage A の FCCD ψ 移流（\texttt{FCCDLevelSetAdvection}；
§~\ref{sec:advection_motivation}）と Stage C の Ridge-Eikonal 再初期化
（\texttt{RidgeEikonalReinitializer}；§~\ref{sec:xi_sdf_reinit}）が主経路となる．
...
\end{align}
$\mathcal{L}\psi = -\nabla\cdot(\mathbf{u}\psi)$ は FCCD 面値演算子による保存形発散である：
セル境界 $x_{i+1/2}$ における面値フラックス $(\psi u)_{f,i+1/2}$ を Hermite 補間で構成し，
その差分でセル中心の発散を求める（\S\ref{sec:advection_motivation}，
変数別割り当て表 \S\ref{sec:scheme_per_variable}）．
節点 CCD 単独（散逸なし）は $\psi$ に適用禁止（\S\ref{sec:scheme_per_variable}）．
WENO5 は比較参照スキーム（付録~\ref{app:weno5}）．
