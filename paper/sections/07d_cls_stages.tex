...
Stage & 操作 & 要件 \\
\hline
A & TVD-RK3 保存形移流 & WENO5 面フラックス + DCCD ハイブリッド (Stage A; \S\ref{sec:advection_motivation})，発散フリー $\mathbf{u}^n$．BC：壁面 $\partial_n\psi=0$（Neumann）/ 周期境界（検証），WENO5 ghost-cell 対称延長 \\
...
\end{align}
$\mathcal{L}\psi = -\nabla\cdot(\mathbf{u}\psi)$ は
WENO5 面フラックス（Jiang--Shu \cite{JiangShu1996}）に DCCD スペクトル散逸を合成した
ハイブリッド演算子（\S\ref{sec:cls_advection}，変数別割り当て表 \S\ref{sec:scheme_per_variable}）．
CCD 単独は $\psi$ に適用禁止（\S\ref{sec:scheme_per_variable}）．
