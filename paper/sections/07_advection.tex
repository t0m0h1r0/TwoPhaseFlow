...
不連続を検知すると，その方向の重みを $\Ord{\varepsilon_{\mathrm{w}}^2}$（$\varepsilon_{\mathrm{w}}$：WENO5 の滑らかさ指標に用いる小パラメータ；界面幅 $\varepsilon$ とは無関係）まで低減して
...
界面厚みを $2\varepsilon \sim \Ord{h}$ に制御する．
...
\begin{enumerate}
  ...
  \item \textbf{Balanced-Force 精度の侵食：}
        ...
        曲率 $\kappa$ の計算と圧力勾配 $\bnabla p$ に\textbf{同一の差分演算子}を
        用いることで寄生流れを $\Ord{h^6}$ のオーダーまで低減する．
        WENO5 の非線形重みは $\psi$ の移流精度を適応的に劣化させるため，
        再初期化後の $\psi$ プロファイル（曲率計算の入力）に振動・ぼやけを残し，
        Balanced-Force によって確保した高精度を実用上無意味にする危険がある．
\end{enumerate}
...
\begin{description}
  ...
  \item[\upshape(ii) 空間離散化スキームの統一]
        ...
        \textbf{打ち切り誤差の非整合}（$\Ord{h^5}$ の移流誤差が $\Ord{h^6}$ の圧力精度を
        律速する非均質な精度階層）を回避できる．
\end{description}
...
これは設計目標であり，数値実験による検証は今後の課題である．形式的上界は \ref{result:dccd_stability} を参照されたい．
...
\end{equation}
すなわち $\sigma \leq \sqrt{3}/\max_\xi\bigl\{H(\xi;\varepsilon_d^{\mathrm{adv}})\,\hat{k}^*(\xi)\bigr\}$ となる．
無次元修正波数 $\hat{k}^*(\xi)$ は特性方程式（式~\eqref{eq:CCD_I} の Fourier 解析）から求まり，
$\varepsilon_d^{\mathrm{adv}} = 0.05$ のとき $\max_\xi\{H(\xi)\,\hat{k}^*(\xi)\} \approx 1.9$
...
\begin{equation}
  ...
  \label{eq:psi_clamp}
\end{equation}
のクランプを適用する．クランプによる質量誤差は $\Ord{h^2}$ であり，
Dissipative CCD の移流フィルタ誤差 $\Ord{\varepsilon_d^{\mathrm{adv}} h^2}$
と同オーダーのため，全体精度のボトルネックにならない．
...
移流項 $\bnabla\cdot(\psi\bm{u})$ の非線形相互作用（$\psi$ と $\bm{u}$ の積）は，
...
\begin{itemize}
  ...
  \item DCCD 散逸機構（§~\ref{sec:checkerboard_solution}）が
