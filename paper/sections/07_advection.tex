...
（§\ref{sec:cls_stages} Stage A，ch13 production の \texttt{FCCDLevelSetAdvection}）．
...
production stack 確立前の旧実装では Dissipative CCD（DCCD：CCD 微分発散形 + 後段スペクトル散逸
$\varepsilon_d^{\mathrm{adv}} = 0.05$）を CLS 移流に使用していた．DCCD は線形フィルタによる
「近似的安定化」であり，質量保存も周期境界以外では近似的にしか成立しない．
ch13 では FCCD の保存型構造による厳密質量保存と界面安定性の両立が確認され，FCCD への移行が決定された．
DCCD の理論的詳細は §~\ref{sec:dissipative_ccd}（§4 DCCD 節）を参照．}
...
ch13 production の \texttt{FCCDLevelSetAdvection} はこの定式化を実装し，
\texttt{experiment/ch13/config/*.yaml} で \texttt{interface.transport.spatial: fccd} と指定する．
...
DCCD のような線形フィルタによる高波数応答低減（近似的安定化）とは構造的に異なる．
...
対象 & スキーム & 精度 & 根拠 \\
\hline
CLS 移流（$\psi$） & FCCD（面値保存型；§~\ref{sec:fccd_def}，§~\ref{sec:advection_motivation}，§~\ref{sec:cls_stages} Stage A；ch13 \texttt{FCCDLevelSetAdvection}） & 面値 $\Ord{h^6}$；保存型 FVM テレスコーピング & 面値評価で節点 CCD overshoot を回避；非周期境界・非一様格子でも厳密質量保存；WENO5 は比較参照（付録~\ref{app:weno5}） \\
運動量移流（$u,v$） & UCCD6（§~\ref{sec:uccd6_def}；ch13 \texttt{UCCD6ConvectionTerm}） & CCD 微分 $\Ord{h^6}$；ハイパー粘性 $\Ord{\sigma h^7}$（次数保存） & 演算子内部散逸；$\omega_2^8$ 選択的 Nyquist（L2/L3 bulk 既定） \\
CLS 再初期化 & Ridge-Eikonal（§~\ref{sec:ridge_eikonal}，§~\ref{sec:cls_compression}；ch13 \texttt{RidgeEikonalReinitializer}） & Eikonal 距離 $\Ord{h^6}$ + Ridge ガウス射影 & DCCD 圧縮項を不使用；§3.4 Part 1 連続定式化の離散実装 \\
