...
圧力 Poisson & 欠陥補正法 PPE + CCD 勾配（§~\ref{sec:pressure}） & $\Ord{h^2}$/$\Ord{h^6}$† & Balanced-Force 整合性（DC k=1） \\
\hline
\end{tabular}
\end{center}
{\small †\ DC k=1 では実効 $\Ord{h^2}$（§~\ref{sec:ppe_discretization_choice}）；速度補正の圧力勾配は CCD $\Ord{h^6}$（§~\ref{sec:pressure_accuracy_summary}）．}
\end{tcolorbox}
