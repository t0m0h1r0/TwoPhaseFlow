...
\begin{description}
  ...
  \item[\textbf{第\ref{sec:time}章：WENO5 移流スキームと時間積分（NS 方程式）}]
        ...
        理論と実装係数を詳述した（§\ref{sec:weno5}）．
        CLS 移流・再初期化には TVD-RK3（Shu--Osher 1988）を適用し，全変動非増大性を保証した．
        全体時間精度が \textbf{Projection 法スプリッティング誤差 $O(\Delta t)$} に律速されること
        および NS 対流項の前進 Euler も同次数であることを明示した（§\ref{sec:time_int}）．
...
\begin{description}
  ...
  \item[\textbf{格子収束解析による理論精度の実証：}]
        ...
        曲率 $O(h^6)$・WENO5 $O(h^5)$・寄生流れ $O(h^2)$（CSF 律速）という
        理論精度との整合を定量的に確認する．
\end{description}
