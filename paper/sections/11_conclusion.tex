...
低次移流による界面にじみ & Dissipative CCD + TVD-RK3（第\ref{sec:time}章）\\
...
\begin{description}
  ...
  \item[\textbf{第\ref{sec:time}章：Dissipative CCD 移流スキームと時間積分（NS 方程式）}]
...
主要な精度設計上のトレードオフとして，
（1）空間精度は CCD-PPE $\Ord{h^6}$・CCD $\Ord{h^6}$・Dissipative CCD $\Ord{h^5}$ と高次で統一されているが，
...
\begin{description}
  ...
  \item[\textbf{格子収束解析による理論精度の実証：}]
        ...
        曲率 $O(h^6)$・Dissipative CCD $O(h^5)$・寄生流れ $O(h^2)$（CSF 律速）という
        理論精度との整合を定量的に確認する．
\end{description}
