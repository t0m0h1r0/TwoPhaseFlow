...
\subsubsection{PhaseRegion 正規状態と圧力反力部分空間}
\label{sec:capillary_pressure_reaction_subspace}

PhaseRegion 正規経路が所有する物理状態は，距離場配列でも
セルごとの体積分率そのものでもなく，相領域である．
離散状態を
\begin{equation}
  ...
  =
  \bigcup_{\alpha=1}^{N_c}\Gamma_{h,\alpha}
  \label{eq:phase_region_owner}
\end{equation}
と書く．ここで $\mathcal{A}_h$ は境界 chart の atlas であり，
成分ごとに graph chart，closed-curve chart，open-curve chart，
または局所 cut-cell chart を持ってよい．
毛管波は $\Omega_g=\{y>\eta(x)\}$ の graph chart，
閉じた振動液滴は閉曲線の内側または外側を所有する closed chart，
上昇気泡と上部気相層は複数成分
$\Omega_g=\Omega_{\mathrm{bubble}}\cup\Omega_{\mathrm{layer}}$
として同じ式で扱う．

セル測度と表面エネルギーは，この所有状態から従属的に読む：
\begin{align}
  q_{h,C} &= Q_h(R_h)_C
  =
  |\Omega_h\cap C|,
  \label{eq:phase_region_measure}\\
  E_h[R_h] &=
  \sigma\,\mathrm{Per}(\Omega_h)
  =
  \sigma\sum_{\alpha=1}^{N_c}|\Gamma_{h,\alpha}| .
  \label{eq:phase_region_surface_energy}
\end{align}
$\phi$ は $\Gamma_h$ を局所表示する gauge，
$q_h$ は有限体積セル上の測度であり，どちらも単独では毛管状態の所有者ではない．
輸送や投影から得たセル測度 $q_T$ は
\begin{equation}
  q_T = Q_h(R_h)+r_h
  \label{eq:phase_region_residual_split}
\end{equation}
と分解して読む．残差 $r_h$ は「保存すべき物質」ではなく，
現在の atlas で表せる滑らかな相領域から外れた非幾何的成分である．
したがって $Q_h(\phi)=q_T$ だけを厳密に満たす再構成は，
曲率を二階微分で増幅する毛管問題では合格条件にならない．
逆に，滑らかな界面を保つために $r_h$ を診断・棄却することは，
物理体積全体を捨てることではなく，有限次元測度の非幾何成分を
界面所有者へ押し付けないという状態空間条件である．

この正規経路が対象にするのは，曲率値そのものではなく，
$E_h[R_h]$ の第一変分として得る表面エネルギー仮想仕事である．
許容面速度空間を $F_h$，その SPD 計量を $M_f$ とし，
表面エネルギーの離散第一変分を共変量 $r_\sigma\in F_h^\ast$ で
\begin{equation}
  \delta E_{\Gamma,h}[w_f]
  =
  -\langle r_\sigma,w_f\rangle
  \qquad (w_f\in F_h)
  \label{eq:capillary_face_covector}
\end{equation}
と表す．ここで $w_f$ は chart 節点変位ではなく，
PPE 補正段，保存形輸送，および非圧縮制約が実際に使う面速度である．
圧力で吸収できる反力は，全てのセル圧力勾配像ではなく，
作用中の PPE，境界制約，圧力代表，および履歴圧力面加速度と同じ契約で許容される
部分空間
\begin{equation}
  R_p(q_T)\subset F_h^\ast
  \label{eq:pressure_reaction_subspace}
\end{equation}
として先に定義する．このとき物理的に速度を変える毛管駆動は
\begin{equation}
  a_{\sigma,\mathrm{bal}}
  =
  M_f^{-1}\left(r_\sigma-\Pi_{R_p}^{M_f}r_\sigma\right),
  \qquad
  \Pi_{R_p}^{M_f}r_\sigma
  =
  \arg\min_{r\in R_p(q_T)}
  \|r_\sigma-r\|_{M_f^{-1}}^2 .
  \label{eq:capillary_pressure_reaction_projection}
\end{equation}
である．平坦界面や静的円形液滴では式~\eqref{eq:capillary_pressure_reaction_projection} が
丸め誤差まで 0 になることを要求し，毛管波や非定曲率界面では
$R_p$ に落ちない成分が復元向きの面加速度として残ることを要求する．

ここで $R_p$ の定義を省略し，節点 Young--Laplace 残差
$e=g+J_q^T\pi$ の非零性だけを非静的駆動の証明としてはならない．
固定アクティブ集合上で，$q$ 互換射影の毛管 Riesz 代表と全セル圧力反力を
同じ Schur 系
\begin{equation}
  S=J_qW^{-1}J_q^T,\qquad
  S\lambda=J_q(-g),\qquad
  S\pi=-J_qg
  \label{eq:ao_full_pressure_cancellation}
\end{equation}
から作ると $\lambda=\pi$ となり，$r_\sigma-T_q^T\pi=0$ が代数的に成立する．
これは非零の節点残差が存在しても，全セル圧力像への過射影では
面上の毛管仕事を全て圧力反力として消してしまうことを意味する．
したがって PhaseRegion 正規経路の成立条件は，
式~\eqref{eq:phase_region_owner}--\eqref{eq:phase_region_residual_split} の状態所有者，
式~\eqref{eq:capillary_face_covector} の幾何共変量構成，
式~\eqref{eq:pressure_reaction_subspace} の圧力反力部分空間，
式~\eqref{eq:capillary_pressure_reaction_projection} の面計量射影の三者である．

式~\eqref{eq:capillary_pressure_reaction_projection} は第2章で固定する理論境界であり，
ここでは計算手順の細部を論じない．
この理論境界を PhaseRegion/InterfaceAtlas 状態空間で満たすための
chart 条件，測度残差，アクティブ集合，近似誤差境界は，
第~\ref{sec:ao_fast_state_space}節で受理条件として定式化する．
