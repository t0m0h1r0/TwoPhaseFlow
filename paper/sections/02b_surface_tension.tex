...
さらに本稿の production 経路では，cut-face で得た毛管ジャンプ cochain を
式~\eqref{eq:capillary_range_projection} の $D_f A_f G_f$ 値域へ射影してから
PPE と corrector へ渡す．
したがって本稿で保持する階層は，
CSF が One-Fluid/BF の連続収縮と対照モデルを与え，
range-projected 分相 pressure-jump PPE が高密度比の圧力閉包を担う，というものである．
CSF 収縮では $\Ord{\varepsilon^2} \approx \Ord{\Delta x^2}$ のモデル誤差が残るため
...
\begin{tcolorbox}[defbox, title={Balanced--Force 条件の 5 つの失敗モード（CSF/BF 経路のプレビュー）}]
...
ここで得た $B_{\Gamma,f}$ は幾何から作る raw cut-face データであり，
PPE/corrector へ入る標準データは
§~\ref{sec:split_ppe_capillary_range_projection} の range-projected capillary cochain である．
