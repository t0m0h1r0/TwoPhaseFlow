...
\paragraph{設定．}
ドメイン高さ 2, 幅 1, $A_0 = 0.005$ の 2 モード摂動 ($\lambda = L_x/2 = 0.5$, $k = 4\pi$, $m=2$)．
$\rho_h = 5, \rho_l = 1$ ($\mathcal{A} = 2/3$), $g = 1$ →
$\gamma_{\mathrm{th}} = \sqrt{\mathcal{A}\, g\, k} = \sqrt{(2/3)\!\cdot\!1\!\cdot\!4\pi} = \sqrt{8\pi/3} \approx 2.894$．
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.92\linewidth]{figures/ch13_v4_rt_snapshot.pdf}
\caption{V4-b：Rayleigh--Taylor 2D 場スナップショット ($N=128$, $\rho_h/\rho_l=5$, $m=2$)．
左から順に $t=0$（初期摂動 $A_0=0.005$ の cosine 形状），
$t \approx t_{\mathrm{end}}/3$（線形成長初期），$t \approx 2t_{\mathrm{end}}/3$（線形成長中），
$t = t_{\mathrm{end}}$（測定窓終端，線形領域端）．
カラーマップは smoothed Heaviside $\psi$，黒線は界面 $\phi=0$ 等高線．}
\label{fig:V4_rt_snapshot}
\end{figure}
...
\begin{itemize}\setlength{\itemsep}{0pt}
  \item 線形理論との誤差 $40$--$55\%$，かつ $\gamma_{\mathrm{num}}$ が
    ...
    \begin{enumerate}\setlength{\itemsep}{0pt}
      \item \textbf{初期 CSF 曲率推定誤差}：$\rho_h \!=\! 5, \rho_l \!=\! 1$ の界面で
