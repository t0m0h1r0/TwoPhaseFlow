...
\textbf{本稿の標準 PPE 実装}は FVM ではなく CCD product-rule 展開（式~\eqref{eq:varrho_product_rule}，§\ref{sec:ccd_poisson_matrix}）である．
滑らかな係数場の検証条件では高次 CCD 設計精度を評価対象にできるが，界面帯・高密度比での実効精度は
HFE，正則化厚み，外側残差に律速されるため，実験結果で判定する．
...
\begin{tcolorbox}[warnbox, title={周期 FVM 行列と前処理：ILU の充填不足に注意}]
  ...
  ILU(0)（\texttt{fill\_factor=1}）を前処理に使用すると BiCGSTAB~\cite{VanderVorst1992} の収束が著しく遅延する．
...
% ══════════════════════════════════════════════════════════════════════════════
\subsubsection{Rhie--Chow 補正の参照定式化}
...
\subsubsection{Rhie--Chow 経路における離散化形式}
\label{sec:rc_implementation}
Rhie--Chow 経路を採る場合，圧力 Poisson 方程式の右辺とフェイス速度を同じ離散発散で閉じるため，
以下の離散化形式を用いる．
\begin{equation}
...
\noindent\textbf{【RC 経路での注意】PPE 右辺での Rhie--Chow 発散：}
Rhie--Chow 経路を用いる場合，PPE 右辺の発散計算にはこのフェイス速度の発散 $\nabla_h^{\mathrm{RC}}\cdot\bu^*$
を使用する
（セル中心発散の代用はチェッカーボードの逆流を招く）．

\noindent\textbf{注：本稿（CSF + HFE + Balanced-Force/FCCD 定式化）では}
Balanced-Force 演算子整合と FCCD 面ジェット sub-system（§\ref{sec:checkerboard_solution}）が
PPE 右辺のチェッカーボード対策を担い，
HFE（§\ref{sec:field_extension}）が界面越し圧力延長を行う．
CSF 体積力は Predictor に保持される（§\ref{sec:ccd_balanced_force}）．

\subsubsection{Rhie--Chow 経路での表面張力項拡張：フェイス速度への Balanced-Force 反映}
\label{app:rc_balanced_force}
\label{app:csf_balanced_force}

Rhie--Chow 経路で Balanced-Force の論理をフェイス速度の計算にも同じ離散レベルで反映させるためには，
...
\begin{itemize}
  ...
  \item $\overline{(\bm{f}_\sigma^{n+1})}_f = \tfrac{1}{2}[(\bm{f}_\sigma^{n+1})_P + (\bm{f}_\sigma^{n+1})_E]$：
        セル中心値の算術平均補間値．\textbf{RC 拡張を使う場合の条件：} セル中心値には CCD 演算子を使用する
...
% ══════════════════════════════════════════════════════════════════════════════
%% 旧 appendix_numerics_schemes_s5.tex の内容を統合
% ══════════════════════════════════════════════════════════════════════════════

\subsubsection{Rhie--Chow 補正と CCD 高次精度の整合性}
\label{app:rc_precision}

RC 検出項のテイラー展開（式~\eqref{eq:rc_face_taylor}--\eqref{eq:rc_detection_taylor}）と
$\Ord{h^2}$ ブラケットの物理的解釈は付録~\ref{app:rhie_chow_full} に収録した．
本節では，表面張力 Balanced-Force 拡張（式~\eqref{eq:rc-face-balanced-app}）の補正項が
CCD の $\Ord{h^6}$ 精度を律速しないことを補足する．

\textbf{表面張力 RC 補正の誤差（式~\eqref{eq:rc-face-balanced-app} の $(\bm{f}_\sigma)_f - \overline{(\bm{f}_\sigma)}_f$）：}
拡張 Rhie--Chow 式~\eqref{eq:rc-face-balanced-app} では，圧力補正項に加えて表面張力検出項
