...
CSF モデルは実装の簡便さという大きな利点を持つが，界面を $\Ord{\varepsilon}$ の厚みを持つ平滑化領域として扱うため，
以下の根本的な限界がある：
（1）モデル誤差 $\Ord{\varepsilon^2} \approx \Ord{\Delta x^2}$ が本稿の空間精度の律速段階となる
（§\ref{sec:pressure_accuracy_summary} の精度バジェット参照）；
（2）体積力として NS 方程式に組み込まれた表面張力は，圧力勾配と同一演算子で離散化しなければ
寄生流れが発生する（Balanced-Force 条件，§\ref{sec:balanced_force}）；
（3）界面近傍で CCD の高次ステンシルが急峻な（しかし滑らかな）プロファイルの解像に費やされ，
スペクトル分解能が低下する．
これらの限界を克服する代替として，次節（§\ref{sec:gfm}）では Ghost Fluid Method (GFM) を導入する．
...
粘性 $\mu_1 = \mu_2$ の単純化条件のもとで，界面 $\Gamma$ における圧力のジャンプ条件は：
\begin{equation}
  [p]_\Gamma \equiv p_1 - p_2 = \sigma\kappa
  \label{eq:gfm_jump}
\end{equation}
ここで $p_1, p_2$ は界面の液相側・気相側の圧力，$\kappa$ は平均曲率，$\sigma$ は表面張力係数である
（無次元形式では $\sigma\kappa \to \kappa/We$）．
...
\begin{equation}
  b_i^{\mathrm{GFM}} = b_i - \frac{\kappa_f}{We \cdot \rho_i \, h^2}
  \quad \text{（界面を右に持つセル $i$ に対して）}
  \label{eq:gfm_rhs_correction}
\end{equation}
符号は界面の向き（$\phi_i > 0$ か $\phi_i < 0$ か）に従い，各軸方向について独立に適用する．
...
GFM によって各相の物理量がゴースト領域まで $C^\infty$ に滑らかに定義されるため，CCD のステンシル $(i-2,\ldots,i+2)$ は界面直近に至るまで滑らかな場を参照できる．この結果，CSF の平滑化プロファイルに解像度を費やすことなく，\textbf{界面直近まで $\Ord{h^6}$ 精度が維持される}．
