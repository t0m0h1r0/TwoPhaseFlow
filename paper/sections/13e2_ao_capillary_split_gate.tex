...
最終採用境界 & $R_p$，Hodge 逆適用済み面補間，移動格子面共鎖，圧力履歴座標，DC 収束判定を同時に満たす経路だけをアクティブ幾何毛管分解として標準物理経路へ入れる． & \checkmark 契約 \\
\bottomrule
\end{tabularx}
\end{table}
...
第~\ref{sec:ao_fast_state_space}節の七条件を本番経路の採用境界として固定する．
したがって第~\ref{sec:val_capillary}節の毛管波は，
幾何セル分率，bundle Young--Laplace work，pressure-component Hodge reaction，
Hodge 逆適用済み面補間，投影済み面共鎖保存，および pressure-coordinate history を同時に使う
