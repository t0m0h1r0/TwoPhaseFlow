...
\paragraph{追加・再試の根拠．}
第14章の最新毛管波結果は，U12 の旧代数反例だけでなく，
$q$ 所有グラフ HFE，pressure-coordinate history の分離，
HFE admission，および界面追随格子の正則層保持に依存する．
V11 では，これらの単体契約を U12 から引き継ぎつつ，
pressure-coordinate と face-acceleration の履歴表現を変えても
旧全圧力像への過射影が非静的波の物理駆動を消すこと，
現行 GPU packet が split-pending 境界で停止して非静的駆動を残すことを
再試する．これにより，旧 fail-close 仕様ではなく，
現在の分相 PPE+HFE 経路へ接続する受理境界として V11 を更新する．
...
$q$ 所有グラフ HFE & 非一様 $x$ 格子で $\|j_{gl}^{G}\|_\infty=11.5323$，$\Delta x$ 加重平均 $1.30\times10^{-16}$，波頂ジャンプは負．pressure-coordinate history へ不連続ジャンプを保存せず，現在格子で面ジャンプを再構築する． & \checkmark 契約 \\
履歴・admission & graph HFE jump は BDF2 pressure history から除外される．HFE admission は $dt$ 倍増分ではなく空間駆動で判定され，$dt=10^{-8}$，増分 $10^{-14}$，空間駆動 $10^{-6}$ の非静的 packet を受理した． & \checkmark 契約 \\
追随格子正則層 & ほぼ水平な毛管波で支配法線方向ガードは $x$ 方向変位 $0$，$y$ 最大変位 $2.50\times10^{-3}$．全 active 軸移動で見られた接線方向格子崩壊は再現しない． & \checkmark \\
GPU split-pending 境界 & pressure-coordinate と face-acceleration の履歴表現のどちらでも，現行 GPU packet は split-pending で停止し，N32 毛管波の非静的駆動 $2.40\times10^{-5}$ を消さない． & \checkmark 境界 \\
...
圧力座標と面加速度の履歴表現を含め，
非静的波では全圧力像への過射影を棄却し，GPU packet が split-pending 境界で
駆動を残すこと，および graph-HFE/追随格子契約が通ることを確認する．}
