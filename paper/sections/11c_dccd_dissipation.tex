...
\begin{itemize}[noitemsep]
  \item 格子 $N=256$，周期境界，CFL $=0.4$，時間積分 RK4\footnote{本検証では CCD 空間離散化の特性を分離して評価するため RK4 を使用した．実ソルバでは TVD-RK3 を採用しており（式~\eqref{eq:tvd_rk3}），時間積分スキームの違いによる散逸特性への影響は微小である．}，1 周期（$T=1$）
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/dccd_dissipation}
\caption{1 次元線形移流（$N=256$，CFL$=0.4$，$T=1$）における各スキームの波形比較．
  Square（不連続），Triangle（$C^0$），Smooth tanh の 3 初期条件で評価．
  DCCD は不連続プロファイルのリンギング（CCD の TV$=10.83$）を $3.15$ に抑制しつつ，
  滑らかなプロファイルでは CCD と同等の精度を維持する．}
\label{fig:dccd_dissipation}
\end{figure}
