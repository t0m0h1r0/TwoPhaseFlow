...
$p$ / 毛管ジャンプ & 界面でジャンプ & Balanced--Force 面評価位置（\S\ref{sec:balanced_force}）+ アフィンジャンプ PPE / 変分毛管 Hodge 閉包（\S\ref{sec:split_ppe_framework}）\\
...
\begin{itemize}
  ...
  \item \textbf{$p$ と毛管ジャンプは節点勾配では閉じない．}
        ...
        分相経路では §~\ref{sec:split_ppe_framework} のアフィンジャンプ面共鎖と
  ...
  \item \textbf{領域内部速度の標準移流は UCCD6．}
        ...
        標準圧力ジャンプ経路では毛管面共鎖と対流面フラックスの評価位置を比較する
...
分相アフィンジャンプ PPE と変分毛管 Hodge 閉包を
