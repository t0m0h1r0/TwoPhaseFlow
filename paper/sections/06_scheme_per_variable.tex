...
$u, v$ bulk & $C^\infty$ 滑 & \textbf{UCCD6}（\S\ref{sec:uccd6_def}；FCCD 面フラックス形は \S\ref{sec:fccd_advection} の面ロカス拡張） \\
...
$p$ / 毛管 jump & 界面で jump & Balanced--Force 面ロカス（\S\ref{sec:balanced_force}）+ affine-jump PPE / range projection（\S\ref{sec:split_ppe_framework}）\\
...
\begin{itemize}
  ...
  \item \textbf{$p$ と毛管 jump は節点勾配では閉じない．}
        ...
        本稿では §~\ref{sec:balanced_force} の同一面ロカス原理に従い，
        一括 CSF 経路では pressure/CSF を同じ面演算子で評価し，
        分相経路では §~\ref{sec:split_ppe_framework} の affine-jump face cochain と
        §~\ref{sec:split_ppe_capillary_range_projection} の capillary range projection に渡す．
  ...
  \item \textbf{bulk 速度の標準移流は UCCD6．}
        ...
        FCCD 面フラックス形は，面ロカスを pressure/CSF/convective flux と揃える
        発展的閉包として \S\ref{sec:fccd_advection} に定義する．
...
圧力 $p$ と毛管 jump の閉包は，Balanced--Force の同一面ロカス原理を
\S~\ref{sec:balanced_force}（§8）で述べ，
分相 affine-jump PPE と capillary range projection を
