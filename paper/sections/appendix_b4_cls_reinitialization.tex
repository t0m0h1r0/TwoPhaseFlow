\subsection{CLS 再初期化方程式の設計動機と不動点証明}
...
\subsubsection*{設計原理（逆算による導出）}

\textbf{目標：} 圧縮法線座標 $s=-\phi$ に沿って
定常解 $\psi = H_\varepsilon(s)=H_\varepsilon(-\phi)$，
すなわち $d\psi/ds = \frac{1}{\varepsilon}\psi(1-\psi)$ を持つ方程式を作る．
...
すなわち方程式
$\partial_\tau\psi + \bnabla\cdot(\psi(1-\psi)\hat{\bm{n}}_c) = \bnabla\cdot(\varepsilon\bnabla\psi)$
の定常解が $H_\varepsilon(-\phi)$ であることが自然に導かれる．

\subsubsection*{不動点の解析的証明}

1次元の定常条件 $\frac{d}{ds}[\psi(1-\psi)] = \varepsilon\,\frac{d^2\psi}{ds^2}$ において，
$\psi = H_\varepsilon(s) = \frac{1}{1+e^{-s/\varepsilon}}$，
$s=-\phi$ がこれを満たすことを示す．
...
\subsubsection*{多次元への拡張}

上記の証明は1次元（法線方向座標 $s$）で与えたが，多次元の再初期化方程式
$\partial_\tau\psi + \bnabla\cdot(\psi(1-\psi)\hat{\bm{n}}_c) = \bnabla\cdot(\varepsilon\bnabla\psi)$
においても，圧縮法線方向 $s = \hat{\bm{n}}_c\cdot\bm{x}=-\phi$ に沿った1次元プロファイル
$\psi(s) = H_\varepsilon(s)$ は不動点である．曲率 $\kappa \neq 0$ の界面では
$\bnabla\cdot\hat{\bm{n}} = \kappa$ による追加項が生じるが，
この項は $\varepsilon\kappa\psi(1-\psi)$ のオーダーであり，
$\varepsilon \to 0$ の極限で消失する（$\Ord{\varepsilon}$ の摂動）．
