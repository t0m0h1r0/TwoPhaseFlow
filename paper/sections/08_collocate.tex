...
どの楕円問題により発散フリー空間へ射影するか．第二に，その射影で得る圧力勾配と
界面張力を，同じ離散空間で釣り合わせるには何を揃えなければならないか．
以下では，連続射影の導出から始め，コロケート格子で生じる離散的な破綻を同定し，
最後に Balanced--Force 条件と FCCD 面フラックス構成を与える．
...
\textbf{格子スケールデカップリング}（FCCD 面フラックス部分系で解消）である．
...
本稿では CSF + Balanced--Force と FCCD 面フラックス部分系（§~\ref{sec:fccd_bf_sub_system}）の組み合わせにより
...
（均一格子および周期 BC）．
非一様格子（$\alpha>1$）+ 壁面 BC の場合は，BF 部分系全体を
...
（CCD-pair / adjoint-pair）に帰着する（face-average 条件は §\ref{sec:fvm_ccd_corrector}，静止液滴での検証は §\ref{sec:bf_static_droplet}）．
...
FCCD 面フラックス部分系（§~\ref{sec:fccd_bf_sub_system}）が加わって初めて
