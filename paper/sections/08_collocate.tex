...
\begin{enumerate}
  ...
  \item \textbf{§~\ref{sec:fccd_bf_sub_system}（FCCD 面フラックス部分系）：}
        ...
        変分毛管面共鎖と成分体積制約付き鞍点系を同じ面の評価範囲へ閉じる．
\end{enumerate}
...
それぞれ独立した機構で対処する：
\textbf{面共鎖の不整合}（Balanced--Force 条件で解消）と
\textbf{格子スケールの圧力・速度分離}（同じ面複体による PPE/補正段共有で解消）である．
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{FCCD 面フラックス部分系}（§~\ref{sec:fccd_bf_sub_system}）：
        ...
        標準圧力ジャンプ閉包では表面エネルギー面共鎖と成分体積反力を
        同じ圧力仕事と随伴な面計量で構成し，
...
$A_f G_f p$ と表面エネルギーの Riesz 面共鎖を同じ面複体で評価し，
成分補正済み面共鎖を速度補正へ渡す
