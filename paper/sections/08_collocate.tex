...
\begin{enumerate}
  ...
  \item \textbf{§~\ref{sec:balanced_force}（Balanced--Force 条件：同一評価位置・随伴整合原理）：}
        圧力面共鎖と毛管ジャンプ面共鎖を同じ評価位置・同じ随伴ペアで
  ...
  \item \textbf{§~\ref{sec:fccd_bf_sub_system}（FCCD 面フラックス部分系）：}
        ...
        変分毛管 cochain と成分体積制約付き鞍点系を同じ面フットプリントへ閉じる．
\end{enumerate}
...
$A_f G_f p$ と毛管ジャンプによる既知面共鎖 $c_f(j_{gl})$ を
...
§~\ref{sec:split_ppe_capillary_range_projection} の面共鎖として扱う．
