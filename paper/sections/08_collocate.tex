...
どの楕円問題により発散フリー空間へ射影するか．第二に，その射影で使う
圧力 face cochain と毛管ジャンプ face cochain を，同じ離散空間で
釣り合わせるには何を揃えなければならないか．
以下では，連続射影の導出から始め，コロケート格子で生じる離散的な破綻を同定し，
最後に pressure-jump 標準経路を支える Balanced--Force 条件と
FCCD 面フラックス構成を与える．
...
\begin{enumerate}
  ...
  \item \textbf{§~\ref{sec:balanced_force}（Balanced--Force 条件：同一ロカス・随伴整合原理）：}
        圧力 face cochain と毛管ジャンプ face cochain を同じ評価ロカス・同じ随伴ペアで
        構成する設計思想を示す．CSF 体積力はこの原理の一括 PPE 縮約形であり，
        標準経路では pressure-jump cochain として扱う．
  ...
  \item \textbf{§~\ref{sec:fccd_bf_sub_system}（FCCD 面フラックス部分系）：}
        ...
        変分毛管 cochain と component saddle を同じ面フットプリントへ閉じる．
\end{enumerate}
...
すなわち射影法とは，$\bu^*$ を発散フリー速度空間（Leray 空間）へ直交射影する操作である．
...
それぞれ独立した機構で対処する：
\textbf{面 cochain の不整合}（Balanced--Force 条件で解消）と
\textbf{格子スケールデカップリング}（同じ face complex による PPE/corrector 共有で解消）である．
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{FCCD 面フラックス部分系}（§~\ref{sec:fccd_bf_sub_system}）：
        PPE 左辺，RHS 発散，速度補正，毛管ジャンプ補正を
        同じ active face complex 上の $D_fG_A$ として組み立てる．
        標準 pressure-jump 閉包では surface-energy cochain と成分体積反力を
        同じ pressure-adjoint metric で構成し，
...
$A_f G_f p$ と毛管ジャンプによる既知面 cochain $c_f(j_{gl})$ を
\textbf{同一の離散ロカスと随伴整合した演算子対}で評価することで，
静止流体における Young--Laplace 平衡
$A_f G_f p=c_f(j_{gl})$ が離散化後にも同じ face space で表現されることを
要求する条件である~\cite{Francois2006}．
CSF モデル（§~\ref{sec:csf}）を一括 PPE 縮約経路として使う場合は，
この条件が $\bnabla p=(\kappa_{lg}/We)\bnabla\psi$ の同一ロカス評価に還元される．
標準 pressure-jump 経路では $j_{gl}=p_g-p_l=-\sigma\kappa_{lg}$ を
§~\ref{sec:split_ppe_capillary_range_projection} の face cochain として扱う．
本節では，寄生流れ（spurious currents，第~\ref{sec:intro}章§~\ref{sec:failure_modes}）を
抑制するためのこの離散要件を定式化する．
...
CSF 縮約経路では，Balanced--Force と FCCD 面フラックス部分系
（§~\ref{sec:fccd_bf_sub_system}）の組み合わせによりこのループを抑える
（§~\ref{sec:bf_static_droplet} 参照）．
一方，本稿の標準 pressure-jump 経路では，表面張力を予測段階の CSF 体積力として入れず，
向き付き圧力ジャンプ $j_{gl}$ を cut-face 代表として使いながら，
表面エネルギーの仮想仕事から毛管 face cochain を構成し，
pressure-adjoint component saddle で静的反力と非平衡駆動を分離してから
PPE/corrector へ渡す（§~\ref{sec:split_ppe_capillary_range_projection}）．

本稿の定式化では，式~\eqref{eq:bf_operator_mismatch} の主要な演算子不整合を抑えるため，
圧力勾配・毛管 cochain・速度補正を同じ面複体に置く．
一様格子の CSF 縮約では $D^{(1)}_\mathrm{CCD}$ 統一として読めるが，
非一様格子（$\alpha>1$）+ 壁面境界条件，および pressure-jump 標準経路では，
BF 部分系全体を face-average 演算子 $\mathcal{G}^\text{adj}$ が定める
同じ面スロットへ移し，PPE・速度補正・ジャンプ cochain を随伴ペアとして扱う
...
一様格子の CSF 縮約経路では $D_x^{(1)}, D_y^{(1)}$（第~\ref{sec:CCD}章）を統一使用する：
\begin{align}
  \text{圧力勾配：} & \quad
...
正帰還ループを通じて計算の発散を招く．
\textbf{本稿の標準 pressure-jump 定式化では，
$A_f G_f p$ と surface-energy Riesz cochain を同じ face complex で評価し，
component-corrected cochain を速度補正へ渡す
（§~\ref{sec:split_ppe_capillary_range_projection}）．}
CSF 縮約経路は，この face-balance 原理を
$\bnabla p$ と $(\kappa_{lg}/We)\bnabla\psi$ の同一ロカス評価として読んだ場合に対応する．
