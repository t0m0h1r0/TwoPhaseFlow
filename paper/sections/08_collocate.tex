...
\begin{enumerate}
  ...
  \item \textbf{§~\ref{sec:fccd_bf_sub_system}（FCCD 面フラックス部分系）：}
        ...
        pressure-jump range projection を同じ面フットプリントへ閉じる．
\end{enumerate}
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{FCCD 面フラックス部分系}（§~\ref{sec:fccd_bf_sub_system}）：
        ...
        同じ face complex 上の $D_f A_f G_f$ として組み立てる．
        標準 pressure-jump 閉包ではこの complex の値域へ毛管 cochain を射影し，
...
向き付き圧力ジャンプ $j_{gl}$ から得る毛管 face cochain を
$D_f A_f G_f$ の値域へ射影してから PPE/corrector へ渡す
...
$A_f G_f p$ と $c_f(j_{gl})$ を同じ face complex で評価し，
range projection 後の実効 cochain を速度補正へ渡す
（§~\ref{sec:split_ppe_capillary_range_projection}）．}
