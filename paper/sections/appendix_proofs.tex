...
本付録は §\ref{sec:ppe_pseudotime} で使用した収束率因子
...
残差 $r^m \equiv \mathcal{L}_h p^m - q_h$ の反復ごとの変化を追う．
式 \eqref{eq:dtau_sweep} に $\mathcal{L}_h$ を作用させ，$q_h$ を消去すると
\begin{equation}
  r^{m+1} = \mathbf{M}\,r^m,
  \qquad
  \mathbf{M} = (\mathbf{I}+\Delta\tau\mathcal{L}_x)^{-1}(\mathbf{I}+\Delta\tau\mathcal{L}_y)^{-1}
              (\mathbf{I} - \Delta\tau^2\mathcal{L}_x\mathcal{L}_y)
\end{equation}
が得られる（$\mathcal{L}_x\mathcal{L}_y$ 交換項を整理した結果）．

\subsection*{スカラー化}

等方格子 $(\Delta x = \Delta y = h)$ かつ一様係数を仮定すると
$\lambda_x = \lambda_y = \lambda/2$（$\lambda$ は $\mathcal{L}_h$ の固有値）．
$t \equiv \Delta\tau\lambda/2$ とおけば，
$\mathbf{M}$ の対応する固有値（収束率因子）は

\begin{equation}
  \gamma(t) = \frac{1}{(1+t)^2}\bigl[(1+t)^2 - 2t\bigr]
            = \frac{1 + t^2}{(1+t)^2}
  \label{eq:gamma_t}
\end{equation}

となる．$t > 0$ に対して $\gamma(t) < 1$ が成立するため，
スウィープ型反復は\textbf{任意の正の $\Delta\tau$ に対して無条件安定}かつ収束する．
...
$\gamma(t) \to 1$（$t \to 0$）：$\Delta\tau$ が小さすぎると更新量がゼロに近づき，収束が遅い．
$\gamma(t) \to 1$（$t \to \infty$）：$\Delta\tau$ が大きすぎると「オーバーシュート」し，収束が遅い（発散はしない）．
最適点 $t=1$ から外れるほど収束は遅くなるが，安定性は保たれる．\qed
