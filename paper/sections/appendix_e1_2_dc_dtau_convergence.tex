\subsubsection{収束率解析と最適 \texorpdfstring{$\Delta\tau$}{Δτ} の導出}
...
各反復ごとに $1/(1+t)^2$ 倍に縮小する（線形スカラー化モデルでは任意の $t > 0$ で縮小率 $< 1$）．
...
implicit--BDF2 粘性 Helmholtz 系（§~\ref{sec:time_viscous}；U8-c/U8-d は
§~\ref{sec:U8_time_integration}）によりこの制約は回避される．
\end{tcolorbox}
