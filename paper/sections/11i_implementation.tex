...
\begin{enumerate}
  \item \textbf{§~\ref{sec:verify_spatial}（空間離散化）：}
        CCD/DCCD 収束性・GCL・散逸特性・Rhie--Chow 補正．
  \item \textbf{§~\ref{sec:verify_interface}（界面処理）：}
        CLS 移流・保存性・HFE・Young--Laplace 圧力跳び．
  \item \textbf{§~\ref{sec:verify_solver}（ソルバと時間発展）：}
        PPE 欠陥補正・TVD-RK3・AB2．
\end{enumerate}
§~\ref{sec:verify_summary} に結果を総括する．

\input{sections/11a_spatial}               %% §11.1 空間離散化の精度検証
\input{sections/11d_interface}             %% §11.2 界面処理の精度検証
\input{sections/11f_solver_time}           %% §11.3 ソルバと時間発展の精度検証
\input{sections/11h_verification_summary}  %% §11.4 検証結果のまとめ
