...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\linewidth]{figures/ppe_convergence}
\caption{PPE 格子収束：欠陥補正 $k=3$（等密度・変密度）．
  等密度では $\Ord{h^6}$ 収束を達成する．
  変密度（$\rho_l/\rho_g=1000$）では平滑密度場で $\Ord{h^5}$，界面型密度場では精度が制限される
  （§~\ref{sec:verify_ppe_varrho} 参照）．}
\label{fig:ppe_convergence}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.62\linewidth]{figures/tvd_rk3_convergence}
\caption{TVD-RK3 の ODE 時間精度（$dq/dt=-q$，$T=1$）．
  $n \geq 16$ で $\Ord{\Delta t^3}$ の正確な収束を示す．}
\label{fig:tvd_rk3_convergence}
\end{figure}
...
% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{DC 反復ソルバの収束解析：ω 緩和とスペクトル半径}
\label{sec:verify_dc_spectral}
% ─────────────────────────────────────────────────────────────────────────────

\noindent\textbf{目的：}
欠陥補正法 Step~2 の求解方法（ADI スイープ vs 直接 LU）による収束挙動の差異を実験的に検証し，
付録~\ref{app:dc_convergence_theory} のスペクトル解析を実験で確認する．

\paragraph{テスト DC-2：Thomas ADI スイープの収束限界．}

2次元 PPE（Neumann BC，製造解 $p^* = \cos(\pi x)\cos(\pi y)$，$N=32,64$）に対し，
Step~2 を Thomas ADI 方向分離スイープ
$[\frac{1}{\Delta\tau}\mathbf{I} - L_{L,x}][\frac{1}{\Delta\tau}\mathbf{I} - L_{L,y}]\,\delta p = d$
で求解した DC 反復を実行した（$c_\tau = 2$，密度比 $\rho_l/\rho_g = 1, 2, 5, 10, 100$）．

\noindent\textbf{結果：} 全条件・全密度比で残差が単調増大（発散）し，300 反復以内に収束しなかった．
スペクトル解析（付録~\ref{app:dc_convergence_theory}）から，
2次元 ADI の反復行列固有値が滑らかモード $(k_x, k_y) = (\pi/N, \pi/N)$ で
\begin{equation}
  \mu_{\mathrm{ADI}} \approx 1 + \lambda_H\,\Delta\tau^2 = 1 + \mathcal{O}(h^4)
\end{equation}
となり，収束に $\mathcal{O}(h^{-4}) = \mathcal{O}(N^4)$ 反復が必要となることが理論的に示された
（$N=32$ で $\approx 10^6$ 反復）．CCD-ADI スイープでも同様の ADI ボトルネックが生じ，
FD-ADI より遅くなることを確認した（実験 exp10\_17）．

\paragraph{テスト DC-3：DC + 直接 LU の $\omega$—密度比収束マップ（exp10\_18）．}

Step~2 に直接 LU（\texttt{spsolve}）を用い，
$\omega \in \{0.1, 0.2, 0.3, 0.5, 0.7, 0.83\}$，
$\rho_l/\rho_g \in \{1, 2, 5, 10, 20, 50, 100, 200, 500, 1000\}$，
$N = 32, 64$ の全組合せで DC 反復を実行した（Neumann BC，tol $= 10^{-8}$，maxiter $= 150$）．

\begin{table}[htbp]
\centering
\small
\caption{テスト DC-3：DC + 直接 LU の $\omega$---密度比収束マップ
（✓ = 収束，\textasciitilde{} = 停滞，✗ = 発散 / maxiter 到達）}
\label{tab:dc_omega_map}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{lcccccc|cccccc}
\toprule
& \multicolumn{6}{c|}{$N = 32$} & \multicolumn{6}{c}{$N = 64$} \\
$\rho_l/\rho_g$ & $\omega=0.1$ & $0.2$ & $0.3$ & $0.5$ & $0.7$ & $0.83$
               & $0.1$ & $0.2$ & $0.3$ & $0.5$ & $0.7$ & $0.83$ \\
\midrule
$1$    & \textasciitilde & ✓ & ✓ & ✓ & ✓ & \textasciitilde
...
\noindent\textbf{考察：}
\begin{itemize}[noitemsep]
  \item \textbf{$\rho = 1$（等密度）：} $\omega \in \{0.2, 0.3, 0.5, 0.7\}$ で収束（22--115 反復）．
        収束率は理論予測（Nyquist 減衰 $|1 - \omega \times 2.4|$）と一致する．
        $\omega = 0.83$ は Nyquist モード減衰率 $0.992 \approx 1$ で停滞し，
        $\omega_{\max} = 0.833$ の理論値を確認した．
        $\omega = 0.1$ は $N=32$ の CCD 離散化誤差床（$\approx 3\times10^{-8}$）に到達前に
        maxiter に達する（平滑モード収束率 $0.9$/反復 → $\approx 230$ 反復必要）．
  \item \textbf{$\rho = 2$，$N = 64$：} $\omega \in \{0.3, 0.5, 0.7\}$ で収束（23--74 反復）．
        界面幅 $3h$ が細くなり CCD/FD 離散化不整合床が tol 以下に低下する．
  \item \textbf{$\rho_l/\rho_g \ge 5$ または $N=32$, $\rho \ge 2$：} 停滞（\textasciitilde）．
        界面近傍で CCD（$\Ord{h^6}$）と FD（$\Ord{h^2}$）の $\nabla\rho$ 評価差異が
        残差床 $\propto (\rho_l/\rho_g)\cdot h^2$ を形成し，tol $= 10^{-8}$ に到達しない．
        高密度比・鋭い界面には Krylov 法（GMRES + FD 前処理）が必要となる．
\end{itemize}

図~\ref{fig:dc_omega_convergence_map} に $N=32, 64$ の収束マップと代表的な残差履歴を示す．

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.48\linewidth]{%
    ../experiment/ch10/results/18_omega_dc_lu/omega_convergence_map_N32}
  \includegraphics[width=0.48\linewidth]{%
    ../experiment/ch10/results/18_omega_dc_lu/omega_convergence_map_N64}
  \caption{テスト DC-3：$\omega$—密度比収束マップ（左：$N=32$，右：$N=64$）．
    ヒートマップは収束反復数，オレンジ \textasciitilde{} は停滞（界面 CCD/FD 不整合床），
    赤 ✗ は発散を示す．}
  \label{fig:dc_omega_convergence_map}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.62\linewidth]{figures/ab2_ode_convergence}
\caption{AB2 の ODE 時間精度（$dq/dt=-q$，$T=1$，初回 Euler）．
  $n \geq 16$ で $\Ord{\Delta t^2}$ 収束を示す．TVD-RK3（図~\ref{fig:tvd_rk3_convergence}）と同一テスト問題．}
\label{fig:ab2_ode_convergence}
\end{figure}
