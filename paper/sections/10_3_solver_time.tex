...
\paragraph{注意：界面型密度場における収束性の破綻と解決方針}
\label{sec:varrho_ppe_limitation}

上記の収束結果は\textbf{滑らかな密度場}（$\rho = 1 + A\sin\pi x\cos\pi y$）における検証であり，
\textbf{界面型密度場}（$\rho = \rho_g + (\rho_l - \rho_g) H_\varepsilon(\phi)$）では状況が根本的に異なる．
この破綻は\textbf{想定内}である：
smoothed Heaviside で $1/\rho$ が $\varepsilon = \Ord{h}$ の幅で $\rho_l/\rho_g$ 倍ジャンプする以上，
FD/CCD を問わず界面を横切るステンシルが $\Ord{1}$ 誤差を生じることは
不連続係数理論から予見される．
...
\noindent\textbf{解決方針：GFM による分相解法}
この問題は§~\ref{sec:extension_pde}の最近接点 Hermite 補間に着想した
Ghost Fluid Method（GFM, §~\ref{sec:gfm_jump}）により解決が見込まれる．
GFM では各相で定数密度 PPE（$\rho_k^{-1}\nabla^2 p = q$）を解き，
界面上のジャンプ条件 $[p] = \sigma\kappa$ を Hermite 延長で課す．
これにより不連続係数を横切るステンシルが排除され，
各相の CCD 離散化が $\Ord{h^6}$ の設計精度を回復する．
GFM の実装と検証は今後の主要課題であり，§~\ref{sec:conclusion}で議論する．
