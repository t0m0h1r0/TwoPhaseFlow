...
ゲージ固定（1点 $p_{0,0} = 0$ の固定）を要する．
本テストでは Neumann BC + ゲージ固定のもとで欠陥補正法が
$\Ord{h^6}$ 精度を維持することを確認する．

\noindent\textbf{評価対象：}
欠陥補正法（$k=3$），全周 Neumann BC，ゲージ固定 $p_{0,0}=0$．
...
\midrule
$8$   & $1/8$   & \textit{（実験値 TBD）} & ---  \\
$16$  & $1/16$  & \textit{（実験値 TBD）} & \textit{TBD} \\
$32$  & $1/32$  & \textit{（実験値 TBD）} & \textit{TBD} \\
$64$  & $1/64$  & \textit{（実験値 TBD）} & \textit{TBD} \\
$128$ & $1/128$ & \textit{（実験値 TBD）} & \textit{TBD} \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{期待される結論：}
Neumann BC でもディリクレテスト（表~\ref{tab:dc_iteration_accuracy}）と同等の
$\Ord{h^6}$ 以上の収束が得られること．
境界スキーム（$\Ord{h^5}$）の影響が顕在化し，
ディリクレテストの超収束 $\Ord{h^7}$ よりも低い次数となる可能性がある．
...
\noindent\textbf{評価対象：}
変密度欠陥補正法（$k=3$），密度比 $\rho_l/\rho_g = 1, 10, 100, 1000$．

\noindent\textbf{手順とパラメタ：}
\begin{itemize}[noitemsep]
  \item 2次元変密度 Poisson 方程式，製造解 $p^* = \sin(\pi x)\sin(\pi y)$
  \item 密度場 $\rho(x,y) = \rho_g + (\rho_l - \rho_g)\,H_\varepsilon(\phi_0)$，
        $\phi_0 = \sqrt{(x-0.5)^2+(y-0.5)^2} - 0.25$（円形界面）
  \item 右辺 $q = \bnabla\cdot(1/\rho\,\bnabla p^*)$ を解析的に構成
  \item 格子 $N = 16, 32, 64, 128$（一様），ディリクレ BC
  \item 各密度比で $L^\infty$ 誤差・収束次数・欠陥補正反復回数を計測
\end{itemize}
...
\begin{tabular}{rcccc}
\toprule
$N$ & $\rho_l/\rho_g = 1$ & $= 10$ & $= 100$ & $= 1000$ \\
\midrule
$16$  & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
$32$  & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
$64$  & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
$128$ & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
\midrule
次数  & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{期待される結論：}
$\rho_l/\rho_g = 1$ では等密度テスト（表~\ref{tab:dc_iteration_accuracy}）と一致する．
密度比の増大とともに条件数が悪化し，収束に必要な反復回数 $k$ が増加するが，
$k$ を十分に取れば（$k \geq 5$）$\Ord{h^6}$ 精度は維持される見込みである．
$\rho_l/\rho_g = 1000$ では仮想時間刻み $\Delta\tau$ の調整
（§\ref{sec:ppe_bc_stability}）が必要となる可能性がある．
...
ステップ数 $n$ & $\Delta t$ & $|q - e^{-1}|$ & 次数 \\
\midrule
  16 & $6.25 \times 10^{-2}$ & \textit{（実験値 TBD）}  & ---  \\
  32 & $3.13 \times 10^{-2}$ & \textit{（実験値 TBD）}  & \textit{TBD} \\
  64 & $1.56 \times 10^{-2}$ & \textit{（実験値 TBD）}  & \textit{TBD} \\
 128 & $7.81 \times 10^{-3}$ & \textit{（実験値 TBD）}  & \textit{TBD} \\
 256 & $3.91 \times 10^{-3}$ & \textit{（実験値 TBD）}  & \textit{TBD} \\
 512 & $1.95 \times 10^{-3}$ & \textit{（実験値 TBD）}  & \textit{TBD} \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{期待される結論：}
$n \geq 16$ で $p \approx 2.00$ の 2 次収束を示す．
初回 Euler ステップの $\Ord{\Delta t}$ 寄与は寄生根減衰（$|\rho_2|=0.5$）により
