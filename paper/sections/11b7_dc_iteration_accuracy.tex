...
\begin{itemize}
  ...
  \item \textbf{$k \geq 3$：} 3回以上の反復で CCD 演算子 $L_H$ の精度が完全に発現する．
        実測収束次数は $p \approx 7.0$ であり，理論的な $\Ord{h^6}$ を上回る．
        これはディリクレ BC の解析解 $\sin(\pi x)\sin(\pi y)$ が境界上で恒等的にゼロであるため
        境界スキーム（$\Ord{h^5}$）の誤差寄与が消滅する超収束現象であり，
        ディリクレ解析解 $\sin\pi x\sin\pi y$ の性質による．
        $k=3, 5, 10$ で精度次数は同一であり，
        誤差の絶対値のみが微減する（$k=3$ と $k=10$ で約 $2$ 倍の差）．
\end{itemize}

以上の結果から，欠陥補正法は \textbf{わずか $k=3$ 回の反復で $\Ord{h^6}$ 以上の空間精度に到達}し，
CCD 演算子の空間精度を損なわないことが確認された．
