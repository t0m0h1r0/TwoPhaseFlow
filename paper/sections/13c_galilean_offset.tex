...
一様速度 offset を加えた run から $\bU$ を差し引いた摂動場が，
...
残差スケールを測る縮約テストである．実装上は界面を $\bU t$ だけ移流せず，
...
\PaperCaptionNote{$\|\bU\|$ で正規化した残差は $O(1)$ scale に収まる．本縮約テストでは厳密ゼロ残差は script 構造上達成不能（後段「誤差源の分解」参照）．}
\centering\small
\begin{tabular}{l r r}
\toprule
集計量 & 絶対値 & $\|\bU\|$ 正規化 \\
...
$\|\Delta u\|_\infty/\|\bU\|$ が $O(1)$ scale に留まることを判定する．
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item 本検証は「固定 Eulerian 格子 + 参照点固定 PPE の制約下で
        Galilean offset 残差が設計通り $O(\|\bU\|)$ に収まる」ことを示す；
