...
%
% V4: 固定壁系の uniform-offset 残差
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{固定壁系の uniform-offset 残差}
...
\begin{table}[!ht]
\caption{V4：固定壁系 uniform-offset 残差}
...
\caption{V4：固定壁系の uniform-offset 残差}
\label{fig:V4_galilean_offset}
\PaperCaptionNote{一様速度 offset 差 $\|\Delta u\|_\infty(t)$ 履歴（offset を差し引いた後）．}
\end{figure}
...
\begin{itemize}\setlength{\itemsep}{0pt}
  \item Galilean 不変性は連続方程式レベルでは厳密だが，
        ...
        $O(\|\bU\|)$ 残差が原理的に避けられない．
  \item \textbf{Type-A criterion 合格 (\checkmark)}：
  ...
  \item 本検証は「固定 Eulerian 格子 + 参照点固定 PPE の制約下で
        uniform-offset 残差が設計通り $O(\|\bU\|)$ に収まる」ことを示す；
