...
本稿の標準 pressure-jump 経路では CSF 体積力としては入れず，
$j_{gl}=p_g-p_l=-\sigma\kappa_{lg}$ から作る毛管 face cochain を
第 3 段階の PPE/corrector に組み込む
...
分相 affine-jump PPE と欠陥補正法（§~\ref{sec:defect_correction_ppe}）で
同じ active face complex $D_fG_A$ の固定点を定める．
...
$D_h(A_f G_h p)$ を FVM 的に積分するのに対し，節点 CCD の速度補正は
...
$B^{\mathrm{adj}}(j_{gl})$ を face-average 勾配と同じ局所物理距離 $H_f$ で定義する．
