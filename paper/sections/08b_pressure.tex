...
射影段の圧力項だけを分離した離散 NS 方程式（無次元形）は：
\begin{equation}
...
\begin{equation}
  \rho^{n+1}\frac{\bu^* - \bu^n}{\Delta t_{\mathrm{proj}}} = \bm{R}^{n+1} - \bnabla p^n
  \quad\Longrightarrow\quad
  \bu^* = \bu^n + \frac{\Delta t_{\mathrm{proj}}}{\rho^{n+1}}\bigl(\bm{R}^{n+1} - \bnabla p^n\bigr)
  \label{eq:predictor}
\end{equation}
...
数値的な切り分けと安定化効果は §~\ref{sec:gadjacent_verification} で検証する．
