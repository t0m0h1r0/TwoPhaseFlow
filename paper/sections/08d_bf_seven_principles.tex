...
\textbf{特に P-5 の位置づけは曲率閉包に依存}：曲率処理が面共鎖の位置を変える場合は
...
% -------------------------------------------------------------------------------
\subsubsection{\texorpdfstring{P-1：$G_h p$ と毛管面共鎖の同一位置評価}{P-1：Gh p と毛管面共鎖の同一位置評価}}
...
節点 CCD 勾配を面に補間し，毛管面共鎖を別の補間で構築する
...
% -------------------------------------------------------------------------------
\subsubsection{\texorpdfstring{P-3：毛管面共鎖を $[p]_\Gamma$ と同じ離散幾何に通す}{P-3：毛管面共鎖を圧力ジャンプと同じ離散幾何に通す}}
...
圧力ジャンプ標準経路では同じ考え方を
$c_f(j_{gl})=A_f B_f(j_{gl})$ として面共鎖に移し，
...
毛管ジャンプ $c_f(j_{gl})$ & $G_h^\text{bf}$ と同じ面複体 & 値域整合 \\
...
\noindent P-5（曲率 $\kappa_{lg}$ の非 CCD 化）と P-3（毛管面共鎖を $p$ と同幾何）は
