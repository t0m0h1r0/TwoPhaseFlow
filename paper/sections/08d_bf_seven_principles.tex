...
\textbf{特に P-5 の位置づけは適用形態に依存}：対称フィルタ適用時は
...
\textbf{コロケート配置の面演算子 FCCD（\S\ref{sec:fccd_def}）で構成する場合}，
...
\begin{equation}
  D_h^\text{bf}\Bigl(\betaf\,G_h^\text{bf}\,p\Bigr)
    = \frac{1}{\Delta t_{\mathrm{proj}}}\,D_h^\text{bf}\,\bm{u}^*,
...
\begin{enumerate}[leftmargin=2em,noitemsep]
  ...
  \item 界面張力正規化 $f_\sigma \propto \betaf\,\sigma\kappa_{lg}\,(\bnabla\psi)_f$（CSF
        標準式 $\sigma\kappa_{lg}\bnabla\psi$ に $\betaf$ 因子を付加した\textbf{変密度
        補正段離散化の特殊形}；等密度では $\betaf$ が定数で標準式に還元する）．
\end{enumerate}
...
$\kappa_{lg}$ は\textbf{界面帯に制限した平滑化}または
...
\begin{itemize}[noitemsep]
  \item \textbf{GFM 面フラックス補正}（標準 pressure-jump 閉包）：
  \begin{equation}
    (G_h^\text{bf} p)_f^\text{corrected}
      = (G_h^\text{bf} p)_f - \frac{j_{gl}}{H_f},
    \label{eq:bf_p6_gfm}
  \end{equation}
  バルク側ステンシルは変更せず，ジャンプだけを面フラックスに注入．
  2 次精度；標準 pressure-jump 閉包の面整合条件に十分．
  \item \textbf{IIM コンパクト行補正}（高次 pressure-jump 閉包）：
...
PPE + 補正 & $G_h^\text{bf},\,D_h^\text{bf}$（面随伴ペア）& 内部整合（$\Ord{h^2}$--$\Ord{h^4}$） \\
界面張力 $f_\sigma$ & $G_h^\text{bf}$ と同じ面スロット & 内部整合 \\
...
\noindent P-5（曲率 $\kappa_{lg}$ の非 CCD 化）と P-3（$f_\sigma$ を $p$ と同幾何）は
