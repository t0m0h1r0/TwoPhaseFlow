...
\begin{table}[htbp]
\centering
\caption{CLS 界面移流テストの精度（DCCD $\varepsilon_d = 0.05$，界面重み付き質量補正あり）．
\textsuperscript{\dag}単一渦テストは適応的再初期化
（$\theta_{\mathrm{reinit}} = 1.10$，§~\ref{sec:adaptive_reinit}）を使用（再初期化 2--3 回）；
Zalesak テストは鋭角形状の維持のため固定頻度（20 ステップ毎）を使用}
...
\midrule
\multirow{3}{*}{Zalesak}
 &  64 & $9.96 \times 10^{-2}$ & $5.31 \times 10^{-1}$ & $\Ord{10^{-15}}$ \\
 & 128 & $9.34 \times 10^{-2}$ & $5.53 \times 10^{-1}$ & $\Ord{10^{-15}}$ \\
 & 256 & $7.62 \times 10^{-2}$ & $5.62 \times 10^{-1}$ & $\Ord{10^{-15}}$ \\
\midrule
\multirow{3}{*}{Single vortex\textsuperscript{\dag}}
 &  64 & $1.53 \times 10^{-1}$ & $7.80 \times 10^{-1}$ & $\Ord{10^{-15}}$ \\
 & 128 & $8.90 \times 10^{-2}$ & $5.43 \times 10^{-1}$ & $\Ord{10^{-15}}$ \\
 & 256 & $7.49 \times 10^{-2}$ & $4.72 \times 10^{-1}$ & $\Ord{10^{-15}}$ \\
\bottomrule
\end{tabular}
\end{table}
...
適応的再初期化（§~\ref{sec:adaptive_reinit}）により再初期化回数を 2--3 回に限定することで，
固定頻度（10 ステップ毎，約 227 回）と比較して $L_2$ 誤差が約 49\% 改善された
（§~\ref{sec:verify_dccd_conservation}）．
残存する $L_\infty$ 誤差（$N = 256$ で 0.47）は，CLS 場の急峻な勾配が
渦の引き延ばしにより格子解像限界を超えることに起因する．
破線は初期界面位置を表す．
...
（表~\ref{tab:cls_advection}，§~\ref{sec:dccd_mass_conservation}）．

\noindent\textbf{結論：}
質量保存誤差は界面重み付き補正（Olsson--Kreiss 基底）により $\Ord{10^{-15}}$ に到達した．
$L_2$ 形状誤差は格子精緻化に伴い緩やかに減少する．
...
形状誤差の支配的要因は\textbf{過剰な再初期化}であり（ベースライン比 49\% の改善），
...
形状誤差の支配的要因は過剰な再初期化（49\%）であり，DCCD 減衰（2\%）ではない．
適応的再初期化（$\theta = 1.10$）への置換を推奨する．
