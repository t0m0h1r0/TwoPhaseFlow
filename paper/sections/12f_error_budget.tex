...
CCD-PPE（$\varrho{=}2$） & --- & $\Ord{h^{1\text{--}2}}$ & $\triangle$ & §~\ref{sec:varrho_grid_convergence} & 変密度圧力場 \\
...
NS 空間（Kovasznay） & $\Ord{h^4}$--$\Ord{h^6}$ & $\Ord{h^{3.96}}$ & \checkmark & §~\ref{sec:kovasznay} & NS 運動量残差 \\
...
毛細管 CFL & $\Delta t_\sigma \propto \Delta x^{3/2}$ & 指数 $= 1.82$ & \checkmark & §~\ref{sec:advection_pressure_coupling} & 時間ステップ制限 \\
\bottomrule
\multicolumn{6}{l}{\footnotesize \textsuperscript{$\dagger$}DCCD 散逸パラメータ $\varepsilon_d$ によらず界面の CLS プロファイル幅が支配する誤差（§~\ref{sec:highre_dccd}）．} \\
