...
交差粘性項（陽的） & $\Ord{\Delta t}$\textsuperscript{$\ddagger$} & $\Ord{\Delta t^{1.0}}$ & §~\ref{sec:verify_summary} & 界面近傍で律速 \\
CCD $D^{(2)}$ スペクトル半径 & $\rho h^2 {\approx} 3.43$ & $3.43$ & §~\ref{sec:verify_summary} & 反復 PPE 解法の制約 \\
...
\multicolumn{5}{l}{\footnotesize \textsuperscript{$\ddagger$}$\mu_l/\mu_g \gg 1$ で陽的交差微分項が $\Ord{\mu_{\max}\Delta t}$ の打切り誤差を生じる（§~\ref{sec:time_integration}）．} \\
\end{tabular}
\end{table}
