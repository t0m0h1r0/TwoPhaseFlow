...
CCD-PPE（$\rho_l/\rho_g{=}2$） & --- & 粗格子: $\Ord{h^{4\text{--}5}}$；$N{>}64$: 収束停止 & $\triangle$ & §~\ref{sec:varrho_grid_convergence} & 変密度圧力場 \\
...
本稿では Balanced-Force 演算子整合と FCCD 面ジェット sub-system により
チェッカーボード不安定性を抑制しており（§~\ref{sec:checkerboard_solution}），
