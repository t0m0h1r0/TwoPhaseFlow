...
§~\ref{sec:csf} の~\ref{par:curvature_stab} で実装クイックスタート（分母クランプ式~\eqref{eq:curvature_clamped}）を提示した．
...
曲率の定義と2次元展開式の導出は §~\ref{sec:curvature}（式~\eqref{eq:curvature_2d}）を参照．ここでは CLS 変数 $\psi$ を用いた曲率評価の適用手順のみを示す．
数学的には厳密であるが，数値計算で直接使用するには注意が必要である．
...
\begin{enumerate}
  ...
  \item \textbf{単体テスト（半径 $R$ の円）：}
        解析値 $\kappa = -1/R$ との誤差 $E_\kappa(h) = \Ord{h^6}$ 収束を
        $N = 16, 32, 64, 128$ で確認する（§~\ref{sec:curvature_circle_example}）．
\end{enumerate}

\medskip
\textbf{代替経路（$\phi$ 経由）}——将来の分相 PPE 解法（§~\ref{sec:split_ppe_motivation}）で
HFE（§~\ref{sec:field_extension}）を使用する場合：
\begin{enumerate}
  \item ロジット逆変換（§~\ref{sec:newton_inversion}）で $\phi^{n+1}$ を取得．
  \item CCD 法で $\phi$ の1次微分を計算し，界面法線 $\hat{\bm{n}} = \bnabla\phi/|\bnabla\phi|$ を求める．
  \item HFE（§~\ref{sec:extension_pde_disc}）により，
        圧力場 $\delta p$ を界面越しに $\Ord{h^6}$ 精度で延長する．
  \item 延長後の滑らかな場に対して CCD $\bnabla(\delta p)$ を適用し，$\Ord{h^6}$ 精度の速度補正を得る．
\end{enumerate}
\end{tcolorbox}

\noindent\textbf{注：HFE について．}
界面 $\Gamma$ では Young--Laplace 則により圧力場に不連続が生じる（§~\ref{sec:csf}）．
分相 PPE 解法では各相の PPE を独立に解くため，圧力場がこの不連続を陽に持つ．
この不連続をそのまま高次差分で微分すると振動誤差が発生するため，
HFE（§~\ref{sec:field_extension}）により圧力場を界面越しに滑らかに延長する．
逆変換で得た $\phi$ は，この延長操作で界面法線 $\hat{\bm{n}} = \bnabla\phi/|\bnabla\phi|$ を
計算するために使用する．
なお，現行の CSF + smoothed Heaviside 一括解法では圧力場が既に滑らかなため
