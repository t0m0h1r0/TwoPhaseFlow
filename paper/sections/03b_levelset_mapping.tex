...
最近接点 Hermite 補間（§~\ref{sec:extension_pde}）による界面越し場延長で $\phi$ が必要な場合は
...
\medskip
\textbf{代替経路（$\phi$ 経由）}——界面越し Hermite 延長（§~\ref{sec:extension_pde}）で $\phi$ が必要な場合：
\begin{enumerate}
  \item ロジット逆変換（§~\ref{sec:newton_inversion}）で $\phi^{n+1}$ を取得．
  \item CCD 法で $\phi$ の1次微分を計算し，界面法線 $\hat{\bm{n}} = \bnabla\phi/|\bnabla\phi|$ を求める．
  \item 最近接点 Hermite 補間（§~\ref{sec:extension_pde_disc}）により，
        圧力場 $\delta p$ を界面越しに $\Ord{h^6}$ 精度で延長する．
  \item 延長後の滑らかな場に対して CCD $\bnabla(\delta p)$ を適用し，$\Ord{h^6}$ 精度の速度補正を得る．
\end{enumerate}
\end{tcolorbox}

% Extension PDE の定式化・離散化・精度解析は，
% 離散化スキーム（§4 CCD）と圧力 Poisson 方程式（§8）を理解した後に
% §~\ref{sec:extension_pde}（§8.4）で詳述する．
% ここでは「界面越し場延長の必要性」のみ注記する．

\noindent\textbf{注：界面越し場延長について．}
界面 $\Gamma$ では Young--Laplace 則により圧力場に不連続が生じる（§~\ref{sec:csf}）．
この不連続をそのまま高次差分で微分しようとすると振動誤差が発生するため，
本稿では圧力場を界面越しに滑らかに延長してから微分を行う
（最近接点 Hermite 補間；§~\ref{sec:extension_pde}）．
