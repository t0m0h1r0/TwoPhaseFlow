...
%
% §6.2  FCCD 移流：構成 B/C と面ジェット共通基盤量 (CHK-223: §6 per-term spatial 配下)
%        既存の節点 CCD 移流を保存型に拡張する FCCD 移流演算子群．
%        §4.5 FCCD の面中心行列 M^FCCD を速度移流に転用する経路を示す．
%
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{FCCD 面移流演算子：面評価位置整合と参照再構成}
...
Balanced--Force の同一面評価位置原理（§~\ref{sec:balanced_force}）と整合する形で
記述するための演算子前提である．本稿の標準領域内部運動量移流は
UCCD6（§~\ref{sec:uccd6_def}）であり，FCCD 面フラックス形は
一括 CSF 一流体経路で圧力・毛管・対流フラックスを同一の面から節点への発散で閉じる場合，
または標準圧力ジャンプ経路で毛管補正量と動的対流フラックスの評価位置を比較する場合の
面評価位置構成として用いる．純 FCCD DNS 構成での役割は §~\ref{sec:pure_fccd_dns} で境界づける．
...
Balanced--Force 原理（\S\ref{sec:balanced_force}）を一括 CSF 一流体経路で読む場合に現れる
「$\bnabla p,\,\sigma\kappa_{lg}\bnabla\psi,\,\bnabla\cdot(\bu\otimes\bu)$
\textbf{3 項すべてが同一の面中心演算子}で離散化される」条件を完結させる．
標準圧力ジャンプ経路では，この条件を毛管補正量と標準の面補正の同一評価位置条件として読み替える．
節点版の運動量移流を圧力・毛管面評価位置と混在させると，
\textbf{移流だけが節点中心に残る}ため，静的 BF 条件は保てても
動的右辺に節点--面の往復誤差が残る．
本節では 2 つの役割を区別する：
\begin{itemize}
  \item \textbf{節点再構成形（構成 C）}：面中心 FCCD 勾配を R$_4$
        Hermite 再構成で節点に戻す参照構成．節点中心右辺と比較するときの
        精度分解に用いる．
  \item \textbf{保存形面フラックス（構成 B）}：
        $\bnabla\cdot(\bu\otimes\bu)$ を FCCD 面値 $\mathbf{P}_f$ で
        組み立て，面ダイバージェンスで節点に戻す．動的 BF 評価位置整合を
        最も直接に満たす構成である．
\end{itemize}

% -------------------------------------------------------------------------------
\subsubsection{\texorpdfstring{共通基盤量：面値演算子 $\mathbf{P}_f$ と面ジェット $\mathcal{J}_f$}{共通基盤量：面値演算子 P\_f と面ジェット J\_f}}
...
統一基盤量 $\FaceJet{u} = (u_f, u'_f, u''_f)$ は
\begin{equation}
...
% -------------------------------------------------------------------------------
\subsubsection{\texorpdfstring{構成 C：R$_4$ Hermite 面→節点再構成}{構成 C：R\_4 Hermite 面→節点再構成}}
...
移流項は従来通り節点ごとの積で組み立てる：
\begin{equation}
  [\mathcal{A}^{(j)}]_i = -\sum_k u_k(x_i)\cdot(\partial_{x_k} u_j)^{\text{node-FCCD}}_i.
  \label{eq:fccd_adv_option_c}
\end{equation}

\noindent\textbf{構成 C の役割}：
出力が節点値なので，節点中心右辺を保ったまま
FCCD 面勾配の影響だけを切り出して比較できる．
したがって構成 C は節点中心経路との精度差を説明する参照構成であり，
標準の二相面上補正を置き換えるものではない．

\noindent\textbf{構成 C の限界}：
R$_4$ の面→節点変換で BF 整合性が\textbf{部分的に}回復するに留まる．
一括 CSF 一流体経路の圧力・毛管項が面中心 FCCD で動く一方，移流だけは R$_4$ 経由の
節点に戻っているため，節点-面-節点の往復で
$\mathcal{O}(H^4)$ 誤差が 2 重に蓄積する．
面評価位置整合を主張する箇所では構成 B を用いる．

% -------------------------------------------------------------------------------
\subsubsection{構成 B：保存形 FCCD 面フラックス}
...
非線形積の補間誤差が $\Ord{H^2}$ 下限に落ちるため採用しない．
...
ここで CCD 面評価位置 $\mathbf{P}_f$ と面勾配 $\mathbf{M}^{\FCCD}$ が
...
\begin{theorem}[一括 CSF 一流体経路での FCCD 面評価位置静的 BF 整合性]\label{thm:fccd_adv_bf}
...
CSF；§\ref{sec:projection_derivation}）の設定．分相圧力ジャンプ PPE 版
（§\ref{sec:split_ppe_pressure_jump_formulation}）は別建ての
系~\ref{cor:fccd_split_ppe_bf}．\\
\textbf{論証}：$\bu\equiv 0$ では対流項は恒等的に零であるため，
静的 BF 残差は圧力勾配と一括 CSF 毛管項の演算子一致だけで決まる．
残る $\bnabla p$ と一括 CSF 毛管項は (A1) より同一の
...
一意に定まることから $G_h p$ の評価が有効．$\square$
\end{proof}

\begin{corollary}[分相圧力ジャンプ PPE での FCCD-BF 整合]
\label{cor:fccd_split_ppe_bf}
分相圧力ジャンプ PPE（§\ref{sec:split_ppe_framework}）を採用した場合，
CSF 体積力は予測段階から外れ，毛管効果は
$J_p^\chi=-\sigma\kappa_{lg}$ から作る面上の圧力ジャンプ量として PPE/補正段に入る．
...
表面エネルギーの Riesz 表現と成分体積制約付き鞍点系から得る
$\widehat c_{\sigma,f}$ を標準の面補正へ渡すことで成立する．
節点延長圧力の未補正勾配を速度補正へ直接渡す経路はこの系の仮定ではない．
\end{corollary}

\noindent\textbf{動的移流評価位置命題}：
非零速度場では，対流フラックスも圧力・毛管補正量と同じ面評価位置に置かれて初めて
運動量右辺全体が同一の面中心空間で閉じる．
構成 C（R$_4$ 経由の節点中心）は節点--面--節点の往復を残すため，
静的 BF 定理の直接の仮定ではないが，動的計算では評価位置不一致の残差源になり得る．
構成 B は $\bnabla\cdot(\bu\otimes\bu)$ を面フラックスとして構成し，
一括 CSF 一流体経路では圧力・毛管・対流の 3 項を，標準圧力ジャンプ経路では
毛管補正量と対流面フラックスの比較基準を，同一の面から節点への発散で読むための設計である．

\begin{tcolorbox}[defbox, title={保存形共通フラックスとしての運動量移流}]
水--空気級の移動界面では，速度場の滑らかさだけで
運動量移流形式を選んではならない．
界面移流が $q=\psi$ を面フラックス $F_q$ で運ぶなら，
密度と運動量は同じ面・同じ段階・同じ発散で
\[
  F_m=\rho_gF_V+(\rho_l-\rho_g)F_q,\qquad
  F_{\bm{p}}=F_m\bu_{\mathrm{up}}
\]
として運ぶ．
この構成では，FCCD は界面面フラックス台帳を作る役割を担い，
UCCD6 は保存形運動量移流の後に再び
$\bu\cdot\bnabla\bu$ を評価する役割を持たない．
保存形運動量式と質量式を組み合わせたときに primitive 形式が得られるのであって，
保存形移流と primitive 移流を足し合わせるのではないからである．

有界性が必要な場合も，端点の
$q\leftarrow\operatorname{clip}_{[0,1]}q$ を単独で行うと，
密度と運動量が同じ写像を受けたことを証明できない．
許される補正は，記録される面フラックス自体を
\[
  F_q=F_{\mathrm{low}}+\alpha(F_{\mathrm{high}}-F_{\mathrm{low}}),
  \qquad 0\le\alpha\le1
\]
へ置き換える flux-corrected transport 型の制限である．
このとき $F_m$ と $F_{\bm{p}}$ は制限後の $F_q$ から作るため，
共通フラックスの同一性は保たれる．
ただしこの証明は，低次 donor フラックス
$F_{\mathrm{low}}$ が現在の CFL と境界条件のもとで
$0\le q\le1$ を保つことを前提とする．
この前提が破れた場合は，端点 clipping で続行するのではなく，
時間刻み棄却または fail-close とする．
\end{tcolorbox}
...
で定義された粘性演算子．この式は一括 CSF 一流体経路の面評価位置を示す代表形であり，
右辺は粘性・圧力・一括 CSF 毛管項・重力を\textbf{面中心}で評価し
面ダイバージェンスで節点に戻す．
PPE 右辺は自然に $\bnabla_h\cdot\bu^*$ が FCCD 面ダイバージェンスで
表現され，ループが閉じる．

% -------------------------------------------------------------------------------
\subsubsection{壁境界条件構成 IV：Dirichlet 無滑り}
\label{sec:fccd_adv_wall_iv}
% -------------------------------------------------------------------------------

\S\ref{sec:ccd_bc} で導入した構成 III（Neumann $\psi, p$）は
移流にも同じ構成を用いる．
Dirichlet 無滑り $u(x_\text{wall}) = 0$ に対する
\textbf{構成 IV（Dirichlet 壁境界）}は，
...
\begin{itemize}
  \item 標準領域内部運動量移流は UCCD6（§\ref{sec:uccd6_def}）で担う．
  \item 一括 CSF 一流体経路の BF 説明では，共通面評価位置として
        圧力・毛管・対流フラックスの整合条件を本節の構成 B で表現できる．
  \item 分相圧力ジャンプ経路では，毛管効果は CSF 体積力ではなく
        §\ref{sec:split_ppe_pressure_jump_formulation} の面上の圧力ジャンプ量として入り，
        表面エネルギーの Riesz 表現を成分体積制約付き鞍点系で補正してから
        標準の面補正へ渡る．
  \item 構成 C は節点中心右辺と面中心右辺の差を切り分ける参照構成であり，
        標準の面上補正の代替ではない．
\end{itemize}
