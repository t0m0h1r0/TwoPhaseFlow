...
pressure/CSF/convective flux を同一 face-to-node divergence で閉じる
発展的な面ロカス構成，および §~\ref{sec:pure_fccd_dns} の純 FCCD DNS 構成で用いる．
...
節点版の運動量移流を pressure/CSF 面ロカスと混在させると，
...
\begin{theorem}[FCCD 面ロカスの静的 BF 整合性]\label{thm:fccd_adv_bf}
...
\begin{itemize}
  ...
  \item 一括 CSF 経路の BF 説明では，共通面ロカスとして
        pressure/CSF/convective flux の整合条件を本節の Option B で表現できる．
