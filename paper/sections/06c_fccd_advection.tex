...
%
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{FCCD 面移流演算子：面評価位置閉包と参照再構成}
...
縮約 CSF 経路で圧力・毛管・対流フラックスを同一の面から節点への発散で閉じる場合，
...
標準圧力ジャンプ経路では，この条件を毛管補正量と正準面補正の同一評価位置条件として読み替える．
...
\begin{theorem}[縮約 CSF 経路での FCCD 面評価位置静的 BF 整合性]\label{thm:fccd_adv_bf}
...
$\widehat c_{\sigma,f}$ を正準面補正へ渡すことで成立する．
...
縮約 CSF 経路では圧力・毛管・対流の 3 項を，標準圧力ジャンプ経路では
...
で定義された粘性演算子．この式は縮約 CSF 経路の面評価位置を示す代表形であり，
...
\begin{itemize}
  ...
  \item 一括 CSF 縮約経路の BF 説明では，共通面評価位置として
