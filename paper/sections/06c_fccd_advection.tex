...
保存形運動量式と質量式を組み合わせたときに速度主変数形式が得られるのであって，
保存形移流と速度主変数移流を足し合わせるのではないからである．

有界性が必要な場合も，端点の
$q\leftarrow\Pi_{[0,1]}q$ を単独で行うと，
...
CCD ブロック三重対角解で得た基盤量を複数の面演算子で共有できる．
この性質により，FCCD は別々の補間・勾配・曲率評価を寄せ集めるのではなく，
同一の面評価位置に基づく一つの演算子族として読める．
