...
「$\bnabla p,\,\sigma\kappa_{lg}\bnabla\psi,\,\bnabla\cdot(\bm{u}\otimes\bm{u})$
...
\begin{itemize}
  ...
  \item \textbf{Option B（面フラックス）}：保存形
        $\bnabla\cdot(\bm{u}\otimes\bm{u})$ を FCCD 面値 $\mathbf{P}_f$ で
...
CSF $\sigma\kappa_{lg}\bnabla\psi$ / Rhie--Chow がすべて無変更．
...
運動量の自然形は $\bnabla\cdot(\bm{u}\otimes\bm{u})$（保存形）である．
非保存形 $(\bm{u}\cdot\bnabla)\bm{u}$ は $\bnabla\cdot\bm{u}\neq 0$
...
$\bnabla p$（FCCD）と $\sigma\kappa_{lg}\bnabla\psi$（FCCD 面形）を
\textbf{共通の面中心演算子}
$\mathbf{M}^{\FCCD}$（\S\ref{sec:fccd_def} 式~\eqref{eq:fccd_composite_matrix}）で
離散化すれば，静止状態 $\bm{u}\equiv 0$ での離散運動量バランスの
BF 残差は
\begin{equation*}
...
\textbf{論証}：$\bm{u}\equiv 0$ では対流項は恒等的に零であるため，
静的 BF 残差は圧力勾配と CSF の演算子一致だけで決まる．
残る $\bnabla p$ と CSF は (A1) より同一の
...
\begin{equation}
  ...
  + \bigl[\mathcal{V}(\bm{u}) - \bnabla p + \sigma\kappa_{lg}\bnabla\psi + \rho\bm{g}\bigr]_i,
  \label{eq:fccd_adv_closure}
\end{equation}
ここで $\mathcal{V}(\bm{u})=\bnabla\cdot[\mu(\bnabla\bm{u}+\bnabla\bm{u}^T)]$ は
§\ref{sec:onefluid}（§2 NS 既定形）と §\ref{sec:viscous_3layer}（§6.3 3 層粘性）
で定義された粘性演算子．右辺は粘性・圧力・CSF・重力すべて\textbf{面中心}で評価し
node\_divergence で節点に戻す．
PPE RHS は自然に $\bnabla_h\cdot\bm{u}^*$ が FCCD 面ダイバージェンスで
表現され，ループが閉じる．
