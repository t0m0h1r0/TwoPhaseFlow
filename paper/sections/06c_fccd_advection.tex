...
または標準圧力ジャンプ経路で毛管面共鎖と動的対流フラックスの評価位置を比較する場合の
...
標準圧力ジャンプ経路では，この条件を毛管面共鎖と正準面補正の同一評価位置条件として読み替える．
...
\begin{itemize}
  \item \textbf{節点再構成形（構成 C）}：面中心 FCCD 勾配を R$_4$
        Hermite 再構成で節点に戻す参照構成．既存の節点右辺と比較するときの
        精度分解に用いる．
...
CSF；§\ref{sec:projection_derivation}）の設定．分相アフィンジャンプ PPE 版
...
\begin{corollary}[分相アフィンジャンプ PPE での FCCD-BF 整合]
\label{cor:fccd_split_ppe_bf}
分相アフィンジャンプ PPE（§\ref{sec:split_ppe_framework}）を採用した場合，
CSF 体積力は予測段階から外れ，毛管効果は
$J_p^\chi=-\sigma\kappa_{lg}$ の面共鎖として PPE/補正段に入る．
...
毛管面共鎖と対流面フラックスの比較基準を，同一の面から節点への発散で読むための設計である．
...
\begin{itemize}
  ...
  \item 分相アフィンジャンプ経路では，毛管効果は CSF 体積力ではなく
        §\ref{sec:split_ppe_pressure_jump_formulation} の面共鎖として入り，
        表面エネルギーの Riesz 面共鎖を成分体積制約付き鞍点系で補正してから
        正準面補正へ渡る．
