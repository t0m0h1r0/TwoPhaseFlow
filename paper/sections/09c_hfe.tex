...
\begin{enumerate}[noitemsep]
  \item \textbf{圧力場の前処理：}
        PPE 求解前に $p^n$ を界面越しに延長し，IPC 予測段階の $-\bnabla p^n$ 項から
        Gibbs 振動を除去する．
  \item \textbf{圧力増分の後処理：}
...
\PaperCaptionNote{左：CSF 表面張力による Laplace ジャンプが CCD ステンシルを跨ぐと Gibbs 振動が発生． 右：ジャンプ補正済みの片側値を気体側へ延長した後は， CCD $\bnabla(\delta p)$ が不連続値を直接跨がずに評価される．}
\end{figure}
