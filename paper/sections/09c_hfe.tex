...
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{Hermite 場延長法（HFE）：高次片側データ構成}
...
ただし §\ref{sec:split_ppe_formulation} の圧力ジャンプ経路では，
圧力履歴と速度補正は節点圧力の界面越し直接微分ではなく，
$A_f(G_fp-B_f(j))$ の面上加速度として保存・適用する．
したがって HFE の役割は，この面上加速度と GFM ジャンプ行を構成するための
...
\begin{itemize}
  ...
  \item GFM ジャンプ行と圧力ジャンプ面上加速度に必要な片側 Hermite データを同じ精度で供給できる
...
\PaperCaptionNote{左：CSF 表面張力による Laplace ジャンプが CCD ステンシルを跨ぐと Gibbs 振動が発生． 右：ジャンプ補正済みの片側値を気体側へ延長した後は，GFM ジャンプ行と面上加速度に必要な片側 Hermite データを不連続値を跨がずに評価できる．}
\end{figure}
...
\begin{enumerate}
  ...
  \item \textbf{面上加速度への集約：}
        圧力ジャンプ標準経路では，延長節点場をそのまま速度補正へ渡さず，
        同じ面・同じ向き・同じ係数で
        $A_f(G_fp-B_f(j))$ を構成して予測圧力履歴と速度補正に用いる．
\end{enumerate}
\textbf{配置の要点}：HFE は界面ジャンプを持つ圧力場を
CCD ステンシルが直接跨がないようにするための高次片側データ構成である．
圧力射影の閉包そのものは，そのデータから作るジャンプ補正付き面上加速度が担う．
