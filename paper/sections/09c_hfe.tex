...
ただし §\ref{sec:split_ppe_formulation} のアフィンジャンプ経路では，
圧力履歴と速度補正は節点圧力の界面越し直接微分ではなく，
$A_f(G_fp-B_f(j))$ の面共鎖として保存・適用する．
したがって HFE の役割は，この面共鎖と GFM ジャンプ行を構成するための
...
\begin{itemize}
  ...
  \item GFM ジャンプ行とアフィンジャンプ面共鎖に必要な片側 Hermite データを同じ精度で供給できる
...
\PaperCaptionNote{左：CSF 表面張力による Laplace ジャンプが CCD ステンシルを跨ぐと Gibbs 振動が発生． 右：ジャンプ補正済みの片側値を気体側へ延長した後は，GFM ジャンプ行と 面共鎖 に必要な片側 Hermite データを不連続値を跨がずに評価できる．}
\end{figure}
