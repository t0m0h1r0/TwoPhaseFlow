...
%   理由：分相 PPE の基盤となる場延長理論であり、本編の重要構成要素
%
%   Aslam (2004) の Extension PDE の定常解を CCD Hermite データから
%   直接構成し，界面越し場延長を高次に実現する．
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{Hermite 場延長法（HFE）：高次片側データ契約}
...
（不連続を陽的に扱う GFM の補完技法としての位置付けは~\cite{Kang2000,Fedkiw1999}）．
1次元または格子軸方向の延長では $(f, f', f'')$ による5次 Hermite 補間で
$\Ord{h^6}$ の補間精度を得る．2次元の斜め界面で同じ設計次数を主張できるのは，
後述の混合微分データを含むテンソル積 Hermite 補間と，
十分に精度の高い最近接点幾何が同時に満たされる場合に限る．

\noindent\textbf{圧力射影段階における位置づけ．}
低密度比の対照モデルとして smoothed-Heaviside 一括 PPE を用いる場合，
圧力場 $p$ は $\varepsilon$ 正則化された滑らかな場であり，
HFE は不要であり，適用すると不要な外挿誤差を加える可能性がある
（§~\ref{sec:ppe_discretization_choice}）．
高密度比では\textbf{分相 PPE 解法}
（§~\ref{sec:split_ppe_motivation}；検証は §~\ref{sec:split_ppe_recovery}）を採用し，
界面条件付きの各相 PPE を解く．圧力場が Young--Laplace ジャンプ
$j_{gl}=p_g-p_l=-\sigma\kappa_{lg}$ を
陽に持つため，HFE による界面越し場延長が\textbf{必須}となる．
本節ではこの分相 PPE 解法の\textbf{基盤となる場延長理論}として HFE を定式化する．

\paragraph{なぜ分相 PPE 解法に界面越し場延長が必要か．}
分相 PPE 解法では各相で定数密度の PPE を独立に解くため，
圧力場が Young--Laplace 則 $j_{gl}=p_g-p_l=-\sigma\kappa_{lg}$ に従う
...
CCD が界面近傍でも同相由来の滑らかな片側データに対して演算できるようにする．
ただし §\ref{sec:split_ppe_formulation} の affine-jump 経路では，
圧力履歴と速度補正は節点圧力の界面越し直接微分ではなく，
$A_f(G_fp-B_f(j))$ の face cochain として保存・適用する．
したがって HFE の役割は，この face cochain と GFM ジャンプ行を構成するための
高次片側 Hermite データを供給することにある．
具体的な適用箇所は以下の通りである：
\begin{enumerate}[noitemsep]
  \item \textbf{GFM ジャンプ行の片側データ：}
        界面近傍で片側場を延長し，各相内の CCD 演算が同一相の滑らかな値だけを参照するようにする．
        これにより不連続圧力を跨ぐ通常 CCD ステンシルを避ける．
  \item \textbf{affine-jump IPC の面 cochain 構成：}
        予測圧力履歴と圧力補正は，HFE で作った片側 Hermite データから
        $G_\Gamma(p;j)=G(p)-B(j)$ を同じ面上で評価し，
        $A_f(G_fp-B_f(j))$ として保存・適用する．
        節点延長圧力の raw 勾配はこの面 cochain の代替経路ではない．
  \item \textbf{節点圧力型の比較経路：}
        圧力増分を節点場として扱う比較・診断では，
        HFE による延長場で Gibbs 振動を避けられる．ただし本章の標準閉包では，
        速度補正の受理対象を face cochain に置く．
...
偏微分方程式は：
\begin{equation}
  \frac{\partial q}{\partial \tau} + \mathrm{sgn}(\phi)\,\hat{\bm n}_{\phi}\cdot\bnabla q = 0
  \label{eq:extension_pde_main}
\end{equation}
ここで $\tau$ は仮想時間（次元は長さ $[L]$；特性速度 $|\hat{\bm n}_{\phi}| = 1$ により $\Delta\tau \sim h$），
$\hat{\bm n}_{\phi} = \bnabla\phi/|\bnabla\phi|$ は HFE の延長方向を与える SDF 法線であり，
Young--Laplace ジャンプの向き付き法線 $\hat{\bm n}_{lg}$ とは役割を分けて用いる．
$\mathrm{sgn}(\phi) = +1$（気相）$/-1$（液相）が延長方向を定める．
液相側（ソース相）の値を固定したまま，気相側（ターゲット相）を仮想時間前進させることで
$\bnabla q \cdot \hat{\bm n}_{\phi} = 0$（法線方向勾配ゼロ）の定常解へ収束する．
...
定常解 $\bnabla q \cdot \hat{\bm n}_{\phi} = 0$ を高次精度で満たすため，
...
\begin{itemize}
  ...
  \item GFM ジャンプ行と affine-jump face cochain に必要な片側 Hermite データを同じ精度で供給できる
  \item Balanced--Force 条件（§~\ref{sec:balanced_force}）と整合したジャンプ補正付き面勾配を構成できる
\end{itemize}
...
\begin{tikzpicture}[scale=0.85]
  ...
  \begin{scope}[xshift=-50mm]
    \node[font=\small\bfseries, text=navy] at (0,2.8) {延長前：片側圧力代表に不連続};
    \draw[->] (-2.5,0) -- (2.5,0) node[right,font=\small]{$x$};
    \draw[->] (0,-0.5) -- (0,2.5) node[above,font=\small]{$q$};
  ...
  % 延長後
  \begin{scope}[xshift=50mm]
    \node[font=\small\bfseries, text=navy] at (0,2.8) {延長後：片側代表 $q_\mathrm{ext}$};
    \draw[->] (-2.5,0) -- (2.5,0) node[right,font=\small]{$x$};
    \draw[->] (0,-0.5) -- (0,2.5) node[above,font=\small]{$q_\mathrm{ext}$};
    \draw[very thick, blue2, domain=-2.2:2.2] plot (\x, {1.8});
    \draw[dashed, gray] (0,-0.3) -- (0,2.3);
    \node[font=\scriptsize] at (0,-0.5) {$\Gamma$};
    \node[font=\scriptsize, blue2] at (1.2, 2.1) {延長済み};
    \node[font=\scriptsize, teal2] at (0, 0.4) {片側 Hermite データ: $\Ord{h^6}$};
  \end{scope}
\end{tikzpicture}
\caption{最近接点 Hermite 補間による片側圧力代表の界面越し延長（概念図）}
\label{fig:extension_schematic}
\PaperCaptionNote{左：CSF 表面張力による Laplace ジャンプが CCD ステンシルを跨ぐと Gibbs 振動が発生． 右：ジャンプ補正済みの片側値を気体側へ延長した後は，GFM ジャンプ行と face cochain に必要な片側 Hermite データを不連続値を跨がずに評価できる．}
\end{figure}
...
$q_\mathrm{ext}(\bx) = q\bigl(\bx_\Gamma(\bx)\bigr)$ に他ならない
（Aslam~(2004)~\cite{Aslam2004}, §2, Proposition 1；$\bnabla q \cdot \hat{\bm n}_{\phi} = 0$ の
特性曲線解が最近接点の値の伝播と一致するという主張，
および \cite{Aslam2004} §3 の数値実験で最近接点延長の一致性が確認されている）．
ここで $\bx_\Gamma$ は $\bx$ から界面 $\Gamma$ への最近接点である．
本稿ではこの定常解を，Extension PDE の仮想時間前進を経由せず，
CCD の Hermite データ $(f, f', f'')$ を用いた直接補間により構成する．

\paragraph{最近接点の構成．}
ターゲット側格子点 $\bx$（$\phi(\bx) \geq 0$）に対し，
法線 $\hat{\bm n}_{\phi} = \bnabla\phi/|\bnabla\phi|$ を CCD $D^{(1)}$（$\Ord{h^6}$）で評価し，
最近接点を
\begin{equation}
  \bx_\Gamma = \bx - \phi(\bx)\,\hat{\bm n}_{\phi}(\bx)
  \label{eq:closest_point}
\end{equation}
で求める．この式が最近接点を高精度に与えるには，$\phi$ 自身の界面位置誤差，
SDF 誤差 $||\bnabla\phi|-1|$，および $\hat{\bm n}_{\phi}$ の微分誤差が同時に小さい必要がある．
したがって $\bx_\Gamma$ の精度は CCD の $\bnabla\phi$ 精度だけではなく，
CLS 再初期化と界面位置の精度にも律速される．

\paragraph{CCD Hermite 5 次補間．}
...
\footnote{$h^j$ は $\xi = (x-x_a)/h$ への無次元化に伴うスケール係数：
...
\paragraph{片側外挿．}
$\bx_\Gamma$ がソース側最端セル $[x_i, x_{i+1}]$ の外側
...
したがって必要な離散データは単一の軸方向評価では閉じず，
各延長帯で軸方向 CCD を複数回組み合わせる．
この完全テンソル版を用いる場合に限り，2次元斜め界面でも Hermite 補間部の
$\Ord{h^6}$ 設計精度を主張できる．混合微分を省略した簡略版はこのデータ条件を満たさず，
...
\subsubsection{HFE の射影段階への組み込み}

射影段階内では次の2層で HFE を適用する：
\begin{enumerate}
  \item \textbf{片側 Hermite データの構成：}
        PPE 求解前後で必要になる $p^n$，$p^{n+1}$，または圧力増分の
        同相由来データを延長し，GFM ジャンプ行と $G_\Gamma(p;j)$ の評価に渡す．
  \item \textbf{face cochain への集約：}
        affine-jump 標準経路では，延長節点場をそのまま速度補正へ渡さず，
        同じ面・同じ向き・同じ係数で
        $A_f(G_fp-B_f(j))$ を構成して予測圧力履歴と速度補正に用いる．
\end{enumerate}
\textbf{配置の要点}：HFE は界面ジャンプを持つ圧力場を
CCD ステンシルが直接跨がないようにするための高次片側データ契約である．
圧力射影の閉包そのものは，そのデータから作るジャンプ補正付き face cochain が担う．
...
密接に関連し，分相 PPE 解法の不可欠な場延長理論であるためである．
§~\ref{sec:split_ppe_motivation} で議論する分相 PPE 解法では，界面上の圧力ジャンプ処理に
HFE が不可欠となり，高密度比の圧力ジャンプ型閉包を支える
主要構成として位置づけられる．
