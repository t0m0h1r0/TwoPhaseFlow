...
なお形状誤差が $N = 64 \to 128$ でわずかに増大する（$7.9 \times 10^{-2} \to 1.2 \times 10^{-1}$）のは，
高解像度では shape error 指標（界面位置の $L^2$ ノルム差）が
より細かい空間スケールの変動まで検出できるようになるためであり，
実質的な並進成分は両解像度で機械精度 $< 10^{-15}$ に保たれている
（$\|\bu_{\mathrm{para}}\|_\infty$ 列参照）．
したがってこのわずかな増大は物理的劣化ではなく測定スケーリングの効果である．
