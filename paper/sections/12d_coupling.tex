...
なお形状誤差が $N = 64 \to 128$ でわずかに増大する（$7.9 \times 10^{-2} \to 1.2 \times 10^{-1}$）一方で，
実質的な並進成分（$\|\bu_{\mathrm{para}}\|_\infty$）は両解像度で機械精度 $< 10^{-15}$ に保たれており，
離散化に起因する物理的な Galilean 不変性の劣化は観測されない．
形状誤差指標が N とともにわずかに増大する点については，
高解像度で形状差分がより細かい空間スケールの変動まで捕捉することによる
測定スケーリングの効果と解釈できるが，定量的な原因分離は将来課題である．
