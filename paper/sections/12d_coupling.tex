...
\noindent\textbf{RT 不安定性（$\sigma = 0$, 動的テスト）：}
テスト A（Galilean 不変性）は定常並進であったが，
ここでは物理的に非自明な動的問題として Rayleigh--Taylor 不安定性を検証する．
$\rho_l/\rho_g = 3$（Atwood 数 $At = 0.5$），$\sigma = 0$，
$64 \times 256$ 格子での界面成長率は
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/ch12_rt_fields}
