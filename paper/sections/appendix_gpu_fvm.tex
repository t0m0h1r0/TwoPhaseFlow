% paper/sections/appendix_gpu_fvm.tex
%
% 付録H  GPU-native FVM 投影：面ローカル演算子カリキュラスと可変バッチ PCR
%       （旧 §9.7 を付録化）
%
% ═══════════════════════════════════════════════════════════════════════════════
\section{GPU-native FVM 投影：面ローカル演算子と可変バッチ PCR}
\label{app:gpu_fvm}
% ═══════════════════════════════════════════════════════════════════════════════
...
本付録では\textbf{PDE・面係数を一切変えず}，
演算子表現とソルバー戦略のみを GPU ネイティブ化する設計を示す．
...
% -------------------------------------------------------------------------------
\subsection{面ローカル演算子カリキュラス}
\label{sec:gpu_fvm_face_local_calculus}
\phantomsection\label{app:gpu_fvm_ops}% 本文→付録バックリンク用 alias
% -------------------------------------------------------------------------------
...
% -------------------------------------------------------------------------------
\subsection{線分解と可変バッチ PCR プリミティブ}
\label{sec:gpu_fvm_variable_pcr}
\phantomsection\label{app:gpu_fvm_pcr}% 本文→付録バックリンク用 alias
% -------------------------------------------------------------------------------
...
% -------------------------------------------------------------------------------
\subsection{プリコンディショナーとしての線ソルブ（ADI 化の禁止）}
...
% -------------------------------------------------------------------------------
\subsection{D2H/H2D 境界規律}
\label{sec:gpu_fvm_d2h_discipline}
\phantomsection\label{app:gpu_fvm_d2h}% 本文→付録バックリンク用 alias
% -------------------------------------------------------------------------------

\begin{tcolorbox}[resultbox,title={ゼロ転送ホットループ条件}]
...
% -------------------------------------------------------------------------------
\subsection{A3 トレーサビリティと実装ロードマップ}
\label{sec:gpu_fvm_a3_roadmap}
% -------------------------------------------------------------------------------

\begin{center}
\footnotesize
\begin{tabular}{@{}p{0.15\textwidth}>{\raggedright\arraybackslash}p{0.78\textwidth}@{}}
\hline
層 & 内容 \\
\hline
...
既存コード & \texttt{\_fvm\_pressure\_grad},
  \texttt{RhieChowInterpolator.face\_velocity\_divergence},
  \texttt{PPEBuilder.build\_values},
  \texttt{\_pcr\_solve\_batched} \\
新規コード & \texttt{PPESolverFVMMatrixFree},
  \texttt{solve\_tridiagonal\_variable\_batched} \\
\hline
\end{tabular}
\end{center}
...
% -------------------------------------------------------------------------------
\subsection{H-01 / A-01 との関係：正確性と性能の分離}
...
\textbf{正確性の軌跡}問題を解いた．
本付録は同じ面ローカル代数の\textbf{性能表現}問題を扱う．
両者は補完的：
\begin{center}
\begin{tabular}{@{}ll@{}}
\hline
H-01 / A-01（FCCD, \S\ref{sec:fccd_def}）& 演算子を\textbf{正しくする} \\
本付録の GPU-native FVM & 同じ演算子を\textbf{GPU で速くする} \\
\hline
\end{tabular}
\end{center}
本付録の手法は面ローカル FVM 方程式を\textbf{厳密に保つ}ため，
...
\noindent\textbf{圧力フィルタ禁止との整合}：
本付録で扱う GPU-native FVM 投影は
\S\ref{sec:dccd_pressure_nofilt} の圧力フィルタ禁止論と
整合する実装バリアントである．
非対称 DCCD を圧力に適用しないため P-1/P-2
（\S\ref{sec:bf_seven_principles}）を破壊せず，
本文の BF 予算がそのまま引き継がれる．
