...
直接引き起こす（1章・失敗例1参照）．したがって，曲率計算には可能な限り高精度の
微分評価が必要である．
...
境界条件 & ゴーストセル値 & 物理的意味 \\
\hline
Dirichlet（ノースリップ壁）: $u|_{\rm wall}=0$ &
  $u_{-1} = -u_1$ &
  鏡像反転（$u_{0} \approx 0$） \\
Dirichlet（強制流入）: $u|_{\rm wall}=U_w$ &
  $u_{-1} = 2U_w - u_1$ &
  $u_{0} = (u_{-1}+u_1)/2 = U_w$ \\
Neumann（流出・対称）: $\partial u/\partial x|_{\rm wall}=0$ &
  $u_{-1} = u_1$ &
  勾配ゼロ（偶関数的反射） \\
\hline
\end{tabular}
...
\begin{itemize}
  ...
  \item 流出境界では $f'_{-1} = f'_1$，$f''_{-1} = f''_1$ とするが，
    圧力の全 Neumann 境界問題（零固有モード）については 7 章の対処法を参照すること．
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[defbox, title={内点のブロック係数行列}]
\textbf{Equation-I の行（$f'$ に関する方程式）から：}
\[
  \underbrace{\begin{bmatrix} \alpha_1 & b_1 h \\ 0 & \beta_2 \end{bmatrix}}_{\mathbf{A}}
  \bm{v}_{i-1}
  +
  \underbrace{\begin{bmatrix} 1 & 0 \\ 0 & 1 \end{bmatrix}}_{\mathbf{B}}
  \bm{v}_i
  +
  \underbrace{\begin{bmatrix} \alpha_1 & -b_1 h \\ 0 & \beta_2 \end{bmatrix}}_{\mathbf{A}^\top}
  \bm{v}_{i+1}
  = \mathbf{r}_i
\]
右辺ベクトル：
\[
  ...
  \begin{bmatrix}
    ...
    \dfrac{a_2}{h^2}(f_{i-1}-2f_i+f_{i+1}) + \dfrac{b_2}{h}(f'_{i+1}-f'_{i-1})\text{ の $f$ のみの項}
  \end{bmatrix}
\]
（Equation-II の $f'_{i\pm1}$ を含む項は左辺のブロックに移項済み）
\end{tcolorbox}

\begin{tcolorbox}[warnbox, title={Equation-II における $f'_{i\pm 1}$ の扱い}]
Equation-II \eqref{eq:CCD_II} の RHS には $b_2(f'_{i+1}-f'_{i-1})/h$ という
\textbf{隣接格子点の $f'$} が現れる．
これは未知数 $\bm{v}_{i\pm1}$ の第1成分であるため，RHS ではなく左辺ブロック $\mathbf{A}$ に組み込む必要がある．
具体的には，第2行（Equation-II）の $\bm{v}_{i-1}$ ブロックに $[{-b_2/h},\ \beta_2]$，
$\bm{v}_{i+1}$ ブロックに $[b_2/h,\ \beta_2]$ が入り，全体が対称なブロック三重対角構造になる．
\end{tcolorbox}
