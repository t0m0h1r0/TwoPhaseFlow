...
\subsection{2次元への拡張：混合偏微分 $D_{xy}^{(11)}$ の計算}
\label{sec:ccd_mixed}
...
$y$ 方向 CCD の微分は「誤差 $E_g$ を持つ入力に対する微分」を行うため：
\[
  E_{\phi_{xy}} \leq \underbrace{O(h^6)}_{\text{$y$ 方向 CCD の打ち切り誤差}}
               + \underbrace{O(E_g / h^?) }_{\text{$E_g$ の微分への増幅}}
\]
$E_g = O(h^6)$ で $h \to 0$ の極限を考えると，$g$ の格子値の誤差が $y$ 方向微分に与える影響は
$\partial E_g / \partial y = O(h^6)$ 程度（$E_g$ の空間変化が滑らか）であるため，
全体の誤差は $O(h^6)$ に保たれる．

\textbf{実用的確認：} $f(x,y) = \sin(x)\cos(y)$（解析解 $f_{xy} = -\cos(x)\sin(y)$）などで
格子精緻化テストを行い，誤差が $h^6$ で収束することを確認すること（10章参照）．
\end{tcolorbox}
