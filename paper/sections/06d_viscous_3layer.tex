...
$[-p\mathbf{I}+\mu_k(\bnabla\bm{u}+\bnabla\bm{u}^T)]_\Gamma\!\cdot\hat{\bm n}_{lg} = \sigma\kappa_{lg}\hat{\bm n}_{lg}$；接線成分は $[\tau_{xy}]_\Gamma=0$）が
...
非圧縮条件 $\bnabla\cdot\bm{u}=0$ 下で：
\begin{equation}
  \bnabla\cdot(2\mu\mathbf{D})
  = \mu\nabla^2\bm{u} + 2\mathbf{D}\cdot\bnabla\mu,
  \label{eq:viscous_exact_decomposition}
\end{equation}
ここで $\mathbf{D} = \tfrac{1}{2}(\bnabla\bm{u} + \bnabla\bm{u}^\top)$
は歪速度テンソル．
...
\begin{itemize}
  ...
  \item 高粘性比流でエネルギー散逸が崩壊（$\bm{u}\cdot\mathbf{F}_\mu$ が
        正の値を取りうる）
\end{itemize}
...
\begin{equation}
  \langle\bm{u},\mathbf{F}_\mu\rangle_h
...
等号は $\mathbf{D}(\bm{u})\equiv 0$（歪速度テンソル恒等零；
...
\begin{equation*}
  L_\mu\bm{u}
  = \mathbb{1}_{\Omega_\text{bulk}}\cdot\mu\Delta_h^{\CCD}\bm{u}
...
$\bigl(I - \tfrac{\Delta t}{2\rho}L_H\bigr)\bm{u}^* = \text{RHS}$ と
...
\textbf{Face 上の $\mu$} は face-flux 型の粘性実装や
CN Helmholtz の主対角構築（§\ref{sec:viscous_cn_defect_v7}，§7.4）へ
拡張する場合に必要となる補助量である．
