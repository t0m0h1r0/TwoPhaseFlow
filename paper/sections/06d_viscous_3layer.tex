...
$\partial_n\bu$ kink を跨がないための応力評価則である．
...
\end{align}
$D_\alpha^{C}$ は cell-centered stress array に作用する物理座標差分であり，
...
界面法線方向では相内 stencil を選び，kink を跨ぐ CCD は使わない．
...
また，ステンシル内に異相のセルを含む場合（phase-indicator mismatch）も界面帯として扱う．
...
は界面帯 indicator の一つであり，標準判定は
$|\phi|\le 3h$ と phase-indicator mismatch を優先する．
...
拡張する場合に必要となる補助量である．
本稿の §6.3 A3 経路は cell-center $\mu_{i,j}$ で
...
\begin{enumerate}
  ...
  \item 本稿の cell-centered 応力発散では cell-center $\mu^{n+1}$ を
        Layer B の応力組立に直接用いる．
...
\begin{itemize}
  ...
  \item \textbf{$\psi$ クランプ前に物性値更新} → $\rho<\rho_g$ または $\rho>\rho_l$．
        Stage B/F の閉鎖と $\psi\in[0,1]$ 値域制限後に更新．
\end{itemize}
