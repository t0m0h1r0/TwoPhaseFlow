% paper/sections/08b_ppe_pseudotime.tex
% 付録 E.5：CCD 陰解法と仮想時間ステップ反復（PPE ソルバの実装詳細）
% ※ 旧: §8.4（本文）→ 付録 E.5 に移動（Modification IV: ソルバーアルゴリズムを付録へ）
%
\section{CCD 陰解法と仮想時間ステップ反復}
\label{sec:ppe_pseudotime}

\subsection{初期圧力場 $p^0$ の設定}
\label{sec:ppe_p0_init}
%% 旧所在: §8.1（08_pressure.tex）→ 付録 E.5 へ移動（CHK-086）
...
\begin{itemize}
  ...
  \item \textbf{一般問題（高密度比を含む）：} $n=0$ でのみ PPE（式~\eqref{eq:PPE_derivation}）を $p^0 = 0$ 初期値で解き，得られた圧力場を $p^0$ として設定する（1回追加求解のコスト）．
  \item \textbf{簡易設定：} $p^0 = 0$ のまま発進する．$O(\Delta t^2)$ 精度は定常後に達成され，初期数ステップでは $O(\Delta t)$ 過渡誤差が生じうるが，多くの実用問題では無視できる．
\end{itemize}
...
\textbf{空間精度}として CCD 法（6次精度）を用いるため $\mathcal{O}(h^6)$ の高精度が得られ
...
\noindent\textbf{本節で解説する欠陥補正法は，\texttt{PPESolverSweep}（\texttt{ppe\_solver\_type="sweep"}）として実装済みである．}
...
\textbf{実装（\texttt{PPESolverSweep}）との対応：}
...
右辺残差は CCD 演算子 $\mathcal{L}_h$（$\mathcal{O}(h^6)$）で評価する．
...
欠陥補正（FD 左辺 + CCD 残差）$\ddagger$ \textbf{[本稿標準]} & $\mathcal{O}(h^6)$ & なし & 残差駆動 & 中 \\
\hline
...
右辺残差に CCD（$\mathcal{O}(h^6)$）を用いることで，分割誤差なしに行列不要の $\mathcal{O}(N)$/ステップを実現する
...
であり，固体壁境界 $\bu^*\cdot\hat{n}=0$ では\textbf{自動的に満たされる}（§\ref{sec:ab2_ipc} の壁面 BC 参照）．
...
\begin{tcolorbox}[mybox, title={ゲージ点選択の比較}]
\begin{center}
