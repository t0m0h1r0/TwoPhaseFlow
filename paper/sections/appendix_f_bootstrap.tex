% paper/sections/appendix_f_bootstrap.tex
% 付録F: ブートストラップ初期化シーケンス（§10 から移動）

\section{ブートストラップ処理：初期化シーケンス}
\label{app:bootstrap}

シミュレーション開始前に，以下の初期化シーケンス（ブートストラップ処理）を一度だけ実行する．
格子生成には符号付距離場 $\phi^{(0)}$ が必要だが，$\phi^{(0)}$ の評価には格子が必要という
...
\begin{enumerate}
  ...
  \item \textbf{Step 3（CCD 演算子構築）：} 非一様格子重みから
        $D^{(1)}, D^{(2)}, M_{\mathrm{CCD}}, L_L, L_H$ の疎行列構造を構築する．
        固定格子比較経路では以降不変であり，標準の追従格子経路では
        格子再生成ごとに同じ契約で再構築する．
