...
\texttt{active\_geometry\_capillary} 設定と過去の
\texttt{sharp\_phase\_volume} / Ridge--Eikonal 補修候補の不一致を閉じる
閉界面体積ゲートである．
...
\PaperCaptionNote{本表は各 V 検証の \textbf{設定}（本文の \texttt{paragraph} ブロック）で繰り返し参照される基準値を集約する．異なる Reynolds 数 / 密度比掃引 / Weber 数掃引を含む検証については代表値または範囲を示し，個別値は本文を参照．V1/V2 は単相 NS のため $\sigma$, $\mathrm{We}$ は適用外であり，参考に粘性条件（V1: $\nu=0.01$，V2: $\Re=40$）を補足列に記す．V10 は NS 非連成 CLS 受動移流のため $\sigma$/$\mathrm{We}$/$\rho$ は適用外．V11 は PhaseRegion 毛管経路ゲート，V12 は閉界面標準設定の採用境界であり，いずれも物理パラメータ掃引ではない．\textsuperscript{$\dagger$}V2 のみ正方領域ではなくドメイン bound $[-0.5,1]\!\times\![0,1]$ の長辺$\times$短辺を示す．}
\centering\small
...
V12 & $0.02$ & $0.0728$ & $829$      & ---           & periodic       & 閉界面体積の採用境界 \\
\bottomrule
\end{tabular}
\end{table}
...
過去の \texttt{sharp\_phase\_volume} 制約と
Ridge--Eikonal 再初期化は現行の閉界面 endpoint では不合格として停止する．
...
V12 & 閉界面体積の採用境界 & \autoref{fig:V12_closed_interface_volume_gate} & 現行設定の構築，過去の sharp-volume/Ridge--Eikonal 経路の不合格停止，初期ステップの体積/P1 面積 drift，2 step 目の格子再構築停止 & \checkmark ゲート \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item \autoref{sec:closed_interface_volume_gate}：V12（閉界面体積の採用境界）．
