...
%
% §13: 統合検証 (Integration Verification) — V1-V12.
...
保存形共通フラックス台帳 U10，制約付き面速度状態空間 U11，PhaseRegion 毛管経路ゲート U12）が
\textbf{単体で}設計仕様（収束率，境界整合，演算子トレース性）を満たすことを確認した．
本章は，これらコンポーネントを\textbf{統合}した完全 Navier--Stokes / 二相流ソルバが
\textbf{物理整合性を保つ領域}と\textbf{設計収束次数が崩れる領域}を，12 個の独立した統合検証実験
V1--V12 で評価する
...
U12 は PhaseRegion 毛管経路の事前ゲートであり，全圧力像への過射影を棄却する反例診断である．
したがって本章では V11 を PhaseRegion 毛管経路の統合ゲートとして追加し，
第14章の標準上昇気泡設定に依存する事前適合性確認は
本章の物理ベンチマークへ数えない．
また V12 は，第~\ref{sec:val_oscillating_droplet}節の現行
\texttt{active\_geometry\_capillary} YAML と旧
\texttt{sharp\_phase\_volume} / Ridge--Eikonal 記述の不一致を閉じる
閉界面体積ゲートである．
本章で読むのは，既存の V6/V7/V9 が担う壁付き圧力ジャンプ/HFE 結合系の有限時間挙動であり，
周期壁の有限振幅上昇気泡応答は第~\ref{sec:benchmarks}章へ送る．
V11 は，$R_p$，PhaseRegion graph chart の HFE 候補，移動格子で保存する面量，
滑らかな圧力履歴座標，正則層ガード，DC 収束判定を
標準構成へ入れるための採用境界である．
今回の PhaseRegion graph/closed chart 実験は reduced oracle/stepper であり，
圧力・速度消費側へは接続していない．
そのため V1--V10 の既存 NS 検証を再実験する必要はなく，
V11 と第14章では \texttt{force\_admissible=0} を明示したまま，
状態所有・測度・表面エネルギー・GPU reduction の整合性を検査する．
V12 はこの方針とは別に，現行 production YAML が閉界面 1 周期受理へ
到達していないこと，および旧 sharp-volume 補修を現行 state space へ
混入できないことを確認する．

\paragraph{本章の検証値の読み方．}
V 系列の数式と指標は，「実験が動いたか」を示す完走記録ではなく，
第12章で確認した部品契約を組み合わせたときに，
どの物理的制約がまだ保たれているかを読むためのものだ．
収束率は設計次数そのものではなく，界面帯，境界条件，圧力ジャンプ，格子再構築を含めた
縮約問題で実際に観測できる次数である．
寄生流れ，圧力差，体積誤差，速度誤差はそれぞれ別の失敗モードを見る窓であり，
一つの値だけで合否を決めない．
読者は各 V test を，入力した部品契約，測った物理量，
残った保証範囲外の三点で読むと，本章の判定区分が直感的になる．
...
Type-D & 連続方程式・離散構造に由来する理論的下限と整合する条件付き合格．V7（圧力ジャンプ・毛管射影結合系の界面帯制約下での実効収束率），V10-a 形状軸（固定格子 CLS 位相/しきい値化誤差 + slot 解像不足）, V10-b 形状軸（格子幅まで薄くなる折り畳み細線）で適用． \\
...
\begin{table}[!ht]
\caption{V1--V12 の物理設定パラメータ}
\label{tab:V_settings}
\PaperCaptionNote{本表は各 V 検証の \textbf{設定}（本文の \texttt{paragraph} ブロック）で繰り返し参照される基準値を集約する．異なる Reynolds 数 / 密度比掃引 / Weber 数掃引を含む検証については代表値または範囲を示し，個別値は本文を参照．V1/V2 は単相 NS のため $\sigma$, $\mathrm{We}$ は適用外であり，参考に粘性条件（V1: $\nu=0.01$，V2: $\Re=40$）を補足列に記す．V10 は NS 非連成 CLS 受動移流のため $\sigma$/$\mathrm{We}$/$\rho$ は適用外．V11 は PhaseRegion 毛管経路ゲート，V12 は閉界面 production YAML の採用境界ゲートであり，いずれも物理パラメータ掃引ではない．\textsuperscript{$\dagger$}V2 のみ正方領域ではなくドメイン bound $[-0.5,1]\!\times\![0,1]$ の長辺$\times$短辺を示す．}
\centering\small
...
V11 & --- & ---   & ---               & ---           & ---            & PhaseRegion 毛管経路ゲート \\
V12 & $0.02$ & $0.0728$ & $829$      & ---           & periodic       & 閉界面体積 admission gate \\
\bottomrule
\end{tabular}
\end{table}
...
質量保存軸 \checkmark / 形状軸 $\triangle$（格子幅まで薄くなる折り畳み細線に対する Eulerian 固定格子の Type-D
...
V11 は Type-A/B/D の精度試験ではなく，PhaseRegion 毛管経路を
標準構成へ混入させる前の採用境界ゲートとする．
このゲートには，PhaseRegion graph chart の HFE 候補，移動格子で保存する面量，
圧力履歴座標，DC 収束判定，および支配法線方向の
正則層ガードを含める．
...
V12 も Type-A/B/D の精度試験ではなく，
閉界面 production 設定の採用境界ゲートである．
現行 \texttt{active\_geometry\_capillary} 設定は構築できるが，
旧 \texttt{sharp\_phase\_volume} 制約と
Ridge--Eikonal 再初期化は現行 closed-interface endpoint では fail-close される．
...
\begin{itemize}\setlength{\itemsep}{0pt}
  \item \textbf{階層構造} — V1--V2 は単相 NS（密度比 $1$）で空間/時間収束を確立し，
        ...
        V11 は PhaseRegion 毛管経路を標準構成へ入れるための統合ゲートとして扱い，
        graph chart HFE 候補と追随格子の正則層保持を同時に確認する．
        V12 は閉界面 production YAML の体積不変量と state-space 整合性を
        §14.2 から逆参照する採用境界として扱う．
...
\begin{table}[!ht]
\caption{V1--V12 統合検証の概観}
\label{tab:V_summary}
\PaperCaptionNote{本表は V 番号順で並び（V1/V10 は a/b 二行分割，V11/V12 はゲート行で計 14 行）．表~\ref{tab:v_accuracy_summary}（§\ref{sec:error_budget}；同一サブテスト集合）はさらに V10-a/V10-b を質量保存軸と形状軸の二軸 (Type-B / Type-D) に分割するため V11/V12 ゲートを含め計 16 行となり，本表 14 行との対応は V1--V9/V11/V12 が 1:1，V10-a → \{mass, shape\}, V10-b → \{mass, shape\} の 1:2 拡張対応となる．本表も判定列を持ち，表~\ref{tab:v_accuracy_summary} と同じ判定ラベルで読む；主要指標欄は本表で全観測指標を列挙し，表~\ref{tab:v_accuracy_summary} では判定キー指標 1 種を軸毎に集約する．}
...
V11 & PhaseRegion 毛管経路ゲート & \autoref{tab:V11_ao_gate} & 平坦界面ゼロ駆動，毛管波の非静的診断，graph chart HFE 候補，履歴分離，正則層保持，未確定分解境界 & \checkmark ゲート \\
V12 & 閉界面体積 admission gate & \autoref{fig:V12_closed_interface_volume_gate} & 現行 YAML 構築，旧 sharp-volume/Ridge--Eikonal 経路の fail-close，current-prefix 体積/P1 面積 drift，2 step 目 grid-rebuild blocker & \checkmark ゲート \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item \autoref{sec:ao_fast_capillary_gate}：V11（PhaseRegion 毛管経路ゲート）．
  \item \autoref{sec:closed_interface_volume_gate}：V12（閉界面体積 admission gate）．
  \item \autoref{sec:error_budget}：V1--V12 横断の精度バジェット総合．
\end{itemize}
...
\input{sections/13e1_nonuniform_ns}
\input{sections/13e2_ao_capillary_split_gate}
\input{sections/13e3_closed_interface_volume_gate}
