...
保存形共通フラックス台帳 U10，制約付き面速度状態空間 U11，AO-Fast gate U12）が
...
U12 は AO-Fast 幾何毛管分解の事前 gate であり，成功した物理ベンチではない．
したがって本章では V11 を AO-Fast 統合前 gate として追加し，
第14章の標準上昇気泡設定に依存する事前適合性確認は
本章の物理ベンチマークへ数えない．
本章で読むのは，既存の V6/V7/V9 が担う壁付き圧力ジャンプ/HFE 結合系の有限時間挙動であり，
周期壁の有限振幅上昇気泡応答は第~\ref{sec:benchmarks}章へ送る．
V11 は AO-Fast packet が標準物理経路へ混入しないための fail-close 境界である．
...
V11 は Type-A/B/D の精度試験ではなく，AO-Fast 幾何毛管分解を
標準物理経路へ混入させる前の fail-close gate とする．
...
\begin{itemize}\setlength{\itemsep}{0pt}
  \item \textbf{階層構造} — V1--V2 は単相 NS（密度比 $1$）で空間/時間収束を確立し，
        ...
        V11 は AO-Fast 幾何毛管分解を本番物理経路へ入れる前の統合前 gate として扱う．
...
\label{tab:V_summary}
\PaperCaptionNote{本表は V 番号順で並び（V1/V10 は a/b 二行分割，V11 は gate 行で計 13 行）．表~\ref{tab:v_accuracy_summary}（§\ref{sec:error_budget}；同一サブテスト集合）はさらに V10-a/V10-b を質量保存軸と形状軸の二軸 (Type-B / Type-D) に分割するため V11 gate を含め計 15 行となり，本表 13 行との対応は V1--V9/V11 が 1:1，V10-a → \{mass, shape\}, V10-b → \{mass, shape\} の 1:2 拡張対応となる．本表も判定列を持ち，表~\ref{tab:v_accuracy_summary} と同じ判定ラベルで読む；主要指標欄は本表で全観測指標を列挙し，表~\ref{tab:v_accuracy_summary} では判定キー指標 1 種を軸毎に集約する．}
...
V11 & AO-Fast 幾何毛管分解 gate & \autoref{tab:V11_ao_gate} & flat zero-drive, wave nonzero probe, GPU fail-close & \checkmark gate \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item \autoref{sec:galilean_offset}：V4 (固定壁 uniform-offset 残差)．
  ...
  \item \autoref{sec:ao_fast_capillary_gate}：V11 (AO-Fast 幾何毛管分解 gate)．
...
\autoref{sec:twophase_static_verification} 内に連続配置し，V4 (固定壁 uniform-offset)
