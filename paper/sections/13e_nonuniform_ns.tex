...
\paragraph{設定．}
$L=1, \sigma=1, r=0.25, \rho_l=\rho_g=1$, $\Delta p_{\mathrm{exact}} = 4 \cdot 0.1 = 0.4$
（$\sigma/r$ 設定; 実装値 $\sigma=0.025, r=0.0625$ → $\Delta p_{\mathrm{exact}} = 0.4$）．
$N \in \{48, 64, 96\}$, $\alpha \in \{1, 2\}$．
$\Delta t = h_{\min}/0.5$, $n_{\mathrm{step}} = 200$．
出典：\texttt{experiment/ch13/exp\_V8\_nonuniform\_ns\_static.py}．
...
\end{itemize}
A3 トレース：\autoref{sec:U3_nonuniform_suite}（非一様格子定義）→
\texttt{src/twophase/grid/nonuniform.py}．
...
出典：\texttt{experiment/ch13/exp\_V9\_local\_eps\_nonuniform.py}．
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item Fixed $\varepsilon = c \cdot h_{\mathrm{avg}}$ が安全な default．
        ...
        を優先すべき，との設計指針が得られた．
\end{itemize}
A3 トレース：\autoref{sec:U5_heaviside_delta}（Heaviside--$\delta$ 定義）→
\texttt{src/twophase/csf/heaviside.py}．
...
出典：\texttt{experiment/ch13/exp\_V10\_cls\_advection\_nonuniform.py}．
...
\end{itemize}
A3 トレース：式~\eqref{eq:cls_advection} → \autoref{sec:cls_advection}（CLS 移流） +
\autoref{sec:fccd_def}（FCCD face value/grad） →
\texttt{src/twophase/cls/advection.py},
\texttt{src/twophase/ccd/fccd.py}．
