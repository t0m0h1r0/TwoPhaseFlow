...
\subsubsection{V10：Zalesak / CLS 移流 非一様}
\label{sec:zalesak_cls_advection}

\paragraph{検証対象．}
古典的 Zalesak slotted disk advection 問題を非一様格子上で CLS 移流として解き，
...
$\alpha=2$ では特に $N=128$ で体積膨張が顕著（$+12\%$）→ 非一様格子上の CLS 再初期化補正が
$N$ 増大に伴い体積誤差を蓄積．}
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item $\alpha=2$ (非一様格子)：centroid err $\sim 0.09$ (slot 部の薄い構造が
        ...
        という「収束逆行」性質は，CLS 再初期化が非一様格子上で
        体積保存性を破る構造的問題を示唆する．
  \item 非一様格子上の CLS 移流には \textbf{保存型再初期化補正}
  ...
  \item \textbf{本論文の制約条件として明示}：
        ...
        本ソルバの保証範囲外である．§3 (CLS 移流)，§10 (非一様格子)，§11 (制約整理) において
        「CLS 移流を伴う非一様格子問題は $\alpha = 1$ を推奨．$\alpha > 1$ の使用は
        体積保存性の追加検証を要する」と注記すべき設計指針．
\end{itemize}
