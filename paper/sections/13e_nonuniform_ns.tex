...
% Verifies: §3 (CLS transport), §5 (Heaviside-delta), §10 (non-uniform grid)
...
非一様格子 $\alpha=2$ では重心収束は弱いが，FCCD を格子更新後に構築すると
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item $\alpha=2$ (非一様格子)：FCCD を格子更新後に構築した production-stack 経路では
        $N=128$ の volume drift は $1.519\%$ であり，旧い WENO/事前構築 FCCD 経路の
        $12\%$ 級膨張は再現しない．一方，centroid order は $0.34$ に留まり，
