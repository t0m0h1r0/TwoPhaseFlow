...
$U$ は初速または重力終端速度を採用する（§~\ref{sec:validation} の各物理検証参照）．
本稿の表記規約として，\textbf{*-付きの変数は有次元量，*-無しは無次元量}とする．
これらを用いて，物理変数を以下のように無次元化する．
\begin{align}
 \bx^* &= L\bx & \bu^* &= U\bu & t^* &= \frac{L}{U}t & p^* &= \rho_l U^2 p \\
...
付録~\ref{app:curvature_2d_derivation}），以下の計算に適した形式が得られる：
\begin{equation}
  \kappa_{lg} = \frac{
    \phi_y^2\,\phi_{xx}
...
CLS 法では $\psi$ が基本変数である．
本稿の標準経路では，ロジット逆変換（§~\ref{sec:newton_inversion}）で
$\phi$ を復元してから曲率を評価するのではなく，
以下に示す\textbf{単調変換に対する曲率の不変性}により，
$\psi$ の微分値から直接曲率を計算する．
$\phi$ による曲率式は，符号規約の確認，解析解との照合，
および比較用の幾何診断として保持する．
...
このとき $\phi(\bx) \in I$ かつ $|\bnabla\phi(\bx)| \neq 0$ を満たす点 $\bx$ において，
\begin{equation}
...
\paragraph{曲率評価の数値的適用域}
曲率式の分母は，参照するレベルセット変数の勾配が消える領域で不安定になる．
ただし，表面張力が運動量方程式へ寄与するのは界面帯だけであるため，
本稿の標準経路は $\psi_{\min}<\psi<1-\psi_{\min}$ の界面帯で
$\psi$ 直接曲率を評価し，飽和領域では曲率を力学量として使わない
（§~\ref{sec:curvature_direct_psi_cls}）．
連続式~\eqref{eq:curvature_2d} を $\phi$ で評価する場合も，
同じ幾何的理由から分母の下限を明示して診断量を有界に保つ：
\begin{equation}
...
ここで $\varepsilon_{\text{norm}}$ は幾何診断のゼロ除算を避ける数値下限であり，
表面張力閉包の別経路ではない．
標準計算で使用する曲率は，第~\ref{sec:levelset}章で定義する
$\psi$ 直接曲率，界面帯制限，壁面トポロジ閉包を組み合わせて評価する．
詳細分析（ゼロ除算の発生メカニズム，誤差評価，界面帯フィルタ，確認項目）は
§~\ref{sec:curvature_stab} を参照．

\medskip
以上で，第~\ref{sec:levelset}章以降に渡す連続レベルの前提が揃った．
次章では，物性補間と曲率評価の入力となる Conservative Level Set 変数 $\psi$ を，
保存形移流・再初期化・$\psi\leftrightarrow\phi$ 写像・曲率直接計算の順に定式化する．
