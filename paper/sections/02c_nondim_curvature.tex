...
\begin{itemize}
    ...
    \item \textbf{表面張力項}:（無次元数 $We$ の導出には CSF 体積力形式 $\bm{f}_\sigma^* = \sigma\kappa^*\bnabla^*\psi$ を用いるが，得られる $1/We$ は Young--Laplace ジャンプ条件~\eqref{eq:gfm_jump} にも共通する）
    \begin{align*}
...
\begin{align}
 ...
 &= - \underbrace{\left[\frac{\rho_l U^2}{L}\right]}_{}\bnabla p
   + \underbrace{\left[\frac{\rho_l U^2}{L}\right]}{}\frac{1}{Re}\,
     \bnabla\cdot\!\bigl[\mu(\psi)(\bnabla\bu+\bnabla\bu^T)\bigr]
   - \underbrace{\left[\frac{\rho_l U^2}{L}\right]}{}\frac{\rho(\psi)}{Fr^2}\hat{\bm z}
...
\begin{equation}
 ...
 \label{eq:dnx_dx_raw}
\end{equation}
分母分子に $(\phi_x^2+\phi_y^2)^{1/2}$ を掛けて通分し，
分子を展開すると $\phi_x^2\phi_{xx}$ の項が相殺される：
\begin{equation}
 \pd{n_x}{x}
