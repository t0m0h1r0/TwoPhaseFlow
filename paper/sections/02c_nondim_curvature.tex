...
表面張力項 $\bm{f}_\sigma^*$ の離散化方針は§\ref{sec:csf}で概説し，詳細は§\ref{sec:gfm}（GFM）および付録~\ref{app:csf_full}（CSF）で述べる．
\begin{equation}
...
\begin{itemize}
    ...
    \item \textbf{表面張力項}:（無次元数 $We$ の導出には CSF 体積力形式 $\bm{f}_\sigma^* = \sigma\kappa^*\bnabla^*\psi$ を用いるが，得られる $1/We$ は GFM のジャンプ条件~\eqref{eq:gfm_jump} にも共通する）
    \begin{align*}
...
$|\bnabla\phi| = \sqrt{\phi_x^2 + \phi_y^2}$ となる．
曲率の定義は以下のようになる：
\[
...
Young--Laplace 則 $[p]=\sigma\kappa$（無次元形式は式~\eqref{eq:gfm_jump}）に代入すると $[p] = \sigma\kappa = -\sigma/R < 0$，
...
% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{擬似ヘヴィサイド関数 \texorpdfstring{$\psi$}{ψ} による曲率の直接計算}
