...
CCD 法（第~\ref{sec:CCD}章）で評価することで，滑らかな領域では高次精度の曲率が得られる．
半径 $R$ の円（$\phi = r - R$）では $\kappa = -1/R$ が解析的に再現される
...
% ═══════════════════════════════════════════════════════════════════════════════
\subsubsection{本稿で参照するショートペーパー系列（SP 索引）}
\label{sec:sp_index}
% ═══════════════════════════════════════════════════════════════════════════════

本稿 §~\ref{sec:levelset}--§~\ref{sec:algorithm} の各章は，
2026-04-20 以降に策定されたショートペーパー
SP-A..SP-N（詳細は \texttt{docs/memo/short\_paper/SP\_INDEX.md}
および計画書 SP-O）を骨格として再構成されている．
各 SP の論文内での主担当章は以下のとおりである：

\begin{center}
\begin{tabular}{l p{0.60\linewidth} c}
\hline
\textbf{SP} & \textbf{主題} & \textbf{主担当章} \\ \hline
SP-A & FCCD 面中心アップウィンド CCD（H-01 救済） & §~\ref{sec:ccd_def}, §~\ref{sec:balanced_force} \\
SP-B & Ridge--Eikonal ハイブリッド再初期化 & §~\ref{sec:levelset} \\
SP-C & FCCD 行列定式化・非一様拡張 & §~\ref{sec:ccd_def}, §~\ref{sec:grid} \\
SP-D & FCCD 移流（Option B/C, BF 整合性） & §~\ref{sec:advection} \\
SP-E & Ridge--Eikonal の非一様格子拡張 & §~\ref{sec:levelset}, §~\ref{sec:grid} \\
SP-F & GPU-native FVM 圧力投影 & §~\ref{sec:pressure} \\
SP-G & DCCD の修正方程式による教示的導出 & §~\ref{sec:ccd_def} \\
SP-H & FCCD face jet $\mathcal{J}_f(u)=(u_f,u'_f,u''_f)$ プリミティブ & §~\ref{sec:ccd_def}, §~\ref{sec:advection}, §~\ref{sec:pressure} \\
SP-I & Level-1/2/3 時間積分（二相 UCCD6-NS） & §~\ref{sec:time_int} \\
SP-J & Balanced-Force 設計原理 P-1..P-7 & §~\ref{sec:balanced_force} \\
SP-K & 粘性項 3 層（応力発散 + 欠陥補正） & §~\ref{sec:advection} \\
SP-L & 変数別移流方策・CLS 6 段階・8-phase 時間ステップ & §~\ref{sec:advection}, §~\ref{sec:algorithm} \\
SP-M & 純 FCCD 二相 DNS（分相 PPE + GFM） & §~\ref{sec:pressure}, §~\ref{sec:algorithm} \\
SP-N & UCCD6 保序ハイパー粘性（旧 SP-H） & §~\ref{sec:ccd_def} \\
SP-O & 論文 §2--§10 SP-core 改稿計画（本計画書） & ---（プロジェクト文書） \\
\hline
\end{tabular}
\end{center}

\noindent
各章は冒頭で該当 SP を明示し，本文内の方程式・離散化・コード位置・検証テストの
対応（A3 トレーサビリティ）を付録~\ref{sec:gpu_fvm_a3_roadmap} に集約する．
