...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{具体例：半径 $R$ の円による曲率式の検証}
...
分母：$\phi_x^2 + \phi_y^2 = 1$（真の符号付き距離関数の性質）．
分子：
\begin{align*}
  ...
  &= \frac{y^2}{r^2}\cdot\frac{y^2}{r^3}
   ...
   = \frac{(x^2+y^2)^2}{r^5} = \frac{1}{r}
\end{align*}
よって $\kappa = -1/r$．界面 $r = R$ 上では $\kappa = -1/R < 0$
（液滴内部が液相で $\hat{\bm{n}}$ が外向き，§~\ref{sec:notation} の符号規約と一致）．
Young--Laplace 則 $[p]=\sigma\kappa$ に代入すると $p_\text{liq} - p_\text{gas} = \sigma/R > 0$
（液滴内部の圧力が外部より高い）となり，物理的に正しい．

実装確認には，$E_\kappa(h) \equiv \|\kappa_h - (-1/R)\|_{L^\infty}$ の $\Ord{h^6}$ 収束テストが有効である
（§~\ref{sec:verify_curvature} 参照）．
