% paper/sections/12e_coupling.tex
%
% §12.4  二相流結合の物理的整合性
% ═══════════════════════════════════════════════════════════════════════════════
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Galilean 不変性}（§~\ref{sec:galilean_invariance}）：
        移動界面下での CLS↔PPE 結合（$\sigma = 0$ は成功，$\sigma > 0$ は破綻）
  \item \textbf{結合の限界の明確化}（§~\ref{sec:coupling_limitations}）：
        上記テスト結果から抽出された根本的制限の解析的裏付けと解決策
\end{enumerate}

\noindent\textbf{主要な発見（概要）：}
静止界面と $\sigma = 0$ の移動界面は正しく動作するが，
\textbf{移動界面＋表面張力（$\sigma > 0$）は 2 ステップで発散する}
（Galilean テスト B，§~\ref{sec:galilean_invariance}）．
根本原因は，IPC の $\bnabla p^n$ が界面の圧力ジャンプ $\Delta p = \sigma\kappa$ を横切り，
CCD の界面横断ステンシルが $\Ord{1}$ の打ち切り誤差を生じることにある．
この限界は分相 PPE 解法により解決される（§~\ref{sec:coupling_limitations}）．
...
\noindent\textbf{目的：}
§~\ref{sec:bf_static_droplet}（§12.1）の Laplace 圧テストを
多ステップ時間積分に拡張し，寄生流れが有界に留まり
Laplace 圧均衡が長時間維持されることを確認する．

\noindent\textbf{§12.1 との相違：}
§~\ref{sec:bf_static_droplet} は $p^0 = 0$ からの\textbf{単ステップ投影}で
Laplace 圧の離散再現精度のみを検証した．
本テストは同一液滴を\textbf{200 ステップ時間積分}し，
PPE を毎ステップ解き直すことで寄生流れの蓄積と圧力場の時間的安定性を検証する．
...
\begin{itemize}[noitemsep]
  ...
  \item 非増分投影法（non-incremental projection）：毎ステップ $p$ を全計算．
        IPC の $\bnabla p^n$ 項は界面の圧力ジャンプを横切るため
        $\Ord{1}$ 誤差を生じうる（§~\ref{sec:galilean_invariance} テスト B で実証）．
        本テストでは $\bnabla p^n$ 項を除去し，CSF＋PPE 結合のみを分離検証する
  \item $N = 64, 96, 128$，$\Delta t = 0.25\,h$，200 ステップ
\end{itemize}
...
\end{tabular}
\end{table}
\footnotetext{$\rho_l/\rho_g = 5$，$N = 64$ でも 200 ステップにわたり安定動作を確認
（$\|\bu_{\mathrm{para}}\|_\infty^{\mathrm{peak}} = 3.1 \times 10^{-4}$，
$\Delta p$ 相対誤差 $1.4\%$）．
$N = 128$ では寄生流れの成長比が 5.3 に達し，条件付き安定と判定した．
紙面の制約上，表には $\rho_l/\rho_g = 2$ のみ掲載する．}
...
§~\ref{sec:interface_crossing} の界面型密度場における格子収束劣化と整合する．
...
\midrule
A（$\sigma{=}0$） &  64 & $< 10^{-15}$ & --- & $3.3 \times 10^{-4}$ & $3.2 \times 10^{-2}$ & ○ \\
\midrule
...
$N = 64$ で形状誤差は $3.2 \times 10^{-2}$ であり，
CLS 移流の数値拡散に起因する界面形状のわずかな変形を反映する．
一方，実質的な並進成分（$\|\bu_{\mathrm{para}}\|_\infty$）は機械精度 $< 10^{-15}$ に保たれており，
離散化に起因する物理的な Galilean 不変性の劣化は観測されない．

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\linewidth]{figures/ch12_hermite_galilean}
\caption{Galilean 不変性テスト（$\rho_l/\rho_g=2$，$U=1$，1 周期）．
...
テスト A は定常並進であったが，
Rayleigh--Taylor 不安定性（$\sigma = 0$）~\cite{Chandrasekhar1961} を動的テストとして追加実施した．
...
（§~\ref{sec:coupling_limitations} で詳述）．
...
$\eta_{tt} = -(\sigma/(\rho_l{+}\rho_g))|D|^3\eta$
の安定限界で検証した（$N = 32$--$256$，RK4，2 分探索）．

再試験では測定されたスケーリング指数は $\mathbf{1.500}$ であり，
理論値 $3/2$ と一致した．
$\Delta t_{\max} / \Delta t_\sigma$ の比率は全解像度で $1.28$ に保たれ，
毛細管 CFL 式が安定限界の解像度依存性を正しく捉えることを確認した．
本稿 7-step フロー Step 0 の $C_{\mathrm{CFL}} = 0.45$ は
