...
\begin{enumerate}
  ...
  \item \textbf{PPE 求解：}
        ...
        変密度条件（§\ref{sec:verify_ppe_varrho}）は $\rho_l/\rho_g = 1$ で等密度テストと一致することを確認済みであり，
        高密度比での検証は今後の課題である．
  \item \textbf{時間積分：}
...
\small
\begin{tabular}{lp{0.14\linewidth}p{0.18\linewidth}l}
\toprule
...
Extension PDE 場延長（1D） & $\Ord{h^1}$ & {（実験値 TBD）} & 上風1次精度（数値実験は今後） \\
Extension PDE + CSF（Laplace） & $\Ord{h^1}$ & {（実験値 TBD）} & Young--Laplace ジャンプ（今後実施） \\
PPE（欠陥補正 $k{=}3$，Dirichlet） & $\Ord{h^6}$ & $\Ord{h^{7.0}}$ & ディリクレ超収束 \\
PPE（Neumann + ゲージ固定） & $\Ord{h^6}$ & $\Ord{h^{5.0}}$ & 境界スキーム $\Ord{h^5}$ が律速 \\
PPE（変密度 $\rho_l/\rho_g{=}1000$） & $\Ord{h^6}$ & {（実験値 TBD）} & 条件数悪化時の収束確認 \\
