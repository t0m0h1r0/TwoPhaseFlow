...
\begin{enumerate}
  ...
  \item \textbf{界面処理：}
        ...
        Extension PDE は場延長の収束テストで正確な $\Ord{h^{1.0}}$ を確認し，
        Young--Laplace 静止液滴テストでは $N = 128$ で相対誤差 $0.9\%$ を達成した
        （§\ref{sec:verify_extension_pde}）．
  \item \textbf{PPE 求解：}
...
Extension PDE 場延長（1D） & $\Ord{h^1}$ & $\Ord{h^{1.0}}$ & 界面サンプリング誤差が支配 \\
Extension PDE + CSF（Laplace） & $\Ord{h^1}$ & 相対誤差 $0.9\%$ & $N{=}128$, $\Delta p = 3.96$（解析 $4.0$） \\
