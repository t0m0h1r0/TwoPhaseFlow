...
\begin{enumerate}
  ...
  \item \textbf{曲率計算：}
        ...
        Extension PDE は $\Ord{h^6}$ を達成するため，界面近傍の律速は
  ...
  \item \textbf{界面処理：}
        ...
        Extension PDE のベースライン（上風差分）実装は場延長収束テストで $\Ord{h^{1.0}}$ を確認した．
...
Extension PDE + CSF（Laplace） & $\Ord{h^6}$† & 相対誤差 $0.9\%$ & $N{=}128$, $\Delta p = 3.96$（解析 $4.0$） \\
