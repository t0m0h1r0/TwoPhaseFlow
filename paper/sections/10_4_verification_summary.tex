...
\begin{enumerate}
  ...
  \item \textbf{曲率計算：}
        ...
        ただし Extension PDE（§\ref{sec:gfm}）の上風1次精度（$\Ord{h^1}$）が
        界面近傍の全体精度を律速するため，曲率計算経路の選択は実用上の精度に影響しない．
  \item \textbf{界面処理：}
        CLS 保存型再マッピングは LS 非保存補間に比べ質量誤差を 1--2 桁低減した
        （§\ref{sec:cls_conservation}）．
        Extension PDE は場延長の収束テストで $\Ord{h^1}$，
        Young--Laplace 精度テストで安定に収束することを確認した（§\ref{sec:verify_extension_pde}）．
  \item \textbf{PPE 求解：}
        欠陥補正法は $k=3$ 回の反復で CCD 演算子の $\Ord{h^6}$ 精度を完全に発現させた（§\ref{sec:verify_ppe}）．
        FD 直接法と比較して $N=128$ で $4{,}900$ 万倍高精度であり，コスト増は $3.4$ 倍に過ぎない．
        Neumann BC + ゲージ固定（§\ref{sec:verify_ppe_neumann}），
        変密度条件 $\rho_l/\rho_g$ = 1--1000（§\ref{sec:verify_ppe_varrho}）での収束も確認した．
  \item \textbf{時間積分：}
        TVD-RK3 は ODE テストで $\Ord{\Delta t^{3.00}}$ の正確な 3 次収束を確認した．
        AB2 は同一テストで $\Ord{\Delta t^{2.00}}$ を確認した（§\ref{sec:verify_ab2}）．
\end{enumerate}
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Heaviside 正則化 $H_\varepsilon$（物性値補間）：}
        §\ref{sec:verify_extension_pde}(b) の Young--Laplace テストにおいて密度場 $\rho(\psi)$ の補間が
...
Extension PDE 場延長（1D） & $\Ord{h^1}$ & \textit{（実験値 TBD）} & 上風1次精度を界面近傍で確認 \\
Extension PDE + CSF（Laplace） & $\Ord{h^1}$ & \textit{（実験値 TBD）} & Young--Laplace ジャンプ；$\varepsilon$ 非依存を確認 \\
PPE（欠陥補正 $k{=}3$，Dirichlet） & $\Ord{h^6}$ & $\Ord{h^{7.0}}$ & ディリクレ超収束 \\
PPE（Neumann + ゲージ固定） & $\Ord{h^6}$ & \textit{（実験値 TBD）} & 零モード処理の検証 \\
PPE（変密度 $\rho_l/\rho_g{=}1000$） & $\Ord{h^6}$ & \textit{（実験値 TBD）} & 条件数悪化時の収束確認 \\
TVD-RK3         & $\Ord{\Delta t^3}$ & $\Ord{\Delta t^{3.00}}$ & ODE テストで確認 \\
AB2（初回 Euler 付き） & $\Ord{\Delta t^2}$ & \textit{（実験値 TBD）} & NS 対流項の時間精度 \\
\bottomrule
\end{tabular}
\end{table}
