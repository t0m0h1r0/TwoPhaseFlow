...
最後に，アルゴリズム Step~1 の CLS 移流で用いる TVD-RK3
（§\ref{sec:verify_tvd_rk3}）と
Step~5 の Predictor で用いる AB2（§\ref{sec:verify_ab2}）の時間積分精度を確認する．
...
$k = 1$ では欠陥修正が 1 回しか入らないため $L_L$ の打ち切り誤差が支配し，
収束次数は $\Ord{h^2}$ にとどまる．
...
\begin{equation}
  \mu_{\mathrm{ADI}}
    \approx 1 + \lambda_H \,\Delta\tau^2
    = 1 + \Ord{h^4}
\end{equation}
%
と見積もられ，$|\mu_{\mathrm{ADI}}| < 1$ を達成するには
...
$\rho_{\max}/\rho_{\min} \approx 999$ においても収束次数は $\Ord{h^{5.8}}$ を維持し，
...
PPE ソルバの空間精度と限界を確認した．
残る検証項目は，アルゴリズム Step~1（CLS 移流）と Step~5（Predictor）で用いる
2 つの時間積分スキームである．
...
時間精度を，スカラー ODE $\mathrm{d}q/\mathrm{d}t = -q$，$q(0)=1$ の初期値問題で確認する．
$t=1$ における厳密解 $q(1) = e^{-1}$ と数値解の差を
$n = 16, 32, 64, 128, 256, 512$ ステップの系列で計測した．
...
TVD-RK3 は CLS 移流に用いられる．
NS 方程式の対流項には AB2（Adams--Bashforth 2 段）を採用しており，
その精度を同一 ODE テストで確認する．
...
§~\ref{sec:ab2_ipc} で導入した AB2（Adams--Bashforth 2 段）は
多段法であるため初期ステップが必要であり，本テストでは前進 Euler で起動する．
同一の ODE $\mathrm{d}q/\mathrm{d}t = -q$ に対し，$t=1$ の誤差を計測した．
