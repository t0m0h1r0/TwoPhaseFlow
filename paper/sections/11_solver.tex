...
次に，本稿の主要 PPE 戦略である分相 PPE + DC $k{=}3$ の密度比非依存性を検証する．
...
各相で定数密度のポアソン方程式を解くため，
§~\ref{sec:verify_ppe} の等密度 DC $k{=}3$（$\Ord{h^7}$）が密度比によらず回復するはずである．
本項ではこれを 3 手法の比較実験で実証する．

\noindent\textbf{手順：}
製造解 $p^* = \sin\pi x\sin\pi y$（Dirichlet BC），
円形界面（$R = 0.25$，中心 $(0.5, 0.5)$），
smoothed Heaviside 密度場（$\varepsilon = 1.5h$）を設定し，以下を比較する：
\begin{enumerate}[noitemsep]
  \item[(a)] \textbf{変密度一括 DC $k{=}3$：}
        $\bnabla\cdot(\rho^{-1}\bnabla p) = q$（CCD $L_H$ + FD $L_L$，変動係数）
  \item[(b)] \textbf{分相 PPE + FD 直接法：}
        $\nabla^2 p = f$（定数密度 FD \texttt{spsolve}，$\Ord{h^2}$）
  \item[(c)] \textbf{分相 PPE + DC $k{=}3$：}
        $\nabla^2 p = f$（定数密度 CCD $L_H$ + FD $L_L$）
\end{enumerate}
...
本稿では分相 PPE 解法を主要戦略として採用するが（§~\ref{sec:split_ppe_framework}），
CCD の変密度演算子自体の精度特性を参考として記録する．
密度場 $\rho(x,y) = 1 + A\sin(\pi x)\cos(\pi y)$
（$A = 0, 0.8, 0.98, 0.998$，$\rho_{\max}/\rho_{\min} \approx 1, 9, 99, 999$）
を設定し，DC $k=3$（Dirichlet BC）で格子収束試験を行った結果，
...
DC が発散する本質的限界が確認されており（§~\ref{sec:interface_crossing}），
これが分相 PPE 解法を必要とする根拠となっている．

以上で PPE ソルバの空間精度を確認した．
