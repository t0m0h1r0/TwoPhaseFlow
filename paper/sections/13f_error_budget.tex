...
V12 は第~\ref{sec:val_oscillating_droplet}節の閉界面標準設定と
過去の sharp-volume 補修候補を照合する採用境界として扱う．
...
V11 も Type-A/B/D ではなく，アクティブ幾何毛管分解を標準構成へ入れる前の
\textbf{採用境界}として別枠で扱う．
V12 も Type-A/B/D ではなく，閉界面標準設定を合格ベンチへ
昇格してよいかを決める\textbf{採用境界}として別枠で扱う．
...
V11 & アクティブ幾何毛管分解ゲート & 全圧力像への過射影を棄却し，$R_p$，グラフ HFE，面 Hodge で加速度へ写した補間，移動格子面量，圧力履歴座標，DC 収束判定を採用境界にする & 厳密全圧力分解は毛管波駆動 $0$；成分体積 Hodge 診断は N32/N64 $2.1176/2.3055$；グラフ HFE $\|j\|_\infty=11.5323$，加重平均 $1.30\times10^{-16}$；正則層 $x$ 変位 $0$ & \checkmark 契約 \\
V12 & 閉界面体積の採用境界 & 現行 \texttt{active\_geometry\_capillary} 設定を構築し，過去の sharp-volume/Ridge--Eikonal 補修を不合格として停止する & 現行構築 ok；sharp-volume 追加と Ridge--Eikonal 追加はともに \texttt{ValueError}；1 step で $\Delta M_\psi/M_{\psi,0}=-1.080216\times10^{-1}$，$\Delta V_{\mathrm{P1}}/V_{\mathrm{P1},0}=-5.993794\times10^{-3}$；2 step 目は面履歴の再射影で停止；1 周期標準実行は不受理 & \checkmark ゲート \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}\setlength{\itemsep}{0pt}
  ...
  \item \textbf{アクティブ幾何毛管分解の圧力反力分解ゲート}．V11 は，
        ...
        第~\ref{sec:benchmarks}章の毛管波結果は縮約チャートの範囲で
        この条件を満たす経路として読む．圧力・速度消費側の
        \texttt{force\_admissible} は引き続き $0$ であり，
        標準実行経路への接続根拠にはしない．
  \item \textbf{閉界面体積の採用境界}．V12 は，第~\ref{sec:val_oscillating_droplet}節の
        現行 \texttt{active\_geometry\_capillary} 設定と過去の
        \texttt{sharp\_phase\_volume} / Ridge--Eikonal 記述を照合する．
        現行設定は構築できるが，過去の sharp-volume 補修候補は現行 endpoint で不合格として停止する．
        現行設定の初期ステップ列は 1 step の時点で
        $\Delta M_\psi/M_{\psi,0}=-1.080216\times10^{-1}$，
        $\Delta V_{\mathrm{P1}}/V_{\mathrm{P1},0}=-5.993794\times10^{-3}$ を示し，
        2 step 目で射影後の面履歴を格子再構築後へ
        再射影できず停止する．
        したがって閉界面 1 周期標準実行は，P1 区分線形界面面積を
        全段で保存する経路が入るまで受理しない．
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item \checkmark \textbf{採用境界ゲート}: V11/V12
        (V11 は PhaseRegion 毛管経路を縮約チャートとして受理し，
        \texttt{force\_admissible=0} を保持する．V12 は閉界面標準
        設定の過去の sharp-volume/Ridge--Eikonal 記述を不合格停止へ移し，
        1 周期振動液滴を不受理のまま保持する).
\end{itemize}
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item V12 (\autoref{sec:closed_interface_volume_gate}) — 閉界面標準設定の
        状態空間整合性，過去の sharp-volume/Ridge--Eikonal 補修の不合格停止，
        初期ステップの体積/P1 面積 drift，および 1 周期標準実行の不受理境界．
\end{itemize}
