...
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{精度バジェット：V1--V12 誤差寄与の統合評価}
\label{sec:error_budget}
\label{sec:accuracy_summary}

\paragraph{構成．}
本節は V1--V12 横断の精度結果を一表に集約し，
...
U12/V11 はアクティブ幾何毛管分解の過射影棄却と統合採用ゲートとして扱う．
V12 は第~\ref{sec:val_oscillating_droplet}節の閉界面 production YAML と
旧 sharp-volume 記述を照合する採用境界ゲートとして扱う．
...
V10-b 形状軸（格子幅まで薄くなる折り畳み細線に対する Eulerian 固定格子限界）は \textbf{Type-D}：
...
V12 も Type-A/B/D ではなく，閉界面 production 設定を合格ベンチへ
昇格してよいかを決める\textbf{採用境界ゲート}として別枠で扱う．

\paragraph{統合検証成績一覧．}

\begin{table}[!ht]
\caption{V1--V12：統合精度まとめ}
\label{tab:v_accuracy_summary}
\PaperCaptionNote{収束率は log-log fitting (3 点以上)；単一値はその設定での代表値．本表は Chapter 12 の成果総括表~\ref{tab:verification_summary} と同じ「ID / テスト / 期待・判定基準 / 実測・観測値 / 判定」文法で読む．表~\ref{tab:U_summary} は成果表ではなく U 系列の検証設計マップである．V10-a/V10-b は質量保存軸 (Type-B) と形状軸 (Type-D) に分離するため，表~\ref{tab:V_summary} の V10-a/b から 1:2 に展開される（V11/V12 ゲートを含め計 16 行）．補助観測指標および全観測指標の網羅的列挙は表~\ref{tab:V_summary} を参照．}
\centering
...
V10-b-shape & 単一渦 reversal 形状軸 & 格子幅まで薄くなる折り畳み細線に対する Eulerian 固定格子下限と整合 & $L^1$ reversal error $=2.248\times10^{-2}$ & $\triangle$ Type-D \\
V11 & アクティブ幾何毛管分解ゲート & 全圧力像への過射影を棄却し，$R_p$，グラフ HFE，面 Hodge で加速度へ写した補間，移動格子面共鎖，圧力履歴座標，DC 収束判定を採用境界にする & 厳密全圧力分解は毛管波駆動 $0$；成分体積 Hodge 診断は N32/N64 $2.1176/2.3055$；グラフ HFE $\|j\|_\infty=11.5323$，加重平均 $1.30\times10^{-16}$；正則層 $x$ 変位 $0$ & \checkmark 契約 \\
V12 & 閉界面体積 admission gate & 現行 \texttt{active\_geometry\_capillary} YAML を構築し，旧 sharp-volume/Ridge--Eikonal 補修を fail-close する & 現行構築 ok；sharp-volume 追加と Ridge--Eikonal 追加はともに \texttt{ValueError}；1 step で $\Delta M_\psi/M_{\psi,0}=-1.080216\times10^{-1}$，$\Delta V_{\mathrm{P1}}/V_{\mathrm{P1},0}=-5.993794\times10^{-3}$；2 step 目は face-history 再射影で停止；1 周期 production admission $0$ & \checkmark ゲート \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}\setlength{\itemsep}{0pt}
  ...
  \item \textbf{アクティブ幾何毛管分解の圧力反力分解ゲート}．V11 は，
        ...
        V11 で検出する．成分体積 Hodge 診断は非静的波の検出器として有用だが，
        それ自体は最終 $R_p$ の代替証明ではない．したがって $R_p$ 未確定の分解は
        未受理とする．最終経路では，これに加えて
        式~\eqref{eq:ao_graph_hfe_jump}--\eqref{eq:ao_graph_hfe_face_law} の
        $q$ 所有グラフ HFE，
        式~\eqref{eq:ao_projection_face_bridge} の面 Hodge で加速度へ写した補間，
        式~\eqref{eq:ao_face_cochain_regrid} の移動格子面共鎖移送，
        式~\eqref{eq:ao_pressure_coordinate_split} の圧力履歴座標，
        式~\eqref{eq:grid_regular_stratum_axis}--\eqref{eq:grid_regular_stratum_shift}
        の正則層ガード，
        式~\eqref{eq:ao_dc_convergence_gate} の DC 収束判定を同時に要求し，
        第~\ref{sec:benchmarks}章の毛管波結果は reduced chart の範囲で
        この条件を満たす経路として読む．圧力・速度消費側の
        \texttt{force\_admissible} は引き続き $0$ であり，
        production runtime への接続根拠にはしない．
  \item \textbf{閉界面 production 体積ゲート}．V12 は，第~\ref{sec:val_oscillating_droplet}節の
        現行 \texttt{active\_geometry\_capillary} YAML と旧
        \texttt{sharp\_phase\_volume} / Ridge--Eikonal 記述を照合する．
        現行設定は構築できるが，旧 sharp-volume 補修候補は現行 endpoint で fail-close される．
        current-config prefix は 1 step の時点で
        $\Delta M_\psi/M_{\psi,0}=-1.080216\times10^{-1}$，
        $\Delta V_{\mathrm{P1}}/V_{\mathrm{P1},0}=-5.993794\times10^{-3}$ を示し，
        2 step 目で projection-native face history の格子再構築 across
        再射影に到達して停止する．
        したがって閉界面 1 周期 production は，P1 sharp area を
        全段で保存する経路が入るまで受理しない．
  ...
  \item \textbf{CLS 形状追跡誤差（質量保存軸 vs 形状精度軸の分離）}．V10 の Zalesak は
        ...
        形状 $L^1$ $8.307\times 10^{-3}$, 重心収束率 $\approx 1.22$ は固定格子
        CLS 位相/しきい値化誤差と slot/h $=6.4$ の鋭角部解像不足に
        律速される．単一渦 (V10-b) も同質量修正下で
        質量ドリフト $0.0428\%$ \checkmark / $L^1$ reversal $2.248\times10^{-2}$ ($T=8$ の
        格子幅まで折り畳まれた細線に対する Eulerian 固定一様格子の理論的下限~\cite{EnrightFedkiwFerzigerMitchell2002}) と
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item $\triangle$ \textbf{Type-D 理論的下限との整合}:
        ...
        形状 $L^1$ $8.307\times 10^{-3}$ — 固定格子 CLS 位相/しきい値化誤差と
        slot/h $=6.4$ 鋭角部解像不足律速),
        V10-b 形状軸 ($T=8$ 単一渦 reversal $L^1=2.248\times10^{-2}$
        — 格子幅まで折り畳まれた細線に対する Eulerian 固定格子原理的下限~\cite{EnrightFedkiwFerzigerMitchell2002}).
  ...
  \item \checkmark \textbf{採用境界ゲート}: V11/V12
        (V11 は PhaseRegion 毛管経路を reduced chart として受理し，
        \texttt{force\_admissible=0} を保持する．V12 は閉界面 production
        YAML の旧 sharp-volume/Ridge--Eikonal 記述を fail-close へ移し，
        1 周期振動液滴を未受理のまま保持する).
\end{itemize}
...
\paragraph{検証対応の統合．}
本節は V1--V12 全結果の横断統合点である．各実験が測定した式・離散化・物理指標の対応を
以下に集約する：
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item V12 (\autoref{sec:closed_interface_volume_gate}) — 閉界面 production YAML の
        state-space 整合性，旧 sharp-volume/Ridge--Eikonal 補修の fail-close，
        current-prefix 体積/P1 面積 drift，および 1 周期 production 不受理境界．
\end{itemize}
