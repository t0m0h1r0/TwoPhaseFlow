...
U12/V11 は AO-Fast 幾何毛管分解の統合前 gate として扱う．
...
V4 は固定壁 uniform-offset 診断の $O(\|\bU\|)$ 有界残差，
...
V11 も Type-A/B/D ではなく，AO-Fast packet を標準物理経路へ入れる前の
\textbf{fail-close gate}として別枠で扱う．
...
V4 & uniform-offset 残差 & $O(\|\bU\|)$ 有界残差，運用上限 $<3$ & $\|\Delta u\|_\infty^{\max}/\|\bU\|=2.63$ & \checkmark Type-A \\
...
V11 & AO-Fast 幾何毛管分解 gate & full-pressure 過射影を棄却し，未解決 GPU packet は fail-close & CPU exact full-pressure split は wave drive $0$；component-volume probe は N32/N64 $2.1176/2.3055$；GPU は fail-close & \checkmark gate \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}\setlength{\itemsep}{0pt}
  ...
  \item \textbf{射影残差誤差と圧力代表誤差の分離}．
        ...
        第13章に追加するものではない．
  \item \textbf{AO-Fast 圧力反力分解 gate}．V11 は，
        式~\eqref{eq:capillary_pressure_reaction_projection} の $R_p$ を定めない
        full pressure image 分解が，毛管波の面上駆動を代数的に消し得ることを
        統合前に検出する．component-volume Hodge probe は非静的波の検出器として有用だが，
        それ自体は最終 $R_p$ の代替証明ではない．したがって現行 GPU AO-Fast packet は
        fail-close し，標準 pressure-jump 経路の V6/V7/V9 と第~\ref{sec:benchmarks}章の
        毛管波結果から分離する．
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item \checkmark \textbf{Type-A 判定基準合格}:
        ...
        V4 (固定壁 uniform-offset $O(\|\bU\|)$ 有界残差, $\|\Delta u\|_\infty^{\max}/\|\bU\| = 2.63$)，
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item V11 (\autoref{sec:ao_fast_capillary_gate}) — AO-Fast 幾何毛管分解の
        full-pressure 過射影反例，component-volume Hodge probe，GPU fail-close gate．
\end{itemize}
