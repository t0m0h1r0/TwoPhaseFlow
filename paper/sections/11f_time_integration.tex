...
（§~\ref{sec:verify_tvd_rk3}），
Step~5（Predictor）で用いる AB2（§~\ref{sec:verify_ab2}），
Crank--Nicolson 粘性項の時間精度と無条件安定性（§~\ref{sec:verify_cn_viscous}），
および交差微分粘性項の陽的処理が時間精度に与える影響（§~\ref{sec:verify_cross_viscous}）
を検証する．
...
設計時間精度 $\Ord{\Delta t^3}$ を，スカラー ODE テストおよび固定格子移流テストで確認する．
前者で漸近精度を，後者で空間離散のスペクトル半径による安定域制約を診断する．

\noindent\textbf{評価対象：}
TVD-RK3（3 段 Shu--Osher 型 Runge--Kutta）の時間精度と安定域．

\noindent\textbf{手順とパラメタ：}
\begin{itemize}[noitemsep]
  \item \textbf{ODE テスト：} スカラー ODE $\mathrm{d}q/\mathrm{d}t = -q$，$q(0)=1$，厳密解 $q(1) = e^{-1}$；
        ステップ数 $n = 4, 8, 16, 32, 64, 128, 256, 512$
  \item \textbf{固定格子移流テスト：} 2 次元周期格子 $N = 256$，
        初期値 $q_0 = \sin(2\pi x)$，空間微分は CCD（$\Ord{h^6}$，周期境界）；
        $\mathrm{d}q/\mathrm{d}t = -\partial q/\partial x$，$T = 1$，
        ステップ数 $n = 10, 20, 40, 80, 160, 320, 640$（$\Delta t$ を粗から細へ精緻化）
\end{itemize}
...
\caption{TVD-RK3 の時間収束．
(a) スカラー ODE では設計通り $\Ord{\Delta t^3}$ の 3 次精度を達成する．
(b) 固定格子移流では粗い時間刻みで安定域を外れるが，
安定域内の細かい時間刻みでは 3 次の低下傾向を示す．}
\label{fig:tvd_rk3}
\end{figure}

\noindent\textbf{結論：}
ODE テストでは収束次数が漸近的に $3.00$ に近づき，
設計通りの $\Ord{\Delta t^3}$ が確認された．
固定格子移流テストは空間離散のスペクトル半径による安定域制約を受けるため，
粗い時間刻みの発散点は時間精度判定から除外する．
...
\noindent\textbf{目的：}
§~\ref{sec:time_level1} で導入した EXT2 外挿（IMEX-BDF2 の対流処理；
AB2 と同じ $(3/2,-1/2)$ 係数の 2 次零安定多段法）の
設計時間精度 $\Ord{\Delta t^2}$ を，AB2 単体の参照テストとして確認する．
...
前進 Euler は $\mathrm{CFL}_\nu = 5$ で $\max|u| \approx 5.95 \times 10^2$（初期値の約 600 倍），
$\mathrm{CFL}_\nu = 10$ で $\approx 6.45 \times 10^5$ と急激な不安定成長を示した．
