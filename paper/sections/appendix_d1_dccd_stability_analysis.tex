% paper/sections/appendix_d1_dccd_stability_analysis.tex
%
...
§~\ref{sec:verification} 既定の dissipative フィルタ強度 $\varepsilon_d^{\mathrm{adv}} = 0.05$
（CCD 移流再構成の散逸係数；式~\eqref{eq:dccd_adv_filter} で定義）のとき
$\max_\xi\{H(\xi)\,\hat{k}^*(\xi)\} \approx 1.9$
（$\xi \approx 0.8\pi$ 付近の数値最大化）であるから：
\[
