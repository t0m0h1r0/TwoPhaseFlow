...
\begin{enumerate}[leftmargin=2em,noitemsep]
  ...
  \item \textbf{Step 3（CCD 演算子構築）：} 非一様格子重みから
        ...
        （格子更新 Mode 1 では以降不変）．
  \item \textbf{Step 4（Young--Laplace 初期圧力）：} 初期曲率 $\kappa^0$ から
        $p^0 = p_\infty + \sigma\kappa^0 H(\phi^{(0)})$（smoothed）で初期圧力場を設定し，
        Step 0 で測定した $\mathbf{u}^0=\mathbf{0}$ と整合させる．
\end{enumerate}
5 段階実行後，時刻ループ Step 1 から通常アルゴリズム（§\ref{sec:algo_flow}）が動作する．
