...
\]
を連続体の基準時刻とする．
ただし，現時点の図表と下記の完走値は，連続 level-set
（CLS）経路で得た履歴結果であり，最新のアクティブ幾何毛管分解
（AO）経路の検証結果ではない．したがって，これらの時刻と振幅履歴を
AO の合格基準として用いてはならない．最新 AO 経路では，
$q$ が所有する列体積グラフ，分相 PPE と HFE による圧力ジャンプ境界式，
pressure-component Hodge reaction，および界面追随格子再構築後の
metric 一貫性を同時に満たすことを別途検証する．
旧 CLS 図として論文に掲載する 2 次元 snapshot は，
...
\begin{itemize}[noitemsep,leftmargin=2em]
  ...
  \item 時間：旧 CLS 掲載窓 $T=0.035379718894\,\mathrm{s}$，
...
以下の数値は旧 CLS 経路の掲載窓結果である．N=32 の掲載窓 run は $1623$ samples で
...
この結果は最新 AO 経路の完走証明ではない．AO 経路の判定では，
CLS 由来の snapshot 窓ではなく，連続理論の初期加速度
$A''(0)=-\omega_\sigma^2A_0$，AO 離散表面エネルギーの一次変分，
分相 PPE+HFE の同一圧力演算子，および $q$ 列高さ振幅の整合を
検証対象とする．U12 で棄却した全圧力像への過射影を成功と読むことも，
旧 CLS の戻り窓を AO の周期再現と読むことも許されない．
