...
N=32 の 1 周期 run は remote GPU 上で完走し，$3452$ samples で
...
本ケースから得る結論は成功主張ではなく，fail-close された残課題である．
毛管波が平坦界面の符号検証なら，振動液滴は移流・粘性項を含む
閉界面エネルギー交換と保存量整合のベンチマークである．
現状の production stack は，1 周期の楕円液滴に対して
...
移動界面二相流の代表的な安定振動・浮力上昇・不安定成長に対して
同じ stack が有効に働く範囲と，閉界面振動でなお破れている
保存量契約の境界を同時に示した，という点にある．
