...
線形増幅から非線形形状形成までを追う．
静止円形液滴は第~\ref{sec:verification}章の BF 検証で扱ったため，
本章では重複させない．
...
静止円形液滴は第~\ref{sec:verification}章で扱ったため，
...
\begin{itemize}[noitemsep,leftmargin=2em]
  ...
  \item 物性：$\rho_l=1000$，$\rho_g=1.2$，
        $\mu_l=10^{-3}$，$\mu_g=1.8\times10^{-5}$，
        $\sigma=0.072$，$g=0$
  \item 時間：$T=40.0$，理論 CFL multiplier $1.0$
...
\texttt{signed\_deformation} は
$D_0\cos(\omega_0 t)$ を第一基準として符号反転し，
約 1 周期にわたって Rayleigh--Lamb 型の往復運動を示した．
粘性項は振動の位相を破壊するのではなく包絡線の減衰として現れ，
\texttt{kinetic\_energy} は形状変形エネルギーとの交換に対応して振動する．
...
毛管波 &
  ...
  \checkmark \\
振動液滴 &
  $128^2$，$T=40$，水--空気，periodic，$D_0=0.10$ &
