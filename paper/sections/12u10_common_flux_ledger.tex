...
%
% U10: Conservative common-flux ledger condition — Tier VIII
% ═══════════════════════════════════════════════════════════════════════════════
...
U10 は，この台帳同一性を単体レベルで検証する．
体積だけを後から合わせる，または $\psi$ だけを切り詰め・射影する経路は，
密度・運動量に同じ保存写像を与えないため合格経路とはしない．

\paragraph{設定．}
周期正方領域上の滑らかな $\psi$ を FCCD 保存形フラックス + TVD-RK3 で 1 step 更新し，
返された段階台帳を \texttt{ConservativeCommonFluxTransport} へ渡す．
密度は
\[
...
負対照は (i) 非アフィン密度，(ii) $\psi$ だけを射影した台帳である．

\paragraph{結果．}

\begin{table}[!ht]
\caption{U10：保存形共通フラックス台帳条件}
\label{tab:U10_common_flux_ledger}
\PaperCaptionNote{全 $N$，密度比，CFL の組合せで閉じた候補は密度・運動量保存とアフィン密度写像を満たし，負対照 2 件は不適合として棄却された．周期格子の保存量は重複端点を除く一意なテンソル格子上で測る．}
\centering\small
\begin{tabular}{l c}
\toprule
指標 & 実測値 \\
...
\PaperCaptionNote{左：閉じた共通フラックス候補の相対エネルギー増分は全 CFL・密度比で非正．右：密度を先に動かして速度を据え置く，または運動量だけを別写像で動かす開いた対照は診断値として分離し，保存形共通フラックスの合格経路へは入れない．}
\end{figure}

\paragraph{判定と次章への接続．}
U10 は \checkmark と判定する．
これは保存形共通フラックス経路の単体条件であり，
第13章の V 系列へ新しい物理検証として追加するものではない．
