...
\noindent\textbf{離散化契約との対応：}
本稿の標準粘性離散化は FVM フェイス積分ではなく，
CCD 積則微分 $\nabla\cdot[\mu(\bnabla\bu+\bnabla\bu^\top)]$ として評価する．
すなわち各格子点で $\mu$ と速度勾配を同じ CCD 系で求め，合成後に再度 CCD 微分をとる．
滑らかな $\mu$ 場ではこの契約により高次 CCD 微分の設計精度を保つ．
一方，界面帯での実効次数は $\mu$ の正則性，CLS 厚み，再初期化品質に律速されるため，
検証結果で判定する．本付録の算術平均導出は，FVM 低次モデルを比較対象にする場合の
参照閉包として掲載している．
