...
本研究では以下の形式を \texttt{rhie\_chow.py} の \texttt{face\_velocity\_divergence} に実装した
（オプション引数 \texttt{kappa}，\texttt{psi}，\texttt{we} を渡すと有効化される；
表面張力支配の問題 $We \ll 1$ での使用を推奨する）：
\begin{equation}
  u_f = \bar{u}_f
...
\noindent\textbf{実装状況：} \texttt{rhie\_chow.py} は式~\eqref{eq:rc-face}（圧力補正のみ，デフォルト）と式~\eqref{eq:rc-face-balanced}（BF-RC，オプション）の両形式を実装している．\texttt{face\_velocity\_divergence} に \texttt{kappa}，\texttt{psi}，\texttt{we} を渡すと式~\eqref{eq:rc-face-balanced} が有効化される．定量的検証は \texttt{experiments/balanced\_force\_rc\_benchmark.py}（§\ref{sec:bench_droplet} 参照）で実施できる．
