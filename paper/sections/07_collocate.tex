...
\textbf{注：} 本節で述べる Balanced-Force 条件は CSF モデル（付録~\ref{app:csf_full}）を前提とする．
本稿では CSF 体積力を予測子ステップに維持し，Balanced-Force 条件（§~\ref{sec:rc_balanced_force}）を適用する．
Extension PDE（§~\ref{sec:extension_pde}）が界面近傍の圧力場を滑らかに延長することで，
CCD の $\Ord{h^6}$ 精度を維持しつつ寄生流れをさらに抑制する．
...
特にコロケート格子では，変密度 PPE の離散演算子がチェッカーボードモード（波長 $2h$）に対してほぼ零固有値を持つ（§~\ref{sec:checkerboard_solution} の圧力デカップリング参照）．この駆動力がその零モード方向に投影されると，PPE ソルバの収束をほぼ妨げず，補正速度に $O(1/\lambda_{\min})$ の増幅が生じる（$\lambda_{\min} \approx 0$ はチェッカーボードの固有値）．デカップリング対策を省略または誤実装した状態では，この増幅がステップごとに積算されて指数的増大を招き，\textbf{最終的に計算の爆発（発散）}を引き起こす．
本稿では CSF + Balanced-Force（§~\ref{sec:balanced_force}）と Extension PDE（§~\ref{sec:extension_pde}）の組み合わせにより，さらに DCCD 散逸（§~\ref{sec:dccd_decoupling}）によりチェッカーボードモード自体を抑制するため，この正帰還ループは本質的に回避される（§~\ref{sec:bench_droplet} 参照）．
...
\paragraph{Balanced-Force 条件（CSF 時代の離散化要件）}
...
本稿では CSF 体積力を予測子ステップに維持しつつ，Rhie--Chow Balanced-Force 拡張（式~\eqref{eq:rc-face-balanced}）により
表面張力と圧力の離散化整合性を確保する（§~\ref{sec:rc_balanced_force}）．
さらに，本稿で用いる Dissipative CCD（§~\ref{sec:dissipative_ccd}）の散逸特性を利用して，
チェッカーボードモードをスキーム内部でも抑制する．
