...
同一の演算子（CCD）を両方に用いると，各項が独立に $O(h^6)$ 精度で連続微分を近似するため
（$D_{\mathrm{CCD}}^{(1)} f = \nabla f + O(h^6)$），
平衡条件 $\nabla p = (\kappa/We)\nabla\psi$ のもとで：
\[
...
（注：$D_{\mathrm{CCD}}(\kappa\psi/We) = (\kappa/We)D_{\mathrm{CCD}}\psi + (\psi/We)D_{\mathrm{CCD}}\kappa$
であるから，$\kappa$ が空間変化する一般の場合は代数的因数分解 $D_{\mathrm{CCD}}(p-\kappa\psi/We)$ は使えない．
上式の $O(h^6)$ は各演算子の精度から直接導かれる．）
...
Balanced-Force 条件の「Balanced」とは「数値離散化誤差成分の相殺（$O(h^6)$ への低減）」を指し，
CSF モデル誤差（$O(\varepsilon^2) \approx O(h^2)$）そのものを除去するものではない点に注意されたい．
また，Rhie--Chow 補正の検出項（付録~\ref{app:rc_precision}）は $O(h^2)$ の安定化項であるが，
これはフェイス速度の非物理振動抑制を担う補助項であり，
$\nabla p$ と $\kappa\nabla\psi$ の主方程式演算子（CCD）の整合性とは独立した役割を持つ
（したがって Balanced-Force の $O(h^6)$ 相殺を損なわない；付録~\ref{app:rc_precision} 末尾参照）．
