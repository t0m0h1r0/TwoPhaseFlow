...
本稿では，GFM 定式化（§~\ref{sec:gfm}）により CSF 体積力が除去されることを活かし，
Rhie--Chow 補正を用いず Dissipative CCD の内在的散逸によってデカップリングを抑制する
（以下 §~\ref{sec:dccd_decoupling}）．
...
\textbf{注：} 本節で述べる Balanced-Force 条件は CSF モデル（付録~\ref{app:csf_full}）を
前提とする．本稿の採用手法（GFM，§~\ref{sec:gfm}）では CSF 体積力が除去されるため
Balanced-Force 拡張は不要である（§~\ref{sec:dccd_decoupling} 末尾参照）．
...
本稿では GFM 採用（§~\ref{sec:gfm}）により CSF 体積力を除去したうえで，DCCD 散逸（§~\ref{sec:dccd_decoupling}）によりチェッカーボードモード自体を抑制するため，この正帰還ループは本質的に回避される（§~\ref{sec:bench_droplet} 参照）．
...
正帰還ループを通じて計算の発散を招く．
\textbf{本稿の GFM 定式化（§~\ref{sec:gfm}）では CSF 体積力が除去されるため，
この Balanced-Force 条件自体が不要となる}（§~\ref{sec:dccd_decoupling} 参照）．
