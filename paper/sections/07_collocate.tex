...
本節では，この補正項の空間精度を CCD の $p'''$ 情報を活用して $\Ord{h^4}$ に改善できることを示す．
...
\paragraph{Richardson 型補正による $\Ord{h^4}$ 化}

式~\eqref{eq:rc_detection_taylor} の主項は $-h^2/12 \cdot p'''(x_f)$ である．
CCD は $p' = D^{(1)}_\mathrm{CCD}\,p$ と $p'' = D^{(2)}_\mathrm{CCD}\,p$ を同時に返すため，
$p''$ をもう一度 CCD で微分すれば $p''' = D^{(1)}_\mathrm{CCD}(p'')$ が $\Ord{h^6}$ で得られる．
フェイス中点値を算術平均で近似すると：
\[
  \bar{p}'''_f = \frac{1}{2}(p'''_P + p'''_E) = p'''(x_f) + \Ord{h^2}
\]
この推定値を用いて RC 検出項を補正すると：
\begin{equation}
  \underbrace{(\nabla p)_f - \overline{(\nabla p)}_f}_{=\;-\frac{h^2}{12}p''' + \Ord{h^4}}
  \;+\; \frac{h^2}{12}\bar{p}'''_f
  \;=\; \Ord{h^4}
  \label{eq:rc_richardson_correction}
\end{equation}
$h^2/12 \cdot \bar{p}'''_f$ の補正項自体が $\Ord{h^2}$ であり，
式~\eqref{eq:rc_detection_taylor} の主項 $-h^2/12 \cdot p'''$ を正確に打ち消す．

\paragraph{補正後の Rhie--Chow フェイス速度}

式~\eqref{eq:rc-face-balanced} の各検出項に同様の補正を加えると：
\begin{equation}
  u_f = \bar{u}_f
    - \Delta t\left(\frac{1}{\rho^{n+1}}\right)_f^{\mathrm{harm}}
    \Biggl[
      \Bigl((\bnabla p^n)_f - \overline{(\bnabla p^n)}_f
            + \tfrac{h^2}{12}\overline{p^{n\prime\prime\prime}}_f\Bigr)
      - \Bigl((\bm{f}_\sigma)_f - \overline{(\bm{f}_\sigma)}_f
            + \tfrac{h^2}{12}\overline{(\bm{f}_\sigma)'''}_f\Bigr)
    \Biggr]
...
\paragraph{計算コスト}
追加コストは各軸あたり CCD 1 回（$p''$ の微分）のみである．
2次元問題では CCD 2 回の追加で済み，PPE 解法コストに比して無視可能である．
