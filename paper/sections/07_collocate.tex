...
CCD の周波数応答は $|\hat{D}_k| \to 0$ when $kh \to \pi$ であり，
...
\begin{tcolorbox}[mybox, title={CSF 時代の条件（参考収録）}]
\textbf{注：} 本節で述べる Balanced-Force 条件は CSF モデル（付録~\ref{app:csf_full}）を
前提とする．本稿の採用手法（GFM，§\ref{sec:gfm}）では CSF 体積力が Predictor から
完全に除去されるため，Balanced-Force 拡張（式~\eqref{eq:rc-face-balanced}）は不要となる
（§\ref{sec:dccd_decoupling} 末尾参照）．本節は設計上のトレードオフ理解のため参考収録する．
\end{tcolorbox}
...
\begin{tcolorbox}[mybox, title={Balanced-Force 条件（CSF 時代の離散化要件）}]
...
\end{align}
演算子の不一致（例：圧力に 2 次中心差分，表面張力に CCD）は
式~\eqref{eq:bf_operator_mismatch} の $\Ord{h^2}$ 残差を生じ，
正帰還ループを通じて計算の発散を招く．
...
\[
  ...
  = \Ord{h^2} \neq 0
\]
この $\Ord{h^2}$ の残差が RC 補正の括弧内に現れ，フェイス速度に非物理的な湧き出しを生じさせる．これが PPE 右辺の発散計算を通じて圧力解に逆流し，ポアソンソルバの収束性を悪化させ，計算の爆発につながる（式~\eqref{eq:bf_operator_mismatch}；詳細は付録~\ref{app:balanced_force_taylor}）．
