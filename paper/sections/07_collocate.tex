...
本稿では代わりに Dissipative CCD 散逸（§\ref{sec:dccd_decoupling}）によりこのモードを内在的に除去する；Rhie--Chow 補正の完全定式化は付録~\ref{app:rhie_chow_full} を参照．
...
\begin{equation}
  \mathcal{L}p = q + \sigma_B(p_B - g_B)\delta_B
  \label{eq:sat_penalty}
\end{equation}
ここで $\sigma_B$ はペナルティ係数，$g_B$ は境界データ（Neumann BC では $g_B=0$），$\delta_B$ は境界デルタ関数である（$\sigma_B$ は安定性解析から決定する；Carpenter \textit{et al.}~\cite{Carpenter1994}）．本稿の現実装では既存の Neumann BC（CCD ブロック三重対角境界処理）を使用しており，SAT の完全実装は今後の課題である（§\ref{sec:future_work}参照）．
