...
% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{CCD を活用した Rhie--Chow 補正の高次精度化}
\label{sec:rc_high_order}
% ─────────────────────────────────────────────────────────────────────────────

前節の Rhie--Chow 補正（式~\eqref{eq:rc-face-balanced}）は $\Ord{h^2}$ のフィルタリング効果を持つ．
本節では，CCD が同時に返す $p''$ の情報を用いて，
\textbf{追加計算コストゼロ}で空間精度を $\Ord{h^4}$ に改善できることを示す．
...
\begin{equation}
  ...
  + \underbrace{\frac{h}{12}\bigl(p''_P - p''_E\bigr)}_{\text{CCD $p''$ 補正項}}
