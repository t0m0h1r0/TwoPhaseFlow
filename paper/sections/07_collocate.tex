...
\noindent\textbf{【注意：本節の拡張形式は将来実装の提案であり，現行コードには未実装である。】}
現行コード（\texttt{rhie\_chow.py}）は §\ref{sec:rc_implementation}（式~\eqref{eq:rc-face}）の
圧力補正のみの形式を実装している．
本節では，Balanced-Force の論理をフェイス速度の計算にも\textbf{完全に}反映させるための
拡張形式を設計提案として詳述する．表面張力支配の問題（$We \ll 1$）への適用を計画している読者はこの提案を参照されたい．
...
\noindent\textbf{現行実装との差異：} 現行実装（\texttt{rhie\_chow.py}）は式~\eqref{eq:rc-face}（圧力補正のみ）を実装している．式~\eqref{eq:rc-face-balanced} の表面張力補正項は未実装であり，\texttt{face\_velocity\_divergence} の引数に \texttt{kappa}，\texttt{psi} を追加する API 変更を伴う．表面張力支配の問題（$We \ll 1$，寄生流れが顕著な系）では式~\eqref{eq:rc-face-balanced} の実装が推奨される．
