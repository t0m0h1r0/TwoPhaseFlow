...
スウィープ型前処理を採用する場合は，$x$ 方向 Thomas 法 $\to$ $y$ 方向 Thomas 法を逐次適用する：
...
\multicolumn{5}{l}{$^{\ddagger}$ 左辺に 2 次精度 FD 演算子 $\mathcal{L}_{FD}$，右辺残差に CCD 演算子 $\mathcal{L}_h$ を用いる（§\ref{sec:defect_correction}，付録 \ref{app:sweep_thomas_coeff}）．}\\
\multicolumn{5}{l}{\phantom{$^{\ddagger}$} 現行実装は \texttt{PPESolverDefectCorrection} の base solver/operator 契約で与える．}\\
\multicolumn{5}{l}{\phantom{$^{\ddagger}$} スウィープ型を使う場合は \ref{warn:ppe_splitting} の分割誤差を別途評価する．}
\end{tabular}
\end{table}

本稿では\textbf{欠陥補正法}（表~\ref{tab:ppe_methods} 5 行目）を標準 PPE ソルバとして採用する：
base solver で補正解を求め，右辺残差に CCD/FCCD/FVM の対象演算子を用いることで，
標準経路の精度評価を外側残差に集約する
（精度保証は §\ref{sec:defect_correction}，スウィープ型前処理の係数導出は付録 \ref{app:sweep_thomas_coeff} 参照）．
