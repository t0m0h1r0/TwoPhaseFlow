...
% 付録 E.2：CCD 陰解法と仮想時間ステップ反復（PPE ソルバの実装詳細）
...
現行実装で標準的に使う PPE 経路は，低次の安定な base solver で補正方程式を解き，
...
条件数，残差フロアを併せて評価しながら式~\eqref{eq:dtau_opt} 近傍を探索する，という実装指針である．
...
\noindent\textbf{現行の標準実装は，}\texttt{PPESolverDefectCorrection}\textbf{である．}
...
\textbf{実装（}\texttt{PPESolverDefectCorrection}\textbf{）との対応：}
欠陥補正法（式~\eqref{eq:defect_correction_linear}）は
\texttt{PPESolverDefectCorrection} として実装されており（表~\ref{tab:ppe_methods} 5行目），
本稿の標準 PPE ソルバ契約である．
base solver が補正方程式を解き，operator 側が対象離散化の残差
$R_h = q_h-\mathcal{L}_h p^{(m)}$ を評価するため，
実装は特定のスウィープ専用クラスに依存しない．
...
CCD Kronecker + 直接 LU（\texttt{PPESolverCCDLU}） & $\Ord{h^6}$ & なし & 直接解 & 高 \\
欠陥補正（base + target 残差）$\ddagger$ \textbf{[標準契約]} & 対象演算子依存 & base 依存 & 残差駆動 & 中 \\
\hline
...
\multicolumn{5}{l}{$^{\ddagger}$ 現行実装は \texttt{PPESolverDefectCorrection} の base solver/operator 契約で与える．}\\
