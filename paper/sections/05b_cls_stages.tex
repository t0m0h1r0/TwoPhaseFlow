...
F & 再初期化後の最終質量閉鎖 & $\phi$ 空間一様シフトを既定；$\psi$ 空間補正は legacy/remap fallback のみ\\
\hline
\end{tabular}
\end{center}
...
\begin{equation}
  \delta\phi
  =
  \frac{M_\text{target}-\sum_i H_\varepsilon(\phi_i)V_i}
       {\sum_i H'_\varepsilon(\phi_i)V_i},
  \qquad
  \phi_i \leftarrow \phi_i+\delta\phi,\qquad
  \psi_i^{\text{new}} = H_\varepsilon(\phi_i).
...
曲率非依存に一様シフトする．高曲率・表面張力ありの再初期化直後に
