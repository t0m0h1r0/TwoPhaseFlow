...
\begin{enumerate}[noitemsep]
  ...
  \item 毛管ジャンプ cochain は式~\eqref{eq:capillary_surface_energy_riesz_cochain}
        ...
        式~\eqref{eq:capillary_hodge_gate} の Hodge 平衡判定で
        静的平衡成分と非平衡駆動成分を分ける
\end{enumerate}
\end{tcolorbox}
...
ただし射影後の面速度を保存する affine-jump IPC では，
...
変分毛管 cochain の Hodge 平衡判定，DC 残差契約，
...
この残りの条件を次項の変分毛管 cochain と Hodge 平衡判定として定式化する．
...
これは現在の solver が実際に発展させる conservative face-\(\psi\) endpoint
（保存形面フラックスが運んだ \(\psi\) 状態）であり，
trace-vertex Riesz 代表は本稿の標準経路ではない．
...
本番の圧力面フラックス演算子と随伴な面計量である．
すなわち，ジャンプを消した本番圧力面フラックスを
\begin{equation}
  G_A p
  \equiv
...
この条件を面空間の Hodge 分解で書く．圧力勾配が到達できる面 cochain 空間を
\begin{equation}
  \mathcal{R}_h
  =
  \left\{
...
この判定は連結成分ごとの体積制約と面空間 Hodge 直交性で決まり，
円・楕円などの形状名を使わない．
...
圧力仕事と随伴な Hodge 平衡判定で
静的平衡・非平衡駆動・離散不整合を同じ面複体上で分離する．
