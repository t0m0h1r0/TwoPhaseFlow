...
分相 PPE は単独の手続き名ではなく，次の 5 つを同時に満たす圧力閉包を指す．
\[
  \text{相内定数密度 Poisson}
  \;+\;
  ...
  \text{大域ゲージ}
\]
この閉包では，一括 smoothed-Heaviside PPE は係数不連続の残るモデル，
...
% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{pressure-jump 閉包と純高次極限の切り分け}
\label{sec:pure_fccd_split_ppe}
% ─────────────────────────────────────────────────────────────────────────────

本節の \textbf{pressure-jump 閉包}は，
相内定数密度 PPE，ジャンプ補正付き面勾配，HFE，DC，
同一面フラックス整合，大域ゲージを同時に満たす閉包である．
これは「全ての部品を純 FCCD だけで閉じる」ことと同義ではない．
DC の低次補正演算子，壁面閉包，面フラックス射影には
低次または FVM 的な補助演算子が入り得るが，
PPE と corrector が共有する面フラックス整合を破らない限り
同じ pressure-jump 閉包に属する．
...
capillary drive が圧力自由度に吸収される．したがって
ジャンプ補正付き面演算子では，cut-face を
$G_f(p)$ と $B_f(j_{gl})$ の\textbf{同じ非零面係数}で扱い，
その連結グラフ上の零モードは全体定数 1 個だけになる．

\noindent\textbf{ゲージ条件：}
pressure jump は圧力の絶対値ではなく面勾配契約であり，
...
\end{equation}
を PPE と corrector が共有する面勾配契約として課す．
...
\end{equation}
として合成する分解とは異なり，楕円 solve が $J_\psi$ を
...
これは $\psi$ の直接線形補間とは別の契約である．
全変数の整合的な再写像が数学的に定義されない限り，
