...
\begin{itemize}[noitemsep]
  ...
  \item $\rho_l/\rho_g = 10$--$100$：DC 発散は再試験（FCCD ベースライン + 改訂 GFM ジャンプ条件，§\ref{sec:interface_crossing}）では再現しないが，界面型密度場で設計精度は回復しない
...
$\rho_l/\rho_g = 830$（水--空気），$h = 1/64$ では
$\kappa_2 \sim 830\cdot 4096 \approx 3.4\times 10^{6}$ となり，
...
\noindent\textbf{命題（2 ピンとジャンプ条件の整合性）}：
IPC 増分形式（\S\ref{sec:ipc_derivation}）では未知量を増分 $\delta p_q^n = p_q^{n+1}-p_q^n$
で解く．初期化時 $\delta p_q^0 \equiv 0$，
かつジャンプ条件 $[\delta p]_\Gamma = \sigma(\kappa^{n+1}-\kappa^n)$ が
GFM ゴーストジェット（\S\ref{sec:gfm_ghost_jet_stitch}）経由で
\textbf{両相 PDE と独立に}注入されるため，
$\delta p_L(i_\text{pin}^L)\equiv 0$ かつ $\delta p_G(i_\text{pin}^G)\equiv 0$ が
両相同時に成立可能で，かつジャンプ条件と整合．
合成圧力 $p_q = p_q^{n}+\delta p_q$ では液相基準ピン
$p_L(i_\text{pin}^L)=0$ に対し気相は
$p_G = p_L + \sigma\kappa$ シフトで自動整合．
よって 2 独立ピンとジャンプ条件は\textbf{両立}する．\hfill$\square$
