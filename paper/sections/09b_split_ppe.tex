...
% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{毛管補正量の変分構成・許容 face-state 空間・平衡判定}
...
一方，圧力仕事と毛管 Hodge 診断には active pressure metric
\begin{equation}
...
したがって壁付きの圧力反力代表は full face 空間の後置補正ではなく，
制約空間 $F_w$ 上の随伴として
\begin{equation}
...
\end{equation}
を満たす候補として検証する．ここで $f^\dagger$ は予測 face state であり，
補正後の状態は
\begin{equation}
  f^{n+1}
  =
...
という順序を明示している．
本稿の標準計算では，この制約付き pressure complex が
\[
  ...
  \quad(\eta_f\in F_w)
\]
を満たすことを診断 gate とし，未検証のまま
旧来の full-space 圧力射影 + wall-only 修復を production fix として使わない．
...
壁反力と圧力反力を同じ complex として閉じられない．
周期軸を含む場合は，この条件を full nodal array ではなく
周期商空間で読む．周期軸 $a$ では終端節点面は物理境界ではなく
$q_{N_a}\sim q_0$ という同値類である．
節点商空間への制限を $R_Q$，同値類代表から full array への展開を $E_Q$，
face-state 商空間への展開を $E_F$ と書く．
周期軸に法線を持つ face 成分はすでに $N_a$ 個の循環 face だけを持つが，
周期軸に接する face 成分は終端 image 面を持つため，
\[
...
\end{equation}
である．圧力内積の control volume も image 行を捨てるのではなく，
source 行へ畳み込んでから $R_Q$ で制限する．
したがって mixed periodic-wall の rank gate は
\begin{equation}
  \operatorname{rank}K_w
  =
...
節点 $\psi$ 直接曲率の最大値をそのまま本番毛管力の大きさとは読まない．
...
\end{equation}
CSF 体積力を使わない pressure-jump 経路では，
節点力との差を取る旧来の balanced-force residual は
active 毛管 cochain の残差ではない．
本稿の標準経路では，表面張力を
