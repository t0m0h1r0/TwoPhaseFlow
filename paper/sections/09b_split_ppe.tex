...
pre-reinit endpoint を毛管仕事の幾何状態とする．
この endpoint で，現在の FCCD face transport が誘導する
節点変分を
\begin{equation}
  T_f(q_c)w_f
  =
...
\end{equation}
と書く．$P_f$ は本番 CLS 移流で用いる face value map であり，
$D_f^\psi$ は同じ保存形 face divergence である．閉界面の
...
\end{equation}
ここで $M_A$ は診断用に任意に選ぶ face mass ではなく，
本番 pressure flux operator と随伴な face metric である．
すなわち，ジャンプを消した active pressure flux を
\begin{equation}
  G_A p
  \equiv
...
\end{equation}
と書くとき，$M_A$ は
$\langle G_Ap,w_f\rangle_{M_A}=\langle p,D_fw_f\rangle_{W_p}$
を満たす重みであり，一様係数の場合は通常の face kinetic mass に縮約する．
体積制約の反力も同じ写像で
\begin{equation}
  B_{m,f}
  =
...
admissible capillary drive が，非圧縮かつ体積保存の仮想速度空間へ
直交射影されたものであることを意味する．
実装ではこの鞍点系を PPE solve のみで消去する．任意 cochain $c$ に対し，
\begin{equation}
  L_A(c)=G_Ap_c,\qquad
...
\end{equation}
と定義する．$Z_A(c)$ は active pressure range による Hodge 残差である．
このとき成分 saddle は小さな線形系
\begin{equation}
  C_{ij}=B_i^TM_AZ_A(B_j),\qquad
  r_i=B_i^TM_AZ_A(s),\qquad
  C\mu=r
...
\end{equation}
を得る．PPE 右辺と第7段の corrector に渡す本番 cochain は
$\widehat c_{\sigma,f}$ であり，最終的な face acceleration は
$h_{\sigma,f}$ である．
...
PPE/corrector がすでに採用している $D_fG_A$ 複体の値域に
...
component-corrected cochain $\widehat c_{\sigma,f}$ を用いる．
...
$\eta_R$ は成分反力を除く前の raw surface cochain が
圧力勾配値域の外へどれだけ出ているかを測る原因診断，
$\eta_a$ は実際に補正段階へ渡る corrected cochain の
face balance norm である．
