...
\begin{align}
  ...
  B_f(J_p^\chi) &= \frac{s_f\,J_{p,f}^\chi}{d_f}, \\
  G_{\Gamma,f}(p;J_p^\chi) &= G_f(p)-B_f(J_p^\chi).
  \label{eq:split_ppe_pressure_composition}
\end{align}
したがって PPE の界面項は
\begin{equation}
  D_f\alpha_fG_{\Gamma,f}(p;J_p^\chi)
  =D_f\alpha_fG_f(p)-D_f\alpha_fB_f(J_p^\chi)
  \label{eq:split_ppe_affine_rhs}
\end{equation}
であるから，投影式を $D_f\alpha_fG_{\Gamma,f}=q$ と書けば
$D_f\alpha_fG_f(p)=q+D_f\alpha_fB_f(J_p^\chi)$ となり，
