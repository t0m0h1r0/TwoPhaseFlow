...
\noindent\textbf{$\kappa_s$ の導出（2 階項の根拠）：}
式~\eqref{eq:gfm_curvature_jump} の $\bnabla_f\!\cdot(\sigma\kappa\,\mathbf{n})$ を
界面座標で展開すると，
$\bnabla_f\!\cdot(\sigma\kappa\,\mathbf{n}) = \partial_n(\sigma\kappa)\,(\mathbf{n}\!\cdot\!\mathbf{n}) + \sigma\kappa\,\bnabla\!\cdot\mathbf{n}
 = \partial_n(\sigma\kappa) + \sigma\kappa^2$
（$\bnabla\!\cdot\mathbf{n}=\kappa$；§\ref{sec:ridge_eikonal}）．
さらに界面接線方向の発散 $\partial_s(\sigma\kappa)$ を分離すると，
2 階ジャンプ成分は $[\partial_n^2 p]_\Gamma = \partial_s(\sigma\kappa)/\sigma \equiv \kappa_s$
として表現される（Liu-Fedkiw-Kang~\cite{Fedkiw1999} Appendix A の
Laplace--Beltrami 分解と整合；詳細な座標系変換は付録~\ref{app:gfm_curvature_jump_derivation} に委ねる）．
