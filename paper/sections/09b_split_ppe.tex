...
分相 PPE は CSF 体積力ではなく圧力ジャンプ $[p]_\Gamma=\sigma\kappa$ を
界面条件として直接課すため，混合密度モデルや GFM ジャンプ縫合を欠いた状態で
分相モードを起動すると合成圧力が両相に整合しない；
本稿では分相モード起動時に圧力ジャンプ縫合と CSF 不在を整合性条件として要請する．
...
前ステップの圧力ジャンプ $[p]_\Gamma=\sigma\kappa$ を継承すると
不連続合成圧力が再投影内部で微分され，人工運動エネルギーを生む．
したがって再投影 PPE には界面ジャンプコンテキスト（$\psi,\kappa,\sigma$）を渡さず，
