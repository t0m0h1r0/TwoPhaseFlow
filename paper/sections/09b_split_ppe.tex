...
分相 PPE は単一の離散化名ではなく，次の 7 つを同時に満たす圧力閉包を指す．
\[
\begin{aligned}
  &\text{相内定数密度 Poisson}
...
\begin{enumerate}[noitemsep]
  ...
  \item 界面近傍の最終律速は，FCCD 面ジェット
        ...
        と読むべきである
  \item 毛管ジャンプ cochain は式~\eqref{eq:capillary_range_projection} の
        $\Pi_{\mathcal{R}_h}$ により圧力勾配値域へ閉じ，
        divergence-free な非勾配成分を速度補正へ渡さない
\end{enumerate}
\end{tcolorbox}
...
\begin{equation}
  \bu^\ast=0,\qquad
  a_f(p_{\mathrm{YL}};c)=0
  \label{eq:discrete_young_laplace_static_face_balance}
\end{equation}
であり，単に $D_fa_f=0$ であることより強い．
実際，$a_f\neq0$ が divergence-free に残ると，補正後速度は
$\bu_f^{n+1}=-\Delta t\,a_f$ となり，
初期速度ゼロでも $\Ord{\Delta t^2\|a_f\|^2}$ の運動エネルギーが生じる．
これは物理的な毛管振動ではなく，スカラー圧力ジャンプとして表すべき
表面張力が face 空間の非勾配成分に漏れたことを意味する．
...
とする．任意の毛管 cochain は
\begin{equation}
  c_f = \Pi_{\mathcal{R}_h}c_f + h_f,\qquad
  D_fh_f=0
  \label{eq:capillary_hodge_decomposition}
\end{equation}
と分解される．$\Pi_{\mathcal{R}_h}c_f$ は，同じ PPE 演算子と同じゲージを用いる
補助 Poisson 問題
\begin{equation}
  D_f A_f G_f\pi_h = D_f c_f,\qquad
  \langle\pi_h,1\rangle_h=0,\qquad
  \Pi_{\mathcal{R}_h}c_f = A_f G_f\pi_h
  \label{eq:capillary_range_projection}
\end{equation}
で定義する．式~\eqref{eq:capillary_range_projection} は
新しい物理モデルでも低次 FD 代替経路でもない．
PPE/corrector がすでに採用している $D_f A_f G_f$ 複体の値域へ，
既知ジャンプ cochain を代数的に閉じるだけである．
したがって
\begin{equation}
  D_f\Pi_{\mathcal{R}_h}c_f = D_f c_f
  \label{eq:capillary_range_projection_same_rhs}
\end{equation}
が solver tolerance まで成り立ち，式~\eqref{eq:capillary_affine_ppe_general}
の発散右辺は変わらない．変わるのは速度補正で差し引く face cochain だけである：
\begin{equation}
  a_f^{\mathrm{range}}(p)
  =
  A_f G_f p-\Pi_{\mathcal{R}_h}c_f .
  \label{eq:range_projected_capillary_face_balance}
\end{equation}
静止 Young--Laplace 状態では $p=\pi_h$ により
$a_f^{\mathrm{range}}=0$ が面ごとに成立する．一方，raw cochain を用いれば
$a_f=-h_f$ が残る．この $h_f$ は $D_fh_f=0$ のため
PPE 残差や発散診断には現れず，速度補正後の face acceleration norm で
初めて検出できる．
...
\begin{equation}
  \eta_H
  =
  \frac{\|c_f-\Pi_{\mathcal{R}_h}c_f\|_{\infty,f}}
       {\max(\|c_f\|_{\infty,f},\epsilon_H)},\qquad
  \eta_a
  =
  \|a_f^{\mathrm{range}}\|_{\infty,f}
  \label{eq:capillary_hodge_gate}
\end{equation}
である．$\eta_H$ は raw 幾何ジャンプがどれだけ
圧力勾配値域の外へ出ているかを測る原因診断であり，
$\eta_a$ は補正段階に渡る実効毛管加速度を測る受理診断である．
圧力ジャンプ形式では非保存的な表面せん断や Marangoni 力を
$h_f$ として暗黙に残してはならない．そのような物理を扱う場合は，
圧力ジャンプとは別の接線応力項として運動量方程式に明示的に入れる必要がある．
本稿の水--空気級 canonical 経路では，
表面張力をスカラー Young--Laplace 圧力ジャンプだけで表すため，
$h_f$ は離散幾何と projection 複体の不整合成分として除去する．
これは damping，曲率 cap，CFL 縮小，平滑化のいずれでもなく，
式~\eqref{eq:discrete_young_laplace_static_face_balance} を満たすための
離散 Hodge 閉包である．
