...
\begin{equation}
  ...
  -
  M_f^{-1}C_w^T
  \left(C_wM_f^{-1}C_w^T\right)^{+}
  C_w ,
  \qquad
  C_wP_w=0,\quad P_w^2=P_w ,
  \label{eq:wall_metric_retraction}
\end{equation}
である．ここで $M_f=Q_f\rho_f$ は速度状態の輸送質量計量であり，
$Q_f$ は非一様格子の face control measure である．
圧力仕事の随伴診断では，同じ retract 形を
active pressure metric
\begin{equation}
  M_A = Q_f/\alpha_f
  \label{eq:pressure_metric_roles}
\end{equation}
で評価する．この二つの計量は互換に取り替えるものではない．
前者は速度状態の壁制約，後者は
...
\begin{equation}
  G_w p = P_wG_Ap,\qquad
  D_w = D_f|_{F_w},\qquad
  D_fP_wG_Ap = D_fP_w f^\dagger
  \label{eq:restricted_pressure_operator}
\end{equation}
を満たす候補として検証する．本稿の標準計算では，
この制約付き pressure complex が
\[
...
全壁トポロジでは $P_w$ の冪等性・自己随伴性・制限 Green 恒等式を
丸め誤差まで検証できる一方，周期壁混在では商空間と壁トレース行を
同じ圧力 gauge で作り直すまで fail-close とする．
