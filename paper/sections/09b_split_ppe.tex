...
分相 PPE は単独の名前ではなく，次の 5 つを同時に満たす圧力閉包を指す．
\[
  \text{相内定数密度 Poisson}
  \;+\;
...
\]
したがって，本節で退ける対象は
「一括 smoothed-Heaviside PPE を高密度比の標準閉包とみなすこと」，
「CSF 体積力を最終的な表面張力閉包とみなすこと」，
「regular pressure field $J$ を後で足す圧力合成を
ジャンプ補正付き面勾配と同等視すること」である．
...
制御体積恒等式としては保証されない．
したがって純 FCCD DNS 版は，標準閉包の必要条件ではなく，
...
面法則として扱い，PPE RHS と corrector が共有する唯一のジャンプ寄与にする．
式~\eqref{eq:gfm_gradient_jump} は $J_n=0$ の標準形である．
一般の $J_n\ne0$ を許すなら，
...
このジャンプ補正付き面勾配形式は，通常の regular 圧力場
\begin{equation}
...
raw な $\sigma\kappa_{lg}$ 場ではない．この条件は毛管波専用ではなく，
...
HFE 延長文脈を補間せず，現在の $\phi,\psi$ と格子幾何データから再構築する．
これは $\psi$ の直接線形補間とは別の契約であり，
$u,v,p,\delta p,\rho,\mu,\kappa_{lg},\hat{\bm n}_{lg}$ の全変数リマップが未完成の Mode 2 では
標準閉包の検証完了を主張しない．
