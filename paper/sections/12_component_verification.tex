...
\textbf{基礎演算 → 特殊離散 → 結合系 → 否定検証 → 状態空間条件 → AO-Fast 事前ゲート}の順に 10 層へ分類した：
...
\begin{description}\setlength{\itemsep}{0pt}
  ...
  \item[Tier X（AO-Fast 幾何毛管分解の事前ゲート）]
        U12 = $R_p$ を定義しない full pressure reaction split が毛管波の面上駆動を消すこと，
        component-volume Hodge probe が非静的波を検出すること，および GPU 実装が未解決条件で fail-close することを確認する
...
AO-Fast 幾何セル分率状態空間 (§\ref{sec:capillary_pressure_reaction_subspace}, §\ref{sec:ao_fast_state_space}) &
U12 が full pressure image 過射影の反例，component-volume Hodge probe，GPU fail-close を事前ゲートとして確認する． &
V11 が標準物理ベンチへ混入させる前の統合前受理境界を確認し，第~\ref{sec:benchmarks}章の毛管波結果とは区別する． \\
...
U10 は保存形共通フラックス台帳，U11 は制約付き面速度状態空間，U12 は AO-Fast gate を単体条件として追加し，それ以外は U1--U9 の検証済み数値要素を入力として扱う． &
V1--V11 と §\ref{sec:error_budget} が更新順序，面データ引き渡し，物理指標，適用限界を検証する． \\
\bottomrule
\end{tabularx}
\end{table}

\paragraph{バックエンド方針（CPU 固定と GPU gate の分離）．}
本章 U1--U11 はすべて \texttt{Backend(use\_gpu=False)} 固定で実行する．
これは \textbf{CPU bit-exact verification}（PR-5）を優先する設計上の選択であり，
収束次数や round-off 床といった単体精度指標が
ハードウェア依存のドリフトを受けないことを保証するためである．
GPU/CuPy 経路は次章 V 系列 (§\ref{sec:verification})・第~\ref{sec:benchmarks}章の
統合検証で運用評価する．本章で確認する数値要素は
\texttt{backend.xp} 経由で書かれており CPU/GPU 切り替え可能だが，
精度判定は CPU 値を canonical reference として記録する．
例外として U12 は，CPU exact split を代数反例として用い，
GPU 経路は成功値ではなく fail-close gate として読む．
したがって U12 の GPU 観測はハードウェア性能評価ではなく，
未解決の AO-Fast packet が標準物理経路へ混入しないことを確認する安全条件である．
...
U9 & VII & DCCD-on-pressure 禁止の否定検証． & V1--V11 全体の圧力勾配 operator policy． \\
U10 & VIII & FCCD 面フラックス台帳を $\psi$，アフィン密度，運動量へ同一に渡す保存形共通フラックス条件． & 第13章の物理ベンチマークには数えず，第14章の保存形共通フラックス物理経路へ送る． \\
U11 & IX & $P_w$ の冪等性・$M$-自己随伴性・$C_wP_w=0$，制限 Green 恒等式，壁/周期壁商空間の階数条件． & V6/V7/V9 の壁付き圧力ジャンプ結合系の状態空間前提を単体で支え，周期壁の物理応答は第14章へ送る． \\
U12 & X & AO-Fast 幾何毛管分解の $R_p$ gate：full pressure image 過射影の反例，component-volume Hodge probe，GPU fail-close． & V11 の統合前 gate へ送り，未通過の AO-Fast packet は第14章の本番毛管波結果とは分ける． \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{Tier X：AO-Fast 幾何毛管分解の事前ゲート}
