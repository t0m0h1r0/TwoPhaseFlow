...
%       11 件の単体テスト (U1–U11) / 9 Tier に分類し,各 primitive が単体で
...
\begin{description}\setlength{\itemsep}{0pt}
  ...
  \item[Tier VIII（保存形共通フラックス台帳）]
        U10 = CLS $\psi$ の FCCD 面フラックス台帳を密度と運動量へ同一写像として渡す component gate
        （§\ref{sec:cls_advection}, §\ref{sec:algorithm}）．
  \item[Tier IX（制約 face-state 空間）]
        U11 = $F_w=\ker C_w$，$P_w$，$G_w=P_wG_A$，および periodic-wall quotient rank の小型代数 gate
...
U10 は保存形共通フラックス台帳，U11 は制約 face-state 空間を単体 gate として追加し，それ以外は U1--U9 の検証済み primitive を入力として扱う． &
...
\paragraph{検証設計マップ．}
表~\ref{tab:U_summary} は，U1--U11 を「どの primitive を単体で確定し，
...
U10 & VIII & FCCD 面フラックス台帳を $\psi$，アフィン密度，運動量へ同一に渡す保存形共通フラックス component gate． & 第13章の物理 benchmark には数えず，第14章の保存形共通フラックス物理経路へ送る． \\
U11 & IX & $P_w$ の冪等性・$M$-自己随伴性・$C_wP_w=0$，制限 Green 恒等式，wall/periodic-wall quotient rank gate． & V6/V7/V9 の壁付き圧力ジャンプ結合系の状態空間前提を単体で支え，periodic-wall 物理応答は第14章へ送る． \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{Tier IX：制約 face-state 空間}
