...
それ以外を\textbf{不合格}（$\times$）と判定する（各 $\triangle$ の分類は表~\ref{tab:verification_summary} 脚注に示す）．
...
\begin{enumerate}
  ...
  \item \textbf{§~\ref{sec:verify_time}（時間積分）：}
        ...
        時間発展スキーム（TVD-RK3・AB2・CN）の精度と安定性を ODE テストで確認する．
\end{enumerate}
...
\small
\begin{tabular}{cl>{\raggedright\arraybackslash}p{4.2cm}>{\raggedright\arraybackslash}p{4.0cm}}
\toprule
ステップ & 構成要素 & 検証項目 & 節 \\
\midrule
...
4 & 曲率 $\kappa$（CCD） & CLS 曲率パイプラインの格子収束 & §\ref{sec:verify_curvature} \\
...
\input{sections/12b_spatial_discretization} %% §12.1 空間離散化基盤の検証（CCD/GCL/DCCD）
\input{sections/12c_spatial_geometry}       %% §12.1続き（曲率・Rhie-Chow・混合微分）
\input{sections/12d_interface_pipeline}     %% §12.2 界面処理パイプラインの検証（CLS/WENO5/DGR/DCCD質量保存）
\input{sections/12e_interface_field}        %% §12.2続き（動的格子再マッピング・HFE・Young-Laplace）
\input{sections/12f_pressure_solver}        %% §12.3 圧力ソルバの検証
\input{sections/12g_time_integration}       %% §12.4 時間積分スキームの検証
\input{sections/12h_summary}                %% §12.5 検証結果の総括
