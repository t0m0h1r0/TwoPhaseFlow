...
%       11 件の単体テスト (U1–U11) / 9 Tier に分類し,各要素が単体で
...
\paragraph{構成（9 層 / 11 単体テスト U1--U11）．}
本章の U 系列は単体検証専用であり，
次章の V 系列（統合検証）とは実験目的と判定基準を分ける．
Part 2 の手法を依存関係の下流方向にトポロジカル整列し，
\textbf{基礎演算 → 特殊離散 → 結合系 → 否定検証 → 状態空間条件}の順に 9 層へ分類した：
...
U10 は保存形共通フラックス台帳，U11 は制約付き面速度状態空間を単体条件として追加し，それ以外は U1--U9 の検証済み数値要素を入力として扱う． &
