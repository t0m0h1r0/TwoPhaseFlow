...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Galilean 不変性}（§~\ref{sec:galilean_invariance}）：
        移動界面下での CLS↔PPE 結合
  \item \textbf{結合の限界と GFM への接続}（§~\ref{sec:coupling_limitations}）：
        テスト結果から特定された限界の整理
\end{enumerate}
...
§~\ref{sec:bf_static_droplet} では $p^0 = 0$（Hermite 延長は実質 no-op）の
単ステップ投影であった．
...
\begin{enumerate}[noitemsep]
  \item IPC の $\bnabla p^n$ が界面の圧力ジャンプ $\Delta p = \sigma\kappa$ を横切る
        → Hermite 延長で回避可能（式~\eqref{eq:p_extension_ipc}）
  \item 変密度 PPE 自体が界面型密度場で精度劣化・発散する
        → GFM 分相解法でのみ解決可能（§~\ref{sec:interface_crossing}）
\end{enumerate}

Hermite 延長のみ（GFM なし）で IPC を修正した追試では，
テスト B の発散は\textbf{解消されなかった}（$N = 64, 128$ の両方で 2 ステップで発散）．
これは原因 2（変密度 PPE の不安定性）が支配的であり，
$\bnabla p^n$ の精度を改善するだけでは不十分であることを示す．
完全な解決には，\textbf{IPC の Hermite 延長と GFM 分相 PPE の両方}が必要である：
\begin{equation}
...
\end{equation}
GFM 分相 PPE による精度回復は §~\ref{sec:gfm_ppe_recovery} で実証し，
移動界面＋表面張力の完全な動的検証は GFM 統合後に
