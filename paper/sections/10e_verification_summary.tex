...
\begin{enumerate}
  ...
  \item \textbf{DCCD 移流特性：}
        1次元移流比較により，DCCD は不連続プロファイルの TV を CCD の約 $1/3$ に抑制し，
  ...
  \item \textbf{界面処理：}
        CLS 保存型再マッピングは LS 非保存補間に比べ質量誤差を 1--2 桁低減する
        （§\ref{app:cls_conservation}）．GFM は粗格子で安定に収束する．
  \item \textbf{PPE 求解：}
        欠陥補正法は $k=3$ 回の反復で CCD 演算子の $\Ord{h^6}$ 精度を完全に発現させる（§\ref{sec:verify_ppe}）．
...
DCCD 移流（不連続） & TV 抑制 & TV: $10.83 \to 3.15$ & CCD 比 $1/3$ \\
