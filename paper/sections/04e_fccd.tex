...
\begin{enumerate}[label=\textbf{(R\arabic*)}]
  ...
  \item GFM ghost pressure jet（§~\ref{sec:split_ppe_framework}）での $[p]=\sigma\kappa$ および $[\alpha p']=0$ 条件課嵌；
...
{\FCCD} 初版（SP-A）の動機は FVM 面平均勾配 $\mathcal{G}^{\text{adj}}$ との
メトリック整合（H-01 診断）にあったが，SP-M 純 FCCD 設計では FVM が除外される．
...
（SP-A 初版の H-01 救済動機は付録~\ref{sec:fccd_motivation_h01} を参照）．
