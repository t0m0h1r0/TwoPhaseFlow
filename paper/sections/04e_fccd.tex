...
\end{equation}
ここで $\omega_2(\xi)$ は CCD 2 階微分の無次元有効波数で
$\hat q_i = -\omega_2(\xi)^2\hat u_i/H^2$ を満たす（§\ref{sec:ccd_def} 参照）．
括弧内因子は $\hat D_2\hat S_{\CCD} = (H^2/24)\hat S_{\CCD}\cdot(2i\sin(\xi/2)/H)$ から導かれ，
$H^2/24\cdot(-\omega_2^2/H^2)=-\omega_2^2/24$ で無次元化が閉じる．
Taylor 展開 $\omega_2(\xi)^2 = \xi^2 + \Ord{\xi^6}$ を代入すると
$\hat M^{\FCCD}(\xi) = i\xi/H + \Ord{\xi^5/H}$ を得，
