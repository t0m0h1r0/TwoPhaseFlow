% paper/sections/11a_spatial_discretization.tex
%
% §11.1  空間離散化基盤の検証
...
2 階微分も $N \leq 128$ で $p = 5.9$--$6.0$ の高次収束を示す．
$N = 256$ では 1 階微分の打ち切り誤差が $\Ord{10^{-14}}$ に到達し，
倍精度浮動小数点の丸め限界が収束次数の見かけ上の低下として現れた（$L_\infty$ で $p \approx 5.36$）．
2 階微分は 1 階微分より 1 段階早く丸め誤差床に到達し，$N = 128 \to 256$ では $p \approx 0.5$ まで低下する．
...
非一様格子（c）では，$\alpha=2$ の集中格子においても高次収束傾向は維持されたが，
表~\ref{tab:gcl_convergence} で後述するように前漸近振動が残る．
...
\caption{CCD 微分演算子の格子収束（3 パネル）．
  ...
  (c) 非一様格子（$\alpha=2$）：高次収束傾向は維持されるが，前漸近振動が残る．
...
非一様格子は絶対誤差こそ一様格子より 2--3 桁大きい．
これは格子圧縮領域での前漸近的効果（pre-asymptotic effect）による定数因子の差である．
ただし，表~\ref{tab:gcl_convergence} の範囲では非一様格子上の収束次数は
$p = 3.4$--$7.9$ と大きく振動しており，
一様格子と同じ $\Ord{h^6}$ の漸近領域に安定して入ったとは判定しない．
本テストが示すのは，連鎖則変換により非一様格子でも高次収束傾向は維持されるが，
$\alpha=2$ の集中格子では前漸近振動が残る，という限定的な結論である．
...
\caption{非一様格子（$\alpha=2$）上での CCD 微分精度と GCL 検証．
  (a) 1 階微分：一様格子は $\Ord{h^6}$，非一様格子は前漸近振動を伴う高次収束傾向．
  (b) 2 階微分：非一様格子では格子集中に伴う前漸近振動が残る．
...
1 階微分誤差は $N=16$--$128$ で単調に減少した．
ただし非一様格子の実測次数は $p = 3.4$--$7.9$ と振動しており，
$\Ord{h^6}$ 到達ではなく条件付き合格とする．
GCL は $2.4 \times 10^{-14} \ll 2.2 \times 10^{-13}$ で計算機精度で成立する．
...
\begin{itemize}[noitemsep]
  \item Part A：DCCD 伝達関数 $H(\xi;\,\varepsilon_d)$ の Nyquist 応答を
        $\varepsilon_d = 0, 0.05, 0.25$ で評価．
        $H(\pi;\,\varepsilon_d = 0.25) = 0$ の解析的証明（§~\ref{sec:checkerboard_solution}）を数値的に補強する
...
チェッカーボード抑制の本質的根拠は $H(\pi;\,\varepsilon_d = 1/4) = 0$ という
伝達関数の代数的ゼロ点にある（§~\ref{sec:checkerboard_solution}）．
数値実験では $(-1)^{i+j}$ の CCD 微分結果にフィルタを適用し
振幅比 $\approx 0.28$ を得たが，CCD が Nyquist 波数を微分する際に
振幅が既に機械精度付近（$\sim\!10^{-13}$）に低下するため，
この数値は定量的検証としては補助的である．
...
なお滑らかな全域 tanh プロファイルでは DCCD の $L_2$ 誤差は
CCD 比で約 3 桁大きいが（$1.47 \times 10^{-2}$ vs $2.56 \times 10^{-5}$），
CLS 界面変数 $\psi$ の特性波長は $\lambda_{\min} \approx 9.4h$ と十分長く，
この波長域でのフィルタ伝達関数は $H \approx 0.98$ であるため，
CLS 設定下での界面移流に対して DCCD の散逸は殆ど影響を与えない
