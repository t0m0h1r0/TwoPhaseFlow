...
%
% §8.4  FCCD BF sub-system：H-01 診断と面ジェット統合
%       （SP-A §6 + SP-H §3 + SP-J §3）
%
% ═══════════════════════════════════════════════════════════════════════════════
...
節点 CCD と FVM の\textbf{メトリック不整合}は H-01 として特定され
（WIKI-E-030，CHK-152），$\BF_\text{res} = \Ord{\Delta x^2}\cdot|d\log J/dx|$
の残差を界面適合非一様格子上で発生させる．
節 \ref{sec:fccd_def} で導出した FCCD 面ジェット
$\FaceJet{u}=(u_f,u'_f,u''_f)$ は，BF sub-system の
P-1..P-4 を\textbf{同時に}満たす最小単位である．

% -------------------------------------------------------------------------------
% H-01 診断の一次定義は §\ref{sec:fccd_motivation_h01}（§4.5, WIKI-E-030, CHK-152）にある．
% 本節では FCCD 面ジェットによる救済に直ちに進むため，診断再掲は外部参照に委ねる．
% 旧 \subsubsection{H-01 診断の再掲と FCCD 救済} は
% paper/archive/v1_pre_sp_core/snippets/h01_recap_08_2.tex に退避（CHK-193 Phase D-1）．
% -------------------------------------------------------------------------------
\phantomsection
\label{sec:h01_diagnosis_fccd_remedy}% 外部参照保持：10_5/08c/10_3/appendix_gpu_fvm

節点 CCD 勾配と FVM の面平均勾配 $\mathcal{G}^\text{adj}$ の\textbf{メトリック不整合}
は，界面適合非一様格子上で
$\mathrm{BF}_\text{res} = \Ord{\Delta x^2}\cdot|d\log J/dx|$ の残差を生む
（一次定義：§\ref{sec:fccd_motivation_h01}, Exp-1 実測 $|\BF_\text{res}|\approx 884$
at step 1, $\rho_l/\rho_g{=}830$）．
FCCD 面ジェット $\FaceJet{u}=(u_f,u'_f,u''_f)$ は
$\mathcal{G}^\text{adj}$ と\textbf{同一の面位置・同一のデータフットプリント}を用い，
\begin{equation}
...
P-1..P-4 を単一オブジェクトで具現する（SP-H \cite{SPH} §3）．
...
\hline
節点 CCD + $\mathcal{G}^\text{adj}$（現行）& $\varepsilon_\text{BF}\sim 10^{-3}$ & $\Ord{\Delta x^2}$ \\
FCCD + GFM（Phase 1）& $\varepsilon_\text{BF}\sim 10^{-6}$ & $\Ord{\Delta x^3}$ \\
...
Level 3（純 FCCD DNS, SP-M \cite{SPM}）= 戦略 C を目標とする分相 PPE
