...
%        SP-H §2 の紹介．完全な定式化は §7（移流），§8–§9（PPE），§10（アルゴリズム）で再登場．
%
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{面ジェット $\FaceJet{u}$：統一プリミティブ}
\label{sec:face_jet_def}
% ═══════════════════════════════════════════════════════════════════════════════

SP-H \cite{SPH} は，面上で値・一次導関数・二次導関数の 3 組
...
これが独立未知数ではなく HFE 再構成と診断に用いる\textbf{補助量}だからである（SP-H §2）．
...
$u_f$ & FCCD 保存形移流 flux / HFE 上流面値 Riemann 入力 &
  §\ref{sec:advection}（SP-D 移流）\\
$u'_f$ & 変密度 PPE 面フラックス $\betaf\,p'_f$ / 曲率・界面力評価 &
  §\ref{sec:pressure}（SP-H §3, SP-M）\\
$u''_f$ & HFE 方向テイラー再構成の 2 次補正項 / 診断 &
  §\ref{sec:advection}（SP-H §4, SP-D）\\
\hline
\end{tabular}
\end{center}
...
SP-H §3 で確立され，§\ref{sec:pressure} および
§\ref{sec:hfe} で具体化される．
...
\begin{itemize}
  ...
  \item \textbf{不連続生フィールドに面値原語を直接適用しない．}
        ...
        不連続を Gibbs 振動として輸送する（WIKI-L-030 の警告）．
