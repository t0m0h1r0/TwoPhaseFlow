...
{\DCCD} (§\ref{sec:dccd_derivation})     & 節点      & 外殻 post-filter              & $\Ord{h^6}$$^\ddagger$ & $\ell^2$ 安定（spectral 拘束下）   & $\Ord{N}$ \\
{\FCCD} (§\ref{sec:fccd_def})            & \textbf{面} & 風上構造（option）          & $\Ord{H^4}$           & 基底 CCD 由来               & $\Ord{N}$ \\
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item \S\ref{sec:advection}（移流）：CLS 移流に DCCD，運動量移流に FCCD/UCCD6 を適用．
