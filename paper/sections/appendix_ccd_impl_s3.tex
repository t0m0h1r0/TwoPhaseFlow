...
\subsubsection{PPE の数値解法：LGMRES 反復法}
...
\subsubsection*{LGMRES 反復法}

LGMRES（拡張 Krylov 部分空間法；\texttt{scipy.sparse.linalg.lgmres}）を唯一の反復ソルバーとして採用する．
メモリコストは $O(n k)$（$k$：内部再スタート数）であり，
...
LGMRES が最大反復回数以内に収束しない場合は，その時点での最良反復解を返し，
警告を発するが解法を停止しない．
...
修正系 $\hat{\mathbf{L}}\bm{p} = \hat{\bm{q}}$ を LGMRES で解く．
...
場合の理論的参照として有効である．
本付録の実装はスウィープ分割を行わず，組み立てた大域行列に対して直接 Krylov 法を
適用するため，$\Delta\tau$ の選択に依存せず，スプリッティング誤差
...
付録~\ref{app:ccd_lu_direct} の LGMRES ソルバーに渡す $n \times n$ スパース行列として
...
LGMRES 反復求解 & $O(n \cdot k)$（$k$：イテレーション数） & 各タイムステップ1回；warm start で $k$ を削減 \\
\hline
\end{tabular}
\end{center}
