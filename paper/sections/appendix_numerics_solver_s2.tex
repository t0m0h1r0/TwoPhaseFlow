...
\subsection*{\texorpdfstring{$t \to 0$}{t → 0} および \texorpdfstring{$t \to \infty$}{t → ∞} の挙動}
...
\subsection*{CCD Laplacian の最大固有値 \texorpdfstring{$\lambda_{\max} \approx 3.43\,a_{\max}/h^2$}{λ\_max ≈ 3.43 a\_max/h²} の導出}
...
\end{tabular}
\end{center}
理論的上限は $9.6/h^2$（Fourier 解析；$N\to\infty$ の極限）であり，
有限格子では境界処理による固有値縮小のため実用値は $\approx 3.43/h^2$ となる
（$N$ が増大すると境界効果が弱まり $\lambda_{\max}\to 9.6/h^2$ に漸近；
