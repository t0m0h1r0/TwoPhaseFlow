% paper/sections/11b2_ccd_pseudotime_verification.tex
%
% §11.3.2  仮想時間陰解法 + CCD の精度検証（3事例）
%          （旧 D.8; \section と \label は 11b_component_verification.tex の
%            \subsubsection で提供する）
...
\begin{table}[htbp]
  ...
  \caption{%
    ...
    CCD の収束次数は境界スキームの精度（Eq-I-bc: $\Ord{h^5}$，Eq-II-bc: $\Ord{h^4}$）に
    制限され，漸近的に $\Ord{h^5}$ に収束する（内部ステンシルの $\Ord{h^6}$ は
    近傍節点を通じて境界誤差の影響を受ける）．
  }
  \label{tab:ccd_case1}
  \begin{tabular}{rcccc}
...
\begin{figure}[htbp]
  ...
  \caption{%
    ...
    CCD は漸近的に $\Ord{h^5}$（破線：$\Ord{h^6}$ 参照線），
    CDS2 は $\Ord{h^2}$ で収束する．
  }
  \label{fig:ccd_case1}
\end{figure}
...
\begin{tcolorbox}[resultbox, title={空間収束次数（CCD vs.\ CDS2）}]
  \begin{itemize}
    \item CCD は内部ステンシル $\Ord{h^6}$ を持つが，コンパクトスキームの結合を通じた
          境界スキーム $\Ord{h^5}$（Eq-I-bc）の影響により，
          全域 $L^\infty$ 収束次数は $\Ord{h^5}$ に制限される（$N=128$ で $p \approx 5.0$）．
    \item $N=128$ の時点で CCD の誤差は CDS2 の約 4500 倍小さい
          （$1.78 \times 10^{-6}$ vs.\ $7.93 \times 10^{-3}$）．
    \item CDS2 は厳密に $\Ord{h^2}$ で収束し，格子収束挙動は単純かつ予測可能である．
  \end{itemize}
\end{tcolorbox}
...
$u_{\mathrm{exact}}(x) = (e^{Pe\,x} - 1)/(e^{Pe} - 1)$．
...
\end{equation}
を scipy の直接法で求解する
（1次元 $N \times N$ 系であり，本稿で不使用とする
2次元 PPE の LU 分解とは異なる；CCD 微分演算子の空間精度評価が目的であるため
ソルバ選択は結果に影響しない）．
...
\begin{figure}[htbp]
  ...
  \caption{%
    ...
    CCD は漸近的に $\Ord{h^6}$ に向かって収束次数が上昇し，
    $N=256$ で $p \approx 5.8$，CDS2 は厳密に $\Ord{h^2}$ で収束する．
  }
  \label{fig:ccd_case3}
\end{figure}
...
\begin{tcolorbox}[resultbox, title={高 Péclet 境界層における CCD の優位性}]
  \begin{itemize}
    ...
    \item 収束次数は粗格子では前漸近的（$N=8$: $p=3.3$，$N=16$: $p=4.6$）だが，
          $N \geq 64$ から漸近 $\Ord{h^6}$ に向かって安定的に上昇する（$N=256$: $p=5.8$）．
    \item CDS2 は中心差分スキームとして安定条件 $Pe \cdot h < 2$ を満たす範囲では
          $\Ord{h^2}$ を維持するが，CCD との誤差差は格子細分化に伴い指数的に拡大する．
  \end{itemize}
\end{tcolorbox}
