...
\begin{align*}
  ...
  &\quad + \frac{1}{2}\Bigl[D_x^{(1)}\!\bigl(\mu^{n+1}(D_x^{(1)} v^* + D_y^{(1)} u^*)\bigr)
                          + D_y^{(1)}\!\bigl(\mu^{n+1}(D_y^{(1)} v^* + D_x^{(1)} u^*)\bigr) \\
  &\qquad\qquad    + D_x^{(1)}\!\bigl(\mu^{n+1}(D_x^{(1)} v^n + D_y^{(1)} u^n)\bigr)
                          + D_y^{(1)}\!\bigl(\mu^{n+1}(D_y^{(1)} v^n + D_x^{(1)} u^n)\bigr)\Bigr]_{i,j} \\
  &\quad - \frac{\rho^{n+1}_{i,j}}{Fr^2}
         + \frac{\kappa^{n+1}_{i,j}\,(D_y^{(1)}\psi^{n+1})_{i,j}}{We}
\end{align*}
\begin{tcolorbox}[warnbox, title={$x$成分と$y$成分の粘性項の対称性}]
$x$成分と $y$成分の粘性項を比べると，$u \leftrightarrow v$ かつ $D_x^{(1)} \leftrightarrow D_y^{(1)}$ の置き換えで一方から他方が得られる．
これは応力テンソル $\tau_{ij} = \mu(\partial_i u_j + \partial_j u_i)$ が対称テンソルであることの直接的な反映である．
\end{tcolorbox}
