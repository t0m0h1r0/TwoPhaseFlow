...
表面張力・圧力・速度補正・界面移流を同一の面中心契約で閉じ，
どの誤差が離散化で除去され，どの誤差がモデルまたは検証範囲として残るかを明確に分離することである．

\textbf{本稿の技術的中核は，寄生流れの離散化起因成分を face-space contract 上で抑えることである．}
結合コンパクト差分法（CCD，第~\ref{sec:CCD}章以降）を基盤に，
FCCD 面フラックス/圧力勾配，HFE による界面近傍データ延長，
向き付き pressure-jump 分相 PPE + DC ($k=3$)，projection-native な正準面速度，
...
\begin{enumerate}
  ...
  \item \textbf{密度ジャンプと寄生流れの安定処理}：
        ...
        分相 pressure-jump 経路では，界面ジャンプ，PPE 補正，projection-native face flux が
        同じ face-space に閉じていないことが同等の不整合を生む．
...
HFE による界面近傍データ延長，向き付き pressure-jump 分相 PPE，projection-native な正準面速度，
...
surface-energy virtual-work pressure-jump/Hodge stack は V6/V7/V9 で，
