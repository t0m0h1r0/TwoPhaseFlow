...
高密度比の移動界面では，この約束にもう一つの保存量の約束が加わる．
速度 $\bu$ だけを主変数として界面を別に移流すると，
同じ見かけの速度場でも，界面移流が見ている面フラックスと運動量移流が見ている面フラックスが
ずれることがある．
水--空気級の密度比では，その小さなずれが
「質量は動いたが運動量は同じ質量で動いていない」という隠れた力になる．
したがって本稿では，必要な箇所で
\[
  q=\psi,\qquad
  m=\rho_g+(\rho_l-\rho_g)q,\qquad
  \bm{p}_m=m\bu
\]
を同じ面フラックスで運ぶ\emph{保存形共通フラックス}の読み方を用いる．
ここで $\bm{p}_m$ は運動量密度であり，圧力 $p$ とは区別する．
この読み方は「より強い安定化」を入れるためではなく，
保存形の質量式と運動量式を同じ離散発散の上で同時に満たすためのものである．
第~\ref{ch:per_term_spatial}章，第~\ref{ch:time_integration}章，
および第~\ref{sec:algorithm}章で，この約束を具体的な更新手順として書き下す．
...
欠陥補正は固定回数または固定緩和係数そのものを精度保証とはせず，
高次 PPE 残差を下げる補正方向だけを受理する射影段階の代数契約として扱う．
...
\begin{enumerate}[noitemsep,leftmargin=2em]
  ...
  \item 壁付き計算では，圧力射影後に壁面を修復するのではなく，
        最初から no-slip 壁トレースを満たす面速度空間
        $F_w=\ker C_w$ を状態空間とし，圧力反力・毛管反力・診断量を
        その空間と active 面計量で定義する
        （式~\eqref{eq:wall_constrained_face_space}--\eqref{eq:restricted_pressure_operator}）．
\end{enumerate}
したがって本稿の毛管閉包は，真円・楕円などの形状名や
曲率上限，人工減衰，時間刻み縮小によって選ぶ近似ではない．
同じ面空間上で「表面エネルギーの仕事」「圧力の仕事」
「成分体積制約の仕事」「壁 no-slip 制約の仕事」を同時に釣り合わせる
離散変分原理から導く．
