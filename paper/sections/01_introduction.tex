...
毛管ジャンプ cochain の $D_f A_f G_f$ 値域射影，
...
界面ジャンプを capillary range projection で同じ $D_f A_f G_f$ 値域へ入れたうえで，
補正段階と投影後 face flux まで同じ値域で閉じ，
節点速度へ戻してから界面を移流する不一致を避ける．

本稿の検証は二層で読む．形式的な精度・安定性の中核 gate は，
§~\ref{sec:density_ratio_sweep}〔V6：静止液滴 8-step 安定性，
密度比 $\rho_r\le 833$〕，
§~\ref{sec:imex_bdf2_twophase_time}〔V7：capillary pressure-jump/projection 界面帯に律速される
coupled-stack 時間精度，観測 slope $1.59$〕，
および §~\ref{sec:zalesak_cls_advection}/§~\ref{sec:single_vortex_reversal}
〔V10-a/b：一様格子・NS 非連成 CLS 強変形移流〕である．
これに加えて，第~\ref{sec:benchmarks}章では同じ production stack 上で
毛管波，振動液滴，単一気泡上昇，Rayleigh--Taylor 不安定性の物理応答を整理する．
非一様 moving-interface CLS の形式収束と，長時間・完全連成 NS 移動界面の
定量的な収束 gate は §~\ref{sec:future_work} の範囲として残す．
...
非一様 moving-interface CLS および長時間・完全連成 NS 移動界面は §~\ref{sec:future_work} の今後の検証ゲートとして残す）．
...
Stage D の Ridge--Eikonal 距離再構成と Stage F の $\phi$ 空間体積閉鎖により，
質量（体積）の離散的保存性と幾何プロファイルを同時に管理する．
