...
FCCD 面フラックス/圧力勾配，HFE 曲率，向き付き pressure-jump 分相 PPE + DC ($k=3$)，
projection-native な正準面速度，および affine jump の履歴圧力面加速度を
同じ face-space contract で閉じる．
この構成により，Balanced--Force 条件の離散化整合性
（本稿で系統化する 7 原則 P-1..P-7；§~\ref{sec:bf_seven_principles}）を
曲率・圧力・速度補正・界面移流の同一面データ共有として維持し，
一括 CSF/BF 縮約経路では CSF モデル誤差 $\Ord{\varepsilon^2}$ が律速する水準まで，
pressure-jump 経路では界面ジャンプと投影後面速度の不整合を直接消す水準まで，
離散化起因の寄生流れを低減する設計とした
...
§~\ref{sec:zalesak_cls_advection}/§~\ref{sec:single_vortex_reversal}〔V10-a/b 一様格子・NS 非連成 CLS 強変形移流〕までとし，
...
また二相結合では capillary pressure-jump/projection 界面帯誤差により実効時間精度は設計の $O(\Delta t^2)$ に届かず，
§~\ref{sec:imex_bdf2_twophase_time}（V7）で観測 slope $1.48$ までの回復に留まることを caveat として明記する．
