...
界面幅パラメータ $\varepsilon$（CLS 推奨スケーリング $\varepsilon = \varepsilon_\xi\, h_{\min}$，$\varepsilon_\xi=O(1)$；§~\ref{sec:reinit} で具体化）に起因する $\Ord{\varepsilon^2} \approx \Ord{h^2}$ のモデル誤差が残り，
...
（具体的検証範囲は §~\ref{sec:density_ratio_sweep}〔V6 静止液滴 8-step 安定性，密度比 $\rho_r\le 833$〕に限定し，
NS パイプライン完全結合および長時間移動界面下の収束性検証は §~\ref{sec:future_work} の残置課題に整理する）．
...
§~\ref{sec:imex_bdf2_twophase_time}（V7）で観測 slope $1.58$ までの回復に留まることを caveat として明記する．
...
CLS 変数 $\psi \in [0,1]$ は界面を滑らかな遷移層として表現する．
本稿では $\varepsilon$ を界面幅パラメータと呼び，中心遷移幅は $\approx 2\varepsilon$，
95\% 遷移帯はおよそ $|\phi|\le 3\varepsilon$（幅 $\approx 6\varepsilon$）である．
