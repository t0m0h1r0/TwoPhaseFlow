...
面フラックス，圧力勾配，界面近傍データ延長，圧力補正，界面移流を
同じ面中心の演算子族へ載せる．
一括 CSF/BF 縮約経路では，Balanced--Force 条件
（本稿で系統化する 7 原則 P-1..P-7；§~\ref{sec:bf_seven_principles}）により，
表面張力と圧力勾配の離散位置を一致させる．
この経路で残る主要誤差は，CSF の拡散界面モデル誤差である．

高密度比・低 We の主経路では，圧力ジャンプを各相の PPE に直接入れる
分相 PPE + 欠陥補正（DC $k=3$）を用いる．
表面エネルギーの仮想仕事から毛管面共鎖を構成し，
再初期化前の物理輸送終点上で，PPE，速度補正，投影後の面フラックス，
および Hodge 平衡判定を同じ面複体で評価する．
ここで Hodge 残差は消去すべき数値ノイズではなく，
体積制約付き表面エネルギーが作る物理的な加速度成分である．
したがって圧力勾配値域への射影は静的平衡判定・原因診断に限り，
本番補正では成分体積制約を取り除いた面共鎖
$\widehat c_{\sigma,f}=s_f-B_h\mu$ を渡す．
履歴圧力のアフィンジャンプ補正も面加速度として持ち越すため，
節点速度へ戻してから界面を移流する不一致だけでなく，
非平衡界面の毛管駆動を圧力値域への射影で消してしまう代数的不整合も避けられる．

本稿の検証済み範囲は，第~\ref{sec:verification}章の V1--V10 で確定する．
単相 NS と縮約 BF 経路の力学的整合性は V1--V5/V8，
高密度比・低 $We$ の二相標準である表面エネルギー仮想仕事に基づく
圧力ジャンプ/Hodge 閉包は V6/V7/V9，
CLS 強変形受動移流は V10-a/b で読む．
とくに V6 は密度比 $\rho_r\le 833$ の静止液滴 8 ステップ安定性，
V7 は毛管圧力ジャンプと圧力投影の界面帯に律速される
結合閉包系の時間精度（観測収束次数 $1.59$），
V9 は同じ閉包系上で固定幅/局所幅 $\varepsilon$ 切替を同時使用しても
有限時間静止液滴を破綻させないことを確認する．
非一様格子上の移動界面 CLS と長時間・完全連成 NS 移動界面の収束性は，
§~\ref{sec:future_work} の今後の検証対象として扱う．
...
HFE による界面近傍データ延長，向き付き圧力ジャンプ分相 PPE，射影後の正準面速度，
...
表面エネルギー仮想仕事に基づく圧力ジャンプ/Hodge 閉包は V6/V7/V9 で，
