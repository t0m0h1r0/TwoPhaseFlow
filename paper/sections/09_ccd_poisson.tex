...
\noindent\textbf{本章の問い：}
二相流の圧力ステップで本当に解くべき問題は，
「Poisson 方程式を高次に解くこと」そのものではない．
投影後の速度を発散なしに戻しつつ，
密度ジャンプ・圧力ジャンプ・表面張力ジャンプを
\textbf{同じ離散面演算子の上で矛盾なく閉じる}ことである．
この問いに対し，本章は候補手段を並列に足し合わせるのではなく，
本質的な失敗を一つずつ除外して，最後に残る pressure-jump 型 FCCD-PPE
閉包を選ぶ．

\begin{tcolorbox}[mybox,title={§9 の選別規則：採用するもの／退けるもの}]
\begin{enumerate}[noitemsep]
  \item \textbf{密度ジャンプ問題}：
        smoothed-Heaviside 一括 PPE は比較経路に留め，
        production では \texttt{phase\_separated} 係数で相内定数密度 PPE に分ける．
  \item \textbf{表面張力ジャンプ問題}：
        CSF 体積力は動機付け・比較経路に留め，
        production では $j_{gl}=p_g-p_l=-\sigma\kappa_{lg}$ を PPE に直接課す．
  \item \textbf{regular jump 相殺問題}：
        \texttt{jump\_decomposition} は legacy 比較経路に留め，
        production では $G_\Gamma(p;j)=G(p)-B(j)$ の \texttt{affine\_jump} を使う．
  \item \textbf{高次ステンシル問題}：
        界面を跨ぐ CCD/FCCD ステンシルを禁止し，HFE で片側 Hermite データを延長する．
  \item \textbf{求解コスト問題}：
        高次演算子を直接「解く」ことに固執せず，欠陥補正で高次 target と低次 base solve を分離する．
\end{enumerate}
\end{tcolorbox}

\noindent\textbf{本章の採用スタック：}
結論として，本章が production 経路として残すのは
\[
...
\texttt{jump\_decomposition} は比較・検証・legacy 用語として残すが，
本章の解決方法としては選ばない．
...
\subsubsection{Balanced--Force 原理から pressure-jump 閉包への選別}
...
最小モデルとして有用だが，production 経路としては二つの欠点を残す：
有限幅 $\delta_\varepsilon$ による $\Ord{\varepsilon^2}$ モデル誤差と，
圧力項とは別の体積力として入ることによる operator-locus のずれである．

したがって本章が採用するのは，CSF 体積力そのものではなく，
Balanced--Force の operator-locus 要請を pressure-jump 形式へ移した
次の閉包である：
\[
...
\]
この形では，表面張力は predictor RHS の体積力ではなく PPE と corrector が共有する
面勾配契約として入る．
つまり，解くべき問題は「CSF を高次化すること」ではなく，
Young--Laplace ジャンプを投影演算子から消えない形で同じ面 flux に載せることである．

\begin{tcolorbox}[resultbox, title={§9 で残す pressure closure と退ける経路}]
\begin{center}
\small
...
候補 & 何を解こうとしたか & §9 の判定 \\
\hline
CSF + BF & 圧力勾配と表面張力体積力の評価位置ずれを減らす & 原理の動機付け．production capillary closure ではない \\
regular jump decomposition & 圧力ジャンプを regular 圧力場 $J$ として合成する & legacy 比較．楕円 solve が $J$ を吸収できるため退ける \\
\textbf{affine jump} & 既知ジャンプを面勾配 $G_\Gamma=G-B$ に直接入れる & 採用．PPE と corrector が同じ面契約を共有する \\
\hline
\end{tabular}
\end{center}
...
\smallskip
\noindent
HFE（§~\ref{sec:field_extension}）が必要になるのは，採用経路が圧力場に
実ジャンプを持たせるからである．CSF のような連続 regular 圧力場では HFE は本質ではないが，
pressure-jump 型分相 PPE では CCD/FCCD ステンシルが不連続値を跨がないための
必須部品になる．
\end{tcolorbox}
...
判断軸は空間精度と face-operator 整合であり，FCCD/CCD を pressure stack の
面演算子として採用する根拠を示す
...
\midrule
FVM baseline（2次中心差分） & $\Ord{h^2}$ & 低次フラックス閉包が律速 & 比較基準 \\
\textbf{FCCD/CCD face operator}（本稿） & $\boldsymbol{\Ord{h^6}}$（平滑領域） & affine jump と同じ面で圧力補正を評価できる & 採用構成 \\
\bottomrule
\end{tabularx}
