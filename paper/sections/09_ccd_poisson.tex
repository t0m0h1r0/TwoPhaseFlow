...
\begin{enumerate}[noitemsep]
  \item \textbf{密度ジャンプ問題}：
        ...
        高密度比の標準閉包では相内定数密度 PPE に分ける．
  \item \textbf{表面張力ジャンプ問題}：
        CSF 体積力は動機付け・比較経路に留め，
        標準閉包では $j_{gl}=p_g-p_l=-\sigma\kappa_{lg}$ を PPE に直接課す．
  \item \textbf{regular jump 相殺問題}：
        regular 圧力合成は legacy 比較経路に留め，
        標準閉包では $G_\Gamma(p;j)=G(p)-B(j)$ のジャンプ補正付き面勾配を用いる．
...
regular 圧力合成は比較・検証・legacy 経路として残すが，
本章の解決方法としては選ばない．
...
候補 & 何を解こうとしたか & §9 の判定 \\
\hline
CSF + BF & 圧力勾配と表面張力体積力の評価位置ずれを減らす & 原理の動機付け．最終的な毛管閉包ではない \\
regular jump decomposition & 圧力ジャンプを regular 圧力場 $J$ として合成する & legacy 比較．楕円 solve が $J$ を吸収できるため退ける \\
\textbf{ジャンプ補正付き面勾配} & 既知ジャンプを面勾配 $G_\Gamma=G-B$ に直接入れる & 採用．PPE と corrector が同じ面条件を共有する \\
\hline
\end{tabular}
\end{center}
