...
%
% ═══════════════════════════════════════════════════════════════════════════════
\section{圧力 Poisson 方程式の解法：分相 PPE・HFE・表面エネルギー毛管閉包・欠陥補正法}
\label{sec:ppe_solve}
...
高次片側データ，表面エネルギー仮想仕事に基づく毛管閉包と平衡判定，欠陥補正，大域ゲージ・再投影分離の七つに分解し，
それぞれに必要な数理構成を順に定める．
背景には射影法~\cite{Chorin1968,vanKan1986,Guermond2006}，
圧力境界条件の精度問題~\cite{BrownCortezMinion2001,PyoShen2007}，
界面ジャンプ処理~\cite{Fedkiw1999,Kang2000,LeVequeLi1994}，
および BF/CSF 系列~\cite{Brackbill1992,Francois2006,Popinet2009} がある．
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{毛管補正量閉包問題}：
        表面エネルギーの仮想仕事を圧力仕事と随伴な面上補正量として構成し，
        成分体積反力を鞍点系で取り除き，静的平衡成分と非平衡駆動を分離する．
  \item \textbf{高次演算子処理問題}：
        欠陥補正により，高次評価演算子と低次補正演算子の役割を分離し，
        反復回数ではなく高次残差で受理する．
  \item \textbf{零モード・再投影問題}：
        圧力の定数自由度を大域ゲージで固定し，再投影 PPE では界面ジャンプ情報を分離する．
\end{enumerate}
\end{tcolorbox}
...
\begin{gathered}
  ...
  +\mathrm{HFE}
  +\widehat c_{\sigma,f}\ \text{（成分体積制約付き鞍点系）},\\
  \mathrm{DC}\{\eta_{\mathrm{DC}}\le\varepsilon_{\mathrm{PPE}}\}
  +\text{大域ゲージ・再投影分離}
\end{gathered}
\]
である．有限体積法/有限差分，CSF，smoothed-Heaviside 一括 PPE，直接解型 CCD，
および滑らかな補助圧力場によるジャンプ表現は，
役割を限定して扱う数理モデルであり，本章の圧力ジャンプ型閉包そのものではない．
精度の全体像と閉包条件は §~\ref{sec:pressure_accuracy_summary} にまとめる．

\paragraph{本章の数式の読み方．}
PPE の式は，圧力値をきれいに求めるためだけの楕円方程式ではない．
直感的には，予測速度が持つ「発散してしまう成分」を，
界面ジャンプ・密度係数・境界条件と矛盾しない圧力反力で打ち消す方程式である．
分相 PPE は密度ジャンプを相内の滑らかな問題へ分けるための分割，
HFE は界面を跨がない片側データを作るための橋，
$G_\Gamma(p;j)$ は圧力値と既知ジャンプを同じ面勾配へ載せるための記法，
DC は高次残差を下げる補正方向だけを受理する代数的な安全装置である．
読者は各節を「どの不連続をどの場所で扱うか」
「その処理が速度補正の面空間まで届いているか」という順に読むと，
長い閉包式を一つの反力構成として追える．
...
この二重の使い方は Chu--Fan CCD~\cite{ChuFan1998} と
compact 差分の楕円/分散解析~\cite{Lele1992,VisbalGaitonde2002} を
圧力 Poisson 文脈へ拡張するものである．

\begin{itemize}
  \item \textbf{役割1（微分演算子）：} 既知の $f$（$\phi$，$\bu$ など）から $\Ord{h^6}$ 精度の
        $f', f'', f_{xy}$ を求める．曲率評価・粘性項評価に用いる．
...
\subsubsection{空間離散化スキームの構造比較（有限体積法基準離散化 vs CCD）}
\label{sec:ccd_vs_fvm}

表~\ref{tab:ccd_vs_fvm} は PPE の\textbf{空間離散化スキーム}（有限体積法基準離散化 vs CCD）を比較する．
判断軸は空間精度と面演算子整合であり，FCCD/CCD を圧力閉包の
面演算子として採用する根拠を示す
（CCD 系を与件とした\textbf{線形求解法}の選択は §~\ref{sec:dc_comparison} で独立に扱う）．
有限体積基準離散化の保存形フラックス評価は Patankar~\cite{Patankar1980}，
projection/FVM 系の二次精度基準は Bell--Colella--Glaz~\cite{BellColella1989}
を参照する．
...
\midrule
有限体積法基準離散化（2次中心差分） & $\Ord{h^2}$ & 低次フラックス閉包が律速 & 基準離散化 \\
