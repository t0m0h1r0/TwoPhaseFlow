...
\textbf{随伴ペア}を構成することが鍵で，これは P-2（§\ref{sec:bf_p2_adjoint}）の
帰結である．
...
\begin{proof}[証明（要旨）]
\textbf{仮定の明示}：本定理は\textbf{CSF-baseline PPE}
（変密度一括 PPE + smoothed Heaviside CSF；§\ref{sec:projection_derivation}）が
$\|G_h p - f_\sigma\|_\infty = \Ord{H^4}$ を満たす設定を前提とする．
分相 PPE / GFM ジャンプ条件版（§\ref{sec:split_ppe_pressure_jump_formulation}）
における精度主張は別建ての Corollary~\ref{cor:fccd_split_ppe_bf} で扱う．
