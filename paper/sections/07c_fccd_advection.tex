...
%
% §6.3  FCCD 移流：Option B/C と面ジェット共通プリミティブ
%        既存の節点 CCD 移流を保存型に拡張する FCCD 移流演算子群．
%        §4.6 FCCD の面中心行列 M^FCCD を速度移流に転用する経路を示す．
%
% ═══════════════════════════════════════════════════════════════════════════════
...
離散化するために用いる．ch13 production stack では bulk 速度に UCCD6 対流
...
\textbf{論証}：$\mathbf{u}\equiv 0$ で保存形・非保存形・スキュー対称形
3 種の対流面フラックス（\eqref{eq:fccd_adv_flux_cons}
\eqref{eq:fccd_adv_flux_skew} および非保存形）はいずれも点積に $\mathbf{u}$ を
