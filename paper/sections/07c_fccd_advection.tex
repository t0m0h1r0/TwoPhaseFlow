...
%
% §7.3  FCCD 移流：Option B/C と面ジェット共通プリミティブ（SP-D）
...
% -------------------------------------------------------------------------------
\subsubsection{共通プリミティブ：面値演算子 $\mathbf{P}_f$ と面ジェット $\mathcal{J}_f$}
...
SP-H の統一プリミティブ $\FaceJet{u} = (u_f, u'_f, u''_f)$ は
\begin{equation}
...
% -------------------------------------------------------------------------------
\subsubsection{Option C：R$_4$ Hermite 面→節点再構成}
...
Option B でのみ H-01 が完全解消する（WIKI-E-030 実測で SP-C 単独の
...
$\mathcal{O}(N)$ 乗算のみで，
演算量は純 CCD 移流の 1.3--1.5$\times$（WIKI-L-019）．
