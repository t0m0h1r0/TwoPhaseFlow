...
\midrule
LS (SDF)     & $1.8 \times 10^{-1}$ & $1.8 \times 10^{-1}$ & $1.8 \times 10^{-1}$ & $1.8 \times 10^{-1}$ \\
CLS ($\psi$) & $1.4 \times 10^{-16}$ & $\Ord{10^{-16}}$    & $\Ord{10^{-16}}$     & $3.7 \times 10^{-2}$ \\
\bottomrule
\end{tabular}
\end{table}
...
（LS は全条件で約 18\% の質量誤差）．
$K = 50$ では DCCD 移流誤差が蓄積し CLS でも $3.7 \times 10^{-2}$ に劣化する．
...
\noindent\textbf{目的：}
§~\ref{sec:grid_ale} の界面適合格子を NS パイプライン
（§~\ref{sec:algorithm} の 7 ステップ Predictor--PPE--Corrector）に統合し，
毎タイムステップで格子を再構築した場合の
質量保存・寄生流れ・格子追従性を検証する．
...
\midrule
一様格子（$\alpha = 1$）    & $6.3 \times 10^{-6}$ & $1.0 \times 10^{-2}$ & 0.0312 \\
界面適合（$\alpha = 2$）    & $6.7 \times 10^{-6}$ & $7.8 \times 10^{-3}$ & 0.0269 \\
\bottomrule
\end{tabular}
\end{table}
...
(a) 質量保存誤差：DGR 再初期化により $\alpha = 2$ でも $6.7 \times 10^{-6}$ に抑制，
(b) 寄生流れ：$\alpha = 2$ は $\alpha = 1$ の約 $3/4$（DGR による界面プロファイル維持），
(c) $h_{\min}$ の時間変化：$\alpha = 2$ で格子が界面に追従し $h_{\min} < h_{\mathrm{uniform}}$ を維持．}
...
\begin{enumerate}[noitemsep]
  \item \textbf{質量保存：}
        一様格子（$\alpha = 1$）では $|\Delta M|/M_0 = 6.3 \times 10^{-6}$，
        界面適合格子（$\alpha = 2$）では $6.7 \times 10^{-6}$ と同等の精度が得られた．
        ...
        質量保存を一様格子と同等に回復した．
  \item \textbf{寄生流れ：}
        $\alpha = 2$ の寄生流れは $\alpha = 1$ の約 $3/4$
        （$7.8 \times 10^{-3}$ vs $1.0 \times 10^{-2}$）．
        DGR による界面プロファイルの正確な維持が，寄生流れの低減にも寄与している．
  \item \textbf{格子追従：}
        図~\ref{fig:ns_grid_rebuild}(c) に示すように，$h_{\min}$ は毎ステップ変化し，
        界面の微小な移動（寄生流れによる）に動的に追従している．
        格子再構築メカニズム自体は正しく機能している．
\end{enumerate}

\noindent\textbf{結論：}
毎ステップ格子リビルドの NS パイプライン統合は数値的に安定（発散なし）であり，
$\alpha = 2$ でも質量保存誤差 $6.7 \times 10^{-6}$ を達成した．
鍵となるのは再初期化に DGR を用いることで，
CN-ADI 拡散の非一様格子上での CFL 違反を回避した点である．
寄生流れは $\alpha = 2$ で $\alpha = 1$ の約 $3/4$ に低減し，
格子追従メカニズムも正しく動作している．
...
製造解に対して確認する．
風上差分経路は §~\ref{sec:verify_interface} で述べた理由（密度ジャンプ界面での NaN）により
廃止済みであるため，Hermite 経路のみを判定対象とする．
...
全格子で解析解 $\Delta p = 4.0$ に対し相対誤差 $0.16$--$0.31\%$ を達成．
...
\noindent\textbf{結論：}
全格子（$N = 32$--$128$）で Young--Laplace 条件を相対誤差 $0.16$--$0.31\%$ で再現した．
格子精緻化に伴う収束は CSF 正則化 $\delta_\varepsilon$（$\Ord{h^2}$）で飽和し，
界面幅 $\varepsilon = 1.5h$ が精度の律速である．
なお，風上差分経路は §~\ref{sec:verify_interface} で述べた通り廃止済みであり，
本テストでは比較対象としない．
