...
%
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{非一様格子上の FCCD：\texorpdfstring{$\mathcal{O}(H^3)$}{O(H3)} 行列定式（$\theta=1/2$ 界面適合で $\Ord{H^4}$）}
...
（ただし，物理座標 $x$ への変換に用いる Jacobian $J_x$ を Simpson 則
または解析積分で構築する場合に限る．台形則で構築すると
$J_x$ 自体が $\Ord{\Delta\xi^2}$ に縛られ，
全体精度は $\Ord{H^2}$ に縮退する；§\ref{sec:grid_metric_caveat} 参照）．
