...
SP-C §5 \cite{SPC} は，\S\ref{sec:fccd_matrix} で導入した
一様格子版行列 $\mathbf{M}^{\FCCD} = \mathbf{D}_1 - \mathbf{D}_2\mathbf{S}_{\CCD}$ を
非一様格子に拡張する道筋を示した．
ここでは WIKI-T-050 の非一様 \textbf{打ち消し係数} $(\mu_i,\lambda_i)$ を
2 つの疎な疎行列に織り込む．
...
SP-C §5 に従い，$\mathbb{R}^{N\times(N+1)}$ の
...
SP-C §5 の Taylor 解析（WIKI-T-050 の $\mu,\lambda$ 導出と整合）により，
...
\S\ref{sec:fccd_h01_remedy} で示した BF 残差 $|\mathrm{BF}_\text{res}| = \Ord{\Delta x^4}$ の性質は，
...
境界行の wall BC 拡張は付録 B.2 および WIKI-T-050/053 参照．
