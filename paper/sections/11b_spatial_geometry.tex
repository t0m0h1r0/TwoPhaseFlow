...
なお，HFE（§~\ref{sec:verify_field_extension}）は 1D で $\Ord{h^7}$，
2D でも $N\le128$ で高次収束を示すが，最高解像度では丸め・補間誤差床の影響を受ける．
したがって界面近傍の実用的な精度律速は主として
CSF 正則化 $\delta_\varepsilon$（$\Ord{h^2}$）であるが，
HFE 2D は条件付き合格として扱う．
