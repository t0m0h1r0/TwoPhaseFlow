...
CLS 変数 $\psi$ から logit 変換 $\phi = \varepsilon\ln[\psi/(1-\psi)]$ で得られる
符号付き距離近似 $\phi$ に対して曲率を計算する（§~\ref{sec:levelset_mapping}）．
計算式は
...
$\varepsilon = 1.5h$ は格子精緻化に伴い等比縮小するため，
CLS プロファイルの滑らかさが格子解像度と連動して変化せず，
高次演算子の優位性が現れない（CCD と CD2 が同等になる理由）．
