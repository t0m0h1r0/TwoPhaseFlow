...
物理空間 $x$（非一様）を計算空間 $\xi$（一様，$\Delta\xi=1$）に写像し，
...
非一様格子（$\alpha > 1$）では CCD 勾配のメトリクス $J_n = 1/\delta v_i$（節点制御体積）と
PPE の FVM Laplacian のメトリクス $J_f = 1/d_f^{(i)}$（面間距離）が乖離し，
残差発散が毎ステップ速度場に注入される
（$\alpha=1.5$ では $\max|J_f-J_n|/J_f = 0.77$，ステップ 51 でブローアップ）．

\textbf{解決：} face-average 勾配 $\mathcal{G}^\text{adj}$ を用いれば
$\mathcal{D}_\text{FVM}(\mathcal{G}^\text{adj}) = \mathcal{L}_\text{FVM}$（厳密整合）が成立する
（定式化・実装の詳細は §~\ref{sec:fvm_ccd_corrector}；
均一格子および周期 BC では CCD をそのまま使用）．
\end{tcolorbox}
