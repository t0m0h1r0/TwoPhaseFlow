...
FCCD（§\ref{sec:fccd_def}：セル界面の面値/面勾配）の 4 種演算子族が，
均一格子・周期境界もしくは壁境界上で\textbf{設計次数の漸近的収束}を達成
することを単体で確認する．

\paragraph{設定．}
製造解は U1-a/c で 2D テンソル積 $f(x,y) = \sin(2\pi x)\sin(2\pi y)$
...
\begin{description}\setlength{\itemsep}{0pt}
  \item[U1-a] CCD 2D テンソル積 MMS（$x$ 軸評価）周期 BC d1/d2 + 壁 BC d1
        （境界閉包の精度保存設計は~\cite{Carpenter1994}）．
  \item[U1-b] DCCD 伝達関数 $H(\xi; \varepsilon_d)$ Nyquist 値 $H(\pi)$
        ($\varepsilon_d \in \{0,\, 0.05,\, 0.25\}$)．
  \item[U1-c] FCCD 周期面値/面勾配（2D テンソル積 MMS, $x$ 軸評価）．
  \item[U1-d] UCCD6 節点 RHS（1D ブロードキャスト，ハイパー粘性含む）．
\end{description}
...
\end{tabular}
\\[2pt]
{\footnotesize FCCD は §\ref{sec:fccd_derivation} の打ち消し設計どおり面値/面勾配ともに $\Ord{\Delta x^4}$；
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item U1-c FCCD：面値/面勾配とも slope $4.00 / 4.00$．
        §\ref{sec:fccd_derivation} 式~\eqref{eq:fccd_fourth_order} の
        打ち消し設計次数 $\Ord{\Delta x^4}$ を完全達成（U3-b の非一様格子でも同 4 次達成）．\checkmark
  \item U1-d UCCD6：MMS 対称性によりスーパー収束 slope $\approx 7$．
        §\ref{sec:uccd6_def} のハイパー粘性の 6 次設計を上回り合格．\checkmark
\end{itemize}
検証対応：式~\eqref{eq:CCD_I} と §\ref{sec:ccd_def} の行列形に対し，
周期 MMS，Nyquist 応答，面評価誤差を測定する．
