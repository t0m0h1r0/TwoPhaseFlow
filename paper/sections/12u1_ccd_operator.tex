...
\begin{table}[htbp]
\caption{U1-a：CCD 1D MMS の格子収束}
\label{tab:U1_ccd_periodic}
\PaperCaptionNote{周期境界，$f=\sin(2\pi x)$．理論期待値 $p_{\mathrm{design}}=6$（d1, d2 とも）．$N = 256$ で d2 が round-off 床 $1.0\times 10^{-10}$ に到達．}
\centering\small
...
\begin{table}[htbp]
\caption{U1-b：DCCD 伝達関数 Nyquist 値 \texorpdfstring{$H(\pi; \varepsilon_d)$}{H(pi; eps_d)}}
\label{tab:U1_dccd_nyquist}
\PaperCaptionNote{解析値 $H(\pi) = 1 - 4 \varepsilon_d$ と一致．}
\centering\small
\begin{tabular}{r r r}
\toprule
...
\begin{table}[htbp]
\caption{U1-c：FCCD 周期 face value/grad}
\label{tab:U1_fccd_uccd6}
\PaperCaptionNote{U1-d：UCCD6 節点 d1． FCCD は面演算子としての設計次数 $\Ord{\Delta x^4}$（§\ref{sec:fccd_derivation}）どおりに収束する．}
\centering\small
\begin{tabular}{r r r r}
\toprule
...
\label{fig:ch12_u1}
\PaperCaptionNote{CCD 周期 d1/d2 slope $6.00 / 5.97$，DCCD Nyquist $H(\pi;\varepsilon_d) = 1-4\varepsilon_d$ 解析一致， FCCD 面演算子設計次数どおり 4 次，UCCD6 対称性スーパー 7 次．}
\end{figure}
