% paper/sections/appendix_e1_3_dc_spectral_convergence.tex
%
% 付録 E.1.3：欠陥補正法の収束理論
...
\subsubsection{収束理論：反復行列のスペクトル解析}
\label{app:dc_convergence_theory}

本付録は §\ref{sec:defect_correction_main} で定義した欠陥補正法（式~\eqref{eq:dc_three_step}）の
収束性を反復行列のスペクトル解析に基づいて評価する．
ここでの解析は $L_L$ と $L_H$ の固有ベクトル共有などの理想化仮定に基づくため，
実装全域の収束保証ではなく，実験結果を解釈するための判定基準として用いる．
...
なお，等密度・smooth RHS の検証（§\ref{sec:dc_iteration_accuracy}）では，
$k = 3$ 反復で $\Ord{h^6}$ 相当の収束が確認されている．
これは直接 LU（\texttt{spsolve}）による Step~2 の厳密求解により
1反復あたりの有効減衰が大きいためであり，変密度・界面問題へ無条件に外挿しない．

\subsubsection*{緩和係数 \texorpdfstring{$\omega$}{omega} の役割}
