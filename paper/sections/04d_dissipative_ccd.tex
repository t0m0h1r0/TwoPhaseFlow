...
\begin{itemize}
  \item \textbf{エイリアシング不安定性：}
        対流項 $\bm{u}\cdot\nabla\bm{u}$ の非線形相互作用によって高波数エネルギーが
  ...
  \item \textbf{界面近傍の圧力・速度振動：}
        ...
        格子スケールノイズが一度流速場に混入すると，次ステップの移流・圧力求解で増幅される．
\end{itemize}

従来の対処法として，（a）CCD 微分場に独立な広ステンシルフィルタ（5〜7 点）を後処理として適用する方法，
または（b）速度場の離散化自体を上流偏向スキームに切り替える方法がある．
前者は CCD のコンパクト性を損ない；後者は速度の低波数成分にも数値拡散を導入し高精度性を毀損する．
...
これにより CCD の $O(h^6)$ 精度と高周波安定性を同一ステンシル内で両立する．
...
\begin{itemize}
  ...
  \item \textbf{低波数近似：} $H(\xi;\varepsilon_d) \approx 1 - \varepsilon_d\xi^2$ for $\xi \ll 1$
        （精度影響は $O(\varepsilon_d\xi^2)$，CCD の $O(h^6)$ 誤差より十分小さい）
\end{itemize}
\end{tcolorbox}
...
フィルタは界面近傍では完全に無効化し，バルク領域でのみ機能させたい．
...
\begin{equation}
  ...
  \label{eq:delta_reg}
\end{equation}
Eikonal 条件 $|\nabla\phi| = 1$ のもとで $|\nabla\psi| = |\delta_\varepsilon(\phi)|\,|\nabla\phi| = \delta_\varepsilon(\phi)$ となる（$\delta_\varepsilon \geq 0$）．
...
\begin{equation}
  \Theta(\psi) \;\equiv\; 4\psi(1-\psi) = \frac{|\nabla\psi|_{\mathrm{eq}}}{\max|\nabla\psi|_{\mathrm{eq}}} \;\in\; [0, 1]
...
フィルタは界面指示子の\textbf{補集合}（界面 $\to 0$，バルク $\to 1$）であるべきであるから，
...
\begin{enumerate}[label=(\alph*)]
  \item \textbf{精度保存：} 低波数域（長波長の物理情報）は CCD の $O(h^6)$ 精度を損なわない．
...
\begin{enumerate}
  ...
  \item \textbf{1 次微分のフィルタ適用：}
        ...
        壁面・流出 BC では $f'$ の境界値に外挿を適用（詳細は §\ref{sec:ccd_bc}）．
...
\end{equation}
すなわちフィルタは $O(\varepsilon_d\,h^2)$ の追加誤差を導入する（$h$ の2乗）．

バルク領域（$\varepsilon_d^{(i)} \leq \varepsilon_{d,\max} = 0.05$）では：
$O(0.05h^2) \sim O(h^2)$——CCD の $O(h^6)$ より 4 次粗い精度である．

しかし，\textbf{この精度低下は意図的かつ許容範囲内}である：
バルク領域での $u$ や $p$ の滑らかな挙動に対しては，
$h^2$ 誤差は格子収束とともに消え（$h \to 0$ で $\varepsilon_d h^2 \to 0$），
かつ表面張力の CSF モデル誤差 $O(\varepsilon^2) \approx O(h^2)$（§\ref{sec:balanced_force}）と同オーダーであり，
全体の精度のボトルネックとならない．

界面近傍（$\varepsilon_d^{(i)} \approx 0$）では，フィルタ誤差がゼロに退化し，
曲率・法線方向の計算には一切の影響を与えない：
$O(\varepsilon_d^{(i)}\,h^2) = O(S(\psi_i)\varepsilon_{d,\max}h^2) \xrightarrow[\psi_i \to 0.5]{} 0$．
...
本稿の Navier--Stokes 離散化は移流項 $\bm{u}\cdot\nabla\bm{u}$ を
\textbf{非保存形}（advective form）で評価しており，
フラックス形式に基づく厳密な保存は当初から保証していない．
この前提のもとで DCCD フィルタの非保存的誤差は
$O(\varepsilon_d^{(i)} h^2)$（式~\eqref{eq:dccd_accuracy_error}）であり，
CSF モデルの界面誤差 $O(\varepsilon^2)\approx O(h^2)$（§\ref{sec:balanced_force}）と
同オーダーとなる．したがって，微分場フィルタは既存の離散化誤差の構造と整合しており，
精度と安定性のトレードオフとして許容される．

\begin{tcolorbox}[resultbox, title={フラックス形フィルタの適用指針}]
...
速度修正ステップ $\bm{u}^{n+1} = \bm{u}^* - \Delta t\,\tilde{\nabla}p$（§\ref{sec:pressure}）が
\textbf{発散自由条件} $\nabla^{\mathrm{RC}}\cdot\bm{u}^{n+1} = 0$ を満たすよう設計されているためであり，
...
\begin{enumerate}
  \item \textbf{圧力勾配のフィルタリング：}
        NS 右辺に現れる $\nabla p$（CCD 微分として評価）に対して DCCD フィルタを適用する．
        $p$ 自体は書き換えず，PPE の構造と発散自由性への影響がない．
  \item \textbf{PPE 右辺の正則化：}
        ポアソン方程式 $\nabla^2 p = \nabla^{\mathrm{RC}}\cdot\bm{u}^*/\Delta t$ の
        右辺 $q = \nabla^{\mathrm{RC}}\cdot\bm{u}^*/\Delta t$（式~\eqref{eq:rc_divergence}）に
...
この操作を一切行っていないことが CHK-036 審査により確認済みである．
\end{tcolorbox}
