...
圧力 Poisson & FD 直接法 PPE + CCD 勾配（§~\ref{sec:pressure}） & $\Ord{h^2}$/$\Ord{h^6}$† & Balanced-Force 整合性（CCD 統一） \\
\hline
\end{tabular}
\end{center}
{\small †\ PPE 求解は FD $\Ord{h^2}$；速度補正の圧力勾配は CCD $\Ord{h^6}$（§~\ref{sec:pressure_accuracy_summary}）．}
\end{tcolorbox}
