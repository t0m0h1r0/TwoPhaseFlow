...
\begin{description}
  \item[\upshape(i) 設定界面構造の長時間保持]
        ...
        WENO5 が数ステップで界面を $\Ord{h}$ 幅だけ肥厚させる状況でも，
...
実用的な安定性が得られることが期待される（設計目標；数値実験による検証は今後の課題；形式的上界は \ref{result:dccd_stability} 参照）．
...
$h \cdot N \cdot \Ord{h^5} = \Ord{h^5 \cdot L}$ となり，
移流ステップあたりの質量保存誤差は $\Ord{h^5\,\Delta t}$ となる
