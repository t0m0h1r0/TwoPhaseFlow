...
不連続を検知すると，その方向の重みを $\Ord{\varepsilon^2}$ まで低減して
...
$N$ 格子点での局所打ち切り誤差 $\Ord{h^6/h}=\Ord{h^5}$（CCD の $\Ord{h^6}$ 積分精度を格子間隔 $h$ で規格化した1格子当たりの誤差密度）が $N$ 点累積して
$h \cdot N \cdot \Ord{h^5} = \Ord{h^5 \cdot L}$ となる．
したがって，移流ステップあたりの質量保存誤差は $\Ord{h^5\,\Delta t}$ となる
