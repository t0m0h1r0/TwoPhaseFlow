...
\end{equation}
（式~\eqref{eq:cls_advection}の再掲）を数値的に解く離散化スキームを示す．
...
\begin{enumerate}
  \item \textbf{§~\ref{sec:advection_motivation}（Dissipative CCD の設計根拠）：}
        WENO5 が CLS 界面と相性が悪い理由を示し，
        CCD フィルタによる安定化の設計を述べる．
  \item \textbf{§~\ref{sec:cls_compression}（圧縮項の離散化）：}
        再初期化方程式の圧縮項 $\bnabla\cdot(\psi(1-\psi)\hat{\bm{n}})$ を
        Dissipative CCD と Crank--Nicolson 法で離散化する．
  \item \textbf{§~\ref{sec:time_int}（時間積分スキーム）：}
        CLS 移流・再初期化には TVD-RK3，NS 方程式には AB2 + IPC を採用する根拠を示す．
