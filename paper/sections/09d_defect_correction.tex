...
\begin{itemize}
  ...
  \item $L_L$：2次精度中心差分（FD）で離散化した近似 Laplacian．
        ...
        基準解析ではこの疎行列を直接法で解き，
        非一様問題では同じ $L_L$ の因子を DC 補正 RHS 間で再利用する．
...
一方，非一様格子では同じ $L_L$ を conservative FD flux 形で組み立て，
...
pressure-jump 閉包を指し，純 FCCD 高次極限そのものと同義ではない．
