...
$k=3$ は設計精度を保証する主契約ではなく，
低・中密度比の対照経路における残差監視付き補正上限として扱う．
...
$L_L$（smoothed-Heaviside 一括 PPE では変密度 FD Laplacian，分相 PPE では相内定数密度 FD Laplacian；
...
したがって，固定回数 $k=3$ そのものは精度契約ではない．
これは分相 PPE の相内 Poisson で観測される有効な標準上限であり，
一括変密度 PPE の界面帯誤差や残差停滞を打ち消す万能条件ではない．

\paragraph{生産計算での残差契約．}
N64 $\alpha=2$ の affine-jump 振動液滴では，
相内 Poisson の成分検証で有効な $k=3$ 上限をそのまま生産計算の契約にすると，
外側 DC の高次残差が射影誤差として残り得る．
したがって実行時の受理条件は反復回数ではなく，
\begin{equation}
  \eta_{\mathrm{DC}}^{(k)}
  =
  \begin{cases}
...
それ自体が数値精度の証明ではない．
この契約で $T\le1$ の N64 $\alpha=2$ 振動液滴を評価すると，
projection DC の最終相対残差は $2.5\times10^{-9}$--$2.9\times10^{-9}$，
投影後発散は $\Ord{10^{-11}}$ まで低下した．
この結果は，圧力振動の原因切り分けにおいて
「楕円方程式を十分に解いていない」誤差を物理閉包誤差から分離するための
運用上の必要条件である．GPU 実装ではこの残差は診断スカラーとしてのみ
host へ同期し，$L_H$ 評価と補正更新の代数は device-resident に保つ．
