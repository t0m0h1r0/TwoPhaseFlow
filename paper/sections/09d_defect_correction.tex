...
本稿では変密度一括 smoothed-Heaviside PPE（本稿のアルゴリズム構成における PPE 求解）
...
実装上はゲージ拘束後の疎行列に対して LU 分解等の直接解法を用いる．
...
別途ベンチで残差保証を確認する（§\ref{sec:verify_dc_spectral}）．
...
分相 PPE + DC + CCD 勾配構成（純 FCCD DNS パス）の詳細は §\ref{sec:split_ppe_framework} および §\ref{sec:fccd_bf_sub_system} を参照．
