...
本節の基準ケースでは疎行列直接法で Step~2 の解法誤差を抑える．
...
行列形ではゲージ拘束後の疎行列に対して LU 分解等の直接解法を用いる~\cite{Kelley2003}．
$x$・$y$ 方向の line sweep/ADI は計算コストを下げる近似前処理として有効だが，
その場合は Step~2 残差が許容誤差に依存するため，本節の「直接法」ケースと区別する．
\textbf{適用範囲：}LU 直接法は総自由度 $N_xN_y \le 256^2$ 程度の小中規模で実効的
（$\Ord{N^{3/2}}$ fill-in が主コスト；$N=128^2$ で 10--30 秒）．
$N_xN_y \ge 512^2$ の大規模反復解法は本節の直接法保証の外にあり，
別途検証で残差保証を確認する（§\ref{sec:verify_dc_spectral}）．
...
production pressure stack を指し，純 FCCD DNS そのものと同義ではない．
