...
1 回の外側 DC 反復内で同じ $L_L$ 因子を複数の補正右辺に再利用し，
第2段の直接法保証を保ったまま再分解コストを避ける．
...
\begin{itemize}
  ...
  \item $L_L$：2次精度中心差分（FD）で離散化した近似 Laplacian．
        ...
        基準解析ではこの疎行列を直接法で解き，
        非一様問題では同じ $L_L$ の因子を DC 補正右辺間で再利用する．
...
一方，非一様格子では同じ $L_L$ を保存形 FD フラックス形で組み立て，
外側 DC 反復の開始時に一度だけ因子化し，初期補正と欠陥補正の複数右辺に再利用する．
