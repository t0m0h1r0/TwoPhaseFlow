% paper/sections/10b6_dccd_dissipation.tex
%
...
滑らかなプロファイルでは CCD と同等の高精度を維持することを検証した．
...
% Figure removed — placeholder only; regenerate from experiments/dccd_comparison.py

\paragraph{考察．}
\begin{itemize}[noitemsep]
  \item \textbf{不連続（Square）：} DCCD は CCD に比べ TV を $10.83 \to 3.15$ に低減（約 $1/3$）し，
        Gibbs 型振動を大幅に抑制した．$L_2$ 誤差も最良（$4.23\times10^{-2}$）であった．
        WENO5 は TVD 条件を満たすため最良 TV（$2.00$）だが，$L_2$ は DCCD より劣る（$6.35\times10^{-2}$）．
  \item \textbf{平滑（Smooth tanh）：} DCCD と CCD の $L_2$ 誤差は同オーダー
        （$3.75\times10^{-5}$ vs $2.57\times10^{-5}$，1.5 倍以内），TV も同値（$2.000$）であった．
        フィルタが高波数成分のみを対象とし，滑らかな成分を保護することを示している．
  \item \textbf{CLS 法への含意：} CLS の界面 $\psi$ は Smooth tanh に相当する平滑化プロファイルであるため，
        DCCD は CCD と同等の精度で界面を移流しつつ，界面近傍の非物理的振動を抑制できる．
