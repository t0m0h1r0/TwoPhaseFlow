...
\subsubsection{CSF + Extension PDE による界面処理と寄生流れの抑制}
...
\begin{tcolorbox}[resultbox, title={Extension PDE + CCD による寄生流れ抑制（FVM + CSF との比較）}]
\begin{center}
\small
...
\textbf{CCD + CSF + Extension PDE + DCCD}（本稿） & Gibbs 振動を除去，数値成分 $\Ord{h^6}$ & $\boldsymbol{\Ord{h^1}}$\textbf{（上風1次律速）} \\
\hline
\end{tabular}
\end{center}
（表~\ref{tab:ccd_vs_fvm}，§~\ref{sec:balanced_force}，表~\ref{tab:accuracy_summary} も参照）

\smallskip\noindent\textbf{注（O(h¹) の根拠）：}
Extension PDE の空間微分 $\hat{\bm{n}}\cdot\bnabla q$ を1次精度風上差分で評価するため，
延長後場 $q_\mathrm{ext}$ の界面近傍（$|\phi| \lesssim 2h$）誤差は O(h) となる．
後段の CCD はこの入力誤差に律速され，界面近傍の圧力勾配精度も O(h) に留まる
