...
\midrule
$1.0$（緩和なし） & $1.40$ → 発散 & $0.00$ & ✗ \\
$0.833$（$\omega_{\max}$） & $1.00$ → 停滞 & $0.17$ & △ \\
$0.70$ & $0.28$ & $0.30$ & ✓（速い） \\
$\mathbf{0.50}$ & $\mathbf{0.20}$ & $\mathbf{0.50}$ & ✓（バランス型） \\
$0.30$ & $0.28$ & $0.70$ & ✓（安全側） \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
等密度，$\omega = 0.50$ & $0.20$ & 38 反復（$N=32$）✓ \\
等密度，$\omega = 0.70$ & $0.28$ & 22 反復（$N=32$）✓ \\
等密度，$\omega = 0.83\,(\omega_{\max})$ & $0.99$ & 停滞 ✗ \\
変密度（$\rho_l/\rho_g = 2$，$N=64$） & $\approx 0.20$--$0.28$ & 39--74 反復 ✓ \\
変密度（$\rho_l/\rho_g \ge 5$，$N=64$） & — & 界面 CCD/FD 不整合床で停滞 ✗ \\
\bottomrule
\end{tabular}
\end{center}
...
高密度比には Krylov 法（GMRES + FD 前処理）が必要となる．
\end{tcolorbox}
