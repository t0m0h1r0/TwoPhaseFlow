...
「速度場そのものの厳密な離散発散フリー」は構造的には保証されない．
...
CLS（保存形 + FCCD 共有面フラックス） & 内部面フラックスは対消滅し，残差を境界・clamp・再初期化・質量閉鎖へ分離 &
  境界フラックス，値域制限，格子転写，Stage B/F 質量閉鎖残差 \\
MAC スタガード格子 & $0$（厳密保存） &
  構造的な離散発散フリー保証 \\
\hline
\end{tabular}
...
したがって体積保存の議論は，フラックス発散そのものではなく，境界，clamp，
再初期化，および Stage B/F の質量閉鎖残差として評価される．
\end{tcolorbox}
...
これは低次の代替スキームではなく，標準パイプラインの幾何修復段である．
...
連続レベルの平衡条件として用い，離散標準経路では Stage D/F の
