...
\textbf{離散化における精度：}
\begin{itemize}
  \item 本稿の標準 CLS 移流では FCCD 面フラックス
        （§~\ref{sec:advection_motivation}）を使用する．
        面値 $(\psi u)_{f}$ のフラックス差分は保存型 FVM の
        テレスコーピング構造を持つため，clamp・再初期化・格子転写を除く
        滑らかな移流段階では内部面フラックスが対消滅する．
        残る体積誤差は境界処理，値域制限，Stage B/F の明示的な質量閉鎖に分離して評価する．
  \item 圧力 Poisson 求解前の中間速度 $\bu^*$（本稿のアルゴリズム構成における predictor 出力）を CLS 移流に用いると，
        壁面法線速度が厳密にゼロでない場合に境界フラックスへの誤差が生じうる．
        標準経路では $\bu^n$（前時刻の投影済み速度，発散誤差が抑制された速度）を使用する．
  \item 第~\ref{sec:verification}章のベンチマーク（体積保存誤差 $\lesssim 1\%$，$N=64^2$ 校正）で
        実際の保存性を定量確認する．
\end{itemize}

\medskip\noindent\textbf{CLS が標準 LS より優れる理由：保存構造の比較}

コロケート格子 + FCCD 面フラックスでは MAC スタガード格子と異なり
「厳密な離散発散フリー」は保証されない．
しかし，以下の保存構造の比較が CLS の優位性を示す：
...
CLS（保存形 + FCCD 面フラックス，滑らかな周期/壁面移流段階） & フラックス差分の内部面和は対消滅 &
  境界フラックス，値域制限，Stage B/F の明示的な質量閉鎖 \\
MAC スタガード格子 & $0$（厳密保存） &
  構造的な離散発散フリー保証 \\
\hline
\end{tabular}
...
CLS（保存形 + FCCD 面フラックス）では保存形の発散定理によりこの項が連続レベルで消去される．
離散残差は境界フラックス，値域制限，格子転写，Stage B/F の質量閉鎖に分かれ，
周期境界または no-through 壁境界の滑らかな移流段階では内部面フラックスの対消滅が
体積保存の主構造となる．
...
ロジット逆変換の分子・分母が数値退化するため，
Ridge--Eikonal 補助場経路（§~\ref{sec:ridge_eikonal}）で零点集合から $\phi$ を再構成する．
