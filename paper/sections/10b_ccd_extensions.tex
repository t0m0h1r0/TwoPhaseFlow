...
座標配列 $x(\xi)$ を CCD 評価対象として $J_x|_i$ と $\partial J_x/\partial\xi|_i$ を同時評価することで解決する
...
圧力 PPE だけが高次の計量係数を使い，曲率や粘性項が低次の計量係数を使う混在は，
表面張力・圧力・粘性の面釣合いを別々に崩すため採用しない．

\subsubsection{計算空間 \texorpdfstring{$\xi$}{xi} 上でのフィルタ適用順序（DCCD/UCCD 共通）}
...
非一様格子上で適用する際には，
\textbf{計算空間 $\xi$ 上でフィルタ／安定化項を作用させ，事後に計量係数 $J_x = \partial\xi/\partial x$ を適用}する．
...
CCD 法は1次元コンパクト系として定式化されているため，混合微分は以下の\textbf{2段階適用}で計算する．
...
この操作を全行（$j = 0, 1, \ldots, N_y-1$）で定義する．
\[
...
\textbf{得られる量：} 混合微分 $\partial^2 f/\partial x \partial y$ が全格子点で定まる．
\end{tcolorbox}

\subsubsection{精度の議論：\texorpdfstring{$O(h^6)$}{O(h\textasciicircum 6)} の維持}

各方向の CCD が $\Ord{h^6}$ 精度を持つとき，段階2での中間誤差増幅により一般には $\Ord{h^5}$ に劣化しうる
