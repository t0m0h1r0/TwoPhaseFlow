...
散逸を外殻後置フィルタとして加える DCCD（§\ref{sec:dccd_derivation}）と
ともに\textbf{節点中心散逸系統}を構成し
（DCCD は後置フィルタ，UCCD6 は演算子内部散逸という対比），
面中心化を行う FCCD（§\ref{sec:fccd_def}）は別系統に属する．
周期格子では Crank--Nicolson 時間積分との結合で無条件 $L^2$ 安定となる
（§\ref{sec:time_int}）．

% -------------------------------------------------------------------------------
\subsubsection{散逸動機の階層：1 次風上から \texorpdfstring{$k=4$}{k=4} ハイパー粘性へ}
...
境界閉包と丸め誤差への感度を増やす~\cite{Carpenter1994}．
したがって指数 7・ハイパー粘性 4 乗は\textbf{次数保存の最小構成}である．

% -------------------------------------------------------------------------------
\subsubsection{半離散 \texorpdfstring{$L^2$}{L2} 安定性（エネルギー法）}
...
この境界影響は §\ref{sec:verify_ccd_convergence} の UCCD6 演算子検証で測定し，
時間積分子としての implicit-BDF2 安定性は §\ref{sec:verify_time} で検証する．
...
DCCD（§\ref{sec:dccd_derivation}） & 節点 & 外殻後置フィルタ & $\Ord{h^6}$ \\
...
{\footnotesize $^\star$ 微分演算子の形式精度．DCCD では後置フィルタによる移流フィルタ誤差 $\Ord{\varepsilon_d^\text{adv}\,h^2}$ が別途付随し，移流の実効精度は $\Ord{h^2}$ に律速される（§\ref{sec:dccd_filter_theory} 参照）．UCCD6 は $\omega_2^8$ 重みで Nyquist 帯域のみ選択減衰し，領域内部で $\Ord{h^6}$ を維持する．}

\noindent
\textbf{DCCD との数値特性比較．}
$\omega_2^8$ の選択的 Nyquist 散逸は DCCD の一様減衰 $H(\pi)=1-4\varepsilon_d$ より
分散精度の保持に優り，領域内部の高周波保持性で上位に位置する；
...
本章の派生スキームは出力評価位置で二つの系統に分かれる：
節点中心系統（CCD / DCCD / UCCD6）と面中心系統（FCCD）である．
UCCD6 は節点中心系統の第 3 員で，演算子内部散逸により Gibbs を抑制する；
FCCD は面位置に値を持つ高次補間で $\Ord{H^4}$ を保ち，
\textbf{面ジェット}（§\ref{sec:face_jet_def}）を経由して節点中心系統の出力にも面入出力形式を提供する．
両系統は同じ基底 CCD のシンボル $\omega_1, \omega_2$ から派生する．
