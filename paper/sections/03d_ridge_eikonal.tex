...
%   トポロジー変化を連続時間発展で扱うガウス補助場 $\xiridge$ と
...
\textbf{(A) 標準 CLS 手順の幾何修復．}~保存形移流後の $\psi$ が
品質閾値を超えた場合，$\psi=0.5$ 零等値面から FMM/FSM を起動して
$\phi$ を再構成し，$H_\varepsilon(-\phi)$ と質量閉鎖で CLS 変数へ戻す
（§~\ref{sec:ridge_fmm_reconstruction}）．
...
品質劣化時とトポロジー変化時に $\phi$ を作り直す\textbf{幾何修復段}を与えるものである．
本章では連続定式化と再構成の論理に焦点を当て，
CCD/FCCD の離散演算子，非一様格子定式化，感度較正の詳細は
...
\subsubsection{ガウス補助場 \texorpdfstring{$\xiridge$}{xi-ridge} の定義（D0）}
...
$\xiridge$ は格子密度用のガウス関数 $\omegagrid(\phi) = \exp(-\alpha(\phi/\varepsilon_g)^2)$
（§~\ref{sec:grid_density}）とは\textbf{役割が直交}する：
$\omegagrid$ は格子間隔を制御し，$\xiridge$ は界面そのものを表す．
また，単純な符号付き距離再構成が格子添字空間の
ユークリッド距離を計算して Eikonal 条件を満たすのに対し，
...
離散計算では再現性を壊さない\textbf{決定論的}な安全策を採るべきである：
\begin{enumerate}[leftmargin=1.5em, itemsep=0pt, topsep=2pt]
  \item \textbf{退化候補の検出．}
...
FMM/FSM へ渡す零点集合を決定論的に固定する離散選択だけを採用する．
\end{remark}
...
\subsubsection{FMM/FSM による \texorpdfstring{$\phi$}{phi} の再構成と一意性}
...
$\Gamma$ から $\Gamma\cup\Gamma_{\mathrm{art}}$ へ置き換え，
...
法線一貫性は Hessian 固有ベクトル符号を連続枝で選択することで確保する．

\subsubsection{表面張力ありの界面幅整合}

純粋 FMM 再構成は，表面張力が有意な場合には
符号付き距離関数の幅と拡散界面幅の不一致に敏感になりやすい．
根本原因は，PPE 側で参照される表面張力残差が
$\sigma\kappa/\varepsilon$（ここで $\sigma$ は表面張力係数）にスケールする一方，
再構成された符号付き距離関数が局所界面幅を必ずしも保持しない点にある．
\textbf{局所界面幅拡張}（式~\eqref{eq:eps_local}）を用いる場合は
...
\begin{itemize}
  ...
  \item 表面張力ありで局所界面幅拡張を用いる場合は，標準経路と分けて明示する．
\end{itemize}
