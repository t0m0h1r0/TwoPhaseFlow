...
\begin{itemize}[noitemsep,leftmargin=2em]
  ...
  \item 界面状態：active-geometry 毛管状態空間，
        geometric swept-volume の $q$ 輸送，
        compatibility projection 再初期化を毎 step 適用する．
        旧 \texttt{sharp\_phase\_volume} / Ridge--Eikonal 補修は
        V12（\autoref{sec:closed_interface_volume_gate}）で負対照として扱い，
        現行 production 設定としては読まない
...
合成写像が閉界面体積を保存していない．
旧 \texttt{sharp\_phase\_volume} / Ridge--Eikonal 補修は，
V12 で示すように現行 \texttt{active\_geometry\_capillary} state space では
fail-close されるため，この不合格を相殺する production 設定ではない．
...
旧設定記述を削除すべき陳腐化項目，現行 1 周期 runtime を
やり直し前の未受理 evidence として固定する．
