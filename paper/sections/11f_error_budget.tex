...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{GFM 導入後の精度回復：}
        GFM は CSF の $\Ord{h^2}$ モデル誤差を原理的に除去し，
        各相の PPE を独立に解くことで
...
\textbf{すべての密度比で GFM が必要}であるとの結論を得た．
§~\ref{sec:gfm_ppe_recovery} では GFM 分相解法により
$\rho_l/\rho_g = 1000$ でも $\Ord{h^2}$ 格子収束が回復されることを実証した
（表~\ref{tab:gfm_convergence}）．
第~\ref{sec:validation}章では GFM を統合した完全な NS ソルバで
