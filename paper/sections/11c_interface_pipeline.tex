...
続いて Hermite 場延長（HFE）の格子収束性を確認する
（§~\ref{sec:verify_field_extension}）．
なお，風上差分による場延長経路は，密度ジャンプを横切る勾配計算で NaN を生じるため
本実装では廃止済みであり，以降のベンチマーク（§~\ref{sec:verification}）でも使用しない．
最後に，曲率計算と場延長を統合した Young--Laplace 圧力跳びテストで
パイプラインの結合精度を確認する（§~\ref{sec:verify_gfm}；
PPE を用いた圧力場検証は §~\ref{sec:bf_static_droplet} で行う）．
なお，§~\ref{sec:verify_cls_advection} と §~\ref{sec:cls_conservation} の間に，
DCCD と WENO5 の移流精度比較（§~\ref{sec:verify_weno5_vs_dccd}），
DGR 厚み制御（§~\ref{sec:verify_dgr}），DCCD の離散質量保存性
（§~\ref{sec:verify_dccd_conservation}），および再初期化ゼロレベルシフト
（§~\ref{sec:verify_reinit_shift}）を補足的に検証する．
...
\begin{itemize}[noitemsep]
  \item (a) Zalesak スロット円板~\cite{Zalesak1979}：中心 $(0.5, 0.75)$，$R = 0.15$，
  ...
  \item 両テストとも DGR 厚み補正（§~\ref{sec:dgr_thickness}）を組み込んだ hybrid モードで再初期化を実行する．
        本パラメタでは $\varepsilon_{\mathrm{eff}}/\varepsilon \approx 1.02$ であり，
        スロット構造への影響は無視できる
\end{itemize}
...
\begin{table}[htbp]
\centering
\caption{CLS 界面移流テストの精度（DCCD $\varepsilon_d = 0.05$，$\varepsilon/h = 1.0$，
界面重み付き質量補正あり）．再初期化設定の詳細は本文を参照．}
...
\midrule
\multirow{3}{*}{Zalesak}
 &  64 & $8.62 \times 10^{-2}$ & $5.03 \times 10^{-4}$ & $\Ord{10^{-15}}$ \\
...
\multirow{3}{*}{Single vortex\textsuperscript{\dag}}
 ...
 & 128 & $1.05 \times 10^{-1}$ & $8.52 \times 10^{-3}$ & $\Ord{10^{-15}}$ \\
...
非一様格子上での CLS 移流検証（Zalesak および単一渦テスト）は
§~\ref{sec:nonuniform_advection} にて，界面適合格子との結合検証として報告する．
...
CCD 一階微分の零和性（\S~\ref{sec:dccd_mass_conservation}）を支持した（壁面 BC でも $\Ord{10^{-13}}$）．
