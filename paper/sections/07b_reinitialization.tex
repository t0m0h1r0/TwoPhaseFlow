...
\end{equation}
ここで $N_{\mathrm{dim}}$ は空間次元数（2D では $N_{\mathrm{dim}}=2$）．
第1項は拡散スケール $\varepsilon$ を分解するための放物型精度条件（CN 陰解法は無条件安定であるが，$\Delta\tau \gg h^2/\varepsilon$ のとき仮想時間の時間分解が粗くなるため；安全率 0.5 を含む），
第2項は圧縮項の双曲 CFL（特性速度 $\max|1-2\psi| = 1$ から $\Delta\tau \leq h$，安全率 0.5 を含む）．
$\varepsilon \sim h$ のとき放物条件 $\Delta\tau \sim h/(4N_{\mathrm{dim}})$ が双曲条件 $0.5h$ より厳しくなり支配する．

\textbf{ステップ数と停止基準：}
本実装では固定ステップ数 $n_{\mathrm{reinit}}$（デフォルト 4）を実行する．
$n_{\mathrm{reinit}} = 4$，$\Delta\tau \approx 0.5h$ のとき仮想積分時間 $\tau_{\mathrm{total}} \approx 2h$ となり，
界面プロファイルの形状修復に必要な仮想拡散距離 $\Ord{\varepsilon} \sim \Ord{h}$ を確保する．
ただし，分裂誤差により界面厚みは平衡値 $\varepsilon$ に収束せず，
呼び出し毎に約 40\% 膨張する（§~\ref{sec:dgr_thickness}）．
\end{tcolorbox}
