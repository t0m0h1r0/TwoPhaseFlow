...
（§~\ref{sec:verify_capillary_wave}，CHK-135 参照）．
...
数値実験（§~\ref{sec:verify_capillary_wave}，Prosperetti 1981 ベンチマーク）により：
\begin{itemize}
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    \hline
    分裂法のみ  & ✓ & $\sim 1.4\varepsilon$（膨張） & ✓ 正確 \\
    DGR のみ    & ✗（フォールド不可） & ✓ & ✗ Blowup \\
    ハイブリッド & ✓ & ✓ & ✗ $D(t)$ 誤差 \\
    \textbf{Eikonal}（本法）& ✓（連続形） & ✓ & ✗ $D(t)$ 誤差（CHK-136）\\
    \hline
  \end{tabular}
...
手法 & $D(T{=}2)$ & $D(T{=}10)$ & 判定 \\
\hline
Eikonal（CHK-136，ZSP なし） & 0.245 & — & ✗ \\
Eikonal + ZSP（Strategy A）   & 0.129 & — & ✗ \\
\textbf{ξ-SDF（Strategy B，本節）} & \textbf{0.050} & \textbf{0.226} & ✗（$T{=}10$ 未達）\\
分裂法のみ（参照，CHK-135）   & 0.037 & 0.0036 & ✓ \\
\hline
\end{tabular}
\end{center}
...
これを検証するため，\citep{Sethian1996} の Fast Marching Method (FMM) を実装した
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccccc}
    ...
    \hline
    分裂法のみ & ✓ & ${\sim}1.4\varepsilon$ & ✓ & 0.037 & ${<}1\%$ @$T{=}10$ & あり \\
    DGR のみ & ✗ & ✓ & ✗ Blowup & — & — & なし \\
    ハイブリッド & ✓ & ✓ & ✗ & 0.129 & 低 & あり \\
    Eikonal（CHK-136）& ✓ & ✓ & ✗ & 0.245 & 低 & あり \\
    Eikonal+ZSP（CHK-137A）& ✓ & ✓ & ✗ & 0.129 & 低 & あり \\
    ξ-SDF（CHK-137B）& ✓ & ✓ & ✗ & 0.050 & $3.5\%$ @$T{=}10$ & なし \\
    \textbf{FMM（CHK-138）}& ✓ & ✓ & ✗ & — & $\mathbf{8.2\%}$ @$T{=}1$ & なし \\
...
\begin{tabular}{lccc}
  ...
  \textbf{ξ-SDF（$f{=}1.4$，CHK-139）} & \textbf{0.018 ✓} & \textbf{0.028 ✓} & \textbf{1.38\% @$T{=}2$} \\
