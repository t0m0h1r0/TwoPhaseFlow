...
$|\sum_i \tilde{f}'_i| < 5 \times 10^{-13}$（$N{=}256$ での最大値；浮動小数点丸めの蓄積に整合）を確認した．
...
（§~\ref{sec:val_capillary}，CHK-135 参照）．
...
CSF 力 $\sigma\kappa\delta_\varepsilon\bnabla\psi$ が指数的に増大して発散する（CHK-133）．
...
% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{統一再初期化：Eikonal 法（WIKI-T-042）}
...
\begin{tcolorbox}[algbox, title={Eikonal 再初期化アルゴリズム（CHK-136）}]
\begin{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{質量保存（前クリッピング）：}Step 4 の $\phi$ 空間補正により
        $\sum\psi_{\mathrm{corr}} = \sum\psi_{\mathrm{old}}$（Thm 2）．
\end{enumerate}
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    \textbf{Eikonal}（本法）& $\checkmark$（連続形） & $\checkmark$ & $\times$ $D(t)$ 誤差（CHK-136）\\
    \hline
  \end{tabular}
  \caption{再初期化手法の比較．WIKI-T-042 参照．}
\end{table}
...
離散ゼロセットドリフトを原理的に排除できる（CHK-137 で ZSP として実装・検証済み；次節参照）．


% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{\texorpdfstring{$\xi$}{ξ}空間符号距離関数法（CHK-137）}
\label{sec:xi_sdf_reinit}
% ─────────────────────────────────────────────────────────────────────────────

CHK-136 の失敗原因（Godunov 差分の離散ゼロセットドリフト累積）を根本的に排除するため，
反復を一切伴わない \textbf{ξ空間符号距離関数（ξ-SDF）法}を提案する．
本手法は再初期化呼び出しごとに，現在の $\psi$ 場から零点交差を直接検出し，
格子インデックス空間（$\xi$ 空間）における符号距離を解析的に構築する．

\begin{tcolorbox}[algbox, title={ξ-SDF 再初期化アルゴリズム（CHK-137 Strategy B）}]
\begin{enumerate}
  \item \textbf{ロジット反転：}
...
Godunov 差分の $O(\Delta\tau\cdot n_{\mathrm{iter}})$ 累積ドリフト（CHK-136 根本原因）を生じない．
\end{description}
...
CHK-137 の失敗原因として当初 Voronoi kink 仮説（ξ-SDF の $C^0$ 不連続が
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccccc}
    ...
    Eikonal（CHK-136）& $\checkmark$ & $\checkmark$ & $\times$ & 0.245 & 低 & あり \\
    Eikonal+ZSP（CHK-137A）& $\checkmark$ & $\checkmark$ & $\times$ & 0.129 & 低 & あり \\
    ξ-SDF（CHK-137B）& $\checkmark$ & $\checkmark$ & $\times$ & 0.050 & $3.5\%$ @$T{=}10$ & なし \\
    \textbf{FMM（CHK-138）}& $\checkmark$ & $\checkmark$ & $\times$ & — & $\mathbf{8.2\%}$ @$T{=}1$ & なし \\
    \textbf{ξ-SDF（$f{=}1.4$，CHK-139）}& $\checkmark$ & $\checkmark$ & $\checkmark$ & \textbf{0.028} & $1.38\%$ @$T{=}2$ & なし \\
    \hline
  \end{tabular}
  \caption{再初期化手法の比較（CHK-139 更新）．WIKI-T-042 参照．ε幅拡大（$f{=}1.4$）が最良の $D(T{=}2)$を実現．}
  \label{tab:reinit_comparison_chk138}
\end{table}

\noindent\textbf{CHK-139（ε幅拡大による修正）：}
CHK-138 の界面幅仮説に基づき，ξ-SDF 再構成のパラメータに $f{=}1.4$ を設定して
...
\begin{tabular}{lccc}
  ...
  \textbf{ξ-SDF（$f{=}1.4$，CHK-139）} & \textbf{0.018 $\checkmark$} & \textbf{0.028 $\checkmark$} & \textbf{1.38\% @$T{=}2$} \\
