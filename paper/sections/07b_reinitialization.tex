...
零点集合の離散ドリフトは排除されたが，
Voronoi 境界付近での界面再配置が呼び出しごとに微小な質量累積誤差を生じ，
長時間積分（$T{=}10$）で顕在化する．
$\sigma>0$ 毛管波ベンチマークには\textbf{分裂法単独}を推奨する方針は不変であり，
ξ-SDF の適用は $\sigma=0$ 問題あるいは短時間積分に限定する．
...
\begin{table}[H]
  ...
  \begin{tabular}{lccccc}
    \hline
    方法 & 形状保存 & 界面幅 $\varepsilon$ & $\sigma>0$ 毛管波 & $D(T{=}2)$ & 反復 \\
    \hline
    分裂法のみ      & ✓ & ${\sim}1.4\varepsilon$（膨張） & ✓ & 0.037 & あり \\
    DGR のみ        & ✗ & ✓ & ✗ Blowup & — & なし \\
    ハイブリッド    & ✓ & ✓ & ✗ & 0.129 & あり \\
    Eikonal（CHK-136）& ✓ & ✓ & ✗ & 0.245 & あり \\
    Eikonal+ZSP（CHK-137A）& ✓ & ✓ & ✗ & 0.129 & あり \\
    \textbf{ξ-SDF（CHK-137B）}& ✓ & ✓（ξ均一）& ✗（$T{=}10$ 未達）& 0.050 & \textbf{なし} \\
    \hline
  \end{tabular}
  \caption{再初期化手法の比較（CHK-137 更新）．WIKI-T-042 参照．}
  \label{tab:reinit_comparison_chk137}
\end{table}
