...
離散実装を示す．ch13 production stack（\texttt{experiment/ch13/config/*.yaml} で
\texttt{interface.reinitialization.algorithm: ridge\_eikonal}）は
\texttt{RidgeEikonalReinitializer} を採用する．
本節の構成は以下である：
\begin{itemize}
  \item \textbf{主軸 Ridge-Eikonal 法}：§~\ref{sec:xi_sdf_reinit}（後述 \texttt{ξ}空間符号距離関数法 = Ridge ZSP）
        が ch13 production の主実装．Eikonal PDE による距離関数構築 + Ridge ガウス射影で
        $\psi$ を再構成する．連続定式化の理論的根拠は §~\ref{sec:ridge_eikonal} 参照．
  \item \textbf{過渡 Eikonal 法}：§~\ref{sec:eikonal_reinit}（後述「統一再初期化：Eikonal 法」）は
        Ridge ZSP の前段である Eikonal 距離構築の単独運用形．
  \item \textbf{旧実装 SplitReinitializer（DCCD 圧縮 + CN 拡散）}：下記
        \paragraph{旧実装} に legacy paragraph として降格．ch13 production では使用しない．
\end{itemize}

\paragraph{旧実装（SplitReinitializer）：圧縮項の DCCD 離散化}
\phantomsection\label{sec:cls_compression_legacy}% ch13 production 非採用; 比較参照のため保持
...
\end{equation}
を演算子分裂し，圧縮項に DCCD（Dissipative CCD），拡散項に Crank--Nicolson 半陰解法を適用する
旧実装（\texttt{SplitReinitializer}）は ch13 production 確立前に使用されていた手法である．
本実装では Ridge-Eikonal 法への移行に伴い legacy 位置づけとなったが，
比較参照および §~\ref{sec:dccd_mass_conservation} の質量保存性議論で必要となるため，
以下に簡略形で記す．

\medskip
\noindent\textbf{陽解法・陰解法の使い分け：}
圧縮フラックス $f(\psi) = \psi(1-\psi)\hat{\bm n}$（特性速度 $\max|f'|=1$）は
§~\ref{sec:advection_motivation} の DCCD 移流と同構成で陽的処理（CFL $\Delta\tau \leq 0.5h$），
拡散項 $\varepsilon\bnabla^2\psi$（陽的 CFL $\Delta\tau \leq h^2/(2\varepsilon)$）は
Crank--Nicolson による半陰的処理で陽的 CFL を除く．

\medskip
\noindent\textbf{圧縮項の DCCD 離散化}（$x$ 成分；$y$ 成分も同様）：
$g_i = \psi_i(1-\psi_i)\hat{n}_{x,i}$，
\begin{align}
  g'_i &= \left[\partial_x g\right]_i \quad\text{（CCD 求解）} \label{eq:comp_ccd} \\
  \tilde{G}_i &= g'_i + \varepsilon_d^{\mathrm{comp}}\!\left(g'_{i+1} - 2g'_i + g'_{i-1}\right)
  \quad(\varepsilon_d^{\mathrm{comp}} = 0.05) \label{eq:comp_filter}
\end{align}
法線 $\hat{\bm n} = \bnabla\psi/|\bnabla\psi|$ は CCD + DCCD 1 次微分場（§~\ref{sec:dissipative_ccd}）
から評価する（$|\bnabla\psi| + \delta$，$\delta = 10^{-8}$）．

\medskip
\noindent\textbf{拡散項の CCD-CN 法}（$\theta=1/2$ 半陰解法）：
$M_2$（CCD 2 階左辺係数；主対角 $1$，両側 $\beta_2=-1/8$），
$B_2$（CCD 2 階右辺係数；主対角 $a_2=3$，両側 $b_2/2=-9/16$）として
\begin{equation}
...
\end{equation}
係数行列は三重対角で Thomas アルゴリズムで $\Ord{N}$ に求解可能．
$\psi^{**}$ は圧縮項陽的処理後の中間値．

\label{alg:cls_reinit_dccd}%
旧実装 SplitReinitializer の完全アルゴリズム（圧縮 DCCD + CN 拡散 + DGR 厚み補正）は
§~\ref{sec:algo_flow}（Step 2）にまとめる．
\textbf{ch13 production の方針：} 旧実装の SplitReinitializer に代わり，
ch13 production では §~\ref{sec:xi_sdf_reinit} の Ridge ZSP（Ridge-Eikonal）を採用する．
Ridge-Eikonal は離散ゼロセットドリフトを原理的に排除し，
DCCD 圧縮項の演算子分裂に伴う形状歪み（§~\ref{sec:dgr_thickness}）を回避できる．
...
本節は §~\ref{sec:reinit}（§3.3 再初期化方程式）の Eikonal 単独運用形の離散実装である．
ch13 production の主軸である Ridge-Eikonal 法（§~\ref{sec:xi_sdf_reinit}；
連続定式化は §~\ref{sec:ridge_eikonal}）は本節の Eikonal 距離構築を前段とする．
...
% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{\texorpdfstring{$\xi$}{ξ}空間符号距離関数法（Ridge-Eikonal；ch13 主実装）}
\label{sec:xi_sdf_reinit}
% ─────────────────────────────────────────────────────────────────────────────

\noindent\textbf{Part 1 連続定式化との関係：}
本節は §~\ref{sec:ridge_eikonal}（§3.4 Ridge-Eikonal 連続定式化）の離散実装であり，
ch13 production stack の \texttt{RidgeEikonalReinitializer}
（\texttt{experiment/ch13/config/*.yaml} の \texttt{algorithm: ridge\_eikonal}）に対応する．
