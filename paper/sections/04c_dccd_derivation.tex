...
%        既存 04d_dissipative_ccd.tex を本節に吸収（DCCD 役割分担のみ残存）．
%
% ═══════════════════════════════════════════════════════════════════════════════
...
WIKI-T-002 の波数選択的設計はこの極限を精緻化したものである．
...
\item \textbf{境界行で $m_{\text{eq}}\ge 0$ 違反の可能性．}
      §\ref{sec:ccd_bc} の Option III/IV で壁近傍の行を置換（WIKI-T-051/056）．
...
\begin{itemize}
  ...
  \item \textbf{UCCD6 との対照}．
        ...
        両者は直交的に選択可能で，Level-2 移流は DCCD，
        Level-3 モーメント移流は UCCD6 を既定とする（§\ref{sec:time_int}）．
\end{itemize}

\noindent
詳細な収束試験値・保存性解析・境界行設計は
WIKI-T-002（DCCD スペクトル設計）・WIKI-T-013（DCCD vs WENO5 比較）を参照．
旧 §~04d 節（\texttt{04d\_dissipative\_ccd.tex}）は本節に吸収されたため
既定コンパイル対象から除外したが，履歴参照用にリポジトリに残存する．
