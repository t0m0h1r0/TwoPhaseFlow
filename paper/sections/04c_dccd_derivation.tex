...
%
% §4.4  DCCD の導出形：中心 CCD 散逸ゼロ → -D_2^CCD 散逸チャンネル → spectral filter
%        DCCD は節点中心 CCD の外殻 post-filter であり，標準 CLS 移流は §6 の FCCD 面フラックスへ委ねる．
...
\medskip\noindent\textbf{導出形と post-filter 形の 2 段構成．}
...
\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=2pt]
  ...
  \item \textbf{post-filter 形}（§\ref{sec:dccd_spectral_filter}--§\ref{sec:dccd_adaptive}）：
...
% -------------------------------------------------------------------------------
\subsubsection{post-filter 形：スペクトルフィルタとの関係}
\label{sec:dccd_spectral_filter}
\phantomsection\label{sec:dccd_filter_theory}%
% -------------------------------------------------------------------------------

DCCD の post-filter 形は定数 $\varepsilon$ を
スペクトル伝達関数：
\begin{equation}
...
\noindent\textbf{Post-filter 形での適用条件．}
...
この $\varepsilon_d=1/4$ は節点中心 post-filter 単独での設計上限であり，
標準 CLS 移流に採用する値ではない．
...
\item \textbf{定数 $\varepsilon$ の精度低下は spectral post-filter で分離する．}
...
以上の導出は，DCCD が\textbf{節点中心 CCD に対する外殻 post-filter} として
作用する設計であることを確立する．
DCCD のスペクトル設計・スイッチ $S(\psi)$・精度影響・境界条件・保存性は，
以下の要約として本節に集約する：

\begin{itemize}
  \item \textbf{DCCD は節点中心 CCD の安定化 post-filter である．}
  ...
  \item \textbf{体積保存は外殻修正}で担保．
        ...
        clamp 後の総量残差を §\ref{sec:cls_mass_correction} の Stage B/F で閉じる．
  \item \textbf{UCCD6 との対照}．
        ...
        CLS $\psi$ は FCCD 面フラックス，運動量 bulk は UCCD6，圧力・毛管ジャンプを含む
...
具体的な用途章割当（標準 CLS 移流・運動量 bulk 移流・圧力面閉包・曲率帯制限など）の
機構詳細は当該章（§~\ref{sec:scheme_per_variable} 以降）に委ねる．

表~\ref{tab:dccd_usage_map} に，DCCD を節点中心 post-filter として用いる場合の
...
\PaperCaptionNote{DCCD は節点中心 CCD の post-filter 設計であり，標準 CLS 移流・圧力面閉包の主演算子ではない．用途章割当の機構詳細は §\ref{sec:scheme_per_variable} 以降で述べる．}
\small
\begin{tabular}{llcl}
\hline
...
\begin{enumerate}
  ...
  \item \textbf{後続計算への引き渡し：}
        ...
        同一ロカス閉包として扱う（§~\ref{sec:collocate}, §~\ref{sec:dccd_pressure_nofilt}）．
\end{enumerate}
