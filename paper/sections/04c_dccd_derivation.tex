...
%        本稿での DCCD は CLS 移流の production 既定（§7）；
...
\textbf{post-filter} として DCCD を導出する．
すなわち本節の主役は基底 CCD の解 $(f, f', f'')$ ではなく，
それに作用する散逸演算子そのものである．
...
DCCD は本稿の\textbf{CLS 移流の production 既定}スキームである
（§~\ref{sec:advection_motivation}：CCD 微分 + 後段スペクトル散逸）．
$\psi$ は $C^0$ 急 tanh プロファイルを持つため UCCD6 では振動が出現し，
DCCD のスペクトル散逸が必須となる．
bulk $u, v$ 移流は Level-1/2/3 すべてで UCCD6（§~\ref{sec:uccd6_def}）を既定とし，
DCCD は CLS 再初期化圧縮項（§~\ref{sec:reinit}）の
非線形フラックス散逸子としても適用される．
WENO5 は付録~\ref{app:weno5} の比較対象スキームに留める．
...
$H(\xi) = 1 - 4\varepsilon_d\sin^2(\xi/2)$ で実装される（§\ref{sec:advection_motivation}）．
...
\begin{itemize}
  ...
  \item \textbf{UCCD6 との対照}．
        ...
        CLS 移流（$\psi$）の production 既定を DCCD とする：
        Level-1 検証では UCCD6/DCCD 両形式を明示的に並列し，
        Level-2 製品（ch13 rising bubble）は UCCD6+AB2 を bulk に，DCCD を $\psi$ 移流に，
