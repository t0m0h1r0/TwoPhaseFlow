...
%
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{非一様格子上の FCCD 面演算子：一般面 \texorpdfstring{$\Ord{H^3}$}{O(H3)} と中央面 \texorpdfstring{$\Ord{H^4}$}{O(H4)}}
...
節点を $x_0 < x_1 < \cdots < x_{N-1}$ とし，
隣接節点間の面を $f_{i-1/2}$（$i=1,\ldots,N-1$）と記す．
局所幾何量を：
\begin{align}
...
% -------------------------------------------------------------------------------
\subsubsection{BF 条件への影響}
\label{sec:fccd_nu_bf_impact}
% -------------------------------------------------------------------------------

\S\ref{sec:fccd_face_residual_order} で示した Balanced--Force（BF）残差 $|\BF_\text{res}| = \Ord{\Delta x^4}$ の性質は，
