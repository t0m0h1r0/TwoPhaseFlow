...
pressure-jump/HFE 型のジャンプ補正閉包では，§~\ref{sec:split_ppe_pressure_jump_formulation} の
$B_f^{\mathrm{nu}}=s_fj_f/H_f$ と同一界面面データを PPE RHS と corrector で共有して初めて
BF 条件が閉じる．
非一様メトリクス $G_f^{\mathrm{nu}}$ だけでは，
既知ジャンプ $B_f^{\mathrm{nu}}$ は自動的には生成されない．

\noindent 詳細な離散化補足・PCR/Thomas 法の非一様バージョン・
