...
壁面近傍，ghost face，HFE 最近接点，GFM 交差点，affine jump 面で
$|\theta_i-1/2|>\theta_{\mathrm{tol}}$ となる場合は，
その面を $\Ord{H^3}$（または境界閉包に応じて $\Ord{H^2}$）の特殊面としてログに記録し，
$\Ord{H^4}$ の検証集合から分離する．
...
\S\ref{sec:h01_diagnosis_fccd_remedy} で示した BF 残差 $|\BF_\text{res}| = \Ord{\Delta x^4}$ の性質は，
...
$p$ 勾配と CSF の同一 face-locus は保たれる．
ただしこれは滑らかな場を同じ FCCD face operator に載せる場合の主張であり，
pressure-jump/HFE 型 affine 閉包では，§~\ref{sec:split_ppe_pressure_jump_formulation} の
$B_f^{\mathrm{nu}}=s_fj_f/H_f$ と同一 face cache を PPE RHS と corrector で共有して初めて
BF 条件が閉じる．
