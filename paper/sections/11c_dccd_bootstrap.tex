...
\noindent
本節は時間発展を開始する前の幾何・圧力・格子メトリクスの整合化と，
その後の $\Delta t$ 制御だけを扱う．
DCCD post-filter は §\ref{sec:dccd_derivation} の節点中心 CCD 外殻設計であり，
§11 の標準更新手順では CLS 移流，圧力面閉包，または二相 pressure-jump stack の
主演算子として再導入しない．
本章の演算子割当は §\ref{sec:scheme_per_variable} に従い，
二相圧力閉包は分相 PPE + HFE + DC + face-cochain 補正で閉じる．
