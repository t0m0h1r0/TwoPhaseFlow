...
% ═════════════════════════════════════════════════════════════════════════════
% §11 統合更新手順周辺（DCCD post-filter 設計は §4c に集約済；
%        Bootstrap は付録 H に集約済；本ファイルは初期整合化とタイムステップ制御のみ保持）
% ═════════════════════════════════════════════════════════════════════════════

\noindent
DCCD post-filter の散逸強度 $\varepsilon_{d,\max}$ の設計値・
適応散逸制御則・離散手順
...
\eqref{eq:adaptive_eps}，手順~\ref{alg:dccd}）は
DCCD 導出章（§\ref{sec:dccd_derivation} 末尾，第~\ref{sec:CCD}章）で定義した．
§11 の標準更新手順は，これを CLS 移流や圧力面閉包の主演算子として再導入せず，
§\ref{sec:scheme_per_variable} の演算子割当に従う．
...
これらが揃わない追従格子は本節の 1 タイムステップ標準経路へ含めない．
