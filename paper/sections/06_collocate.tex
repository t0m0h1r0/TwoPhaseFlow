...
$\bnabla^2\phi$ が $\bnabla\cdot(1/\rho\,\bnabla p)$ に変わる（第7章§\ref{sec:projection_derivation}）．
\end{tcolorbox}
...
\textbf{実装上の確認：} 静止液滴ベンチマーク（10章）において寄生流れ振幅が $O(h^4)$ 以上で
...
本節では，第2章（§\ref{box:balanced_force_ref}）で予告した
\textbf{Balanced-Force 離散化条件}の詳細を述べる．
これは寄生流れ（spurious currents，第1章§\ref{sec:failure_modes}）を
抑制するための最も重要な実装要件である．
...
本稿の実装では CCD 法による1次微分演算子 $D_x^{(1)}, D_y^{(1)}$（第4章）を用いる：
\begin{align}
  \text{圧力勾配：} & \quad
...
\begin{itemize}
  ...
  \item 圧力 Poisson 方程式の係数行列と Rhie-Chow 係数で
        異なる密度補間（算術平均 vs 調和平均）を混在させる（§\ref{sec:collocate} §6.2.2 参照）．
...
\]
となり，寄生流れは $O(h^6)$ まで抑制される（第10章の静止液滴ベンチマークで確認）．
