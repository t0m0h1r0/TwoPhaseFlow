...
\begin{equation}
 u_f = \bar{u}_f - \Delta t\left( \frac{1}{\rho^n} \right)^{\mathrm{harm}}_f \left[ (\bnabla p^n)_f - \overline{(\bnabla p^n)}_f \right]
 \label{eq:rc-face}
\end{equation}
...
係数 $(1/\rho^n)_f^\mathrm{harm} = 2/(\rho^n_P + \rho^n_E)$ には\textbf{前時刻密度 $\rho^n$ の調和平均}を用いる
（$\rho^{n+1}$ は PPE の解が未確定のため使用できない；第\ref{sec:pressure}章§\ref{sec:rc_divergence} 参照）．
その導出根拠を以下で丁寧に説明する．
