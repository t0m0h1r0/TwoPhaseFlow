...
$u_\infty^{\mathrm{end}}\le3.3\times10^{-10}$，
$|\Delta V_\psi|/V_{\psi,0}\le2.4\times10^{-16}$ を確認した．
...
寄生流れの抑制（V5 で CCD $u_\infty^{\mathrm{end}}=1.93\!\times\!10^{-2}$ @ $\rho_r=1$, $N=32$（FD 比 $1/5.44$ 補助）；§\ref{sec:spurious_multistep}）と
	高密度比下の pressure-jump PPE スタック安定性（V6 で $\rho_r \le 833$ までブローアップなし，$|\Delta V_\psi|/V_{\psi,0} \le 2.4\!\times\!10^{-16}$；§\ref{sec:density_ratio_sweep}）を同時に達成している．
...
\begin{description}
  ...
  \item[V6（密度比 sweep；§\ref{sec:density_ratio_sweep}）：]
        ...
        $u_\infty^{\mathrm{end}}\le3.3\times10^{-10}$，
        $|\Delta V_\psi|/V_{\psi,0}\le2.4\times10^{-16}$．
  ...
  \item[V9（local-$\varepsilon$ switch；§\ref{sec:local_eps_validation}）：]
        ...
        $|\Delta V|/V_0\le3.81\times10^{-16}$．空間可変 $\varepsilon(\bx)$ は
