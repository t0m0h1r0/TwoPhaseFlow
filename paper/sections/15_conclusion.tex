...
\begin{description}
  ...
  \item[\textbf{第\ref{sec:grid}章：界面適合非一様格子と CCD/FCCD 非一様拡張}]
        ...
        格子密度幅 $\varepsilon_g$ を計算空間 $\xi$ のセル数で定義する方式
        （$\varepsilon_g = N_{g,\text{cells}} \cdot L/N$，式~\eqref{eq:eps_g_xi}）を導入し，
        $\xi$ 空間でのメトリクス解像度を $N$・$\alpha$ によらず保証した
        （§\ref{sec:cls_on_nonuniform} で移流精度が一様格子と同等まで回復することを確認）．

  \item[\textbf{第\ref{sec:collocate}章：圧力--速度連成と Balanced-Force/FCCD}]
        コロケート格子でのチェッカーボード不安定のメカニズム（$2\Delta x$ モード解析）を示し，
        Balanced-Force 演算子整合と FCCD 面ジェット sub-system による
        チェッカーボード抑制の設計原理を示した（§\ref{sec:checkerboard_solution}，§\ref{sec:balanced_force}，
        §\ref{sec:fccd_bf_sub_system}）．

  \item[\textbf{第\ref{sec:ppe_solve}章：圧力 Poisson 方程式の解法と分相 PPE・HFE・欠陥補正法}]
  ...
  \item[\textbf{第\ref{sec:algorithm}章：Navier--Stokes ソルバの統合アルゴリズム}]
        NS 方程式の各項と離散ソルバーの対応関係（演算子マッピング），
        7 ステップの全体フロー，
        および CFL・毛管波制約を統合したタイムステップ制御を本稿全体で整理した．
        AB2 + IPC による対流項処理・Crank--Nicolson 粘性項の ADI 求解・
        IPC 増分圧力 Poisson 求解の完全な離散形式は，本稿 Part II の各章で個別に提示している．
\end{description}
