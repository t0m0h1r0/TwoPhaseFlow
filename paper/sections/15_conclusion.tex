...
HFE Hermite 場延長 1D $\Ord{h^{5.91}}$ / 2D max $\Ord{h^{11.35}}$ med $\Ord{h^{8.72}}$（U6-c；§\ref{sec:hfe}），
...
（第\ref{sec:verification}章）・物理ベンチマーク（第\ref{sec:validation}章）
の三段階で実証されている．以下に headline 成果を要約する．
...
\begin{description}
  ...
  \item[U3（非一様格子上の空間離散；§\ref{sec:U3_nonuniform_suite}）：]
        ...
        GCL 機械零（$7.5\times 10^{-14}$），Ridge--Eikonal D1--D4 機械零を確認．
  ...
  \item[U6（分相 PPE + HFE + DC；§\ref{sec:U6_split_ppe_dc_hfe}）：]
        ...
        1D $\Ord{h^{5.91}}$，2D max $\Ord{h^{11.35}}$ med $\Ord{h^{8.72}}$ を達成．
...
% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{物理ベンチマーク §\ref{sec:validation} の検証成果}
% ─────────────────────────────────────────────────────────────────────────────

第\ref{sec:validation}章の物理ベンチマーク 3 種（毛管波・気泡上昇・Rayleigh--Taylor）は，
古典理論・既往 DNS との定量比較で受入基準を満たした
（集計：表~\ref{tab:ch14_summary}）．

\begin{description}
  \item[毛管波（§\ref{sec:val_capillary}）：]
        線形分散関係 $\omega^2 = \sigma k^3/(\rho_l + \rho_g)$ を Prosperetti~\cite{Prosperetti1981}
        参照と比較し，位相・周期が許容誤差内で一致．
        体積保存 $|\Delta V|/V_0 = \Ord{10^{-4}}$（Popinet~\cite{Popinet2009} 慣行値以下），
        Denner--van~Wachem~\cite{DennerVanWachem2015} の毛管 CFL 安定境界を遵守．
  \item[気泡上昇（§\ref{sec:val_rising_bubble}）：]
        Hysing ら~\cite{Hysing2009} の単一気泡幾何アーキタイプを再現．
        体積保存 $|\Delta V|/V_0 = \Ord{10^{-4}}$，重心 $y_c(t)$ 単調上昇，
        平均上昇速度の漸近，Francois ら~\cite{Francois2006} 類概念に基づく浮力分解の閉包確認，
        Aland--Voigt~\cite{AlandVoigt2019} diffuse-interface 対応 benchmark との整合．
  \item[Rayleigh--Taylor 不安定（§\ref{sec:val_rayleigh_taylor}）：]
        $\mathrm{At}=0.0256$, $k=4\pi$ で線形成長率
        $\gamma \approx 1.78\,\mathrm{s^{-1}}$（Chandrasekhar~\cite{Chandrasekhar1961}
        古典理論）を $0 < t < 3/\gamma$ 線形窓で再現．
        $T=7$ で非線形 mushroom 領域に到達（Tryggvason~\cite{Tryggvason2011} DNS 規約），
        Tryggvason~\cite{Tryggvason1988} 低 Atwood DNS との定量比較で整合．
\end{description}

これら 3 ベンチマークの成功により，
三本柱設計（CCD 高次群／分相 PPE+HFE+DC／BF+FCCD 面ジェット）の物理的有効性が
本稿で完結したと宣言できる．
...
第\ref{sec:numerical_integrity}章の単体検証 U1--U9・第\ref{sec:verification}章の統合検証 V1--V10・
第\ref{sec:validation}章の物理ベンチマーク（毛管波～\cite{Prosperetti1981}・気泡上昇～\cite{Hysing2009}・
Rayleigh--Taylor～\cite{Chandrasekhar1961,Tryggvason1988}）はすべて受入基準を満たし，
本稿の三本柱設計（CCD 高次群／分相 PPE+HFE+DC／BF+FCCD 面ジェット sub-system）の
物理的有効性が実証された（§\ref{sec:achievements_summary}）．
今後の検証課題として，以下を挙げる．
