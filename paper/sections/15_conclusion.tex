...
（CLS 移流・PPE・運動量更新の非線形連結）律速であり，条件付き $\triangle$
（Type-D, Lie 1 次分割漸近 slope $\sim 1$ を実質上回る near-optimal coupled-stack regime）
として記録した．
さらなる高次化（Guermond--Shen 2006 等の高次 Projection）は今後の課題である．

これらの特性は意図的な設計選択であり，
三本柱（CCD 高次群／分相 PPE+DC+CCD 勾配／BF+FCCD 面ジェット sub-system）の整合により，
寄生流れの抑制（V5 で CCD $u_\infty^{\mathrm{end}}=1.76\!\times\!10^{-2}$ @ $\rho_r=1$, $N=32$（FD 比 $1/5.85$ 補助）；§\ref{sec:spurious_multistep}）と
高密度比下の pressure-jump PPE スタック安定性（V6 で $\rho_r \le 833$ までブローアップなし，$|\Delta V_\psi|/V_{\psi,0} \le 2.4\!\times\!10^{-10}$；§\ref{sec:density_ratio_sweep}）を同時に達成している．
...
V1-b/V2/V4/V5 は判定基準を実装 identity に合わせて見直し，
それぞれ standard PPE $O(\Delta t)$，Kovasznay wall residual $O(h^4)$，
fixed-wall offset residual $O(\|\bm U\|)$，
...
\begin{description}
  ...
  \item[V4（fixed-wall Galilean offset；§\ref{sec:galilean_offset}）：]
        $\|\Delta u\|_\infty^{\mathrm{end}}=2.07\times10^{-1}$，
        $\|\Delta u\|_\infty^{\max}/\|\bm U\|=2.63<3$．
        厳密ゼロ残差ではなく固定壁 offset residual として合格．
  \item[V5（寄生流れ multistep；§\ref{sec:spurious_multistep}）：]
        CCD $u_\infty^{\mathrm{end}}=1.76\!\times\!10^{-2}$ @ $\rho_r=1$, $N=32$（FD 比 $1/5.85$ 補助）．
        CCD は寄生流れ抑制に有効．
  \item[V6（密度比 sweep；§\ref{sec:density_ratio_sweep}）：]
        $|\Delta p_\mathrm{corr}|/(\sigma/r) = 1.07\times 10^{-1}$ @ $\rho_r=833$, $N=32$
        （pressure-jump 補正 diagnostic; Laplace 絶対圧誤差ではない）．
        分相 PPE + DC + HFE が高密度比でも機能することを実証．
  \item[V8（非一様格子 NS 静止液滴；§\ref{sec:nonuniform_ns_static}）：]
        $|\Delta p_\mathrm{rel}| = 2.75\%$ @ $N=96$．非一様 D1--D4 と FCCD の整合を確認．
  \item[V9（local-$\varepsilon$ §\ref{sec:local_eps_validation}; §14 stack 条件付）：]
        $C/A = 0.11$ @ $N=32$．空間可変 $\varepsilon(\bx)$ の §\ref{sec:validation} 適用条件を確定．
  \item[V10-a（Zalesak; §\ref{sec:zalesak_cls_advection}）：]
        $\alpha=1$ の一様格子で質量保存軸 mass drift $6.13\!\times\!10^{-4}\%$ @ $N=128$ \checkmark (Type-B)，
        形状軸 centroid err $4.911\!\times\!10^{-3}$ ($\triangle$ Type-D, slot/h$=6.4$ 律速)．
        詳細は表~\ref{tab:V10_zalesak}．
  \item[V10-b（単一渦反転; §\ref{sec:single_vortex_reversal}）：]
        $\alpha=1, N=128, T=8$ で質量保存軸 mass drift $0.0936\%$ \checkmark (Type-B)，
        形状復元軸 $L^1$ reversal error $2.690\times 10^{-2}$
        ($\triangle$ Type-D, filament 厚 $<h$ Eulerian 固定格子下限)．
        いずれも Olsson--Kreiss 質量修正 10 step ごと適用後．
\end{description}

条件付き $\triangle$ は V7 IMEX-BDF2 effective $\Ord{\Delta t^{1.58}}$ coupled-stack 律速
（Type-D, Lie 1 次分割漸近 slope $\sim 1$ を実質上回る），
V10-a Zalesak $\alpha=1$ 形状軸 (centroid err $4.911\!\times\!10^{-3}$，slot/h=6.4 律速；
質量保存軸は \checkmark)，
V10-b 単一渦 reversal $L^1=2.690\times10^{-2}$ に限定する．
V1-b/V2/V4/V5 は本章の revised criterion により合格へ再分類済みであり
...
定量的ベースラインを与えている．
残る V7/V10 は格子増強だけでなく，operator splitting，norm 選択，
参照解品質，reinitialization cadence，非可逆な形状復元を分離する
追加検証 gate として残す．
...
古典理論・既往 DNS との比較対象，YAML 条件，出力診断，受入基準を固定している．
表~\ref{tab:ch14_summary} の通り，毛管波については
向き付き affine jump の符号検証まで完了し，気泡上昇・Rayleigh--Taylor は
査読時点で未完了検証ゲートとして残る．

\begin{description}
  \item[毛管波（§\ref{sec:val_capillary}）：]
        旧 \texttt{jump\_decomposition} では regular 圧力場が鋭いジャンプを吸収し，
        旧 \texttt{affine\_jump} では $p_g-p_l$ の符号が逆で anti-restoring となった．
        現行の向き付き contract
        $j_{gl}=p_g-p_l=-\sigma\kappa_{lg}$ は $N=32,T=10$ run を完走させ，
        初期加速度 $A''$ の符号と大きさを
        Prosperetti~\cite{Prosperetti1981} の線形分散関係に対して確認した．
        ただし振幅成長と曲率 cap 到達が残るため，位相・周期誤差の合格判定は
        Denner--van~Wachem~\cite{DennerVanWachem2015} の毛管 CFL 安定境界を含む
        sweep 後に限定する．
