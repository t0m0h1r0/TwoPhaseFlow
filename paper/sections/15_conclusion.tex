...
二相 V7（§\ref{sec:imex_bdf2_twophase_time}）の effective $\Ord{\Delta t^{1.48}}$ は coupled-stack
（CLS 移流・PPE・運動量更新の非線形連結）律速であり，条件付き $\triangle$
（Type-D, Lie 1 次分割下限を上回るが BDF2 単体 $2.0$ に届かない coupled-stack regime）
として記録した．
...
条件付き $\triangle$ は V7 IMEX-BDF2 effective $\Ord{\Delta t^{1.48}}$ coupled-stack 律速
（Type-D, Lie 1 次分割下限を上回るが BDF2 単体 $2.0$ に届かない），
