...
$|\Delta p_\mathrm{corr}|/(\sigma/r) = 2.006$ @ $\rho_l/\rho_g = 833$, $N=32$ を実証済みである．
...
高密度比下の pressure-jump PPE スタック安定性（V6 で $\rho_r \le 833$ までブローアップなし，$|\Delta V_\psi|/V_{\psi,0} \le 6.4\!\times\!10^{-16}$；§\ref{sec:density_ratio_sweep}）を同時に達成している．
...
\begin{description}
  ...
  \item[V6（密度比 sweep；§\ref{sec:density_ratio_sweep}）：]
        $|\Delta p_\mathrm{corr}|/(\sigma/r) = 2.006$ @ $\rho_r=833$, $N=32$
  ...
  \item[V9（local-$\varepsilon$ §\ref{sec:local_eps_validation}; §14 stack 条件付）：]
        $C/A = 1.00$，$C/B=1.00$ @ $N=32$．空間可変 $\varepsilon(\bx)$ は
