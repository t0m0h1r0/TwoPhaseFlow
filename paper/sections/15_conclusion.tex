...
AO-Fast の高速化境界 & 幾何セル分率 $q_C$，互換射影，active table，GPU fail-close 契約（§\ref{sec:ao_fast_state_space}）\\
...
\begin{description}
  ...
  \item[\textbf{第\ref{sec:algorithm}章：統合アルゴリズム（7 ステップ）}]
        ...
        分相 PPE + HFE + DC 経路は固定格子・現行 projection-native face closure には統合済みだが，
...
production static gate では $N=32,T=10$ で
...
復元振動，閉界面保存量 gate，浮力上昇，不安定成長という
異なる物理応答を読み分けた．
...
\begin{description}
  ...
  \item[U11（制約 face-state 空間；§\ref{sec:U11_constrained_face_state_space}）：]
        wall trace $\le2.581\times10^{-12}$，冪等性 $4.851\times10^{-12}$，
        Green residual $\le1.294\times10^{-16}$，rank gate wall $19/19$，
        periodic-wall quotient $25/25$ を確認した．
  \item[U12（AO-Fast 幾何毛管分解 gate；§\ref{sec:U12_ao_capillary_split_gate}）：]
        full pressure image は毛管波の balanced drive を $0$ にする反例であり，
        component-volume Hodge probe は N32/N64 の非静的 balanced norm
        $2.1176/2.3055$ を検出した．現行 GPU packet は
        diagonal active-Schur residual と zero-drive により fail-close した．
\end{description}
...
\begin{description}
  ...
  \item[V3（静止液滴 long-term）：]
        ...
        production pressure-jump/Riesz 静止 gate では
  ...
  \item[V11（AO-Fast 統合前 gate；§\ref{sec:ao_fast_capillary_gate}）：]
        U12 の反例を統合検証文脈へ接続し，
        AO-Fast packet が標準物理経路へ暗黙混入しないことを確認した．
        これは成功ベンチではなく，§\ref{sec:ao_fast_state_space} の
        $R_p$/GPU/YAML 契約を満たすまで fail-close する境界である．
\end{description}
...
sharp 体積保存 gate を満たしておらず，閉界面の保存量契約を
未解決課題として切り出す fail-close ケースである．
...
\begin{description}
  ...
  \item[\textbf{AO-Fast 幾何セル分率経路の本番化：}]
        §\ref{sec:ao_fast_state_space} で定義した $q_C$ 所有，互換射影，
        active table，PCG/DC proposal，GPU fail-close 契約は，
        現時点では U12/V11 の事前 gate までを本稿の検証済み範囲とする．
        今後の本番化では，dense direct-AO を oracle として
        active-vs-dense equality，dirty/flux/target support refresh，
        device-resident $J/J^T$/Schur/PCG，明示 fallback 台帳，
        および Chapter 14 YAML の
        \texttt{geometric\_cell\_fraction} activation を段階的に通す必要がある．
...
同じ production stack が代表的な移動界面現象で物理応答を保つ範囲と，
...
\begin{enumerate}
  \item 本稿の三本柱（CCD 高次群／分相 PPE+HFE+DC／BF+FCCD 面ジェット）を
        ...
        U11 制約 face-state ／ U12 AO-Fast gate），
        §\ref{sec:verification}（V1--V11 統合検証），
