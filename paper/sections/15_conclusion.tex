...
\begin{description}
  ...
  \item[気泡上昇（§\ref{sec:val_rising_bubble}）：]
        ...
        閉包 benchmark として位置付ける．まずは Mode 1 固定非一様格子で
        $B_f^{\mathrm{nu}}=s_fj_f/H_f$ の PPE/corrector 共有を検証し，
        Mode 2 は jump face cache 再構築を別ゲートとする．
        体積保存，重心 $y_c(t)$，平均上昇速度，
        変形量の時系列を未完了の判定項目とする．
...
\begin{description}
  ...
  \item[\textbf{長時間移動界面下での分相 PPE + HFE 結合の統計解析と高密度比拡張：}]
        ...
        (iv) Kang ら~\cite{Kang2000} の Ghost Fluid Method との完全整合
