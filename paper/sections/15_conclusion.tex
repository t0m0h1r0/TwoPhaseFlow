...
アクティブ幾何毛管分解の採用境界 & 幾何セル分率 $q_C$，互換射影，圧力反力部分空間 $R_p$，近似誤差境界（§\ref{sec:ao_fast_state_space}）\\
...
\begin{description}
  ...
  \item[V11（アクティブ幾何毛管分解の統合前ゲート；§\ref{sec:ao_fast_capillary_gate}）：]
        U12 の反例を統合検証文脈へ接続し，
        アクティブ幾何毛管分解の未受理分解が標準物理経路へ暗黙混入しないことを確認した．
        これは成功ベンチではなく，§\ref{sec:ao_fast_state_space} の
        $R_p$ 分解，幾何互換，近似誤差境界を満たすまで
        標準物理経路へ入れない境界である．
\end{description}
...
FCCD/UCCD6/pressure-jump/Ridge--Eikonal/IMEX--BDF2 構成を用い，
...
\begin{description}
  ...
  \item[\textbf{アクティブ幾何毛管分解の検証拡張：}]
        ...
        アクティブ集合，$R_p$ 分解，近似誤差境界は，
        現時点では U12/V11 の事前ゲートまでを本稿の検証済み範囲とする．
        今後の採用には，幾何互換，非静的波での毛管仕事，
        および近似誤差境界を同時に満たす検証を追加する必要がある．
