...
寄生流れの抑制（V5 で CCD/FD 比 $1/5.85$ @ $\rho_r=1$, $N=32$；§\ref{sec:spurious_multistep}）と
...
\begin{description}
  ...
  \item[V5（寄生流れ multistep；§\ref{sec:spurious_multistep}）：]
        CCD/FD 比 $1/5.85$ @ $\rho_r=1$, $N=32$．CCD は寄生流れ抑制に有効．
  ...
  \item[V10-a/b（Zalesak / 単一渦反転）：]
        Zalesak は $\alpha=1$ の一様格子で mass drift $1.535\%$ @ $N=128$，
...
V10-a Zalesak $\alpha=1$ mass drift 1.535\%・V10-b 単一渦 reversal $L_1=2.690\times10^{-2}$）は，
