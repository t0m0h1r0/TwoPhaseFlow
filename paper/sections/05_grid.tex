...
有限台を持つコサイン型の代替定義も利用可能であり，
界面から離れた点で厳密にゼロとなる利点を持つ（付録~\ref{app:grid_delta_cosine}）．
...
\begin{tcolorbox}[algbox, title={1次元格子点生成アルゴリズム（台形則累積和）}]
...
\paragraph{CCD $\Ord{h^6}$ 精度の平滑性条件}
...
2次元テンソル積格子の Python 実装例（擬似コード）は付録~\ref{app:grid_2d_impl} に掲載する．
...
ただし，界面が初期精細化領域から大きく離れると経時的に解像度が失われる（目安：界面変位量 $\delta \gtrsim 0.5\,h$ で顕在化）．
...
非一様格子の Jacobian $J(x)$ は CFL 条件 $\Delta t \le \min_i(\Delta x_i/|u_i|)$ を
介して許容時間刻みを支配し，界面近傍の精細化領域ほど制約が厳しくなる．
空間 $\Ord{h^6}$ の恩恵を時間積分で失わないために，
次章（§~\ref{sec:time_int}）では精度整合性を満たす時間積分フレームワークを設計する．
