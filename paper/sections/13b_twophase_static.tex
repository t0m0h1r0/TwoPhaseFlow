...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item $u_\infty^{\max}$ には定量的な合格上限を設けない．
        $\sigma/\mathrm{Re}_c$ 寄生流れスケール（$\sim 10^{-2}$）未満であることと
        単調増加しないことを定性確認する（全 $N$ で満たされる；
        \autoref{fig:V3_droplet} 中の点線スケール参照）．
        この寄生流れは CSF $\kappa$ 推定の高周波誤差が支配的で，
        V5 で $\rho_r$ / 演算子（CCD vs FD）依存性を final-time ベースで切り分ける
        （V3 と V5 で異なる集計量を用いるため，同一設定でも値の桁が異なる；
        V3 は peak，V5 は final）．
\end{itemize}
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item \textbf{$N=64\!\to\!128$ の非単調増加について}：CCD 列は $\rho_r$ 全条件で
        $u_\infty^{\mathrm{end}}$ が増加する．$\rho_r=1$ でも増加するため
        高密度比由来の励起では説明できない．本効果は CCD 高次微分作用素が
        短波長 spurious mode を励起するためと解釈する：
