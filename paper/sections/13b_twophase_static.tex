...
\PaperCaptionNote{$\Delta p_{\mathrm{rel}}$ は $N$ につれ $1\%$ 程度に収束（$N=64\!\to\!128$ で $1.23\%\!\to\!0.97\%$，slope $\sim 0.34$ — BF 平衡が解像度限界誤差床に到達）． $u_\infty^{\max}$ (peak over 200 step) は $N=64\!\to\!96$ で 4 倍上昇 → $N=128$ で微減． これは CSF $\kappa_{lg}$ 推定が解像度向上に伴って高周波モードを励起するため， 履歴ピーク値は $N$ に対し非単調になる（V5 では final-time 指標で multistep + CCD/FD の寄与を切り分ける）．}
\end{figure}
...
\begin{itemize}\setlength{\itemsep}{0pt}
  \item $|\Delta p_{\mathrm{rel}}| \le 1.3\%$ × 3 解像度で BF 平衡再現\checkmark（当初設計基準を満たす合格；
        ...
        snapshot を示す（Laplace 圧力跳びが円形界面に沿って維持され，寄生流れは
        界面近傍の薄い帯に局在）．
...
\paragraph{pressure-jump range projection による静止液滴 face-balance gate．}
上の V3 は CSF/BF 縮約経路の長時間診断である．
一方，水--空気級の production 経路では，表面張力を
CSF 体積力ではなく §~\ref{sec:split_ppe_pressure_jump_formulation} の
向き付き pressure-jump として閉じる．この経路の静止液滴平衡は，
圧力差 $\Delta p=\sigma/r$ だけでなく，
式~\eqref{eq:discrete_young_laplace_static_face_balance} の
面ごとの $a_f^{\mathrm{range}}=0$ を満たす必要がある．
したがって本稿では，§~\ref{sec:split_ppe_capillary_range_projection} の
Hodge 閉包に対する短時間 gate を V3 の補助診断として追加する．

\begin{table}[!ht]
\caption{pressure-jump range projection：静止液滴 \texorpdfstring{$N=32,T=0.2$}{N=32,T=0.2} face-balance gate}
\label{tab:V3_pressure_jump_range_gate}
\PaperCaptionNote{水--空気物性，interface-fitted $\alpha=2$，periodic，pressure-jump affine PPE，Ridge--Eikonal every step，CFL 制御．Raw Hodge residual は式~\eqref{eq:capillary_hodge_gate} の原因診断であり，補正へ渡る実効 cochain は range projection 後の値で判定する．}
\centering\small
\begin{tabular}{l r}
\toprule
診断量 & 実測値 \\
\midrule
最終運動エネルギー $K(T)$ & $7.783\times10^{-38}$ \\
体積 drift $|\Delta V|/V_0$ & $1.777\times10^{-15}$ \\
補正後 $\|a_f^{\mathrm{range}}\|_{\infty,f}$ & $3.816\times10^{-17}$ \\
$D_fa_f^{\mathrm{range}}$ residual & $4.028\times10^{-15}$ \\
raw $\|c_f-\Pi_{\mathcal{R}_h}c_f\|_{\infty,f}$ & $1.139\times10^{-3}$ \\
$\|D_h\bu\|_\infty$ & $1.282\times10^{-17}$ \\
\bottomrule
\end{tabular}
\end{table}

表~\ref{tab:V3_pressure_jump_range_gate} は，
PPE 残差だけでは検出できない divergence-free な毛管 cochain が
raw 幾何ジャンプには存在すること（$1.139\times10^{-3}$）と，
それを $D_f A_f G_f$ 値域へ射影した後は補正段階の実効 face acceleration が
丸め誤差まで消えることを同時に示す．この結果は
「静止液滴の KE 上昇は物理的毛管振動ではなく，
pressure-jump cochain の Hodge 非勾配成分である」という
§~\ref{sec:split_ppe_capillary_range_projection} の推論を支持する．
ここで用いた修正は damping，CFL 縮小，曲率上限，平滑化ではなく，
PPE と速度補正が共有する face 複体の値域条件である．

この gate 以降の圧力可視化は，式~\eqref{eq:capillary_pressure_face_cochain}
の face cochain から復元した Hodge 代表を用いる．
したがって本稿の production snapshot では `pressure\_hodge' を
`pressure\_t*.pdf' として出力し，interface band を NaN で消す
`pressure\_bulk' 型の図は採用しない．
実際，保存済みの静止液滴データ（$N=64,T=1$ および $N=32,T=4$）は
plot-only で Hodge 圧力へ再描画でき，それぞれ 6 枚および 21 枚の
`pressure\_t*.pdf' を生成する．この確認は数値軌道を変えず，
「何を圧力として読むか」という出力契約だけを検証するものである．
...
\begin{itemize}\setlength{\itemsep}{0pt}
  \item \textbf{CCD 寄生流れ絶対値（主指標）}：$\rho_r=1$ で
        ...
        $\sim 10^{-2}$ と同じ order の $2\times10^{-2}$ 以下）．\textbf{CCD 勾配は寄生流れ抑制に有効}\checkmark
        （Type-A revised criterion；表~\ref{tab:V_verdict_types}：CCD/FD 比から CCD 絶対値へ判定キー変更）．
