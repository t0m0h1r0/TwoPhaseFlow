...
\paragraph{production stack 静止液滴 gate．}
上の V3 は CSF/BF 縮約経路の長時間診断である．
一方，水--空気級の pressure-jump 経路では，表面張力を
CSF 体積力ではなく §~\ref{sec:split_ppe_pressure_jump_formulation} の
向き付き pressure-jump と閉界面 Riesz 毛管源として閉じる．
この production-stack 静止液滴は，第~\ref{sec:validation}章の
移動界面 benchmark ではなく，本章の統合検証に置く．
第~\ref{sec:validation}章では静止円形液滴を重複させず，毛管波，振動液滴，
気泡上昇，Rayleigh--Taylor のように有限界面運動を伴う物理応答だけを扱う．

本 gate は \texttt{ch14\_static\_droplet.yaml} 系譜で用意した
production 設定を，V3 の pressure-jump 側補助診断として取り込むものである．
構成は periodic，interface-fitted $\alpha=2$，FCCD/TVD--RK3 界面輸送，
UCCD6/IMEX--BDF2 運動量更新，pressure-jump affine PPE，
closed-interface Riesz 毛管源，pressure-adjoint residual contract，
pressure-component Hodge 反応，variational operator pairing，
静止 reinitialization なしである．
したがって，CSF/BF 縮約経路の Laplace 圧誤差だけでは読めない
production pressure-jump/Riesz stack の幾何保持，体積保持，および
小さな運動エネルギー漏れを同じ V3 節で評価する．

\begin{table}[!ht]
\caption{V3：production stack 静止液滴 gate}
\label{tab:V3_production_static_droplet}
\PaperCaptionNote{水--空気物性，periodic，interface-fitted $\alpha=2$，pressure-jump affine PPE，closed-interface Riesz 毛管源，pressure-adjoint residual contract，静止 reinitialization なし．$D$ は signed/deformation 診断の絶対変形量である．}
\centering\small
\begin{tabular}{l r r r r r}
\toprule
\multicolumn{1}{c}{run} & $K(T)$ & $K_{\max}$ & $\max|\Delta V|/V_0$ & $D(T)$ & $\|u(T)\|_\infty$ \\
\midrule
...
表~\ref{tab:V3_production_static_droplet} は，production stack が
静止円形液滴の形状と体積を保ち，有限時間の kinetic leakage を
$K_{\max}\le1.718\times10^{-6}$，$\|u(T)\|_\infty\le1.686\times10^{-4}$
に抑えることを示す．ただし，これは「解析円を標本化しただけで
丸め誤差まで完全静止する」ことの証明ではない．標本化円は一般には
離散面積汎関数の制約付き臨界点ではないため，production stack の gate は
幾何保持・体積保持・bounded kinetic leakage を判定対象とする．
