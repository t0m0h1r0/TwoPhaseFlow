...
圧力ジャンプ/Riesz 本番構成の幾何保持，体積保持，および
小さな運動エネルギー漏れを同じ V3 節で評価する．

\begin{table}[!ht]
\caption{V3：本番構成の静止液滴判定}
\label{tab:V3_production_static_droplet}
\PaperCaptionNote{水--空気物性，periodic，interface-fitted $\alpha=2$，圧力ジャンプ付きアフィン PPE，閉界面 Riesz 毛管源，圧力補正と随伴な残差受理条件，静止 reinitialization なし．$D$ は signed/deformation 診断の絶対変形量である．}
\centering\small
...
表~\ref{tab:V3_production_static_droplet} は，本番構成が
...
離散面積汎関数の制約付き臨界点ではないため，本番構成の判定は
