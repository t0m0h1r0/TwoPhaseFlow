...
%
% §12.8  検証結果の総括
%   11 単体テスト (U1–U11) の pass/fail 総括 + 後続ベンチマーク章への接続
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{単体検証の総括：Tier I--IX の適合判定}
\label{sec:verify_summary}
% ═══════════════════════════════════════════════════════════════════════════════

9 Tier (I--IX) で構成される 11 単体テスト (U1--U11) が完了した．
表~\ref{tab:verification_summary} に各テストの目的・期待/判定基準・実測/観測値・判定を総括する．
11 件の単体テストは Part 2 (§\ref{sec:ccd_def}--§\ref{sec:grid}) のうち
単体化できる primitive を依存順に検査する．
...
\midrule
U6-a & DC $\omega$ route guard（$\rho_l \in \{1,100,1000\}$, $k\in\{2,3\}$, $\omega \in \{0.2..0.9\}$） & $\rho_l=1$ clean，高密度比一括 PPE は保証対象外 & $\rho_l=1$: $5.19\!\times\!10^{-5}$；高密度比は残差改善しても分相 PPE+HFE へ切替 & \checkmark guard \\
...
\midrule
U10 & 共通 FCCD ledger による $\psi$・密度・運動量の同一保存写像 & 密度/運動量保存，アフィン密度誤差 $0$，負対照 fail-closed & $\max|\Delta M_\rho|=1.137\!\times\!10^{-13}$, $\max|\Delta P_i|=2.842\!\times\!10^{-14}$, affine error $0$, negative $2/2$ rejected & \checkmark \\
\midrule
\multicolumn{5}{l}{\textbf{Tier IX：制約 face-state 空間}} \\
\midrule
U11 & $P_w$ と $G_w=P_wG_A$ の wall/periodic-wall quotient gate & $C_wP_w=0$，冪等性，metric self-adjointness，Green identity，rank 一致 & wall trace $\le2.581\!\times\!10^{-12}$, idempotence $4.851\!\times\!10^{-12}$, Green $\le1.294\!\times\!10^{-16}$, rank $19/19$ and $25/25$ & \checkmark \\
\bottomrule
\multicolumn{5}{p{0.95\linewidth}}{\footnotesize
$\triangle$ の類型——
\checkmark: 設計精度達成，$\bullet$: negation 成立（規則違反量が予測通り発散；合格扱い），$\times$: 未達（本章では該当なし）．
本表は Tier 順で配置する（26 サブテスト；親テスト U6-b は U6-b1/U6-b2，親テスト U7-a は U7-a1/U7-a2 として
表内で一意に引用できる枝番へ分割）．
U6 の一括 PPE 高密度比 sweep は保証対象 primitive ではないため本成果表から除外し，U6 本文では分相 PPE+HFE を必要とする route guard として扱う．
...
\begin{tcolorbox}[resultbox, title={U1--U11 単体検証の総括}]
11 件の単体テストで，正の検証項目は\textbf{合格}（$\checkmark$），
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{保存形共通フラックス：}U10 で closed common-flux candidate は
        最大 $|\Delta M_\rho|=1.137\times10^{-13}$，
        最大 $|\Delta P_i|=2.842\times10^{-14}$，アフィン密度誤差 $0$ を示し，
        非アフィン密度と $\psi$-only ledger の負対照は $2/2$ reject した．
  \item \textbf{制約 face-state 空間：}U11 で $P_w$ は wall/periodic-wall の双方で
        $C_wP_w=0$，冪等性，metric self-adjointness，制限 Green identity を丸め誤差まで満たし，
        rank gate は wall $19/19$，periodic-wall quotient $25/25$ で合格した．
...
次章（第~\ref{sec:verification}章）は，U1--U11 で確立した primitive を
...
\PaperCaptionNote{U 系列は component verification，V 系列は integration verification であり，判定基準は異なる．本表は単体検証だけでは判定できない結合効果を次章の V test に対応付ける橋渡しである．}
\centering\small
...
非一様格子・局所厚み & U3, U5 & V8（非一様 NS 静止）と V9（圧力ジャンプ・毛管射影結合系条件下の nominal/local $\varepsilon$ switch）． \\
...
制約 face-state / periodic-wall quotient & U11 & V6/V7/V9 の壁付き pressure-jump/HFE 統合経路の状態空間前提を支える．periodic-wall の有限振幅物理応答は第~\ref{sec:benchmarks}章で扱う． \\
7 ステップ更新順序・面データ引き渡し & U1--U11 全体 & V1--V10 と表~\ref{tab:v_accuracy_summary}（更新順序，射影後面速度閉包，アフィン圧力履歴面加速度，圧力ジャンプ結合系の適用限界）． \\
\bottomrule
\end{tabularx}
\end{table}
