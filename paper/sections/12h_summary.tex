...
9 件の単体テストは Part 2 (§\ref{sec:ccd_def}--§\ref{sec:grid}) の primitive を
依存順に検査し，複合要素（face-jet $J_f$, CLS A→F 6-stage, 運動量対流, IPC projection）は
次章の V 系列で統合効果として再検査する．
...
\midrule
U7-a & 縮約 BF 整合：$j_{gl}=-\sigma\kappa_{lg}$ vs 計算値 & $N\ge64$ で ${<}3\%$ & $0.61\%$（$N=128$） & \checkmark \\
U7-a & BF mismatch one-step 負対照 & 合格経路ではなく判別力の低さを示す & FD2 mismatch でも $N\ge64$ で ${<}3\%$ に見えるため長時間 V5 へ送る & $\bullet$ \\
...
高密度比 pressure-jump stack & U6, U8, U9（U7 は縮約 BF 対照） & V6（密度比 sweep）と V7（FCCD, HFE, 分相 PPE/DC, capillary range projection, projection-native face closure, affine pressure-history faces を含む coupled-stack 時間精度）．U7 は Neumann BF 縮約との差分整理にのみ用いる． \\
