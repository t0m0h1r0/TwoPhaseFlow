...
高密度比 pressure-jump stack & U6, U7, U8 & V6（密度比 sweep）と V7（FCCD, HFE, 分相 PPE/DC, projection-native face closure, affine pressure-history faces を含む coupled-stack 時間精度）． \\
非一様格子・局所厚み & U3, U5 & V8（非一様 NS 静止）と V9（projection-native / affine-history §14 stack 条件下の nominal/local $\varepsilon$ switch）． \\
CLS 保存移流 & U4, U5, U8 & V10-a/V10-b（一様格子・NS 非連成 CLS 強変形移流；非一様 moving-interface CLS は future verification gate）． \\
\bottomrule
\end{tabularx}
\end{table}
