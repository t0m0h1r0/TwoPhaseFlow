...
U6 の一括 PPE 高密度比 sweep は保証対象 primitive ではないため本成果表から除外し，U6 本文では分相 PPE+HFE を必要とする補助診断として扱う．
}
\end{tabularx}
\end{table}
...
高密度比 pressure-jump stack & U6, U7, U8 & V6（密度比 sweep）と V7（FCCD, HFE, 分相 PPE/DC, capillary range projection, projection-native face closure, affine pressure-history faces を含む coupled-stack 時間精度）． \\
非一様格子・局所厚み & U3, U5 & V8（非一様 NS 静止）と V9（projection-native / affine-history stack 条件下の nominal/local $\varepsilon$ switch）． \\
