...
9 件の単体テストは Part 2 (§\ref{sec:ccd_def}--§\ref{sec:grid}) のうち
単体化できる primitive を依存順に検査する．
一方，face-jet $J_f$ の複数経路共有，CLS A→F 6-stage，運動量対流，
pressure-jump IPC projection，履歴圧力面加速度，capillary range projection は
...
本表は Tier 順で配置する（24 サブテスト；親テスト U6-b は U6-b1/U6-b2，親テスト U7-a は U7-a1/U7-a2 として
表内で一意に引用できる枝番へ分割）．
...
9 件の単体テストで，正の検証項目は\textbf{合格}（$\checkmark$），
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{面データ引き渡し}：projection-native 面速度，affine pressure-history faces，capillary range projection が同じ面複体を共有するか
\end{itemize}
...
運動量対流・全応力粘性の NS 残差 & U1, U8 & V1/V2（単相 NS）と V7（二相 coupled-stack 時間診断）． \\
BF 圧力--界面バランス & U5, U7, U9 & V3/V5（静止液滴と寄生流れ），V8（非一様静止液滴）． \\
高密度比 range-projected pressure-jump stack & U6, U8, U9（U7 は縮約 BF 対照） & V6（密度比 sweep）と V7（FCCD, HFE, 分相 PPE/DC, capillary range projection, projection-native face closure, affine pressure-history faces を含む coupled-stack 時間精度）．U7 は Neumann BF 縮約との差分整理にのみ用いる． \\
...
7 ステップ更新順序・面データ引き渡し & U1--U9 全体 & V1--V10 と表~\ref{tab:v_accuracy_summary}（更新順序，projection-native face closure，affine pressure-history faces，pressure-jump stack の適用限界）． \\
\bottomrule
\end{tabularx}
\end{table}
