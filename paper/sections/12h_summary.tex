...
アクティブ幾何毛管分解の受理境界 & U12 & V11 で全圧力像への過射影を棄却し，$R_p$，$q$ 所有グラフ HFE，面 Hodge で加速度へ写した補間，移動格子面量，圧力履歴座標，正則層ガード，DC 収束判定を採用境界にする． \\
閉界面標準実行の体積境界 & U4, U5, U10, U12 & V12 で第~\ref{sec:val_oscillating_droplet}節の現行 \texttt{active\_geometry\_capillary} 設定を監査し，過去の \texttt{sharp\_phase\_volume}/Ridge--Eikonal 補修は現行 closed endpoint へ混入できないこと，1 周期標準実行は不受理であることを固定する． \\
7 ステップ更新順序・面データ引き渡し & U1--U12 全体 & V1--V12 と表~\ref{tab:v_accuracy_summary}（更新順序，射影後面速度閉包，アフィン圧力履歴面加速度，圧力ジャンプ結合系の適用限界，アクティブ幾何毛管分解の最終条件，閉界面体積の採用境界）． \\
\bottomrule
\end{tabularx}
\end{table}
