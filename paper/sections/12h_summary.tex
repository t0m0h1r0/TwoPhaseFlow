...
\midrule
\multicolumn{5}{l}{\textbf{Tier X：AO-Fast 幾何毛管分解 gate}} \\
\midrule
U12 & $R_p$ gate と full-pressure 過射影反例 & flat zero-drive，wave nonzero probe，GPU fail-close & flat $0$；N32/N64 wave の CPU exact full-pressure split は drive $0$，component-volume probe は $2.1176/2.3055$；GPU は diagonal active-Schur residual と zero drive で fail-close & \checkmark diagnostic \\
\bottomrule
\multicolumn{5}{p{0.95\linewidth}}{\footnotesize
$\triangle$ の類型——
\checkmark: 設計精度達成，$\bullet$: negation 成立（規則違反量が予測通り発散；合格扱い），$\times$: 未達（本章では該当なし）．
本表は Tier 順で配置する（U12 の 4 gate ケースを含む 30 サブテスト相当；親テスト U6-b は U6-b1/U6-b2，親テスト U7-a は U7-a1/U7-a2 として
表内で一意に引用できる枝番へ分割）．
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{AO-Fast gate：}U12 で，full pressure image への過射影は
        毛管波の CPU exact split において balanced drive を $0$ にすることを確認した．
        component-volume Hodge probe は N32/N64 wave で $2.1176/2.3055$ の非静的残差を検出し，
        現行 GPU AO-Fast packet は diagonal active-Schur residual と zero balanced drive で fail-close した．
...
AO-Fast 幾何毛管分解の受理境界 & U12 & V11 で標準物理経路への混入可否を判定する．U12/V11 未通過の AO-Fast packet は第~\ref{sec:benchmarks}章の本番毛管波結果とは別扱いにする． \\
7 ステップ更新順序・面データ引き渡し & U1--U12 全体 & V1--V11 と表~\ref{tab:v_accuracy_summary}（更新順序，射影後面速度閉包，アフィン圧力履歴面加速度，圧力ジャンプ結合系の適用限界，AO-Fast gate）． \\
\bottomrule
\end{tabularx}
\end{table}
