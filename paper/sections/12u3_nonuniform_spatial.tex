...
\texttt{update\_from\_levelset} に与える経路で実現する（実装:
\texttt{experiment/ch12/exp\_U3\_nonuniform\_spatial\_suite.py:104}）．
...
本節の D1--D4 は \textbf{実験スクリプト固有の集約スカラー}であり，
production API として \texttt{src/twophase} に独立関数として
公開してはいない（実体は格子量 \texttt{ridge\_sigma\_eff\_field} /
\texttt{eps\_local} / \texttt{dJ\_dxi}（§\ref{sec:ridge_eikonal_nu_grid}）
の振幅幅 \textit{max-min} を実験側で集計したもの）．
これは D1--D4 が「メトリック補正の活性化を一桁単位で確認する診断指標」であって
量的な収束次数を主張する量ではないためで，
production 経路では生成過程の各場が
