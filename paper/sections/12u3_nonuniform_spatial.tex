...
格子は \texttt{Grid.update\_from\_levelset(psi, eps)}
（\texttt{src/twophase/core/grid.py:91}）が実装する
gauss 密度 $\rho_d(\xi) = 1 + (\alpha-1)\exp\!\bigl(-(\psi/\eps)^2\bigr)$
...
均一格子 U1-c (4 次律速) に対し非一様 face evaluation では
連鎖律補正により設計 6 次が回復する．}
...
\midrule
\multicolumn{2}{r}{slope} & $\mathbf{5.99}$ & $\mathbf{5.97}$ \\
\bottomrule
\end{tabular}
\end{table}
...
$\alpha=2$ 界面集中ガウス密度上で CCD 1 階 d1 slope $5.98$ + FCCD face value/grad slope $5.99 / 5.97$（U1-c の 4 次律速が連鎖律補正で解消）；
Ridge--Eikonal D1--D4 メトリック補正は $\alpha=1$ で全零，$\alpha>1$ で D4（連鎖律）が $O(10^{-11})$ で発火．
出典：\texttt{experiment/ch12/exp\_U3\_nonuniform\_spatial\_suite.py}．}
\label{fig:ch12_u3}
\end{figure}
...
\begin{itemize}\setlength{\itemsep}{0pt}
  \item U3-a CCD 非一様：slope $5.98$．設計 6 次の機械零域近接，
        \textbf{合格} \checkmark．
  \item U3-b FCCD 非一様：slope $5.99$（face value）/ $5.97$（face grad）．
        U1-c の 4 次律速が解け，§\ref{sec:fccd_def} の設計 6 次が
        実現することを確認．\checkmark
...
\end{itemize}
A3 トレーサビリティ：式~\eqref{eq:grid_delta} → §\ref{sec:grid_density} 密度関数 →
\texttt{src/twophase/core/grid.py} の \texttt{Grid.update\_from\_levelset}．
