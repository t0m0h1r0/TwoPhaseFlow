...
% 付録 D.x: Predictor ADI 分裂の実装詳細（§10 本文から移動）

\subsection{Predictor ADI 分裂の実装詳細}
\label{app:predictor_adi}

本節では，§~\ref{sec:algo_flow} Step~5 の運動量予測ステップにおける
Crank--Nicolson + ADI 分裂の実装詳細と，$y$ 成分の完全な離散形式を補足する．

% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{$y$ 成分の離散形式}
...
§~\ref{sec:algo_flow} Step~5 の離散形式は形式的な CN 離散化を示す．
...
\end{split}
\end{equation*}
$(R^{\text{explicit}}_x)_{i,j}$ は対流項・クロス粘性・CSF 表面張力の陽的寄与
