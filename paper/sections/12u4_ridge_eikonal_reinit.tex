...
\begin{description}\setlength{\itemsep}{0pt}
  \item[U4-a] Ridge--Eikonal 反復回数 $n_\tau \in \{0,1,5,10,20,50,100\}$，
...
\begin{table}[!ht]
\caption[U4-a: Ridge--Eikonal band gradient error]{%
U4-a: Ridge--Eikonal 再初期化のバンド内 $|\nabla\phi|-1$ 誤差
（$N=128$, $h_{\min} = 7.81\times 10^{-3}$, $\mathrm{d}\tau = 0.3 h_{\min}$）．
$n_\tau = 0$ の初期残差 $0.5$ から $n_\tau = 100$ で
$9.16\times 10^{-3}$（$\sim h$）まで単調減衰．設計通りの $\eps$--Eikonal 流．}
\label{tab:U4_reinit}
\centering\small
\begin{tabular}{r r r}
\toprule
...
理論予測の改善率 $\sim 1.03$ を大幅に上回る精度（連鎖律補正の効果）．}
\label{tab:U4_dgr}
\centering\small
...
\caption{U4：Ridge--Eikonal 再初期化 + DGR バンド厚補正．
...
出典：\texttt{experiment/ch12/exp\_U4\_ridge\_eikonal\_reinit.py}．}
\label{fig:ch12_u4}
\end{figure}
...
\begin{itemize}\setlength{\itemsep}{0pt}
  \item U4-a：$n_\tau = 20$ 反復で band 誤差 $2.2\times 10^{-2}$，
        ...
        §\ref{sec:eikonal_reinit} の Ridge--Eikonal 設計通り．\checkmark
  \item U4-b：DGR 1 ステップで ratio が $2.0 \to 1.0000033$ に補正．
        §\ref{sec:dgr_thickness} 理論期待値（残差 $\sim 0.03$）を 4 桁上回る精度．
        DGR の連鎖律重み付け実装が設計通りに機能．\checkmark
\end{itemize}
A3 トレーサビリティ：
再初期化擬似時間刻み定義（Hamilton--Jacobi 形）→ §\ref{sec:eikonal_reinit} 離散化 →
\texttt{src/twophase/levelset/reinit\_eikonal.py} および
\texttt{src/twophase/levelset/ridge\_eikonal\_reinitializer.py}；
DGR スケーリング規則 → §\ref{sec:dgr_thickness} →
\texttt{src/twophase/levelset/reinit\_dgr.py}．
