...
$n_\tau = 0$ の初期残差 $0.5$ は biased SDF $\phi_0 = \phi_{\sdf}/2$ に起因する設計値．
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item U4-b：DGR 1 ステップで ratio が $2.0 \to 1.0000033$ に補正．
        ...
        理論期待値「残差 $\sim 0.03$」は biased $\psi$ の漸近 1 次補正項 $(2\eps - \eps^\star)/\eps^\star \cdot O(h^2)$ に
        対応し，実測 $3.3\times 10^{-6}$ は biased 入力の高次項残差が $h^4$ レベルまで効くことを示す
        （$N=128$ の機械精度床近接）．DGR の連鎖律重み付け補正が設計通りに機能．\checkmark
\end{itemize}
検証対応：
