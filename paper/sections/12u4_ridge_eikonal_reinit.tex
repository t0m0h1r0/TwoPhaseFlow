...
本 U4 の実体は \texttt{twophase.levelset.\allowbreak reinit\_eikonal\_godunov.\allowbreak godunov\_sweep}
（Hamilton--Jacobi 擬似時間 sweep）と
\texttt{twophase.levelset.\allowbreak reinit\_dgr.\allowbreak DGRReinitializer}（DGR 1 段補正）
であり，名称が類似する \texttt{RidgeEikonalReinitializer}（single-shot FMM 型；
$n_\tau$ 反復ではなく ridge--extraction の \texttt{eps\_scale} で band 床に達する）
は本 U4 の対象ではない．label \texttt{sec:U4\_ridge\_eikonal\_reinit} は章ファイル
命名の歴史的経緯による artefact で，検証対象は上記 2 primitive である．
