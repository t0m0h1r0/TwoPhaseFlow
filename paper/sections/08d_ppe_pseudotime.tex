...
\begin{itemize}
  \item \textbf{空間精度}：CCD 法（6次精度）を用いるため，$\mathcal{O}(h^6)$ の高精度
        ...
        後述の収束基準~\ref{eq:etol_criterion} による）
...
枠組みに直接対応する．現行 Krylov 実装（表~\ref{tab:ppe_methods}）では左辺にも $\mathcal{L}_h$ を
用いているが，式~\eqref{eq:defect_correction_linear} が示すように左辺は低次 $\mathcal{L}_{FD}$ への
置き換えが可能であり，Thomas スウィープ実装（省メモリ・$\mathcal{O}(N)$）への移行の理論的根拠を与える．
