...
\begin{itemize}
  \item \textbf{空間精度}：CCD 法（6次精度）を用いるため，$\mathcal{O}(h^6)$ の高精度
        ...
        後述の収束基準~\ref{result:etol_criterion} による）
...
\noindent\textbf{本節は将来の改良候補として提示する理論的枠組みである（現行コードへの実装は未完了；現行スウィープ実装は§\ref{sec:pseudo_adi_comparison}）．}
...
\begin{itemize}
  ...
  \item \textbf{右辺 $R_h$：} CCD 演算子 $\mathcal{L}_h$ の $\mathcal{O}(h^6)$ 精度で評価された残差であり，
        収束先の解の空間精度を決定する．
\end{itemize}
...
\textbf{実装との位置付け：}
欠陥補正法（式~\eqref{eq:defect_correction_linear}）は，本稿が現在採用する
\textbf{スウィープ型 CCD}（\ref{warn:ppe_splitting}）とは\textbf{異なる手法}である点に注意する．
スウィープ型実装は左辺に CCD 演算子 $\mathcal{L}_x^\mathrm{CCD}$，$\mathcal{L}_y^\mathrm{CCD}$ を用いた
ADI 因子分解であり，$\mathcal{O}(\Delta\tau^2)$ の分割誤差が生じる（表~\ref{tab:ppe_methods} 3行目）．
一方，欠陥補正法は左辺を低次 $\mathcal{L}_{FD}$ に置き換えることで，
Thomas $\mathcal{O}(N)$ スウィープでの求解を可能にしつつ分割誤差なしに CCD 精度を実現する
\textbf{将来の改良候補}である（表~\ref{tab:ppe_methods} 5行目）．
両者は収束後に同一の $\mathcal{L}_h(\delta p) = q_h$ 解を与えるが，
スウィープ型には分割誤差 $\mathcal{O}(\Delta\tau^2)$ が残るのに対し，
欠陥補正法は残差収束のみで精度を保証する点が本質的な違いである．
...
欠陥補正（FD 左辺 + CCD 残差）$\ddagger$ & $\mathcal{O}(h^6)$ & なし & 残差駆動 & 中 \\
\hline
...
\multicolumn{5}{l}{$^{\ddagger}$ 左辺に 2次精度 FD 演算子 $\mathcal{L}_{FD}$，右辺残差に CCD 演算子 $\mathcal{L}_h$ を用いる（§\ref{sec:defect_correction}）．}\\
\multicolumn{5}{l}{\phantom{$^{\ddagger}$} 密度適応 LTS（§\ref{sec:lts_dtau}）と組み合わせることで高密度比流れの収束を改善できる．}
\end{tabular}
\end{table}
...
スウィープ実装では線形系は $\mathcal{O}(\Delta\tau^2)$ の誤差を残すため，
...
式~\eqref{eq:dtau_opt} の最適 $\Delta\tau_\mathrm{opt} \approx 0.58\,h^2/a_\mathrm{max}$ は
...
PPE 演算子 $\mathcal{L}_h = \nabla\cdot({\rho^{-1}}\nabla)$ の最大固有値は
$a_{i,j} \equiv 1/\rho_{i,j}$ に比例するため，各セルの有効仮想時間刻みと固有値の積は
\begin{equation*}
  \Delta\tau_{i,j} \cdot a_{i,j}
  = \frac{C_\tau\,\rho_{i,j}\,h^2}{2} \cdot \frac{1}{\rho_{i,j}}
  = \frac{C_\tau\,h^2}{2} \quad (\text{定数})
\end{equation*}
となり，密度依存性が完全に打ち消される．
