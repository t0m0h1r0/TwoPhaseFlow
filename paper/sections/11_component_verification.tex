...
\input{sections/11a_spatial_discretization} %% §11.1 空間離散化基盤の検証（CCD/GCL/DCCD）
\input{sections/11b_spatial_geometry}       %% §11.1続き（曲率・Rhie-Chow・混合微分）
\input{sections/11c_interface_pipeline}     %% §11.2 界面処理パイプラインの検証（CLS/WENO5/DGR/DCCD質量保存）
\input{sections/11d_interface_field}        %% §11.2続き（動的格子再マッピング・HFE・Young-Laplace）
\input{sections/11e_pressure_solver}        %% §11.3 圧力ソルバの検証
\input{sections/11f_time_integration}       %% §11.4 時間積分スキームの検証
\input{sections/11g_summary}                %% §11.5 検証結果の総括
