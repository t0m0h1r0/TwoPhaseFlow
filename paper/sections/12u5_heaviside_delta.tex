...
平滑化幅 $\eps$ の関数形と離散モーメントの整合性についての標準解説は~\cite{OsherFedkiw2003}を参照．
...
$N \ge 128$ で 0 次モーメント誤差が saturation（$\sim 10^{-11}$）に到達．
slope $N\!=\!16\!\to\!64$ は $\approx 11$（解析的には超収束）．
saturation 値は $c=1.5$ 固有の Richardson 残差床であり double-precision floor ではない（U5-b 参照）．}
...
$c = 1$ は $N=128$ で $2.11\!\times\!10^{-7}$ に stall（$\eps$ サポート境界）．
$c \in [1.5, 2]$ で $h$ 細分化により Richardson 残差 saturation に到達し，
$c = 2$ は $N=128$ で $1.83\times 10^{-14}$（double-precision floor 近傍；本実験の最良）．
本実験での推奨は $c \in [1.5, 2]$．}
...
$\Ord{10^{-5}},\,\Ord{10^{-4}}$ と悪化）．これは粗格子では $\eps\!=\!c h$ のサポートが
...
$c=1.5$ で $N=128$ までに 0 次モーメント誤差が Richardson 残差床へ到達；
