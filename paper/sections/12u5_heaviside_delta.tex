...
\PaperCaptionNote{$\eps = 1.5 h$．$N \ge 128$ で 0 次モーメント誤差が Richardson 残差 saturation（$\sim 10^{-11}$）に到達．slope $N\!=\!16\!\to\!64$ は $\approx 11$（解析的には超収束）．saturation 値は $c=1.5$ 固有の Richardson 残差床であり double-precision floor ではない（U5-b 参照）．}
\centering\small
...
\PaperCaptionNote{$c \le 1$ は $\eps$ サポート不足で stall（$c=0.5$ は $h\to 0$ で改善せず；$c=1$ は $N=128$ で $2.11\!\times\!10^{-7}$ に stall）．$c \in [1.5, 2]$ で $h$ 細分化により Richardson 残差 saturation に到達し，$c = 2$ は $N=128$ で $1.83\times 10^{-14}$（double-precision floor 近傍；本実験の最良）．本実験での推奨は $c \in [1.5, 2]$．}
\centering\small
