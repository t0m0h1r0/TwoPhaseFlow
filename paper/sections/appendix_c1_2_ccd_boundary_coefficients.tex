...
\subsubsection{Equation-II 境界スキームの 4 次精度昇格（\texorpdfstring{$\Ord{h^4}$}{O(h\textasciicircum 4)}）}
...
$n_\mathrm{pts} \ge 6$ を paper-exact 条件として式~\eqref{eq:bcII_left_h4} を用いる．
$n_\mathrm{pts} < 6$ は 6 点 $\Ord{h^4}$ 境界閉包を構成できないため明示的に拒否し，
低次公式への暗黙フォールバックは行わない．右境界では
...
境界を含む係数検証では平均スロープ ${\approx}4.8$ を確認した．\qed
