...
\begin{table}[htbp]
\centering
\caption{RC ブラケットの格子収束：標準形 $\Ord{h^2}$ vs Richardson 補正形 $\Ord{h^4}$}
\label{tab:rc_bracket_convergence}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{rcccccc}
\toprule
$N$ & $\|\text{std}\|_\infty$ & 次数 & $\|\text{cor}\|_\infty$ & 次数 & 改善倍率 \\
\midrule
 16 & $7.89 \times 10^{-2}$ & ---  & $1.22 \times 10^{-3}$ & ---  & $65\times$ \\
 32 & $2.01 \times 10^{-2}$ & 1.97 & $7.74 \times 10^{-5}$ & 3.98 & $259\times$ \\
 64 & $5.04 \times 10^{-3}$ & 1.99 & $4.86 \times 10^{-6}$ & 3.99 & $1037\times$ \\
128 & $1.26 \times 10^{-3}$ & 2.00 & $3.04 \times 10^{-7}$ & 4.00 & $4150\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
CCD $p'''$ による Richardson 補正は収束次数を\textbf{正確に} $\Ord{h^4}$ に引き上げ，
$N = 128$ で 4150 倍の改善を達成する．
追加コストは CCD 2 回（各軸 1 回，$p''$ の微分）のみであり，
高次精度 CCD の情報を活用した効率的な精度改善である．
