...
\noindent\textbf{目的：}
§~\ref{sec:rc_high_order} で導出した CCD $p''$ 係数相殺補正（式~\eqref{eq:rc_d2fd_bracket}）が，
RC 検出項（ブラケット）の収束次数を $\Ord{h^2}$ から $\Ord{h^4}$ に改善することを数値的に確認する．

\noindent\textbf{評価対象：}
RC ブラケット $(\nabla p)_f - \overline{(\nabla p)}_f$（式~\eqref{eq:rc_detection_taylor}），
および補正済みブラケット $(\nabla p)_f - \overline{(\nabla p)}_f^{\,*}$（式~\eqref{eq:rc_d2fd_bracket}）．
...
\begin{table}[htbp]
\centering
\caption{RC ブラケットの格子収束：標準形 $\Ord{h^2}$ vs CCD $p''$ 補正形 $\Ord{h^4}$}
\label{tab:rc_bracket_convergence}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{rcccccc}
\toprule
$N$ & $\|\text{std}\|_\infty$ & 次数 & $\|\text{d2fd}\|_\infty$ & 次数 & 改善倍率 \\
\midrule
...
CCD $p''$ による係数相殺補正は収束次数を\textbf{正確に} $\Ord{h^4}$ に引き上げ，
