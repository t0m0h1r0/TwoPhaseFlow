...
第~\ref{sec:grid}章 §~\ref{sec:grid_gen} のステップ5では $J_x$ を $\Ord{h^2}$ 中心差分で評価しているため，
そのままでは $\partial J/\partial\xi$ の精度が $\Ord{h}$ 以下となり，
...
\begin{tcolorbox}[algbox, title={混合微分 $\partial^2 f/\partial x\partial y$ の計算手順}]
...
\begin{itemize}
  ...
  \item \textbf{交換可能性の確認：} 滑らかな $f$ に対して Schwarz の定理から $f_{xy} = f_{yx}$（理論値として）．
        実装の検証として $D_x^{(1)}(D_y^{(1)} f)$ と $D_y^{(1)}(D_x^{(1)} f)$ の差が $\Ord{h^6}$ 以内であることを確認する．
...
\begin{itemize}
  ...
  \item \textbf{役割2（楕円型ソルバー）：} 未知の圧力 $p$ に対して CCD を適用し，$\Ord{h^6}$ 精度の演算子 $\mathbf{L}_{\mathrm{CCD}}^{\rho}$ を構成，仮想時間陰解法で PPE を解く（§~\ref{sec:ppe_pseudotime}）．
\end{itemize}
...
\medskip
\noindent\textbf{標準 CCD の残された課題：数値散逸の欠如．}
以上で述べた標準 CCD は，係数の中心対称性によってあらゆる波数モードを\textbf{振幅減衰なく}伝播させる．
これは高精度スキームとして理想的な一方，非線形の気液二相流では対流項の非線形相互作用によって
格子スケール（$kh \approx \pi$）の高周波ノイズが蓄積し，最終的に計算が発散するという問題を引き起こす．
次節（§~\ref{sec:dissipative_ccd}）ではこの欠点を補う\textbf{散逸的 CCD（Dissipative CCD）}を導入する．
