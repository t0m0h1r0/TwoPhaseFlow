...
\begin{tcolorbox}[mybox, title={実装との対応：CCD 積則微分の使用}]
本稿の標準実装（\texttt{ns\_terms/viscous.py}）では，粘性項を FVM フェイス積分ではなく
\textbf{CCD 積則微分} $\nabla\cdot[\mu(\nabla\bm{u}+\nabla\bm{u}^\top)]$ として計算する：
各格子点で $\mu$ と各速度勾配を CCD で求め，そのまま $\mu\cdot(\partial u_\alpha/\partial x_\beta + \partial u_\beta/\partial x_\alpha)$ を合成して再度 CCD 微分をとる．
この実装は FVM 算術平均とは\textbf{異なるアプローチ}だが，$\mu$ が界面近傍以外では滑らかな場合には
等価な $\mathcal{O}(h^6)$ 精度の結果を与える．
本付録の算術平均証明は，代替 FVM 実装を選択する場合の数学的正当化として掲載している．
\end{tcolorbox}
