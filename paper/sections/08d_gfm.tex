...
\label{sec:gfm}% backward-compat: §10–§12 cross-refs（将来ラベル移行後に削除）
% ═══════════════════════════════════════════════════════════════════════════════
...
\paragraph{なぜ界面越し場延長が必要か.}
CCD 法は同一ステンシル $(i-2,\ldots,i+2)$ で $\Ord{h^6}$ の微分を求解する．
しかし二相流では，圧力場が Young--Laplace 則 $[p]_\Gamma = \sigma\kappa$ に従う
不連続を持つため（式~\eqref{eq:young_laplace_jump}），
界面を跨ぐ CCD ステンシルは Gibbs 振動を引き起こし精度が著しく低下する．

界面越し場延長はこの問題を根本的に解消する：
...
% ─────────────────────────────────────────────────────────────────────────────
% Backward-compatibility labels（将来ラベル移行後に削除）
