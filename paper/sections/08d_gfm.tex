...
\noindent\textbf{現行ソルバにおける位置づけ．}
現行の CSF + smoothed Heaviside 一括解法では，圧力場 $p$ は
$\varepsilon$-regularized な滑らかな場であり，界面ジャンプを持たない．
この場合 HFE は不要であり，むしろ有害である
（§~\ref{sec:ppe_discretization_choice} で定量的に実証）．
HFE が有効なのは，\textbf{分相 PPE 解法}（§~\ref{sec:split_ppe_motivation}）のように
各相で独立に PPE を解き，圧力場が Young--Laplace ジャンプ $[p]_\Gamma = \sigma\kappa$ を
陽に持つ場合である．
本節では将来の分相 PPE 解法に向けた\textbf{基盤技術}として HFE を定式化する．
