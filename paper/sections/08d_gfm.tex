...
\begin{itemize}
  \item \textbf{空間微分：} $\hat{\bm{n}}\cdot\bnabla q$ を1次精度風上差分で評価する．
        CCD はパデ型コンパクトスキームであり単調性保存性（ENO/TVD 性）を持たない．
        仮想時間積分開始直後の不連続フィールド $q$ に CCD を適用すると
        Gibbs 振動が発生して延長場が汚染されるため，CCD は適用できない．
        1次風上差分はその固有の数値拡散により単調性を維持し，この問題を回避する．
        その結果，延長後場 $q_\mathrm{ext}$ の界面近傍（$|\phi| \lesssim 2h$）での誤差は
        $\Ord{h}$ となり，たとえ後段の CCD が $\Ord{h^6}$ であっても界面付近の圧力勾配精度は
        $\Ord{h}$ に律速される（これは不連続ソース項に起因する物理的下限であり，
        スキームの選択によって解消できない）．
        延長\textbf{後}の滑らかな場には CCD を安全に適用できる（本節前段参照）．
