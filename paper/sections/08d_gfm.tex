...
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{Hermite 場延長法（HFE）：$\Ord{h^6}$ 界面越し場延長}
...
CSF モデルは $\delta_\varepsilon$ 正則化により体積力を $2\varepsilon$ 幅に分散させるが，
\textbf{PPE の解 $p$ 自体は $\varepsilon$ 内で急峻な勾配を持つ}%
\footnote{CSF の $\delta_\varepsilon$ は運動方程式の表面張力項を正則化するものであり，
PPE の解の勾配を $\varepsilon$ スケールで平坦化するわけではない．
Smoothed Heaviside 一括解法では $p$ は $\varepsilon$-regularized な滑らかな場となるが，
IPC や分相 PPE 解法では $p$ は界面で真のジャンプ $\sigma\kappa$ を持つ．}．
このため界面を跨ぐ CCD ステンシルは Gibbs 振動を引き起こし精度が著しく低下する．
