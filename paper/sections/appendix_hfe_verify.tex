...
格子 $N = 32, 64, 128, 256$ で風上差分（Aslam~2004）と HFE を同一条件で比較した．
...
(a) 1 次元テスト：風上差分（$\Ord{h^1}$）と Hermite（$\Ord{h^7}$）の比較．
...
風上差分ベースラインの同格子における $1.23 \times 10^{-2}$
