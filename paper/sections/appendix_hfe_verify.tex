...
\begin{table}[htbp]
\centering
\caption{1 次元場延長の格子収束}
\label{tab:field_extension_convergence}
\PaperCaptionNote{$L^\infty$ 誤差で風上差分と Hermite を比較する．}
...
\begin{table}[htbp]
\centering
\caption{2 次元 HFE の格子収束}
\label{tab:hfe_2d_convergence}
\PaperCaptionNote{$L^\infty$ 誤差，円形界面 $R = 0.25$．}
\begin{tabular}{rrrr}
\toprule
$N$ & $h$ & $L^\infty$ 誤差 & 次数 \\
\midrule
...
\PaperCaptionNote{(a) 1 次元テスト：Hermite は $N=32$--$128$ で $\Ord{h^7}$ の収束を示す． (b) 2 次元テスト：テンソル積逐次補間は $N\le128$ で高次収束し， $N=256$ では丸め・補間誤差床の影響を受ける．}
\end{figure}
