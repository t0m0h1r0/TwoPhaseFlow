...
% §5  CLS 専用アルゴリズム (CLS-specific): Ridge--Eikonal 再初期化 + A-F 6 段階
%      §5.1 (本ファイル)  Ridge--Eikonal 主軸再初期化
%      §5.2 (05b)         CLS A-F 6 段階アルゴリズム
%
% ═══════════════════════════════════════════════════════════════════════════════
\section{CLS 専用アルゴリズム}
...
\noindent\textbf{本章の構成：}
\begin{itemize}
  \item \S~\ref{sec:cls_compression}（§5.1）\textbf{Ridge--Eikonal 再初期化系}：
        本稿の主軸再初期化．Eikonal redistancing による距離関数構築と
        Heaviside 再構成で $\psi$ を復元する．
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{CLS 再初期化：Ridge--Eikonal 主軸}
...
離散実装を示す．本稿の主軸再初期化は Ridge--Eikonal 系であり，
零点集合から構築した符号距離場と Heaviside 再構成で $\psi$ を復元する．
本節の構成は以下である：
\begin{itemize}
  \item \textbf{主軸 Ridge--Eikonal 系}：§~\ref{sec:xi_sdf_reinit}（後述 $\xi$ 空間符号距離関数法）
...
本稿の主軸再初期化はここで定式化する Ridge--Eikonal 系
...
\begin{enumerate}
  ...
  \item \textbf{Eikonal PDE 反復（疑似時間反復；反復回数は
...
\textbf{反復有無の切り分け：}
本稿の Ridge--Eikonal／$\xi$-SDF／FMM 主経路は 1 ショット距離再構成であり，
疑似時間ループを持たない．以下の停止条件は，
...
（律速合成式）は時間積分章にまとめている．
擬似時間 $\Delta\tau$ の参考安定条件は \S~\ref{warn:cls_dtau_stability_v7}（§7）に，
物理 $\Delta t$ の律速合成式は \S~\ref{sec:dt_synthesis}（§7.9）に集約．
本稿の Ridge--Eikonal 主軸（\S~\ref{sec:xi_sdf_reinit}）は
反復不要であり，擬似時間 CFL は物理 $\Delta t$ には乗らない．
