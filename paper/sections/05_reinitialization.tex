...
各呼び出しの零交差集合移動量は線形補間誤差 $\Ord{h^2}$ のみであり，
...
\end{equation}
を採用する（$f$ の選定および $f{=}1$ との性能差は
§~\ref{sec:val_capillary} で定量比較）．
...
\begin{itemize}
  ...
  \item $\sigma>0$ 問題（界面振動を伴う系）：ξ-SDF（$f>1$ で
        ...
        定量比較・$f$ 較正は §~\ref{sec:val_capillary}．
\end{itemize}
...
疑似時間ループを持たない．以下の $\Delta\tau$ と停止条件は，
Godunov/WENO--HJ 型の反復 Eikonal solver を fallback として選ぶ場合にのみ適用する．
反復型では $\Delta s = \min(h_x, h_y)$ として，
...
\begin{enumerate}
  ...
  \item \textbf{ステップ数上限：}
        ...
        校正値は §~\ref{sec:val_summary} のベンチマークに基づく）
\end{enumerate}
