...
\noindent\textbf{本章の構成：}
\begin{itemize}
  \item \S~\ref{sec:cls_compression}（§5.1）\textbf{Ridge--Eikonal 再初期化}：
        本稿の主軸再初期化．Eikonal PDE による距離関数構築 +
        Ridge ガウス射影で $\psi$ を再構成．
...
本稿の主軸再初期化はここで定式化する Ridge-Eikonal 法
...
\textbf{証明：}$\phi_{\mathrm{raw}}(i,j)=0$ のとき，
...
% ─────────────────────────────────────────────────────────────────────────────
% CHK-223 Phase 3.3/3.4: 時間積分関連 2 節を §7 NEW (time integration) へ移行．
% backward-compat: 旧 \ref{warn:cls_dtau_stability} と \ref{sec:stability} は
% phantomsection alias で外部参照保護．本ファイルでは §7 への redirect 1 行のみ残す．
% ─────────────────────────────────────────────────────────────────────────────
% backward-compat aliases for warn:cls_dtau_stability / sec:stability are placed
% at the migration destination (§7) to avoid multiply-defined labels.
