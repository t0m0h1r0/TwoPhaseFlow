...
Stage A が担う．Ridge--Eikonal は物理時間の輸送方程式ではなく，
移流後の $\psi^\ast$ を同じ零点集合・許容界面幅・体積制約を満たす
CLS プロファイルへ戻す幾何射影である．したがって Stage D 単独を
「質量保存演算子」と呼ばず，体積の閉鎖は Stage F の
...
$\Delta M_{\mathrm{closure}}$ は Stage B/F の明示的な質量閉鎖である．
非一様格子ではすべての和をセル体積重み付き
$M_h=\sum_{i,j}\psi_{i,j}\Delta V_{i,j}$ で評価し，
動的格子再構成後も現在時刻の $\Delta V_{i,j}$ を用いる．
ただし $M_h$ は拡散 CLS 質量であり，界面形状が占める sharp phase volume
そのものではない．本稿では
\begin{equation}
...
Stage A の輸送端点で評価し，Stage D/F の
$\Delta M_{\psi,\Pi}$，$\Delta V_{\Gamma,\Pi}$，
$\Delta S_{\Pi}$ は別の ledger に載せる．
...
各検証・ベンチマークは，Ridge--Eikonal を毎 step 適用するか，
...
許容プロファイルを保っているかを判定する gate として読む．
...
\end{equation}
を満たす離散プロファイル集合であり，$V_{\Gamma,h}$ は sharp phase volume，
$M_{\psi,h}$ は拡散 CLS 質量，
$d_\Gamma$ は $\psi=1/2$ 線の離散変位，$S_h$ は界面測度
（または surface-energy proxy）である．
実装上は，まず符号付き距離場の一様シフト
\begin{equation}
...
\end{equation}
で零等値面の sharp volume を閉じる．一様シフトは
$|\bnabla(\phi+\lambda_\Gamma)|=|\bnabla\phi|$ を保つため，
pinning band を除けば距離性を壊さない．次に
...
\end{equation}
を満たす．この二つの scalar constraint は，sharp volume を動かして
diffuse mass を合わせる補正を禁止し，物理界面の位置と
...
毛管 face-space Hodge gate を別々の離散 identity として監視する．
...
標準アルゴリズムでは，Stage D が零点集合から符号距離場を再構成し，
Stage F が式~\eqref{eq:cls_phi_mass_correction} の
...
\begin{enumerate}
  \item \textbf{零点集合保存：}
        標準経路では，零点集合を入力の交差集合または物理壁接触 seed から定義し，
...
これは圧縮--拡散型比較経路の幅 rescaling 誤差とは別の，
反復型距離化に固有の系統誤差である．
...
$\phi_{\mathrm{raw}}=0$ となる退化点は離散上の符号クランプで片側セルへ帰属させ，
...
\begin{tcolorbox}[mybox, title={受理条件：Stage D/F の離散契約}]
Ridge--Eikonal 再構成を受理する条件は，
\begin{enumerate}[noitemsep]
  \item 入力の零点集合または壁面接触 seed と整合すること，
  \item 再構成後の $\phi$ が距離性 $|\bnabla\phi|\approx1$ を満たすこと，
  \item 再構成後の sharp phase volume $V_{\Gamma,h}$ が
