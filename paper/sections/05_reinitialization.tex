...
Gaussian 補助場 $\xiridge$ と非一様 FMM を含む完全な Ridge--Eikonal 定式化は
...
$\partial\psi/\partial t + \bnabla\cdot(\psi\bu) = 0$ は，
領域積分により $\mathrm{d}/\mathrm{d}t \int\psi\,\mathrm{d}V = 0$
（周期／壁面 BC）を保証する．離散化においても，
空間演算子が $\sum_i [\bnabla\cdot(\psi\bu)]_i = 0$ を満たせば
質量は厳密に保存される．
...
$A\bm{z}=\bm{b}$ に左から $\bm{1}^{\top}$ を掛けると
$c\sum_i z_i=\sum_i b_i$ である．したがって $\bm{b}$ が零和なら
...
DCCD/圧縮--拡散型の比較経路における質量損失メカニズムと
...
そこで，再初期化の呼び出しは固定頻度ではなく，界面帯の品質量
$Q_\Gamma = \int \psi(1-\psi)\,\mathrm{d}V$ に基づいて判定する：
\begin{equation}
  \text{再初期化の実行条件：}\quad
...
\end{equation}
を満たす離散プロファイル集合であり，$V_h$ は液相体積，
$d_\Gamma$ は $\psi=1/2$ 線の離散変位，$S_h$ は界面測度
（または surface-energy proxy）である．
したがって 1 ステップの界面変化は
\begin{equation}
...
の閉包条件で棄却するか，別個の metric dissipation として明示的に扱う必要がある．
...
毛管 face-space 射影を別々の離散 identity として監視する．
...
\textbf{分裂誤差による界面膨張．}
DCCD/圧縮--拡散型の節点中心比較経路（§~\ref{sec:dissipative_ccd}）は，
...
\textbf{Direct Geometric Reinitialization（DGR）の位置付け．}
DGR はこの構造的膨張を同定し，分裂型比較経路の幅誤差を切り分けるための
診断的な厚み補正である．標準の Stage D/F 経路は
Ridge--Eikonal 距離再構成と $\phi$ 空間質量閉鎖で完結し，
DGR を隠れた後処理として追加しない．DGR を比較・診断目的で明示的に用いる場合だけ，
logit 逆変換と勾配正規化に基づく次の補正を別経路として適用する：
\begin{enumerate}
...
\textbf{注意：} 標準経路では，次の $\psi$ 空間補正を再初期化後に用いない．
...
\textbf{比較経路での用法：}
DGR は圧縮--拡散型の分裂経路が作る幅膨張を分離評価するために用いる．
標準アルゴリズムの 1 物理時間ステップでは，Stage D が零点集合から
符号距離場を再構成し，Stage F が式~\eqref{eq:cls_phi_mass_correction}
と同じ $\phi$ 空間閉鎖で液相体積を閉じる．
...
鋭い幾何形状では，零点集合からの Ridge--Eikonal/FMM/ξ-SDF 距離再構成を
検証条件として明示する．
...
\textbf{DGR の後段追加を用いない}．必要な再構成は Ridge--Eikonal 系の
零点集合保存・体積閉鎖条件で扱い，発散シナリオの定量検証は
§~\ref{sec:val_capillary} に置く．
...
\begin{enumerate}
  ...
  \item \textbf{Eikonal PDE 反復（仮想時間反復；反復回数は
        ...
        変密度・分相条件での高精度比較経路；等密度 SSPRK3 検証では Godunov で十分．
  ...
  \item \textbf{$\phi$ 空間質量補正：}
        ...
        ただし壁面接触 seed が存在する場合は
        $\phi \leftarrow \phi+\delta\phi\,\chi_{\mathrm{free}}$ とし，
        contact-line band では $\chi_{\mathrm{free}}=0$ として零集合を固定する
...
\begin{enumerate}
  ...
  \item \textbf{勾配収束：}十分な反復で $|\bnabla\phi|\to 1$ が保証され，
  ...
  \item \textbf{質量閉鎖（線形化補正）：}Step 4 の $\phi$ 空間補正
        ...
        contact-line band で零にし，質量補正が接触 seed を移動しない
        制約付き線形化として用いる．
  \item \textbf{周期商空間閉性と対称性：}
        ...
        したがって FMM の Dirichlet seed は実際の零交差集合
        $S_h(\phi_0)$ と物理壁接触拘束に限られ，補助 ridge の局所極大を
        ``近いから'' という理由で $\phi=0$ seed に昇格してはならない．
...
\begin{tcolorbox}[mybox, title={運用指針：再初期化の定量的条件}]
\textbf{反復有無の切り分け：}
本稿の Ridge--Eikonal／$\xi$-SDF／FMM 補助経路は 1 ショット距離再構成であり，
仮想時間ループを持たない．以下の停止条件は，
Godunov/WENO--HJ 型の反復 Eikonal solver を明示比較経路として選ぶ場合にのみ適用する．
反復型 Eikonal では $\Delta s = \min(h_x,h_y)$ とし，
Hamilton--Jacobi CFL に基づく $\Delta\tau_\mathrm{HJ}\le C_\mathrm{HJ}\Delta s$
（例：$C_\mathrm{HJ}=0.3$）を用いる．圧縮--拡散型比較経路の
放物型 CFL は \S~\ref{warn:cls_dtau_stability} の比較参照に限る．

\medskip
\noindent\textbf{品質モニター：} $Q_\Gamma = \int_\Omega \psi(1-\psi)\,dV$ を使用する．
基準値
\begin{equation}
  Q_{\Gamma,\mathrm{ref}} = \int_\Omega H_\varepsilon(-\phi_0)\bigl(1-H_\varepsilon(-\phi_0)\bigr)\,dV
\end{equation}
に対して $Q_\Gamma/Q_{\Gamma,\mathrm{ref}}$ が閾値 $\theta_{\mathrm{reinit}}$ を超えたときのみ再初期化を実行する
（適応的再初期化；§~\ref{sec:adaptive_reinit}．具体値は対象検証の条件として明示する）．
軽微な乱れでは3～5ステップ，界面トポロジー変化直後は10～20ステップ程度が目安．

\medskip
\noindent\textbf{終了判定（反復型 Eikonal 経路の停止条件）：}
再初期化ループを反復ごとに以下の条件でチェックし，いずれかを満たしたとき停止する：
\begin{enumerate}
  \item \textbf{残差収束：}
        $\|\psi^{m+1} - \psi^m\|_{L^\infty} < \delta_{\tau}$（推奨：$\delta_\tau = 10^{-4}$）
  \item \textbf{ステップ数上限：}
        $m \ge N_{\tau,\max}$（推奨：$N_{\tau,\max} = 5$；反復比較経路では
        少数反復で $Q_\Gamma/Q_{\Gamma,\mathrm{ref}}$ を閾値以下まで戻すことを確認する．
        トポロジー変化直後のみ $N_{\tau,\max} = 20$ に引き上げる．
        反復数による Eikonal 誤差の減衰は §~\ref{sec:U4_ridge_eikonal_reinit}
        の単体検証で確認する）
\end{enumerate}
反復型 Eikonal 経路で物理タイムステップあたりの再初期化ステップ数（1--5 程度）が
少なすぎる場合は $Q_\Gamma$ の時系列を記録し，蓄積的な乱れを検知すること．
