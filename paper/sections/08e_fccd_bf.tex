...
圧力勾配，速度補正，HFE/GFM ジャンプ行，毛管面共鎖を同じ面の評価範囲へ写すことで，
連続平衡
...
HFE はこの GFM/アフィンジャンプ行に必要な片側 Hermite データを供給し，
$c_f=A_f B_f(j_{gl})$ または表面エネルギーから得る
Riesz 面共鎖は同じ面補正枠で速度補正へ渡す．
本稿の標準構成では GFM/アフィンジャンプ + HFE + DC 残差受理条件 +
変分毛管 Hodge 平衡判定を用いる．
固定回数 $k=3$ は §~\ref{sec:defect_correction_ppe} の成分検証での実用上限であり，
補正段階の標準受理条件は高次残差が規定許容値へ落ちることである．
...
同一面随伴ペア & PPE と補正の面複体共有 & 合格 \\
圧力ジャンプ Hodge 平衡判定 & 静的平衡でない毛管 Hodge 成分の消去 & 不合格 \\
\hline
\end{tabular}
\end{center}
\noindent 圧力ジャンプ経路の実測値は §~\ref{sec:bf_static_droplet} の
面釣合判定量で示す．そこでは未補正毛管面共鎖の非勾配成分と，
...
構成 & 内容 & 用途 \\
\hline
\textbf{A：標準アフィンジャンプ面複体} &
$D_fG_A$，$B_f(j_{gl})$，変分毛管 Riesz 面共鎖，
および成分体積制約付き鞍点系 / Hodge 平衡判定を同じ圧力仕事と随伴な面複体で構築．
...
PPE，速度補正，HFE/GFM ジャンプ行，毛管面共鎖は同じ BF 面演算子へ集約する．
標準圧力ジャンプ閉包ではアフィンジャンプ面法則と変分毛管 Hodge 平衡判定を用い，
...
\begin{itemize}[leftmargin=1.5em,noitemsep]
  \item 高次 $G_h p$，低次毛管面共鎖（切断誤差非対称）
  \item 節点 CCD 全量 $\kappa_{lg}$（界面帯ノイズ増幅）
  \item PPE $G_h$ と補正 $G_h$ が別演算子（F-2）
  \item 毛管面共鎖が節点，$G_h p$ が面（F-1）
  \item $A_f/\betaf$ 混用（PPE 調和 / 補正算術 / ジャンプ面共鎖別メトリクス；F-4）
  \item 界面越え CCD ステンシル無補正（F-5）
  \item 表面エネルギーの仮想仕事を経ない未補正曲率面共鎖を物理力として採用する
  \item 非平衡界面の毛管 Hodge 成分を値域射影で本番補正前に消す
\end{itemize}
\end{tcolorbox}
