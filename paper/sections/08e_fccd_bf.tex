...
$\kappa_{lg}$ 更新時のみ再計算．
GFM 実用閉包の BF 精度は 2 次で，H-01 診断と production face 契約に十分．
...
静止平衡は構造的厳密．
& 研究由来の圧力ジャンプ閉包．本稿の production では
\texttt{affine\_jump}+HFE+大域ゲージとして限定採用．
\\
\hline
\end{tabular}
\end{center}
...
変密度一括 PPE は比較経路に退ける；
production pressure stack = 戦略 C の affine 実装を
戦略 A/B の face-operator 整合で支える構成；
