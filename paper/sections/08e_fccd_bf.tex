...
§~\ref{sec:split_ppe_capillary_range_projection} の値域へ射影し，
補正段階に divergence-free な毛管 cochain を残さない．
...
$c_f=A_f B_f(j_{gl})$ は range projection 後に速度補正へ渡す．
本稿の標準構成では GFM/affine-jump + HFE + DC 残差契約 + range projection を用いる．
...
pressure-jump range projection & divergence-free 毛管 cochain の補正段流入 & PASS \\
\hline
\end{tabular}
\end{center}
\noindent pressure-jump 経路の実測値は §~\ref{sec:bf_static_droplet} の
face-balance gate で示す．そこでは raw 毛管 cochain の非勾配成分と，
range projection 後に補正へ渡る実効 cochain を分けて評価する．
...
$D_f A_f G_f$，$B_f(j_{gl})$，range projection を同じ face complex で構築．
...
標準 pressure-jump 閉包では affine-jump face law と range projection を用い，
...
\begin{itemize}[leftmargin=1.5em,noitemsep]
  ...
  \item raw 毛管 cochain を range projection せず補正段へ渡す
\end{itemize}
\end{tcolorbox}
