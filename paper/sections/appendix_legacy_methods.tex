...
（§\ref{sec:csf_young_laplace_app} 参照）．

\medskip
\phantomsection\label{sec:csf_young_laplace_app}
...
本付録では，第\ref{sec:collocate}章§\ref{sec:rhie_chow} で参照した
...
\begin{itemize}
  ...
  \item \textbf{$d_f = \Delta t (1/\rho)_f^{\mathrm{harm}}$（密度係数）→ 調和平均}：
    密度 $\rho$ は界面で不連続であり，フラックスの連続性から調和平均が自然に導かれる（第\ref{sec:pressure}章）．
\end{itemize}
...
係数 $(1/\rho^{n+1})_f^\mathrm{harm} = 2/(\rho^{n+1}_P + \rho^{n+1}_E)$ には現時刻密度の調和平均を用いる．

\paragraph{調和平均係数の根拠}
算術平均と調和平均の差は密度比が大きいほど顕著になる：
\begin{center}
\begin{tabular}{lccc}
\hline
...
（式~\eqref{eq:rc_divergence}）を使用しなければならない
（セル中心発散の代用はチェッカーボードの逆流を招く；第\ref{sec:pressure}章 §\ref{sec:rc_divergence} 参照）．
