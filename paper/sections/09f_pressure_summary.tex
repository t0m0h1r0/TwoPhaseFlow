...
raw compact 勾配は，この随伴性が証明されない限り標準圧力反力ではない \\
...
壁付き面速度状態 & 圧力射影後に壁速度だけを修復すると，輸送で使う面速度状態と節点速度が分裂する &
$F_w=\ker C_w$ を速度状態空間とし，$P_w^{(M)}$ と $G_w=P_w^{(A)}G_A$ を制約付き空間の受理条件とする &
圧力補正後の壁面だけの修復や全自由度への加法的 KKT 補正は標準閉包にしない \\
診断契約 & 非標準の節点曲率や部分 PPE 右辺を現在作用している演算子の量として読むと誤判定する &
作用中の面曲率，ソルバ直前 RHS，$M_A=Q_f/\alpha_f$ 上の制約付き Hodge 残差を記録する &
\texttt{kappa\_max} や CSF 型残差を圧力ジャンプ型の毛管力判定に代用しない \\
...
\[
  ...
  +R_p(q_T)\ \text{（PPE 互換の圧力反力部分空間）},\quad
   a_{\sigma,\mathrm{bal}}=M_f^{-1}(r_\sigma-\Pi^{M_f}_{R_p}r_\sigma) \\
  +F_w=\ker C_w,\quad G_w=P_w^{(A)}G_A\ \text{（壁付き面速度状態の受理条件）} \\
  +\mathrm{DC}\{\eta_{\mathrm{DC}}\le\varepsilon_{\mathrm{PPE}}\}
  +\text{大域ゲージ・再投影分離・作用中演算子の診断契約}
  \end{gathered}
  }.
\]
...
アクティブ幾何毛管分解を使う場合も，この原則は変わらない．
対象になるのは $r_\sigma$ とその圧力反力分解であり，
全セル圧力像へ $r_\sigma$ を射影してから残差を読む方法ではない．
式~\eqref{eq:ao_full_pressure_cancellation} のように同じ Schur 系で
毛管 Riesz 代表と全圧力反力を作ると，非定曲率波でも
面上駆動が $0$ になり得るため，$R_p(q_T)$ は
PPE 互換のスカラー圧力代表・境界制約・履歴面加速度を保つ部分空間として定義する．
...
壁付き計算では，この判定を全自由面空間ではなく
式~\eqref{eq:wall_constrained_face_space} の $F_w$ と
作用中の圧力計量 $M_A=Q_f/\alpha_f$ 上で読む．
圧力射影後に壁トレースだけを修復する方法，全自由度へ加法的 KKT 補正を
そのまま標準閉包とする方法，および $s_f\leftarrow L_A(s_f)$ による
一律の範囲射影置換は採用しない．
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{圧力補正後の壁面だけの修復}：
        壁トレースを消しても発散と面/節点状態の同一性を同時に保つとは限らないため，
        壁付き標準経路では $F_w=\ker C_w$ を状態空間として先に定義する．
  \item \textbf{非標準診断量の切り分け}：
        節点曲率最大値，中間段階の PPE 右辺，CSF 型平衡残差を，
        圧力ジャンプ経路の作用中の毛管力・右辺・Hodge 残差として読まない．
  \item \textbf{アクティブ幾何毛管分解の暗黙採用}：
        $R_p$ が未確定，近似誤差境界が未達，移動格子で投影済み面共鎖を保持しない，
        圧力履歴座標と現在の幾何反力を分離しない，または非静的波の毛管駆動を
        全圧力像へ過射影して消してしまう場合に，
        その分解を標準物理経路へ入れることは採用しない．
        これらを満たす最終契約だけをアクティブ幾何毛管分解として扱う．
\end{itemize}
\end{tcolorbox}
