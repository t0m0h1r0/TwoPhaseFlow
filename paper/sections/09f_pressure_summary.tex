...
界面を跨いだ通常 CCD 強行は不可 \\
毛管 cochain & $D_fa_f=0$ でも非勾配毛管成分が face 空間に残ると静止界面へ速度を注入する &
$\Pi_{\mathcal{R}_h}c_f$ による圧力勾配値域への射影 &
...
さらに毛管ジャンプ cochain $c_f=A_fB_f(j)$ は
式~\eqref{eq:capillary_range_projection} の値域射影で
$D_fA_fG_f$ 複体に閉じてから速度補正へ渡す．
