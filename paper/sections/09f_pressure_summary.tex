...
AO-Fast 幾何毛管分解を使う場合も，この原則は変わらない．
高速化されるのは $r_\sigma$ とその圧力反力分解の組み立てであり，
full cell-pressure image へ $r_\sigma$ を射影してから残差を読む方法ではない．
式~\eqref{eq:ao_full_pressure_cancellation} のように同じ Schur 系で
capillary Riesz 代表と full pressure reaction を作ると，非定曲率波でも
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{非標準診断量の切り分け}：
        節点曲率最大値，途中段階の PPE 右辺，CSF 型平衡残差を，
        圧力ジャンプ経路の作用中の毛管力・右辺・Hodge 残差として読まない．
  \item \textbf{AO-Fast の暗黙 fallback}：
        DC が残差受理条件を満たさない，PCG が式~\eqref{eq:ao_fast_error_budget} の
        誤差境界を満たさない，または wave nonzero-drive gate が不成立の場合に，
        自動で別ソルバ・別圧力空間・ゼロ駆動継続へ切り替える運用は採用しない．
        fallback を許す場合は YAML/実験記録で明示し，標準経路とは別の検証対象として扱う．
\end{itemize}
\end{tcolorbox}
