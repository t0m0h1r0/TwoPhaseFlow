...
界面を跨いだ通常 CCD 強行は不可 \\
毛管 cochain & 表面エネルギーの離散第一変分と PPE/corrector の face complex がずれると，静的液滴には寄生流れを，非平衡界面には過小駆動を生む &
surface-energy Riesz cochain $s_f$，成分体積反力 $B_h\mu$，component-corrected cochain $\widehat c_{\sigma,f}=s_f-B_h\mu$ &
曲率 cap・damping・CFL 縮小による症状処理ではない \\
高次演算子処理 & 高次評価演算子と低次補正演算子を同一視すると，設計次数と補正安定性の役割分担が崩れる &
DC 残差契約 $\eta_{\mathrm{DC}}\le\varepsilon_{\mathrm{PPE}}$: 高次評価 $L_H$ + 低次補正 $L_L$ &
...
\[
  ...
  +\mathrm{HFE} \\
  +\widehat c_{\sigma,f}\ \text{from surface-energy component saddle} \\
...
さらに毛管 cochain は pre-reinit endpoint 上の
surface-energy virtual work から $s_f$ として構成し，
同じ pressure-adjoint metric 上の component saddle で
$\widehat c_{\sigma,f}=s_f-B_h\mu$ に補正してから速度補正へ渡す．
式~\eqref{eq:capillary_range_projection} の $L_A/Z_A$ は
静的平衡 gate と原因診断のための代表であり，本番力を置換しない．

この閉包が保証するのは，PPE と速度補正が同じ面フラックス整合を共有し，
圧力ジャンプが滑らかな圧力自由度として消えないことである．
また，affine-jump 経路で物理的に仕事をする圧力は
$A_f(G_fp-B_f(j))$ の面 cochain であり，
diffuse interface band 内の raw 節点圧力は相内スカラー圧力そのものではない．
したがって snapshot のスカラー圧力は保存された面 cochain から復元した
Hodge 代表として定義し，raw 節点値は診断用の内部自由度として扱う．
界面帯を ``未定義'' として隠す表示や raw 節点値を相内圧力として読む表示は，
本章の圧力代表ではない．
...
毛管 cochain を表面エネルギー仮想仕事と component saddle で閉じ，
DC で高次演算子を扱い，
大域ゲージと再投影分離で閉じる．
