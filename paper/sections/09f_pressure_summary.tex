...
界面を跨いだ通常 CCD 強行は不可 \\
毛管面共鎖 & 表面エネルギーの離散第一変分と PPE・補正段の面複体がずれると，静的液滴には寄生流れを，非平衡界面には過小駆動を生む &
表面エネルギーの Riesz 面共鎖 $s_f$，成分体積反力 $B_h\mu$，成分補正済み面共鎖 $\widehat c_{\sigma,f}=s_f-B_h\mu$ &
...
静的平衡判定と原因診断のための代表であり，本番力を置換しない．

この閉包が保証するのは，PPE と速度補正が同じ面フラックス整合を共有し，
圧力ジャンプが滑らかな圧力自由度として消えないことである．
また，アフィンジャンプ経路で物理的に仕事をする圧力は
$A_f(G_fp-B_f(j))$ の面共鎖であり，
拡散界面帯内の未補正節点圧力は相内スカラー圧力そのものではない．
したがってスナップショットのスカラー圧力は保存された面共鎖から復元した
Hodge 代表として定義し，未補正節点値は診断用の内部自由度として扱う．
...
毛管面共鎖を表面エネルギー仮想仕事と成分体積制約付き鞍点系で閉じ，
DC で高次演算子を扱い，
大域ゲージと再投影分離で閉じる．
