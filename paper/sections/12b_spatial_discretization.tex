...
\medskip
\noindent\textbf{U1 補足：曲率の精度参考データ}

CCD operator の同時微分出力は，曲率計算（§~\ref{sec:U7_bf_static_droplet} で利用）の
精度を上界する．
曲率パイプラインの実効精度は CSF 正則化幅 $\varepsilon = \Ord{h}$ により $\Ord{h^1}$ に
律速される（参考データは旧 §~\ref{sec:verify_curvature} を統合）．
\label{sec:verify_curvature}%   backward compat (CSF 曲率パイプライン)
\phantomsection\label{sec:verify_rc_bracket}%  backward compat (C/RC ブラケット, historical)
