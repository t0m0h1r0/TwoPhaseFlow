...
\section{界面適合非一様格子：格子生成・メトリクス係数・CCD/FCCD 非一様拡張}
\label{sec:grid}
...
CCD 差分演算子（§~\ref{sec:CCD}）を適用するための座標変換理論を整備する
...
本章が標準経路として支える範囲は，
\textbf{Mode 1 固定非一様格子 + 固定 $\varepsilon$ + 静止または短時間・小変形界面}
である．
移動界面で格子を追従再生成する Mode 2 は，ALE 補正，保存的な全変数リマップ，
HFE 延長データと界面面幾何の再構築，壁面 BC での PPE--corrector 面演算子統一を
同時に満たすまでは検証ゲートであり，一般の標準経路とはみなさない．
\end{tcolorbox}
...
壁面を持つ非一様格子では，界面の曲率・物性ジャンプ層とは独立に，
非すべり壁のせん断層，接触線閉包，および投影法の半コントロールボリューム行が
壁法線方向の短い長さスケールを作る．したがって格子密度は界面項だけでなく，
非周期壁だけに作用する壁項を含む合成 monitor として定義する：
\begin{align}
...
並進対称性を破る．したがって壁項は非一様格子を明示的に選択した壁境界の場合にだけ有効であり，
一様格子または周期境界では $A_{W,a}=0$ の legacy reduction に厳密に戻る．
...
\begin{itemize}
  \item \textbf{従来定義の問題点：}
        ...
        では，格子点数 $N$ の一様計算格子幅を
        $\Delta\xi=L/(N-1)$ と書けば $\varepsilon_g = C_g C_\varepsilon \Delta\xi$ なので
        $\xi$ 空間での幅は
        \[
...
\noindent\textbf{格子生成 gate．}
非一様格子を用いる離散化は，少なくとも
$W_\xi\ge4$，
$\max_i H_{i+1}/H_i$（および逆比）の上限，
CCD で評価した $J_x,\partial_\xi J_x$ の格子収束を診断値として出力する．
台形則生成のまま高次格子収束を主張してはならず，
その場合の主張は「メトリクス評価は高次だが，座標生成は
§~\ref{sec:grid_metric_caveat} の $\Ord{\Delta\xi^2}$ 律速を受ける」に限定する．
...
\noindent\textbf{精度律速 caveat．}
\label{sec:grid_metric_caveat}
以上より，\textbf{CCD 微分評価自体は $\Ord{h^6}$ だが，
座標生成の台形則による $\Ord{\Delta\xi^2}$ 求積誤差が全体精度を律速する}．
高次格子収束を厳密に主張する場合は，ステップ3をシンプソン則（$\Ord{\Delta\xi^4}$）
または解析的 $x(\xi) = \int_0^\xi \omega^{-1}(\phi(\xi'))\,\mathrm{d}\xi'$ に置換する．
本稿の標準構成（台形則）は界面近傍の格子集中が滑らかに変化する限り
実用上十分であるが，精度主張は「CCD 微分演算子の内在精度」と
「格子生成の全体精度」を区別して記述する．
\textbf{定量目安：}$N=64$ では台形則求積誤差 $\sim 2.4\times 10^{-4}$ オーダー，
シンプソン則採用時 $\Ord{\Delta\xi^4}\sim 10^{-7}$ オーダー（界面集中分布
$\omega(\phi)$ が $C^4$ 滑らかな場合）．
...
\textbf{本稿の離散化選択（CCD による $J_x$ 評価）の代替案：}
\begin{enumerate}
  ...
  \item \textbf{高次片側差分による $J_x$ 計算：} 4次以上の差分で $J_x$ を評価することで中間的な解決策となる（導入負荷は低い）．
\end{enumerate}
格子収束テストで $\Ord{h^6}$ 収束が確認されれば，
...
各格子線上の界面帯交差数
\[
  m_x(i)=\#\{j:\ |\phi_{i,j}|<c_\Gamma\varepsilon\},\qquad
  m_y(j)=\#\{i:\ |\phi_{i,j}|<c_\Gamma\varepsilon\}
\]
を監視し，$\max_i m_x(i)$ または $\max_j m_y(j)$ が単一閉曲線の想定を超える場合は，
テンソル積非一様格子の過剰細分化モードとして記録する．
...
\subsection{格子更新モードと ALE 補正の省略：固定格子設計の根拠}
\label{sec:grid_ale}

格子は物理時間 $t=0$ における初期水準集合値 $\phi^{(0)}$ から
§~\ref{sec:grid_gen} のアルゴリズムで生成する（初期化手順全体は付録~\ref{app:bootstrap} 参照）．
...
$0.5h$ 閾値は再生成に伴う $\psi$ 転写誤差がセル幅を超える点に対応する．

\textbf{Mode 2 における場の転写：}
格子再生成時の CLS 場の転写には $\psi$ 場の\textbf{直接線形補間}を用いる．
$\phi = \varepsilon\ln[(1-\psi)/\psi]$ 経由の 3 次補間ではロジット空間でのオーバーシュートと
飽和域クリップにより質量誤差が $\Ord{10^{-2}}$（3\%）に達するのに対し，
$\psi$ 線形補間はモノトン性（$\psi \in [0,1]$ 保持）から $\Ord{10^{-5}}$（0.005\%）に抑制される
（検証は §~\ref{sec:cls_conservation}）．
格子生成自体には $\phi$ を用いるが，場の転写は常に $\psi$ を直接補間する．
ただしこれは CLS 場だけの転写規約である．
移動界面を標準閉包へ拡張するには，
$u,v,p,\delta p,\rho,\mu,\kappa_{lg},\hat{\bm n}_{lg},\phi,\psi$，
HFE 延長文脈，粘性陰解法の履歴量を同一時刻・同一格子へ保存的にリマップする設計が必要であり，
本稿の Mode 2 はこの全変数リマップを完了した経路ではない．

\textbf{Mode 2 における pressure-jump 面幾何：}
...
圧力履歴，IPC 圧力増分，HFE 延長場，界面面幾何データを前格子から補間して再利用してはならない．
速度再投影 PPE には前ステップのジャンプ条件を渡さず，
メインステップ PPE と corrector だけが再構築済みの同一界面面データを共有する
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{空間可変 Heaviside 幅 \texorpdfstring{$\varepsilon(\bx)$}{epsilon(x)} — CLS 界面厚みの非一様格子適応}
...
本稿の既定は固定 $\varepsilon$ であり，空間可変 $\varepsilon(\bx)$ は
pressure-jump/GFM 系の実験フラグに限定する．
...
$N_{\varepsilon,\text{cells}}$ が候補となるのは，\textbf{表面張力を GFM（Ghost Fluid Method）または
pressure-jump PPE で計算する場合に限られる}：デルタ関数を介さず圧力ジャンプを直接課すため，
CSF の振幅不整合を避けられる可能性がある（§~\ref{sec:pressure_accuracy_summary} 参照）．
ただし V9 は短時間・静止液滴 probe であり，長時間移動界面の汎用合格を意味しない．
ただし，座標変換メトリクスは滑らかな場の微分を非一様化する仕組みであり，
ジャンプ補正項 $B_f(j)$ を自動的に非一様化するわけではない．
pressure-jump 経路では $B_f^{\mathrm{nu}}=s_fj_f/H_f$ を
面演算子として明示的に構成し，PPE と corrector に接続する必要がある．
