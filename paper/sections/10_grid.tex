...
\section{界面適合非一様格子：固定格子幾何から高次演算子へ}
\label{sec:grid}

\noindent\textbf{本章の位置付け：}
前章までの圧力ジャンプ閉包と Balanced--Force 条件は，同じ界面面幾何を
圧力勾配，表面張力，補正速度が共有することを要求した．
非一様格子を導入する目的は，この共有面を動かさずに，
界面帯と壁近傍だけへ格子点を配分し直すことである
（解析的格子伸長関数の体系は~\cite{Vinokur1983}）．
本章では，初期界面から定まる固定の界面適合格子をまず定義し，
その格子上で座標変換，CCD/FCCD，Ridge--Eikonal が同じ幾何量を参照する流れを構成する
...
界面近傍の解像度を上げる最小構成は，Level Set 値に連動した密度関数
\begin{equation}
  \omega(\phi) = 1 + (\alpha - 1)\,\varepsilon_g\,\delta_\varepsilon^*(\phi)
\end{equation}
である．$\alpha > 1$ は界面帯の格子密度倍率，
$\delta_\varepsilon^*(\phi)$ は界面近傍に集中した正規化スムーズデルタ関数である．
$\delta_\varepsilon^*$ は $[\text{長さ}]^{-1}$ の次元を持つため，
$\varepsilon_g\,\delta_\varepsilon^*$ により無次元化する．
...
\begin{itemize}
  \item \textbf{界面幅を物理幅だけで指定する場合：}
        $\varepsilon_g = C_{g} \varepsilon$（$\varepsilon = C_\varepsilon h$）と置くと，
...
\textbf{ステップ5：} メトリクス係数を CCD 微分で評価する：
\[
...
CCD の $\Ord{h^6}$ 精度を著しく損なうため本稿では採用しない（ステップ5参照）．
...
ステップ5の CCD 評価が適切に機能していることを確認できる．
...
\textbf{Mode 2（追従格子，本稿では標準経路に含めない）：}
各タイムステップで $\phi^{(n)}$ から格子を再生成し，界面周辺の解像度を追従させる．
ただし追従格子を標準化するには，格子速度 $\bm{v}_\text{mesh}$ を含む ALE 項を
保存則へ入れる必要がある．
これを欠くと，界面変位が ${\sim}0.5h$ を超える場合は $\Ord{\Delta t}$ への精度劣化が生じる．
...
一様格子では $h_{\mathrm{local}} = h$ となり固定スカラー $\varepsilon$ と一致する．
本稿の既定は固定 $\varepsilon$ であり，空間可変 $\varepsilon(\bx)$ は
デルタ関数型体積力を用いない pressure-jump/GFM 型の閉包条件に限って検討する．
...
ただしこの主張は短時間・静止界面の範囲に限られ，長時間移動界面の汎用性を意味しない．
