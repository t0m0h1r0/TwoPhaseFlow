...
本章が標準経路として支える範囲は，
\textbf{固定非一様格子 + 固定 $\varepsilon$（CLS/CSF 界面厚み） + 静止または短時間・小変形界面}
である．
...
§~\ref{sec:grid_2d}--\ref{sec:grid_ale} ではテンソル積格子と固定格子の適用範囲を明確にし，
§~\ref{sec:eps_spatial_variable} では固定 $\varepsilon$ 標準経路から外れる空間可変幅の制約を述べる．
...
保存則へ入れる必要がある．
これを欠くと，格子の再配置そのものが保存則の欠落項として現れ，時間精度を一次へ落としうる．
...
単一の界面指示場だけを補間しても，運動量，密度，圧力履歴，界面幾何が同じ離散面を共有しないため，
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{固定 \texorpdfstring{$\varepsilon$}{epsilon} 標準経路と空間可変 Heaviside 幅の制約}
\label{sec:eps_spatial_variable}
% ─────────────────────────────────────────────────────────────────────────────

本章の標準経路では，CLS/CSF の界面厚み $\varepsilon$ は空間的に固定する．
この選択により，Heaviside/デルタ関数の振幅，圧力ジャンプ面幾何，
速度補正が同じ界面厚みを参照する．
一方，非一様格子では一様な $\varepsilon = C_\varepsilon h$ の代わりに，
局所格子幅に比例する空間可変場
\begin{equation}
...
\end{equation}
を考えることはできる（$N_{\varepsilon,\text{cells}}$ は $\xi$ 空間でのセル数）．
...
したがって本稿の既定は固定 $\varepsilon$ であり，空間可変 $\varepsilon(\bx)$ は
...
$N_{\varepsilon,\text{cells}}$ を空間可変幅として採用しうるのは，\textbf{表面張力を GFM（Ghost Fluid Method）または
