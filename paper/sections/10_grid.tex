...
本章が本文で確定するのは，
\textbf{固定 $\varepsilon_\Gamma$（CLS/CSF 界面厚み）を保った非一様格子上で，
CCD/FCCD，Ridge--Eikonal，PPE/速度補正が同じ格子幾何を参照するための離散契約}
である．
現行の検証済み非一様経路は，固定非一様格子上の静止 BF 縮約試験と，
range-projected pressure-jump/HFE/projection-native stack 上の有限時間
nominal/local-$\varepsilon$ switch 診断である（§~\ref{sec:nonuniform_grid_ns}）．
毎ステップ界面追随格子は，保存的再写像，速度再投影，履歴リセット，
HFE 延長データと界面面幾何の再構築，および壁面境界条件での
PPE--速度補正面演算子統一が同時に成立する場合にだけ標準化できる
\emph{閉包条件}として述べる．
...
そのため追従格子を採用する場合は，$\psi$ の質量を新セル体積で補正し，速度を再投影し，
...
固定格子（Mode 1）は現行検証済み縮約経路，毎ステップ再生成（Mode 2）は
上記閉包が揃う場合に限る追従格子経路である．
...
追従格子を採用する移動界面 & $\approx0.5$ & 毎ステップ追随 + 保存的再写像 \\
