...
である．
本稿で検証する非一様経路は，固定非一様格子上の静止 BF 縮約試験と，
surface-energy virtual-work pressure-jump/HFE/projection-native stack 上の有限時間
固定幅・局所幅診断である（§~\ref{sec:nonuniform_grid_ns}）．
体積，sharp phase volume，表面測度は一様計算座標ではなく
現在時刻の物理セル体積・物理 face 距離・物理界面交差位置で評価する．
...
固定格子（Mode 1）は縮約検証経路，毎ステップ再生成（Mode 2）は
