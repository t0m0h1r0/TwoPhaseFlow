...
\paragraph{本章の数式の読み方．}
非一様格子の式は，単に界面近くを細かくするレシピではない．
一様な計算座標 $\xi$ で演算子の形を保ち，物理座標 $x$ へ写すときに
計量係数とセル体積を一緒に運ぶための座標写像である．
格子密度関数 $M_a$ は「どこを細かく見るか」を決めるレンズ，
等分布式はそのレンズで見た長さを均等に割る規則，
連鎖律の式は $\xi$ 空間で得た微分を物理空間の力やフラックスへ戻す翻訳である．
したがって本章で追うべき量は，格子点の位置そのものだけでなく，
その格子上で圧力勾配，表面張力，速度補正，界面輸送が
同じ物理面と同じ体積測度を参照しているかである．
