...
本章が標準経路として支える範囲は，
\textbf{固定非一様格子 + 固定 $\varepsilon$ + 静止または短時間・小変形界面}
である．
移動界面で格子を追従再生成する経路は，ALE 補正，保存的な全変数リマップ，
...
\paragraph{格子密度デルタ関数 \texorpdfstring{$\delta_\varepsilon^*(\phi)$}{delta*eps(phi)} の定義}
...
ここで $J_x = d\xi/dx$ はメトリクス係数であり，CCD 法を用いて高精度に評価される（評価手順は §~\ref{sec:grid_gen} 段階5；CCD 理論は §~\ref{sec:CCD}）．
...
\textbf{段階2：} 各格子点で $\omega_i = \omega(\phi^{(0)}_i) = 1 + (\alpha-1)\,\varepsilon_g\,\delta_\varepsilon^*(\phi^{(0)}_i)$ を計算する．
...
格子収束テストでは，座標生成の求積次数と CCD メトリクス評価の次数を分けて評価し，
