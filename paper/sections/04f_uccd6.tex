...
$\tau_{\max}\le \Ord{h/(\sigma|a|\,\omega_{2,\max}^8)}$
（$\sigma$：UCCD6 散逸係数，$|a|$：移流速度の最大絶対値）の
厳しい拘束を課す．中程度の $\sigma$ でも実質使えず，CN との組合せが必然．
\end{remark}
...
{\footnotesize $^\star$ 微分演算子の formal 精度．DCCD では post-filter による移流フィルタ誤差 $\Ord{\varepsilon_d^\text{adv}\,h^2}$ が別途付随し，移流の実効精度は $\Ord{h^2}$ に律速される（§\ref{sec:dccd_filter} 参照）．UCCD6 は $\omega_2^8$ 重みで Nyquist 帯域のみ選択減衰し bulk $\Ord{h^6}$ を維持．}
