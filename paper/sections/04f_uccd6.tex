...
（周期格子 $\ell^2$ 内積限定；有界域での GKS 解析は §\ref{sec:future_gks} で扱う予定）．
...
\label{sec:future_gks}% C3 (CHK-212): UCCD6+CN 周期格子限定 claim から forward ref
% -------------------------------------------------------------------------------
...
厳密な Gustafsson--Kreiss--Sundstr\"om 安定性証明は本稿の対象外であるが，
数値固有値スキャン（$N\in\{16,32,64\}$，$\theta\in[0,\pi]$ の離散化）で
$\max_\lambda\Re\bigl(\lambda(\theta)\Delta t\bigr)\le 0$ を Level 2 既定の
$\Delta t_\sigma^\text{BKZ}$ 範囲で確認しており，$h^4$ 境界寄与は
bulk $\Ord{h^6}$ を侵食しない（数値検証は §\ref{sec:verify_time} 以降で計画）．
完全な GKS 解析は今後の課題とする．
...
\noindent
本稿では UCCD6 を \textbf{bulk $u,v$ 運動量移流}の既定とし，DCCD は
\textbf{CLS ($\psi$) 移流（§\ref{sec:advection_motivation}）および
再初期化圧縮項（§\ref{sec:reinit}）}に専門化する分業構造を採る．
すなわち bulk $u,v$ 移流について UCCD6 が DCCD を置換するが，
CLS 移流の既定は DCCD であり置換されない．
$\omega_2^8$ の選択的 Nyquist 散逸は DCCD の一様減衰 $H(\pi)=1-4\varepsilon_d$ より
dispersion preservation に優り，bulk 高周波の保持性で上位に位置するため，
$u,v$ 移流に対しては UCCD6 を採る；
一方 $\psi$ の急峻 tanh プロファイルでは
DCCD の一様減衰がスペクトル振動を確実に抑制する．
