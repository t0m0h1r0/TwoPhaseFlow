...
面中心化により面ジェットを供給する FCCD（§\ref{sec:fccd_def}）に対する
\textbf{第 3 の経路}（演算子内部散逸）であり，
Crank--Nicolson 時間積分との結合で無条件 $L^2$ 安定となる
（§\ref{sec:time_int}）．
...
FCCD は\textbf{面位置}に値を持つ高次補間で $\Ord{H^4}$ を保ち，
UCCD6 は\textbf{節点位置}のまま演算子内部散逸で Gibbs を抑制する．
両者は同じ基底 CCD のシンボル $\omega_1, \omega_2$ から派生し，
出力ロカス（面 vs 節点）と散逸機構（補間 vs 演算子内部）が異なるのみである．
