...
%
% §4.6  UCCD6：6 次節点中心 CCD ＋ 次数保存ハイパー粘性（SP-N）
%        旧 SP-H は 2026-04-23 リナンバで SP-N となった．
%
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{{\UCCD[6]}：次数保存ハイパー粘性付き節点中心 CCD}
\label{sec:uccd6_def}
% ═══════════════════════════════════════════════════════════════════════════════

SP-N \cite{SPN} は，Chu--Fan の CCD 演算子 $D_1^{\CCD}$ の
\textbf{6 次分散精度を保ったまま}
...
指数 5（$h^{5}(-D_2^{\CCD})^3$）では主項の次数を削り，
指数 9 では $\theta=\pi$ 近傍モードを減衰しきれない；
指数 7・ハイパー粘性 4 乗は\textbf{次数保存の最小構成}である（SP-N §3.2）．
...
従って SP-N §4 に従い，UCCD6 は全ての $\theta\in(0,\pi]$ で
...
\end{equation}
$\Re\lambda(\theta)\le 0$（式~\eqref{eq:uccd6_symbol}）より $|G(\theta)|\le 1$ が
...
中程度の $\sigma$ でも実質使えず，CN との組合せが必然（SP-N §5 Remark）．
\end{remark}
...
FCCD は Balanced-Force H-01（§\ref{sec:balanced_force}）を面配置で救済し，
UCCD6 は節点配置のまま演算子内部散逸でモーメント移流の Gibbs を抑制する．
UCCD6 はモーメント移流（§\ref{sec:advection}）で主に用いられ，
CN と組み合わせて§\ref{sec:time_int} の Level-2/3 時間積分スタックに組み込まれる．
