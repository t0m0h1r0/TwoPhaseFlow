...
界面適合（$\alpha = 2$）    & $2.3 \times 10^{-1}$ & $1.3 \times 10^{-1}$ & 0.0290 \\
\bottomrule
\end{tabular}
\end{table}
...
(a) 質量保存誤差：$\alpha = 2$ では毎ステップの補間に伴う質量損失が蓄積し 23\% に達する，
...
\begin{enumerate}[noitemsep]
  \item \textbf{質量保存：}
        一様格子（$\alpha = 1$）では $|\Delta M|/M_0 = 2.3 \times 10^{-4}$ であるのに対し，
        界面適合格子（$\alpha = 2$）では $2.3 \times 10^{-1}$（23\%）まで劣化した．
        毎ステップの格子再構築に伴う $\psi$ の線形補間が大きな質量損失を生み，
        界面重み付き補正が十分に機能していない．
        非一様格子の $h_{\min} \approx 0.029$ 近傍での補間精度低下が主因と考えられ，
        3 次補間または保存型リマッピングの導入が必要である（将来課題）．
  \item \textbf{寄生電流：}
        $\alpha = 2$ の寄生電流は $\alpha = 1$ の約 1.2 倍（$1.3 \times 10^{-1}$ vs $1.1 \times 10^{-1}$）．
...
\noindent\textbf{結論：}
毎ステップ格子リビルドの NS パイプライン統合は数値的に安定（発散なし）である．
ただし $\alpha = 2$ では線形補間に伴う質量損失が 23\% に達し，
一様格子（$2.3 \times 10^{-4}$）と比べ 3 桁劣化するため，質量保存性は未達である．
保存型リマッピングまたは高次補間の導入が将来課題である．
寄生電流の増大は $\alpha = 2$ では穏やかであり（1.2 倍），
格子追従メカニズム自体は正しく動作している．
