...
（§~\ref{sec:fccd_def}）と投影で得た正準面速度 $\bu_f$ を組み合わせ，
運動量 $(u,v)$ には演算子内部散逸の
...
$(P_f\psi)_{i+1/2}$ と，直前の投影で得た正準面速度
...
対象 & スキーム & 精度 & 根拠 \\
\hline
CLS 移流（$\psi$） & FCCD $\psi$ 面値 + 射影後の正準面速度（§~\ref{sec:fccd_def}，§~\ref{sec:advection_motivation}，§~\ref{sec:cls_stages} 段階 A） & 面値 $\Ord{H^4}$；値域制限前は保存型有限体積法の隣接面相殺構造 & 面値評価で節点 CCD 上振れを回避；総量変化時は段階 B，再初期化後は段階 F で閉じる \\
...
圧力/毛管閉包 & 二相標準は切断面圧力ジャンプ PPE + 表面エネルギー仮想仕事に基づく毛管閉包（§~\ref{sec:pressure}，§~\ref{sec:split_ppe_framework}）；一括 PPE は縮約・対照経路 & $\Ord{h^5}$--$\Ord{h^7}$† & Balanced--Force 面評価位置と射影後の面速度まで共有する面閉包 \\
\hline
\end{tabular}
\end{center}
{\small †\ DC 残差の受理条件を満たす分相 PPE 成分検証では Dirichlet $\Ord{h^7}$，Neumann BC $\Ord{h^5}$（§~\ref{sec:verify_split_ppe_dc}）．欠陥補正は残差受理条件で判定し，固定回数だけを精度保証として扱わない．速度補正の圧力勾配は CCD $\Ord{h^6}$（§~\ref{sec:pressure_accuracy_summary}）．}
\end{tcolorbox}
