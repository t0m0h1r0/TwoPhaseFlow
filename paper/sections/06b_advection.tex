...
値域制限で生じた総量誤差を戻す
（完全なアルゴリズムは 7-step フロー Step 1 に対応）．
...
対象 & スキーム & 精度 & 根拠 \\
\hline
CLS 移流（$\psi$） & FCCD（面値保存型；§~\ref{sec:fccd_def}，§~\ref{sec:advection_motivation}，§~\ref{sec:cls_stages} Stage A） & 面値 $\Ord{h^6}$；保存型 FVM テレスコーピング & 面値評価で節点 CCD overshoot を回避；非周期境界・非一様格子でも厳密質量保存 \\
