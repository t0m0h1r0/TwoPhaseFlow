...
WENO5 は本稿の主経路には用いない（§\ref{sec:verify_weno5_vs_dccd} で FCCD と WENO5 を定量比較）．\footnote{FCCD 標準化前の節点中心比較経路では帯状散逸付き節点 CCD を CLS 移流に使用していた．
当該手法は線形フィルタによる近似的安定化に依存し，非周期境界での厳密質量保存を満たさない．
FCCD への移行は保存型構造による flux-level conservation と界面安定性の両立を理由とする．
節点中心比較経路の理論的詳細は §~\ref{sec:dissipative_ccd}（§4.4）を参照．}
...
\begin{align}
  ...
  \quad\text{（FCCD 面値，$\Ord{H^4}$；\S\ref{sec:fccd_adv_primitives}）}
...
値域制限で生じた総量誤差を所定許容誤差へ戻す
（完全なアルゴリズムは本稿 7-step フロー Step 1 に対応）．
...
対象 & スキーム & 精度 & 根拠 \\
\hline
CLS 移流（$\psi$） & FCCD（面値保存型；§~\ref{sec:fccd_def}，§~\ref{sec:advection_motivation}，§~\ref{sec:cls_stages} Stage A） & 面値 $\Ord{H^4}$；clamp 前は保存型 FVM テレスコーピング & 面値評価で節点 CCD overshoot を回避；clamp 後は §~\ref{sec:dccd_mass_conservation} の質量補正で総量を所定許容誤差へ戻す \\
