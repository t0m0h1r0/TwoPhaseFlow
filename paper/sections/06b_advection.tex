...
FCCD による面ロカス拡張は §~\ref{sec:fccd_advection}（§6.2）で展開し，
...
面値 $(P_f\psi)_{i+1/2}$ を構成し，projection-native な正準面速度
...
\begin{align}
  ...
  \label{eq:dccd_adv_ccd} \\[4pt] % legacy label; active concept = FCCD face flux
...
\medskip
\noindent\textbf{質量保存性．}
clamp 前の FCCD フラックス評価は\textbf{保存型 FVM のテレスコーピング構造}を持つ：
...
§~\ref{sec:cls_stages} の Stage B が界面重み付きに総量を閉じる．
再初期化を実行した場合の最終閉鎖は Stage F の $\phi$ 空間補正に委ねる．
...
\label{alg:dccd_adv}%
TVD-RK3 + FCCD 面値演算子 + 値域制限は本稿の 7-step フロー第1段に対応し，
その後の Stage B--F は §~\ref{sec:cls_stages} の責務分離に従って適用する．
...
対象 & スキーム & 精度 & 根拠 \\
\hline
CLS 移流（$\psi$） & FCCD $\psi$ 面値 + projection-native 正準面速度（§~\ref{sec:fccd_def}，§~\ref{sec:advection_motivation}，§~\ref{sec:cls_stages} Stage A） & 面値 $\Ord{H^4}$；clamp 前は保存型 FVM テレスコーピング & 面値評価で節点 CCD overshoot を回避；総量変化時は Stage B，reinit 後は Stage F で閉じる \\
運動量移流（$u,v$） & UCCD6（§~\ref{sec:uccd6_def}） & CCD 微分 $\Ord{h^6}$；ハイパー粘性 $\Ord{\sigma h^7}$（次数保存） & 演算子内部散逸；$\omega_2^8$ 選択的 Nyquist（L2/L3 bulk 既定） \\
CLS 再初期化 & Ridge--Eikonal 補助経路（§~\ref{sec:ridge_eikonal}，§~\ref{sec:cls_compression}） & 零交差線形補間は $\Ord{h^2}$；FMM/ξ距離は距離場整合 & 飽和帯・トポロジー変化時の 1 ショット距離再構成；§3.4 の補助経路を離散手順として具体化 \\
曲率・速度微分 & CCD（§~\ref{sec:CCD}） & $\Ord{h^6}$ & 高精度・コンパクト \\
圧力/毛管閉包 & 二相標準は affine-jump PPE + capillary range projection（§~\ref{sec:pressure}，§~\ref{sec:split_ppe_framework}）；一括 PPE は縮約・対照経路 & $\Ord{h^5}$--$\Ord{h^7}$† & Balanced--Force 面ロカスと projection-native face closure \\
\hline
\end{tabular}
\end{center}
{\small †\ DC 残差契約を満たす分相 PPE 成分検証では Dirichlet $\Ord{h^7}$，Neumann BC $\Ord{h^5}$（§~\ref{sec:verify_split_ppe_dc}）．固定回数 $k=3$ は実用上限の観測値であり，標準契約ではない．速度補正の圧力勾配は CCD $\Ord{h^6}$（§~\ref{sec:pressure_accuracy_summary}）．}
\end{tcolorbox}
