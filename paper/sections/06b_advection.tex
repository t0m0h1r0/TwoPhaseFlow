...
CLS 再初期化 & Ridge--Eikonal 補助経路（§~\ref{sec:ridge_eikonal}，§~\ref{sec:cls_compression}） & 零交差線形補間は $\Ord{h^2}$；FMM/ξ距離は距離場整合 & 飽和帯・トポロジー変化時の 1 ショット距離再構成；§3.4 の補助経路を離散実装 \\
曲率・速度微分 & CCD（§~\ref{sec:CCD}） & $\Ord{h^6}$ & 高精度・コンパクト \\
圧力/毛管閉包 & 一括 PPE または affine-jump PPE + capillary range projection（§~\ref{sec:pressure}，§~\ref{sec:split_ppe_framework}） & $\Ord{h^5}$--$\Ord{h^7}$† & Balanced--Force 面ロカスと projection-native face closure \\
\hline
\end{tabular}
\end{center}
