...
対象 & スキーム & 精度 & 根拠 \\
\hline
CLS 移流（$\psi$） & FCCD $\psi$ 面値 + 射影後の正準面速度（§~\ref{sec:fccd_def}，§~\ref{sec:advection_motivation}，§~\ref{sec:cls_stages} Stage A） & 面値 $\Ord{H^4}$；clamp 前は保存型 FVM テレスコーピング & 面値評価で節点 CCD overshoot を回避；総量変化時は Stage B，reinit 後は Stage F で閉じる \\
...
圧力/毛管閉包 & 二相標準は affine-jump PPE + 変分毛管 Hodge 閉包（§~\ref{sec:pressure}，§~\ref{sec:split_ppe_framework}）；一括 PPE は縮約・対照経路 & $\Ord{h^5}$--$\Ord{h^7}$† & Balanced--Force 面評価位置と射影後の面速度まで共有する面閉包 \\
\hline
\end{tabular}
\end{center}
