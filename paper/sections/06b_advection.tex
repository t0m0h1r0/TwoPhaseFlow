...
FCCD は\textbf{保存型有限体積法の隣接面相殺構造}を持ち，
...
\medskip
\noindent\textbf{質量保存性．}
値域制限前の FCCD フラックス評価は\textbf{保存型有限体積法の隣接面相殺構造}を持つ：
...
圧力/毛管閉包 & 二相標準はアフィンジャンプ PPE + 変分毛管 Hodge 閉包（§~\ref{sec:pressure}，§~\ref{sec:split_ppe_framework}）；一括 PPE は縮約・対照経路 & $\Ord{h^5}$--$\Ord{h^7}$† & Balanced--Force 面評価位置と射影後の面速度まで共有する面閉包 \\
\hline
\end{tabular}
\end{center}
{\small †\ DC 残差の受理条件を満たす分相 PPE 成分検証では Dirichlet $\Ord{h^7}$，Neumann BC $\Ord{h^5}$（§~\ref{sec:verify_split_ppe_dc}）．固定回数 $k=3$ は実用上限の観測値であり，標準前提ではない．速度補正の圧力勾配は CCD $\Ord{h^6}$（§~\ref{sec:pressure_accuracy_summary}）．}
\end{tcolorbox}
