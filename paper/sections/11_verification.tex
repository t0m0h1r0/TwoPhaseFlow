...
本章は単相 NS ソルバの物理的整合性（§~\ref{sec:force_balance}--§~\ref{sec:ns_time_accuracy}）に加え，
現行実装の範囲内で実施可能な\textbf{低密度比二相流時間積分}（§~\ref{sec:advection_pressure_coupling}，
$\rho_l/\rho_g \leq 10$）と，smoothed Heaviside 一括解法の
\textbf{破綻限界の定量化}（§~\ref{sec:interface_crossing}）を対象とする．
高密度比（$\rho_l/\rho_g \gg 10$）での界面通過検証は GFM 実装後に実施し
（§~\ref{sec:future_work}），第~\ref{sec:validation}章の物理ベンチマークへ接続する．
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{§~\ref{sec:ns_time_accuracy}（総合精度）：}
        NS 方程式の時間・空間収束次数．
  \item \textbf{§~\ref{sec:advection_pressure_coupling}（二相流時間積分）：}
        CLS↔PPE 結合の整合性（$\rho_l/\rho_g \leq 10$，多ステップ積分）．
  \item \textbf{§~\ref{sec:interface_crossing}（変密度 PPE 破綻限界）：}
        smoothed Heaviside 一括解法の適用限界の定量化と GFM 必要性の数値的根拠．
  \item \textbf{§~\ref{sec:error_budget}（精度バジェット）：}
        全コンポーネントの精度を統合し，律速段階と現行実装の限界を特定する．
\end{enumerate}
...
\begin{tabular}{p{38mm}p{38mm}p{30mm}p{22mm}}
\toprule
検証項目 & 理論値/解析解 & チェック方法 & 許容基準 \\
\midrule
静水圧バランス & $p = \rho g z$，$\bu = \bm{0}$ & $\|\bu\|_\infty$ & $< 10^{-12}$\\
Laplace 圧均衡（単ステップ） & $\Delta p = \sigma/R$ & $\Delta p$ 相対誤差 & $< 1\%$（$N \geq 128$）\\
...
二相流時間積分（$\rho_l/\rho_g \leq 5$） & CLS 質量保存，div$(\bu)=0$ & 時系列 & 表~\ref{tab:twophase_ti_criteria}\\
PPE 破綻限界（$\rho_l/\rho_g = 2$--$100$） & DC 収束・発散の閾値 & 残差履歴 & 表~\ref{tab:varrho_breakdown_sweep}\\
\bottomrule
\end{tabular}
\end{table}
