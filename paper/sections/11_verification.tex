...
\begin{enumerate}[noitemsep]
  ...
  \item[\textbf{第~\ref{sec:validation}章}]
    \textbf{二相流ベンチマーク} — 公表参照解との定量比較（GFM 統合後に実施）
\end{enumerate}
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{§~\ref{sec:advection_pressure_coupling}（二相流結合）：}
        CLS↔PPE 結合の物理的整合性（$\rho_l/\rho_g \leq 5$）と
        Galilean 不変性．
  \item \textbf{§~\ref{sec:interface_crossing}（一括解法の適用限界）：}
        smoothed Heaviside の破綻閾値と GFM 必要性の数値的根拠．
  \item \textbf{§~\ref{sec:error_budget}（精度バジェット）：}
        全コンポーネントの実測精度を統合し，律速段階と GFM 導入の展望を示す．
\end{enumerate}
...
GFM 精度回復 & 密度比非依存 & 格子収束 & $\Ord{h^2}$ 確認\\
\bottomrule
\end{tabular}
\end{table}
