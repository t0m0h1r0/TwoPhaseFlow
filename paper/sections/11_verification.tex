...
本章は主に\textbf{単相ナビエ・ストークスソルバ}の物理的整合性を対象とする．
ただし §~\ref{sec:bf_static_droplet} では変密度 PPE + CSF による
Laplace 圧均衡（$\rho_l/\rho_g = 1000$）を単ステップ投影レベルで検証する．
時間積分を伴う二相流の質量保存性および寄生流れ収束は，
第~\ref{sec:validation}章にて定量的に検証する．
...
Laplace 圧均衡 & $\Delta p = \sigma/R$ & $\Delta p$ 相対誤差 & $< 1\%$（$N \geq 128$）\\
