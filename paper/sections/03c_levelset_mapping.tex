...
\medskip
\textbf{逆変換（$\psi \to \phi$）：}
CLS 法の移流計算は $\psi$ で行い，曲率は $\psi$ 微分から直接計算する．
一方，法線ベクトルや HFE など $\phi$ を明示的に使う処理では，
以下の解析的逆変換で $\psi$ と整合する $\phi$ を復元する．
...
曲率計算に寄与しない．通常はロジット式を直接評価し，$\psi$ を
$[\psi_{\min},1-\psi_{\min}]$ にクリップして $|\phi|\le\phi_{\max}$ に飽和させるだけで十分である．
万一の診断用補助として Newton 更新式を書くと：
\begin{equation}
...
\end{equation}
である．この反復は界面帯域内で初期値が十分近く，
$H_\varepsilon'(\phi)$ が小さすぎない場合に局所二次収束を示す
（付録~\ref{app:newton_quad_conv}）．
飽和域では導関数が小さく Newton 更新が不安定になりうるため，
本稿の実装では Newton 反復ではなくロジット式とクリップ処理を標準とする．
飽和クランプを含む具体的な実装手順は §~\ref{sec:algorithm}（Step 3）を参照．
