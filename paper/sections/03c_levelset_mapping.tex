...
Conservative Level Set（CLS）変数 $\psi$ と符号付き距離関数 $\phi$ は以下の関係にある~\cite{OlssonKreiss2005, Desjardins2008}：
\begin{equation}
  \psi(\bx,t) \equiv H_\varepsilon(-\phi(\bx,t)), \qquad
  \bnabla\psi = -\delta_\varepsilon(\phi)\,\bnabla\phi
\end{equation}
物性値補間 $\rho(\psi), \mu(\psi)$ には $\psi$ を直接用いる．
曲率 $\kappa$ は標準経路では $\psi$ の微分値から直接計算する
（定理~\ref{thm:curvature_invariance}，§~\ref{sec:curvature_direct_psi_cls}）．
$\phi$ は HFE（Hermite 場延長法；§~\ref{sec:field_extension}），壁面幾何，
および距離再構成後の $\psi$ 再生成に必要な補助量として用いる．

\medskip
\textbf{逆変換（$\psi \to \phi$）：}
CLS 法の移流計算と標準曲率計算は $\psi$ で行うが，HFE・壁面幾何・距離再構成後の再生成には $\phi$ が必要である．
$\psi=H_\varepsilon(-\phi)=1/(1+e^{\phi/\varepsilon})$ はシグモイド型関数であり，
その逆関数（ロジット関数）は\textbf{解析的に求まる}：
\begin{equation}
  \phi = -H_\varepsilon^{-1}(\psi) = \varepsilon\ln\!\left(\frac{1-\psi}{\psi}\right)
  \label{eq:logit_inverse}
\end{equation}
$\psi(\phi)$ は単調減少関数（$d\psi/d\phi < 0$）であるから，解は一意に定まる．
計算上の注意は，$\psi \to 0$ または $\psi \to 1$ の\textbf{飽和領域}で
...
$\phi = \varepsilon\ln((1-\psi)/\psi)$
によって点ごとに $\Ord{1}$ で実行できる．
$\psi(\phi)=H_\varepsilon(-\phi)$ は狭義単調減少であるため逆関数は一意に定まり，
...
曲率計算に寄与しない．
本稿ではロジット逆変換を局所的な $\psi\to\phi$ 写像の\textbf{標準手順}とし，
飽和制限 $|\phi| \le \phi_{\max}$ でオーバーフローを防ぐ
（アルゴリズム上は第~\ref{sec:algo_flow}節の界面幾何・物性更新段に位置する）．
...
\hline
計算式 & $\phi = \varepsilon\ln((1-\psi)/\psi)$（解析的） & $\partial\phi/\partial\tau + \mathrm{sgn}(\phi_0)(|\bnabla\phi|-1)=0$ \\
...
本稿の $\psi$ は液相で $1$，気相で $0$ なので，
$\bnabla\psi$ は気相から液相を向く．
したがって $\hat{\bm n}_{lg}=-\bnabla\psi/|\bnabla\psi|$ として
式~\eqref{eq:curvature_2d} を $\psi$ で書き換えた
\begin{equation}
  \kappa_{lg} = -\frac{
    \psi_y^2\,\psi_{xx}
...
\end{equation}
は連続・非飽和領域では式~\eqref{eq:curvature_2d} と同一の
液相$\to$気相向き曲率を与える
...
% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{実効界面幅 \texorpdfstring{$\varepsilon_{\textup{eff}}$}{ε\_eff} によるプロファイル品質監視}
% ─────────────────────────────────────────────────────────────────────────────

$\psi$ による直接曲率計算では $\varepsilon$ の値は不要であるが，
数値拡散や再初期化の不完全さによるプロファイル歪みを
\textbf{診断}する目的で，各格子点における実効界面幅を定義できる：
\begin{equation}
...
$\phi$ 経由で曲率または HFE 法線を評価する場合は，再初期化の不完全性により
分母 $|\bnabla\phi|$ が局所的に小さくなるリスクがある．
この場合は分母下限制限（式~\eqref{eq:curvature_clamped}，$\varepsilon_{\text{norm}} = 10^{-3}$）で
$\kappa$ を有界に保つ．ただし標準の $\psi$ 直接曲率で第一に監視すべきものは
Eikonal 条件そのものではなく，界面帯が単調な $H_\varepsilon(-\phi)$ 型プロファイルとして
保たれているかである；$\phi$/HFE 経路では Eikonal 品質が法線精度の保証となる．

\medskip
以上で CLS 法の基本定式化（保存形移流・再初期化・$\psi$--$\phi$ 写像・曲率の直接計算）が整った．
