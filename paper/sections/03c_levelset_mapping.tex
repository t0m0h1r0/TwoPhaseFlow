...
分相 PPE 解法で使用する HFE（§~\ref{sec:field_extension}）で
...
曲率計算に寄与しない．ロジット逆変換を主手法とし，Newton 更新式は診断・補助用途に限って利用できる：
\begin{equation}
...
\end{equation}
この反復の二次収束は，根に十分近い初期値を用い，
$H_\varepsilon'(\phi)$ が小さすぎない局所領域でのみ成り立つ（付録~\ref{app:newton_quad_conv}）．
飽和クランプはオーバーフローを防ぐ実装上の防御策であり，
単独で大域的な二次収束を保証するものではない．
...
（ただし界面近傍で $\psi$ が滑らかな単調プロファイルを保つことを前提とする）．
