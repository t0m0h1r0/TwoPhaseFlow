...
変数 & 特性 & 推奨スキーム \\
\hline
$\psi$（CLS）& $C^0$ 急 tanh & Dissipative CCD（CCD 微分 + 後段スペクトル散逸；\S\ref{sec:advection_motivation}） \\
...
\begin{itemize}
  ...
  \item \textbf{bulk 速度は CCD 推奨．}
        ...
        DCCD または UCCD6（\S\ref{sec:uccd6_def}）が実装デフォルト推奨．
...
\noindent\textbf{本章の構成}：
\S\ref{sec:advection_motivation} で CLS 移流 DCCD（Stage A），
\S\ref{sec:fccd_advection} で FCCD 移流 Option B/C，
