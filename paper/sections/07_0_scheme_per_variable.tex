...
%
% §6.0  変数別スキーム方針
%        §6 冒頭に配置し, 後続の §6.1 (CLS 移流) / §6.3 (FCCD 移流) /
%        §6.6 (粘性 3 層) の選択根拠を先取りする．
%
% ═══════════════════════════════════════════════════════════════════════════════
...
変数 & 特性 & 推奨スキーム \\
\hline
$\psi$（CLS）& $C^0$ 急 tanh & FCCD（面値保存型；\texttt{FCCDLevelSetAdvection}；ch13 production；\S\ref{sec:advection_motivation}） \\
...
\begin{itemize}
  \item \textbf{$\psi$ に節点 CCD を適用してはならない．}
        ...
        振動源を切り，\texttt{FCCDLevelSetAdvection} を実装デフォルトとする．
        旧実装の DCCD スペクトル散逸（\S\ref{sec:dccd_derivation}）はレガシー位置づけ．
  ...
  \item \textbf{bulk 速度は CCD 推奨．}
        ...
        （\texttt{UCCD6ConvectionTerm}）．DCCD は legacy 位置づけ．
...
\begin{equation*}
  ...
  \!\to\!\mathbf{u}^*\!\to\!\text{PPE}_{n+1}\!\to\!\mathbf{u}^{n+1}
\end{equation*}
（詳細は \S\ref{sec:algorithm}）．

\noindent\textbf{本章の構成}：
\S\ref{sec:advection_motivation} で CLS 移流 FCCD（Stage A），
\S\ref{sec:fccd_advection} で FCCD/UCCD6 運動量移流，
\S\ref{sec:cls_compression} で CLS 再初期化（Ridge-Eikonal 主軸），
\S\ref{sec:cls_stages} で CLS A--F 6 段階，
\S\ref{sec:viscous_3layer} で粘性 3 層アーキテクチャ．
