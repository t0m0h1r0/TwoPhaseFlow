...
%
% §4.7  面ジェット $\FaceJet{u}$：基底 CCD 出力の面入出力形式（FCCD と等価）
%        本節では数学的定義のみを与える；消費先章での具体使用は章末まとめ §\ref{sec:ccd_summary} を参照．
%
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{\texorpdfstring{面ジェット $\FaceJet{u}$：基底 CCD 出力の面入出力形式（FCCD と等価）}{面ジェット J\_f(u)：基底 CCD 出力の面入出力形式（FCCD と等価）}}
...
\]
を\textbf{面ジェット}として定義する．
構成式は §\ref{sec:fccd_def} の FCCD 公式と数式上\textbf{等価}であり，
FCCD はこの面入出力形式を面中心スキームとして直接定式化した形態と見なせる；
この位置づけは，compact Hermite データから評価点を移す際にも
同じ修正波数・同じ補間整合性を追跡する Lele 型 compact 解析
~\cite{Lele1992,ChuFan1998} の文脈で読む．
UCCD6（§\ref{sec:uccd6_def}）など他の節点中心 CCD 派生も
この面入出力形式を経由して面位置の表現を得る．
本節では数学的定義と各成分の数値的意味のみを与え，
後続章（§\ref{sec:time_int} 以降）ではこの入出力形式を
\textbf{入力前提}として再利用する；
...
この入出力形式の核心である．成分ごとに別ステンシル・別補間を用いると
微分演算子の離散的随伴性 $\langle D_h u, v\rangle = -\langle u, D_h^{*} v\rangle$ が
破れ，下流の連立離散において自己整合性を失う；
離散随伴性と運動エネルギー整合を保つ非圧縮離散の一般原理は
Verstappen--Veldman~\cite{VerstappenVeldman2003} も参照．
面ジェットはこの整合性を\textbf{受け渡し形式の段階}で保証する．

% -------------------------------------------------------------------------------
\subsubsection{公開入出力形式としての設計帰結}
\label{sec:face_jet_design_consequences}
% -------------------------------------------------------------------------------

\begin{itemize}
  \item \textbf{面ジェットは公開入出力形式．}
        呼び出し側は $(u_f, u'_f, u''_f)$ の 3 成分だけを仮定し，
        それらが基底 CCD の Hermite ペアから構成されたものか，
        等価な面中心連立系から得られたものかには依存しない．
  \item \textbf{不連続な生の場に面値式を直接適用しない．}
        ジャンプ処理なしの $u_f$ 適用は，
        $H^2 q$ 項（式~\eqref{eq:face_value_formula} 第 2 項）が
        不連続を Gibbs 振動として輸送する；
        この入出力形式の数値的妥当性は被作用関数の $C^k$ 滑らかさに依存する．
  \item \textbf{連立構造の局所性．}
...
その出力を統一する 3 成分の面入出力形式である面ジェット $\FaceJet{u}=(u_f,u'_f,u''_f)$ を導出した．
...
\noindent
{\footnotesize
\begin{tabularx}{\linewidth}{@{}l l >{\raggedright\arraybackslash}X l >{\raggedright\arraybackslash}X l@{}}
\hline
...
{\DCCD} (§\ref{sec:dccd_derivation})     & 節点      & 外殻後置フィルタ              & $\Ord{h^6}$$^\ddagger$ & スペクトル非増幅（係数拘束下）   & $\Ord{N}$ \\
{\FCCD} (§\ref{sec:fccd_def})            & \textbf{面}$^\flat$ & なし（散逸ゼロ）          & $\Ord{H^4}$           & 基底 CCD 由来               & $\Ord{N}$ \\
{\UCCD[6]} (§\ref{sec:uccd6_def})        & 節点      & \textbf{演算子内部}（ハイパー粘性） & $\Ord{h^6}$  & CN で無条件安定（周期格子）    & $\Ord{N}$ \\
$\FaceJet{u}$ (§\ref{sec:face_jet_def})  & 面（入出力形式） & ---                           & $\Ord{H^4}$/$\Ord{H^2}$$^\S$ & 基底スキーム継承   & $\Ord{N}$ 後処理 \\
\hline
\end{tabularx}}
...
後置フィルタによる移流フィルタ誤差 $\Ord{\varepsilon_d^{\mathrm{adv}} h^2}$ が別途付随し
...
本章で導入した演算子・入出力形式は以下の章で再利用される（章名と節番号のみ；
個別機構の詳細は各章で展開）：

\begin{itemize}\setlength{\itemsep}{0pt}
  \item \S\ref{sec:time_int}（時間積分）：UCCD6 の安定化機構を IMEX-BDF2/implicit-BDF2 + DC の時間更新へ接続する．
...
\begin{description}\setlength{\itemsep}{0pt}
  ...
  \item[DCCD] Dissipative CCD（外殻スペクトル後置フィルタ付加）（§\ref{sec:dccd_derivation}）
  ...
  \item[面ジェット $\FaceJet{u}$] $(u_f, u'_f, u''_f)$ を一括して提供する 3 成分の面入出力形式（§\ref{sec:face_jet_def}）
\end{description}
