...
$\betaf^\text{harm}$ は \textbf{$1/\rho$ の調和平均}（厳密形式）：
$\bigl[\bigl(1/\rho_L+1/\rho_R\bigr)/2\bigr]^{-1}=2\rho_L\rho_R/(\rho_L+\rho_R)$．
他方，2 値の場合は\textbf{密度の算術平均の逆数}とも書ける：
$\bigl[(\rho_L+\rho_R)/2\bigr]^{-1}=2/(\rho_L+\rho_R)$．
両者の差は $(\rho_L-\rho_R)^2/(2\bar\rho(\rho_L+\rho_R))$ オーダーであり，
$\Delta\rho/\bar\rho \to 0$ の等密度極限では $\Ord{(\Delta\rho/\bar\rho)^2}$ で一致する一方，
$\rho_l/\rho_g\sim 10^3$ では絶対値で大きく乖離する点に注意．
本稿は FVM フェイス係数連続性（§\ref{sec:bf_p4_beta}）と整合する
\textbf{密度算術平均の逆数} $2/(\rho_L+\rho_R)$ を採用し，
表記 $\betaf^\text{harm}$ は慣用に従う．
$n\ge 3$ 値では両者が分離するため
（§\ref{app:fvm_face_coeff} 参照），定義の取違いに注意．
...
片側だけフィルタは非対称減衰を生み P-1 を事後的に破壊する：
P-5 は対称適用時に「次数改善・設計予防」として機能するが，
非対称適用時は P-1 違反として F-1/F-2 失敗モードに直接寄与する．
この意味で P-5 は P-1 維持の必要条件として連動する
...
% -------------------------------------------------------------------------------
\subsubsection{Rhie--Chow の限定使用（F-2/F-4 失敗モードへの該当）}
\label{sec:bf_rhie_chow_restriction}
% -------------------------------------------------------------------------------

\noindent
本項は P-1--P-7 原則の独立スロットではなく，
\textbf{失敗モード F-2（演算子非整合）と F-4（ジャンプ平滑化）が
Rhie--Chow（RC）補間で同時発現する典型例}として位置付ける副節である．
P-1--P-7 と F-1--F-5 の対応は次の §\ref{sec:bf_failure_principle_map} にまとめる．
...
\textbf{RC 単独の Balanced-Force 経路は禁止}（プロダクションの BF 圧力補正には使用しない）．
\textbf{ただし}，§\ref{sec:collocate} の RC + CSF 拡張（§\ref{sec:rc_balanced_force}）で
\textbf{曲率・$\betaf$ 同一演算子適用}を満たす\textbf{拡張 RC--CSF}は，
本稿のコロケート Level 2（\S\ref{sec:level_selection} 参照）既定の BF 経路として独立に成立する．
すなわち\textbf{RC 単独 = 不可，RC + CSF 整合拡張 = 既定}であり，
この区別を曖昧にすると F-2/F-4 を踏む．
本稿の C/RC--DCCD 拡張（§\ref{sec:rc_high_order}）は
さらに\textbf{高周波散逸の面中心修正}として位置付け，
面フラックス整合を補強する．
