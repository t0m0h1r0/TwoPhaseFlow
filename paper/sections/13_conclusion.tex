...
変密度 PPE の収束悪化 & GFM 分相解法（§\ref{sec:gfm}，§\ref{sec:varrho_ppe_limitation}）\\
...
最近接点 Hermite 補間（§\ref{sec:gfm}）により Gibbs 振動は除去されるが，
CSF $O(\varepsilon^2)$ モデル誤差の根本的除去には GFM 分相解法（§\ref{sec:future_work}）が必要である（付録~\ref{app:csf_full}）；
...
\begin{description}
  ...
  \item[\textbf{Ghost Fluid Method（GFM）による界面表現の鋭利化：}]
        GFM の導入は，精度向上と変密度 PPE 収束性の双方の観点から\textbf{最優先課題}である．
        ...
        CSF の界面厚み $\varepsilon$ に起因する $\Ord{\Delta x^2}$ モデル誤差を原理的に排除する．
        ...
        界面のジャンプ条件を最近接点 Hermite 補間（§\ref{sec:extension_pde}）で課すため，
        不連続係数を横切るステンシルが排除され，
