...
CCD $O(h^6)$ による高精度曲率計算，界面越し場延長による高精度な界面処理，
...
\begin{description}
  \item[\textbf{第\ref{sec:governing}章：支配方程式と One-Fluid 定式化}]
        ...
        CSF 体積力形式（付録~\ref{app:csf_full}）と界面越し場延長（§\ref{sec:field_extension}）の
        両面から整理した．
  ...
  \item[\textbf{第\ref{sec:collocate}章：圧力--速度連成と DCCD 散逸}]
        ...
        界面越し場延長（§\ref{sec:field_extension}）により CCD の Gibbs 振動が除去される下で，
...
最近接点 Hermite 補間（§\ref{sec:field_extension}）により Gibbs 振動は除去されるが，
...
\begin{description}
  ...
  \item[\textbf{分相 PPE 解法による変密度 PPE の収束性回復：}]
        ...
        各相で定数密度 PPE（$\rho_k^{-1}\nabla^2 p = q$）を解き，
        界面のジャンプ条件を最近接点 Hermite 補間（§\ref{sec:field_extension}）で課すことで，
        不連続係数を横切るステンシルが排除され，
