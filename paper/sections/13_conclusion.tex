...
\begin{description}
  ...
  \item[\textbf{Ghost Fluid Method（GFM）による界面表現の鋭利化：}]
        §\ref{sec:pressure_accuracy_summary} で整理した通り，
        CSF モデルの近似誤差 $\Ord{\varepsilon^2} \approx \Ord{\Delta x^2}$ が
        本稿の全空間コンポーネント（CCD-PPE $\Ord{h^6}$，CCD 移流 $\Ord{h^6}$）に対する
        律速段階となっており，界面モデルの改善が精度向上の主要な鍵である．
        GFM~\cite{Kang2000} は圧力ジャンプ条件 $[p] = \sigma\kappa$ を
        PPE 離散化式にゴースト値として直接組み込み，
        界面 $\Gamma$ を数学的不連続面として扱うことで，
        CSF の人工的な界面厚み $\varepsilon$ に起因する $\Ord{\Delta x^2}$ モデル誤差を
        原理的に排除する．
        CCD の高次ステンシルはゴースト領域において $C^\infty$ 滑らかな場を参照するため，
        界面直近まで $\Ord{h^6}$ 精度を維持でき，
        寄生流れの CSF 律速成分を除去できる理論的可能性を持つ．
