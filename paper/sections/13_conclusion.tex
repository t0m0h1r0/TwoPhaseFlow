...
CCD $O(h^6)$ による高精度曲率計算，GFM による鋭利な界面処理，
...
\begin{description}
  \item[\textbf{第\ref{sec:governing}章：支配方程式と One-Fluid 定式化}]
        ...
        表面張力の離散化には Ghost Fluid Method（GFM，§\ref{sec:gfm}）を採用し，
        界面ジャンプ条件 $[p]_\Gamma=\kappa/We$ を PPE 右辺に直接組み込む定式化を示した
        （CSF モデルの完全定式化は付録~\ref{app:csf_full}）．
  ...
  \item[\textbf{第\ref{sec:collocate}章：圧力--速度連成と DCCD 散逸}]
        ...
        GFM 定式化（§\ref{sec:gfm}）により CSF 体積力が除去される下で，
  ...
  \item[\textbf{第\ref{sec:pressure}章：変密度圧力 Poisson 方程式}]
        ...
        変係数 PPE $\nabla\cdot(1/\rho\,\nabla p) = (1/\Delta t)\nabla\cdot\bu^*$ を導出し，
        \textbf{欠陥補正法}（§\ref{sec:defect_correction_main}）を
        本稿の標準求解手法として定式化した：
        CCD 演算子（$\Ord{h^6}$）で残差を評価し，
        低次 FD 演算子の方向分離スイープで $\mathcal{O}(N)$ コストの補正を行う．
        実装詳細は付録~\ref{sec:ppe_pseudotime} に示す．
...
主要な精度設計上のトレードオフとして，
（1）空間精度は CCD-PPE $\Ord{h^6}$・CCD $\Ord{h^6}$・Dissipative CCD $O(\varepsilon_d h^2)$（フィルタ律速；低波数近似 $H\approx1-\varepsilon_d k^2h^2$ より導出）と高次で統一され，
GFM 採用により CSF $O(\varepsilon^2)$ モデル誤差が除去された（§\ref{sec:gfm}，付録~\ref{app:csf_full}）；
