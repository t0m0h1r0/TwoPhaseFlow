...
\subsubsection{時間変化格子と ALE 効果（本稿では未実装）}
\label{app:grid_ale}

本付録は §\ref{sec:grid_gen} の格子が時間発展する場合の ALE（Arbitrary Lagrangian-Eulerian）補正を説明する．
\textbf{本稿の標準経路では格子を時間固定として扱う（$\bm{v}_\text{mesh} = 0$）}．
時間変化格子を標準経路へ含めるには，§\ref{sec:grid_ale} の
離散幾何保存則，ALE 保存更新，全変数再写像，および界面面データ再構築ゲートを全て満たす必要がある．
...
したがって，標準経路ではこの経路を有効化せず，上昇気泡ベンチマークなど大変形問題では
一様格子，固定適合格子の有効時間窓，または ALE 閉包済み追従格子を別途検証する．
...
長時間・長距離移動を伴う計算では，一様格子を選択するか，Mode 2 の全ゲートを満たす別経路として検証する．
...
さらに，Mode 2 は $\psi$ 転写だけで閉じない．
$u,v,p,\delta p,\rho,\mu,\kappa_{lg},\hat{\bm n}_{lg},\phi,\psi$ の保存的リマップと，
HFE 延長文脈・jump face cache の現在格子上での再構築を同時に満たす必要がある．
これらが欠ける場合，格子追従の利得より operator mismatch が支配的になりうる．
したがって本稿では，Mode 2 を正規の精度主張には含めず，
式~\eqref{eq:ale_grid_gcl}--\eqref{eq:regrid_face_geometry} を満たす実装が揃うまで fail-closed とする．
