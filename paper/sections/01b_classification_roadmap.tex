...
\begin{description}
  ...
  \item[\textbf{Conservative Level Set（CLS）法}~\cite{OlssonKreiss2005,OlssonKreissZahedi2007}（\textbf{本稿採用}）]
        ...
        高次 compact 離散と CLS を結ぶ既存研究としては，Compact Conservative Level Set
        （CCLS）法~\cite{MoghadamShafieefarPanahi2016} もあり，
        本稿の CCD/FCCD 利用はこの系譜と問題意識を共有する．
\end{description}
...
Ridge--Eikonal 再初期化・保存形フラックス・体積閉鎖の手順が追加で必要なため，
...
同じ Balanced--Force の問題意識は Level Set 系にも展開されており，
Herrmann~\cite{Herrmann2008} は refined level set grid と曲率評価を組み合わせて
寄生流れを低減する構成を示した．
...
GFM と Extension PDE の面ジェット統合（HFE，§~\ref{sec:field_extension} で定式化）を CCD 高次精度の枠内で定式化する．
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{分相圧力ジャンプ PPE + DC + 面上補正量}：
        ...
        各相内で欠陥補正法を残差受理条件まで適用して高次の圧力補正を得る．
        固定緩和の反復を増やすことは受理条件ではなく，
        各補正は作用中の PPE 演算子に対する残差低下で判定する．
...
\end{enumerate}
高密度比の移動界面では，界面を運ぶ質量フラックスと運動量を運ぶフラックスが
離散的にずれるだけで非物理的な運動量交換が生じる．
この問題は VOF 系では Rudman~\cite{Rudman1998} の一貫質量・運動量輸送，
Level Set 系では Raessi--Pitsch~\cite{RaessiPitsch2012} の
同一フラックス密度による保存形運動量輸送として整理されている．
本稿の保存形共通フラックス分岐はこの系譜に属するが，
CLS の $\psi$ 面フラックス台帳から密度と運動量を同時に更新し，
その後の PPE・毛管補正へ同じ射影後面速度を渡す点を設計契約として明示する．
...
（§~\ref{sec:U6_split_ppe_dc_hfe} では完全テンソル構成で 2D 低次化が解消する）．
以上により，界面近傍の場延長，圧力補正，速度補正を同じ離散構造で読める定式化を実現する．

本稿の毛管閉包は，標準の圧力ジャンプ NS 結合経路と，鋭い界面極限へ向かう
PhaseRegion 正規経路を明確に分けて扱う．
標準経路では $j_{gl}=-\sigma\kappa_{lg}$ を切断面ジャンプとして PPE と補正段へ渡す．
一方，PhaseRegion 正規経路は，物理状態を
相領域 $\Omega_h$ とその境界 atlas $\Gamma_h=\partial\Omega_h$ に置き，
$q_h=Q_h(\Omega_h)$ を測度，$\phi$ やグラフ高さを chart/gauge として読む．
表面張力は $E_h[\Omega_h]=\sigma\,\mathrm{Per}(\Omega_h)$ の第一変分であり，
節点曲率ではなく面速度空間上の共変量として構成する．
そのうち圧力反力で吸収できない成分だけを毛管加速度として残すことを狙う．
ただし，この分解は単に $Q_h(\phi)=q_T$ を満たす
Young--Laplace 残差を計算するだけでは成立しない．
全セル $q_T$ を絶対視したり，全セル圧力像を圧力反力として許したりすると，
輸送ノイズを曲率へ増幅するか，非平衡波の面上駆動を代数的に消す．
したがって本稿では，圧力反力部分空間 $R_p$ を PPE と同じ面計量・同じ圧力代表・同じ境界制約から先に定義し，
$r_\sigma-\Pi^{M_f}_{R_p}r_\sigma$ を物理的な毛管駆動として読む
（§~\ref{sec:capillary_pressure_reaction_subspace}）．
PhaseRegion/InterfaceAtlas 状態空間，$q_T=Q_h(R_h)+r_h$ の残差分解，
アクティブ集合，移動格子で保存する面量，圧力履歴座標，
DC 収束判定，近似誤差境界は
§~\ref{sec:ao_fast_state_space} で数理条件として述べる．
この条件を満たさない分解は，ゼロ駆動継続や標準経路への暗黙混入を許さず，
事前ゲートで採用条件を満たさないと判定する．
U12/V11 は全圧力像への過射影を棄却し，
第~\ref{sec:benchmarks}章ではこの最終契約を満たす
PhaseRegion 正規経路を毛管ベンチマークへ用いる．
...
5  & CLS 再初期化
   ...
   & 2, 3, 4 \\
6  & 方程式項別空間離散
   & CLS $\psi$ 移流，運動量移流，保存形共通フラックス，粘性 3 層，物性補間
   & 3, 4, 5 \\
7  & 時間積分
   & CLS は TVD-RK3，NS は EXT2/AB2（IMEX-BDF2）+ IPC，粘性は陰的 BDF2 + DC，時間刻み制約
   & 2, 3, 4, 6 \\
8  & 圧力・速度連成
...
10 & 界面適合格子
   ...
   & 3, 4, 6, 8, 9 \\
11 & ソルバ統合
   & 演算子マッピング，1 タイムステップ手順，保存形共通フラックス分岐，純 FCCD DNS 構成
   & 4--10 \\
12 & コンポーネント検証
...
13 & 統合検証
   ...
   & 12 \\
14 & 多相流物理検証
   & PhaseRegion 正規ルートの毛管波・振動液滴 chart 診断と，既存 NS 物理ベンチマーク；長時間・高 We/低 We の定量拡張は §~\ref{sec:future_work} の課題
   & 13 \\
15 & まとめ
   & 技術的貢献の整理，今後の拡張方向
   & --- \\
...
\paragraph{1--14章の数式読解ロードマップ．}
本稿の数式は，章ごとに別々の技巧を積み上げるためではなく，
界面を運ぶ量，界面を測る量，圧力・毛管仕事を釣り合わせる量を
段階的に接続するために導入する．
以下を各章の読解軸として先に持っておくと，式の目的を見失いにくい．
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  \item 第1章：二相流の困難を，界面台帳・幾何台帳・圧力仕事台帳の不一致として読む．
  \item 第2章：$\phi,\psi,\kappa_{lg},j_{gl}$ を，全章で使う符号と物理量の辞書として読む．
  \item 第3章：$\psi$ は保存形に運ぶ量，$\phi$ は法線・曲率を読む物差しとして分けて読む．
  \item 第4章：CCD 族は高次公式の集合ではなく，狭い3点窓で値・傾き・曲がりを整合させる連立系として読む．
  \item 第5章：再初期化は界面の物理移動ではなく，移流後の形を測り直せるプロファイルへ戻す幾何射影として読む．
  \item 第6章：演算子選択は精度順ではなく，場の滑らかさと出力位置に対する適材適所として読む．
  \item 第7章：時間積分は公式の選択ではなく，陽的に進める項と同時刻で閉じる拘束の仕分けとして読む．
  \item 第8章：圧力は値そのものより，中間速度から発散成分を抜く面上反力として読む．
  \item 第9章：PPE は密度ジャンプ，圧力ジャンプ，HFE，DC を同じ速度補正面へ届ける反力構成として読む．
  \item 第10章：非一様格子は細かい格子の配置ではなく，計算座標の演算を物理面・物理体積へ翻訳する写像として読む．
  \item 第11章：7段更新は計算順序だけでなく，PhaseRegion，運動量，圧力仕事へ所有者が移る台帳遷移として読む．
  \item 第12章：U 系列は最終ソルバの証明ではなく，部品ごとの式・離散化・指標の契約検査として読む．
  \item 第13章：V 系列は部品を組んだとき，どの物理制約が残るかを失敗モード別に読む．
  \item 第14章：ベンチマークは見た目の再現ではなく，保存性，初期応答，有界性，形状発展の同時診断として読む．
\end{enumerate}
...
以降の章を読み進める前に，各時間ステップで何が行われるかの手順の地図を把握しておくことが
不可欠である．アルゴリズム全体は（1）界面移流・再初期化・物性更新・曲率/界面ジャンプデータ構成という
界面部分系（ステップ1--4）と，（2）予測・PPE 求解・補正という
流体力学部分系（ステップ5--7）の7ステップで構成される．
両部分系は物性値 $\rho,\mu$，圧力ジャンプ $j_{gl}$，
および面上毛管補正を介して結合する．
PhaseRegion 正規経路は，この標準7段更新の後処理ではなく，
界面状態そのものを所有し直す発展的経路として位置付ける．
すなわち，相領域 $\Omega_h$ と境界 atlas $\Gamma_h=\partial\Omega_h$ を主状態とし，
$q_h=Q_h(\Omega_h)$ を測度，$\phi,\psi$ を派生ゲージとして作り直したうえで，
その同じ $\Omega_h$ から毛管分解を構成する．
各ステップの離散方程式・受理条件・収束判定の詳細は，
...
\begin{tikzpicture}[
  ...
  flowbox/.style={rounded corners=5pt, font=\scriptsize\bfseries, inner sep=4pt,
                  minimum width=31mm, minimum height=15mm, text width=28mm,
                  align=center},
  iface/.style={flowbox, draw=navy!70, fill=lightblue},
  capstd/.style={flowbox, draw=red2!70, fill=red2!8},
  capdev/.style={flowbox, draw=darkgray!75, fill=darkgray!7},
  gate/.style={flowbox, draw=darkgray!75, fill=darkgray!7},
  fluid/.style={flowbox, draw=red2!70, fill=red2!8},
  arr/.style={-Stealth, thick, darkgray!80},
  dev/.style={-Stealth, thick, darkgray!70, dotted, line width=1.1pt},
  lbl/.style={font=\scriptsize\bfseries, text=darkgray},
]

%% ── 列見出し ─────────────────────────────────────────────────────────
\node[font=\normalsize\bfseries, text=navy] (hdI) at (0, 0)
  {輸送};
\node[font=\normalsize\bfseries, text=navy] (hdR) at (37mm, 0)
  {界面再構成};
\node[font=\normalsize\bfseries, text=red2] (hdC) at (74mm, 0)
  {毛管量};
\node[font=\normalsize\bfseries, text=red2] (hdF) at (111mm, 0)
  {更新・採用};

%% ── 標準経路 ─────────────────────────────────────────────────────────
\node[iface, below=8mm of hdI] (s1)
  {1.\ \ CLS 移流\\[1pt]\footnotesize（$\psi$）};
\node[iface, below=8mm of hdR] (s2)
  {2.\ \ 標準再初期化\\[1pt]\footnotesize（$\phi,\psi$）};
\node[capstd, below=8mm of hdC] (s3)
  {3--4.\ \ 標準毛管量\\[1pt]\footnotesize（$\rho,\mu,j_{gl}$）};
\node[fluid, below=8mm of hdF] (s4)
  {5--7.\ \ 流体更新\\[1pt]\footnotesize（予測・PPE・補正）};

%% ── PhaseRegion 候補 ───────────────────────────────────────────────
\node[iface, below=11mm of s1] (a1)
  {A.\ \ PhaseRegion\\[1pt]\footnotesize（$\Omega_h$）};
\node[capdev, below=11mm of s2] (a2)
  {B.\ \ 境界 atlas\\[1pt]\footnotesize（$\Gamma_h$）};
\node[capdev, below=11mm of s3] (a3)
  {C.\ \ 毛管分解\\[1pt]\footnotesize（$Q_h,E_h,r_\sigma,R_p$）};
\node[gate, below=11mm of s4] (a4)
  {D.\ \ 事前ゲート\\[1pt]\footnotesize（U12/V11）};
...
\draw[dev] (a1) -- (a2);
\draw[dev] (a2) -- node[lbl, below]{派生ゲージ} (a3);
\draw[dev] (a3) -- node[lbl, below]{採用条件} (a4);
\draw[dev] (a4.north) -- node[lbl, right]{合格時のみ} (s4.south);

\node[lbl, above=2mm of s4.east, anchor=west] {$t^n \to t^{n+1}$};

\end{tikzpicture}
\caption{1 タイムステップの処理フロー}
\label{fig:algo_flow}
\PaperCaptionNote{概念図．上段の実線は本稿の標準7段更新を役割ごとに束ねた経路であり，CLS 移流，Ridge--Eikonal 再初期化，物性・標準毛管量の確定，流体更新の順に進む．下段の点線は PhaseRegion 正規経路である．この経路は圧力ジャンプの後処理ではなく，$\Omega_h$ を主状態として所有し，境界 atlas $\Gamma_h$ から測度 $q_h=Q_h(\Omega_h)$ と派生ゲージ $\phi,\psi$ を読む界面再構成として始まる．そのうえで，$q_T=Q_h(\Omega_h)+r_h$，毛管共変量 $r_\sigma$，圧力反力部分空間 $R_p$，面 Hodge で加速度へ写した補間，移動格子で保存する面量，圧力履歴座標を満たす毛管分解を構成する．この採用境界を通過した経路だけを流体更新へ接続する．}
\end{figure}
