...
本稿の毛管閉包は，標準の圧力ジャンプ経路と，鋭い界面極限へ向かう
アクティブ幾何毛管分解を明確に分けて扱う．
標準経路では $j_{gl}=-\sigma\kappa_{lg}$ を切断面ジャンプとして PPE と補正段へ渡す．
一方，アクティブ幾何毛管分解は表面エネルギーの仮想仕事を
節点曲率ではなく面速度空間上の共変量として直接構成し，
そのうち圧力反力で吸収できない成分だけを毛管加速度として残すことを狙う．
ただし，この分解は単に Young--Laplace 残差を計算するだけでは成立しない．
全セル圧力像を圧力反力として許すと，表面エネルギー Riesz 代表と
圧力反力が同じ Schur 系に落ち，非平衡波であっても面上駆動が代数的に消える．
...
幾何セル分率状態空間，互換射影，アクティブ集合，移動格子で保存する面量，
圧力履歴座標，DC 収束判定，近似誤差境界は
...
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  ...
  \item 第11章：7段更新は計算順序だけでなく，所有者が界面，運動量，圧力仕事へ移る台帳遷移として読む．
...
および面上毛管補正を介して結合する．
アクティブ幾何経路は，この標準7段更新の後処理ではなく，
再初期化に近い界面状態再構成として位置付ける．
すなわち，幾何セル分率 $q_C$ を主状態として輸送し，
互換界面 $\Gamma_h$ から派生ゲージ $\phi,\psi$ を作り直したうえで，
その幾何から毛管分解を構成する発展的経路である．
...
\node[iface, below=11mm of s1] (a1)
  {A.\ \ $q_C$ 輸送\\[1pt]\footnotesize（幾何セル分率）};
\node[capdev, below=11mm of s2] (a2)
  {B.\ \ 互換界面\\[1pt]\footnotesize（$\Gamma_h$）};
\node[capdev, below=11mm of s3] (a3)
  {C.\ \ 幾何毛管分解\\[1pt]\footnotesize（$\phi,\psi,r_\sigma,R_p$）};
...
\label{fig:algo_flow}
\PaperCaptionNote{概念図．上段の実線は本稿の標準7段更新を役割ごとに束ねた経路であり，CLS 移流，Ridge--Eikonal 再初期化，物性・標準毛管量の確定，流体更新の順に進む．下段の点線はアクティブ幾何経路である．この経路は圧力ジャンプの後処理ではなく，$q_C$ を主状態として輸送し，互換界面 $\Gamma_h$ から派生ゲージ $\phi,\psi$ を作り直す界面再構成として始まる．そのうえで，$Q_h(\phi)=q$，毛管共変量 $r_\sigma$，圧力反力部分空間 $R_p$，面 Hodge で加速度へ写した補間，移動格子で保存する面量，圧力履歴座標を満たす幾何毛管分解を構成する．この採用境界を通過した経路だけを流体更新へ接続する．}
\end{figure}
