...
3  & CLS 法
   ...
   & 2 \\
4  & CCD 法
   & 記号と略語・技術要素一覧（冒頭ロードマップ），6 次精度コンパクト差分，係数導出，DCCD（散逸強度設計・適応制御），UCCD6
   & --- \\
5  & 時間積分
...
9  & 界面適合格子
   ...
   & 3, 4, 6, 7, 8 \\
10 & 完全アルゴリズム
   & 離散演算子マッピング・7ステップフロー・タイムステップ制御・PPE 解法 routing (等密度検証/変密度一括/分相) 集約・GPU 二路選択・純 FCCD DC 収束率
   & 全章 \\
11 & コンポーネント検証
...
\begin{enumerate}
  ...
  \item \textbf{空間離散化スキーム}（第~\ref{sec:CCD}, \ref{sec:advection}, \ref{sec:collocate}, \ref{sec:ppe_solve}, \ref{sec:grid} 章；表~\ref{tab:chapter_overview} の第 4 章・第 6--9 章に相当）：
        CCD 法，CLS 移流・再初期化，Rhie--Chow 補正，分相 PPE，界面適合格子
