...
\label{fig:algo_flow}
\PaperCaptionNote{概念図．上段の実線は本稿の標準7段更新を役割ごとに束ねた経路であり，CLS 移流，Ridge--Eikonal 再初期化，物性・標準毛管量の確定，流体更新の順に進む．下段の点線はアクティブ幾何経路である．この経路は圧力ジャンプの後処理ではなく，$q_C$ を主状態として輸送し，互換界面 $\Gamma_h$ から派生ゲージ $\phi,\psi$ を作り直す界面再構成として始まる．そのうえで，$Q_h(\phi)=q$，毛管共変量 $r_\sigma$，圧力反力部分空間 $R_p$，移動格子面共鎖，圧力履歴座標を満たす幾何毛管分解を構成する．この採用境界を通過した経路だけを流体更新へ接続する．}
\end{figure}
