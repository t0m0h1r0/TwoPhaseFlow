...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{分相 PPE + 欠陥補正法 + CCD 勾配}：
        ...
        平滑領域で高次の離散化整合性を確保する（§~\ref{sec:ppe_discretization_choice}；統合検証は第~\ref{sec:verification}章）．
...
\end{enumerate}
さらに，高密度比対応の分相 PPE 解法（§~\ref{sec:split_ppe_motivation}；第~\ref{sec:verification}章で検証）の基盤技術として，
...
10 & 界面適合格子
   ...
   & 3, 4, 6, 8, 9 \\
11 & ソルバ統合
   & 演算子マッピング，1 タイムステップ手順（7 ステップ），DCCD ブートストラップ，純 FCCD DNS アーキテクチャ
   & 4--10 \\
12 & コンポーネント検証
