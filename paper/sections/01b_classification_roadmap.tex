...
その高速化に必要な幾何セル分率状態空間，互換射影，アクティブ集合，
PCG/DC，GPU 契約，YAML/fail-close は §~\ref{sec:ao_fast_state_space} に分離して述べる．
この条件を満たさない AO-Fast 実装は，PCG/DC への暗黙 fallback や
ゼロ駆動継続ではなく，事前ゲートで fail-close させる．
標準経路の統合実験は第~\ref{sec:verification}章と第~\ref{sec:benchmarks}章で扱い，
AO-Fast 経路は U12/V11 の事前ゲートを通過するまで発展的構成として扱う．
...
\node[boxr, right=18mm of s3] (s5)
  {5.\ \ 運動量予測\\[1pt]\footnotesize（粘性・重力・履歴項）};
...
\label{fig:algo_flow}
\PaperCaptionNote{概念図．界面部分系（CLS 移流・Ridge--Eikonal 再初期化・物性更新・曲率/ジャンプデータ構成，ステップ1--4）と流体力学部分系（予測・PPE・補正，ステップ5--7）が，物性値 $\rho,\,\mu$，曲率 $\kappa$，および圧力ジャンプ面データを通じて結合している．各ステップの離散方程式・受理条件は，第~\ref{sec:algorithm}章を入り口として第II部の対応章で述べる．}
\end{figure}
