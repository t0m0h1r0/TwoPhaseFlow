...
全 cell-pressure image を圧力反力として許すと，表面エネルギー Riesz 代表と
...
\begin{tikzpicture}[
  ...
  flowbox/.style={rounded corners=5pt, font=\scriptsize\bfseries, inner sep=4pt,
                  minimum width=28mm, minimum height=14mm, text width=25mm,
                  align=center},
...
\node[font=\normalsize\bfseries, text=navy] (hdI) at (0, 0)
  {界面輸送};
\node[font=\normalsize\bfseries, text=navy] (hdR) at (33mm, 0)
  {界面再構成};
\node[font=\normalsize\bfseries, text=red2] (hdC) at (66mm, 0)
  {界面量・毛管};
\node[font=\normalsize\bfseries, text=red2] (hdF) at (99mm, 0)
  {流体更新};

%% ── 標準経路 ─────────────────────────────────────────────────────────
\node[iface, below=8mm of hdI] (s1)
  {1.\ \ CLS 移流\\[1pt]\footnotesize（$\psi$）};
\node[iface, right=4mm of s1] (s2)
  {2.\ \ 標準再初期化\\[1pt]\footnotesize（$\phi,\psi$）};
\node[capstd, right=4mm of s2] (s3)
  {3--4.\ \ 標準界面量\\[1pt]\footnotesize（$\rho,\mu,j_{gl}$）};
\node[fluid, right=4mm of s3] (s4)
  {5--7.\ \ 流体更新\\[1pt]\footnotesize（予測・PPE・補正）};

%% ── アクティブ幾何候補 ───────────────────────────────────────────────
\node[iface, below=9mm of s1] (a1)
  {発展的輸送\\[1pt]\footnotesize（$q_C$）};
\node[capdev, right=4mm of a1] (a2)
  {幾何互換再構成\\[1pt]\footnotesize（$q_C\!\to\!\Gamma_h\!\to\!\phi,\psi$）};
\node[capdev, right=4mm of a2] (a3)
  {幾何毛管分解\\[1pt]\footnotesize（$r_\sigma,\,R_p$）};
\node[fluid, right=4mm of a3] (a4)
  {流体更新候補\\[1pt]\footnotesize（採用後のみ）};

%% ── 経路 ─────────────────────────────────────────────────────────────
\draw[arr] (s1) -- (s2);
\draw[arr] (s2) -- (s3);
\draw[arr] (s3) -- node[lbl, above]{$\rho,\mu,j_{gl}$} (s4);

\draw[dev] (a1) -- (a2);
\draw[dev] (a2) -- node[lbl, below]{派生ゲージ} (a3);
\draw[dev] (a3) -- node[lbl, below]{U12/V11} (a4);
\draw[dev] (s1.south) -- node[lbl, left]{発展的} (a1.north);
\draw[dev] (a2.north) -- node[lbl, right]{$\phi,\psi$} (s2.south);
\draw[dev] (a4.north) -- node[lbl, right]{採用後} (s4.south);
