...
幾何セル分率状態空間，互換射影，アクティブ集合，近似誤差境界は
§~\ref{sec:ao_fast_state_space} で数理条件として述べる．
この条件を満たさない分解は，ゼロ駆動継続や標準経路への暗黙混入を許さず，
事前ゲートで未受理と判定する．
標準経路の統合実験は第~\ref{sec:verification}章と第~\ref{sec:benchmarks}章で扱い，
アクティブ幾何毛管分解は U12/V11 の事前ゲートを通過するまで発展的構成として扱う．
...
\label{fig:algo_flow}
\PaperCaptionNote{概念図．上段の実線は本稿の標準7段更新を役割ごとに束ねた経路であり，CLS 移流，Ridge--Eikonal 再初期化，物性・標準毛管量の確定，流体更新の順に進む．下段の点線は発展的なアクティブ幾何経路である．この経路は圧力ジャンプの後処理ではなく，$q_C$ を主状態として輸送し，互換界面 $\Gamma_h$ から派生ゲージ $\phi,\psi$ を作り直す界面再構成として始まる．そのうえで，$Q_h(\phi)=q$，毛管共変量 $r_\sigma$，圧力反力部分空間 $R_p$ を満たす幾何毛管分解を構成する．U12/V11 の採用境界を通過した場合だけ流体更新へ接続し，それまでは標準物理経路へ混入させない．}
\end{figure}
