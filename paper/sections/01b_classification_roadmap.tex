...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{分相圧力ジャンプ PPE + DC + 面共鎖投影}：
        ...
        表面エネルギー仮想仕事と成分体積制約付き鞍点系を含む圧力ジャンプ閉包は V6/V7/V9 で，
        一様格子・NS 非連成 CLS 強変形移流は V10-a/b で確認し，
        非一様格子上の移動界面 CLS と長時間・完全連成 NS 移動界面の収束性は
        §~\ref{sec:future_work} の今後の検証対象に残し，
...
\end{enumerate}
さらに，高密度比対応の分相 PPE 解法（§~\ref{sec:split_ppe_motivation}；§~\ref{sec:density_ratio_sweep}〔V6〕で密度比 $\rho_r\le 833$ までの結合閉包系の安定性を確認）の基盤技術として，
...
\begin{enumerate}
  ...
  \item \textbf{完全アルゴリズムの実装}：
        第~\ref{sec:algorithm}章で 7 ステップ手順として統合し，各項の離散化は Part 2 の対応章で確認する
  ...
  \item \textbf{統合検証}（第~\ref{sec:verification}章）：
        U1--U9 で確立した基本検証済み要素を V1--V10 の単相・二相・非一様・CLS 強変形テストへ接続し，
...
\label{fig:algo_flow}
\PaperCaptionNote{概念図．界面サブシステム（CLS 移流・Ridge--Eikonal 再初期化・物性更新・曲率/ジャンプデータ構成，ステップ1--4）と流体力学ソルバー（予測・PPE・補正，ステップ5--7）が，物性値 $\rho,\,\mu$，曲率 $\kappa$，および圧力ジャンプ面データを通じて結合している．各ステップの離散方程式・実装詳細は，第~\ref{sec:algorithm}章を入り口として Part 2 の対応章で述べる．}
\end{figure}
