...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{分相 pressure-jump PPE + DC + face-space 投影}：
        高密度比向けの主たる設計戦略として向き付き pressure-jump 分相 PPE を採用し，
        各相内で欠陥補正法（DC $k=3$）を適用して高次の圧力補正を得る．
        速度補正・浮力残差・界面移流は同じ face-space contract
        $D_fA_fG_f$ と projection-native な正準面速度で閉じ，
        affine jump の履歴圧力も面加速度として持ち越す．
        一括 CSF + Balanced--Force 縮約経路（§~\ref{sec:balanced_force}）では
...
13 & 統合検証
   ...
   & 12 \\
14 & 多相流ベンチマーク
   & 毛管波・振動液滴・気泡上昇・Rayleigh--Taylor smoke の物理検証；長時間・高 We/低 We の定量拡張は §~\ref{sec:future_work} の課題
   & 13 \\
15 & まとめ
   & 技術的貢献の整理，今後の拡張方向
   & --- \\
