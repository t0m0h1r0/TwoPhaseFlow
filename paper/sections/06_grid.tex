...
\noindent\textbf{CCD の $\Ord{h^6}$ 出力精度が保証される前提条件：}
CCD は入力配列 $\{x_i\}$ が $C^8$ 級（8階微分可能かつ有界）である場合に
$\Ord{h^6}$ 精度の微分を保証する（打ち切り誤差は $f^{(8)}_i$ に依存，式~\eqref{eq:CCD_TE_II}）．
本稿の座標配列 $x(\xi)$ は前進矩形則（ステップ3）による $\omega(\phi)$ の累積積分として生成されるが，
$\omega(\phi)$ は $\delta_\varepsilon^*(\bar\phi^x)$ の滑らか（$C^\infty$）な関数であるため，
$x(\xi)$ は高階微分まで有界な滑らかな関数となる．
累積和の打ち切り誤差 $\Ord{\Delta\xi}$ は $\{x_i\}$ の滑らかさには影響しない．
したがって CCD の $\Ord{h^6}$ 精度保証の平滑性前提は満たされる．
ただし CCD の $\Ord{h^6}$ 精度は入力値が厳密な関数値である場合に保証される；
前進矩形則の大域誤差 $\Ord{h}$ を含む $\{x_i\}$ に対しては，CCD が返すメトリクス係数の実効精度は $\Ord{h}$ に制限される
（全体スキームの精度 $\Ord{h^2}$ との整合は取れている）．
中心差分による $\Ord{h^2}$ 近似 $J_x|_i \approx 2\Delta\xi/(x_{i+1}-x_{i-1})$ は
CCD の空間精度 $\Ord{h^6}$ を著しく損なうため，本稿では採用しない
...
ただし，界面が初期精細化領域から大きく離れると経時的に解像度が失われる（目安：界面変位量 $\delta \gtrsim 0.5\,h$ で顕在化；詳細：付録~\ref{app:grid_ale}）．
...
（条件と回避策については付録~\ref{app:grid_ale} 参照；本稿では ALE 補正は未実装）．
\end{tcolorbox}
...
格子を毎タイムステップ更新しつつ ALE 補正を省略した場合の $\Ord{\Delta t}$ 精度劣化についても付録~\ref{app:grid_ale} で詳述する．
ALE 定式化が必要となる条件（大密度比・高速界面変形）についても同付録を参照されたい．
