...
\begin{enumerate}
  ...
  \item 以後の時間発展では，界面が大きく移動した場合に格子を再生成することができる
        （その際はステップ1の $\phi^{(0)}$ を最新の $\phi^n$ で置き換える）．
\end{enumerate}

\textbf{$x$ 方向格子：}
\begin{enumerate}
  \item $x$ 方向の密度関数 $\omega^x_i = 1 + (\alpha-1)\,\varepsilon_g\,\delta_\varepsilon^*(\bar{\phi}^x_i)$ を計算する．
...
\textbf{$y$ 方向格子：}
\begin{enumerate}
  \item $y$ 方向の密度関数 $\omega^y_j = 1 + (\alpha-1)\,\varepsilon_g\,\delta_\varepsilon^*(\bar{\phi}^y_j)$ を計算する．
...
格子は物理時間 $t=0$ における初期水準集合値 $\phi^{(0)}$（問題定義として与えられた Level Set 場）から
§~\ref{sec:grid_gen} のアルゴリズムで生成する．
この「\textbf{格子ブートストラップ}」操作——初期 $\phi^{(0)}$ を用いて一度だけ格子を構築すること——は
シミュレーション開始前に一度だけ実行され，以降の時間発展では格子を更新しない：
