...
\begin{tcolorbox}[resultbox, title={座標変換公式（厳密形）}]
...
\]
すなわち，通常は物理場 $f(\xi_i)$ を入力する CCD ソルバーに，座標配列 $f(\xi_i) \leftarrow x_i$，一様格子幅 $\Delta\xi = \xi_{i+1}-\xi_i$（$= L/(N-1)$）を渡す．ソルバーは1・2階微分を同時に返すので，$dx/d\xi$ と $d^2x/d\xi^2$ が一度の Thomas 法求解で $\Ord{h^6}$ 精度で得られる
...
$x(\xi)$ は高階微分まで有界な滑らかな関数となる（累積和の打ち切り誤差 $\Ord{\Delta\xi}$ は
$\{x_i\}$ の滑らかさには影響しない）．
...
\S\ref{sec:grid_gen} のアルゴリズムで生成する．
...
が，界面が初期精細化領域から大きく離れると経時的に解像度が失われる
（目安：界面変位量 $\delta \gtrsim 0.5\,h$ で顕在化；詳細：付録~\ref{app:grid_ale}）．
...
格子を毎タイムステップ更新しつつ ALE 補正を省略した場合の $\Ord{\Delta t}$ 精度劣化，および
