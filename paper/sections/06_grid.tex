...
$x(\xi)$ は高階微分まで有界な滑らかな関数となる（累積和の打ち切り誤差 $\mathcal{O}(\Delta\xi)$ は
$\{x_i\}$ の滑らかさには影響しない）．
したがって CCD の $\mathcal{O}(h^6)$ 精度保証の平滑性前提は満たされる．
ただし CCD の $\mathcal{O}(h^6)$ 精度は入力値が厳密な関数値である場合に保証される；
前進矩形則の大域誤差 $\mathcal{O}(h)$ を含む $\{x_i\}$ に対しては，CCD が返すメトリクス係数の実効精度は $\mathcal{O}(h)$ に制限される
（全体スキームの精度 $\mathcal{O}(h^2)$ との整合は取れている）．
...
ALE 誤差はゼロであり，全体時間精度 $O(\Delta t^2)$ が保持される．
...
界面の移動速度が大きい場合（$\delta/h > 0.5$）は $O(\Delta t)$ への精度劣化が生じる
（条件と回避策については付録~\ref{app:grid_ale} 参照；本稿では ALE 補正は未実装）．
\end{tcolorbox}

本稿の標準実装は \textbf{Mode 1}（\ref{box:grid_modes}）であり，全体時間精度 $O(\Delta t^2)$ が保持される．
