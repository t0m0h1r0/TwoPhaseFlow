...
\begin{equation}
  ...
  \label{eq:grid_delta}
\end{equation}
ここで $\varepsilon_g$ は界面近傍の格子細分化幅（有効半幅）を制御するパラメータであり，中点値は $\varepsilon_g\,\delta_\varepsilon^*(0) = 1/\sqrt{\pi} \approx 0.564$ となる（式~\eqref{eq:grid_delta}）．
...
$\omega(\phi) > 1$ となる界面近傍では $\Delta\tilde{x} = \Delta\xi/\omega < \Delta\xi$ となり物理格子幅が細くなり（$\omega$ 倍の格子密度），
