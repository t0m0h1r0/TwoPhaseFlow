...
以下に，前進矩形則（左端点則）による累積和の具体的な手順を示す．

\begin{tcolorbox}[algbox, title={1次元格子点生成アルゴリズム（前進矩形則累積和）}]
...
\textbf{ステップ3：} 前進矩形則（左端点則）で累積和を計算し，物理座標の「未正規化」値を求める：
\[
  \tilde{x}_0 = 0, \qquad
...
\noindent\textbf{CCD $\Ord{h^6}$ 精度の平滑性条件．}
...
$x(\xi)$ 自体は $C^\infty$ 級の滑らかな関数として生成される——累積和の打ち切り誤差 $\Ord{\Delta\xi}$ は
$\{x_i\}$ の滑らかさに影響しない．
