...
\end{equation}
%
を独立に解き，界面のジャンプ条件 $[p] = \sigma\kappa$ は
...
\begin{enumerate}[noitemsep]
  \item 各相で定数密度の PPE を独立に解くため，
        界面を横切るステンシルが\textbf{完全に排除}される
  \item DC $k{=}3$ は各相内で $\Ord{h^7}$（Dirichlet）/
        $\Ord{h^5}$（Neumann BC 律速）を達成する
  \item 上記精度は\textbf{全密度比}（$\rho_l/\rho_g = 1$--$1000$）で不変である
        （§~\ref{sec:verify_split_ppe_dc} で実証）
...
圧力場を HFE の $\Ord{h^6}$ Hermite 補間で滑らかに延長することで，
