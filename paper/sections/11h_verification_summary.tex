...
\begin{enumerate}
  \item \textbf{空間離散化：}
        ...
        Rhie--Chow 補正ブラケットは C/RC 形式で $\Ord{h^4}$ を達成し，
        標準形 $\Ord{h^2}$ に対して $N{=}128$ で 14{,}229 倍の精度向上を確認した（§\ref{sec:verify_rc_bracket}）．
  ...
  \item \textbf{曲率計算：}
        ...
        HFE は $\Ord{h^6}$ を達成するため，界面近傍の律速は
  ...
  \item \textbf{界面処理：}
        ...
        HFE（§~\ref{sec:extension_pde_disc}）への切り替えにより
        1 次元で $\Ord{h^6}$，2 次元テンソル積で $\Ord{h^3}$ が達成された
        （表~\ref{tab:hfe_2d_convergence}）．
        Young--Laplace 静止液滴テストでは上風・Hermite 両パイプラインで完全一致し，
        $N = 128$ で相対誤差 $2.2\times10^{-3}$ を達成した（§\ref{sec:verify_field_extension}）．
  \item \textbf{PPE 求解：}
...
Rhie--Chow C/RC ブラケット & $\Ord{h^4}$ & $\Ord{h^{4.0}}$ & 標準 $\Ord{h^2}$ 比 14{,}229 倍（$N{=}128$） \\
...
場延長・ベースライン（1D）& $\Ord{h^1}$ & $\Ord{h^{1.0}}$ & 上風差分；Hermite 補間†で $\Ord{h^6}$ へ改善（次行参照） \\
HFE 2D（製造解） & $\Ord{h^6}$† & $\Ord{h^{3.0}}$ & テンソル積逐次補間；$N{=}256$ で $7.5 \times 10^{-9}$ \\
HFE + CSF（Laplace） & $\Ord{h^6}$† & 相対誤差 $2.2\times10^{-3}$ & $N{=}128{,}256$ で単調収束；DCCD 曲率フィルタ有効 \\
...
\bottomrule
\multicolumn{4}{l}{\footnotesize †HFE（§~\ref{sec:extension_pde_disc}）採用時の設計精度；
  ベースライン上風差分は $\Ord{h^1}$（表第1行参照）．} \\
\end{tabular}
\end{table}

\begin{tcolorbox}[resultbox, title={コンポーネント検証の総括}]
全 13 コンポーネントが設計精度を達成した：
CCD 微分は $\Ord{h^{6.0}}$，PPE 欠陥補正（$k{=}3$）は Dirichlet で $\Ord{h^{7.0}}$，
TVD-RK3 は $\Ord{\Delta t^{3.0}}$，AB2 は $\Ord{\Delta t^{2.0}}$ をそれぞれ確認した．
CLS 移流は質量保存誤差 $\Ord{10^{-5}}$（$N{=}256$），
HFE は 2D で $\Ord{h^{3.0}}$ を達成し，
いずれも目標精度を満たしている．
\end{tcolorbox}
