...
\begin{enumerate}
  ...
  \item \textbf{DCCD 移流特性：}
        ...
        不連続プロファイルでは CCD と同程度の TV（$17.14$ vs $10.96$）を示すが，
...
CCD 微分（非一様格子）    & $\Ord{h^6}$        & $\Ord{h^6}$（5.86--6.65）      & \checkmark & GCL 合格（$2.4 \times 10^{-14}$） \\
...
CLS 保存型再マッピング     & LS 比改善          & $\Ord{10^{-16}}$（$K{\leq}20$）& \checkmark & LS は全条件で ${\sim}20\%$ \\
...
Young--Laplace              & $\Delta p = 4.0$   & 相対誤差 $0.2$--$0.3\%$        & \checkmark & $N{=}32$--$128$，CSF 飽和 \\
...
NS 毎ステップ格子リビルド  & 安定・質量保存             & $|\Delta M|/M_0 = 2.3 \times 10^{-1}$ & $\triangle$ & $\alpha{=}2$ 補間質量損失；一様 $2.3 \times 10^{-4}$ \\
