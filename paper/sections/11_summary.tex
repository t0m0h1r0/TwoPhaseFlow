...
全 19 件の基本コンポーネントテストおよび 8 件の追加検証（ギャップ充填）が完了した．
表~\ref{tab:verification_summary} に各テストの期待精度・実測精度・備考を総括する．
主要な知見を§~\ref{sec:verify_spatial}--§~\ref{sec:verify_time}の 4 小節に対応させて以下に整理する．
...
\begin{itemize}[noitemsep]
  \item \textbf{Crank--Nicolson 粘性項（$\Ord{\Delta t^2}$）：}
        §~\ref{sec:verify_cn_viscous} で独立に $\Ord{\Delta t^{2.0}}$ と無条件安定性を確認済み．
        ADI 分裂誤差は第~\ref{sec:verification}章の Taylor--Green 渦テストで結合検証される．
...
\end{itemize}
次章（第~\ref{sec:verification}章）では，Taylor--Green 渦・静止液滴・
Rayleigh--Taylor 不安定性など解析解が既知の問題を用いて，
7 ステップ全体の統合検証を行う．
