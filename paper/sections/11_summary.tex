...
全 19 件の基本コンポーネントテストおよび 12 件の追加検証（ギャップ充填）が完了した．
...
\begin{enumerate}
  \item \textbf{空間離散化：}
        ...
        非一様格子で $\Ord{h^6}$（GCL 合格）を達成した．
        DCCD フィルタは Nyquist モードを完全に抑制し（$H(\pi)=0$），
        Rhie--Chow C/RC ブラケットは $\Ord{h^4}$ を達成して
        標準 $\Ord{h^2}$ に対し $N{=}128$ で 24{,}876 倍の精度向上を確認した．
  \item \textbf{DCCD 移流特性：}
        不連続プロファイルの TV を CCD の約 $1/3$ に低減し（$10.83 \to 3.15$），
        平滑プロファイルでは精度を維持する．
        この特性が CLS 界面移流への DCCD 適用の根拠となる．
  \item \textbf{曲率計算：}
        経路 A（$\phi$ 直接微分）は $\Ord{h^5}$ の高次収束を示す．
        経路 B（$\psi \to \phi$ ロジット逆変換）は界面厚み $\varepsilon$ で精度飽和が生じるが，
        DCCD 安定化により $\Ord{h^2}$ の単調収束を達成した．
        CSF 正則化 $\delta_\varepsilon$（$\Ord{h^2}$）が全体精度の律速である．
  \item \textbf{界面処理：}
        CLS 質量保存誤差は $\Ord{10^{-15}}$（機械精度，界面重み付き補正），
        面積誤差は $\Ord{10^{-4}}$（$N{=}256$，単一渦），
        保存型再マッピングは LS 非保存補間比で 85 倍改善（$K{=}10$），
        HFE は 1D で $\Ord{h^{7.0}}$（上風 $\Ord{h^1}$ 比 $10^5$ 倍改善），
        2D テンソル積で $\Ord{h^{3.0}}$（$N{=}256$ で $L_\infty = 7.5 \times 10^{-9}$）を達成した．
  \item \textbf{PPE 求解：}
        欠陥補正 $k{=}3$ は等密度 Dirichlet で $\Ord{h^{7.0}}$（超収束；FD 比 $4.9 \times 10^7$ 倍高精度，コスト増 3.4 倍），
  ...
  \item \textbf{PPE 演算子特性：}
        CCD 2 階微分行列のスペクトル半径は $\rho(D^{(2)}) h^2 \approx 3.43$（FD の $4.0$ と比較）であり，
        2D Kronecker 積では $\rho(L_{\mathrm{2D}}) = 2\,\rho(L_{\mathrm{1D}})$ が機械精度で成立する．
        §~\ref{sec:dc_ppe_iteration} の反復ベース PPE ソルバ放棄の根拠が定量的に確認された．
  \item \textbf{再初期化界面シフト：}
        CLS 再初期化 1 回あたりのゼロレベル位置シフトは $\Ord{h^2}$--$\Ord{h^3}$ でスケールし，
        10 回累積で略線形蓄積（${\approx}8$--$9$ 倍）することを確認した．
\end{enumerate}
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Block Thomas 直接解法：}
        ...
        block Thomas アルゴリズムを介して $\Ord{h^6}$ 精度を達成しており，
        その正確性を含意する．
\end{itemize}
...
CCD 微分（非一様格子）    & $\Ord{h^6}$        & $\Ord{h^6}$（5.86--6.65）      & \checkmark & GCL 合格（$5.4 \times 10^{-14}$） \\
DCCD フィルタ              & $H(\pi) = 0$       & $H(\pi) = 0$                   & \checkmark & チェッカーボード完全抑制 \\
DCCD 移流（不連続）        & TV 低減            & TV: $10.83 \to 3.15$           & \checkmark & CCD 比 $1/3$ \\
Rhie--Chow C/RC            & $\Ord{h^4}$        & $\Ord{h^{4.0}}$                & \checkmark & 標準 $\Ord{h^2}$ 比 24{,}876 倍 \\
曲率（正弦波，$\phi$ 直接）& $\Ord{h^5}$        & $\Ord{h^{5.0}}$                & \checkmark & ベースライン \\
CLS 質量保存               & 保存的             & $\Ord{10^{-15}}$（機械精度）   & \checkmark & 界面重み付き補正 \\
CLS 面積精度               & 格子収束           & $\Ord{10^{-4}}$（$N{=}256$）   & \checkmark & 形状誤差は $\varepsilon$ 律速 \\
CLS 保存型再マッピング     & LS 比改善          & 85 倍（$K{=}10$）              & \checkmark & 動的非一様格子 \\
HFE 1D                     & $\Ord{h^6}$        & $\Ord{h^{7.0}}$                & \checkmark & 上風比 $10^5$ 倍改善 \\
HFE 2D（製造解）           & $\Ord{h^6}$\textsuperscript{†} & $\Ord{h^{3.0}}$ & $\triangle$\textsuperscript{‡} & テンソル積逐次補間 \\
Young--Laplace              & $\Delta p = 4.0$   & 相対誤差 $0.22\%$              & \checkmark & $N{=}128$，CSF + CCD-PPE \\
PPE DC $k{=}3$（Dirichlet） & $\Ord{h^6}$       & $\Ord{h^{7.0}}$                & \checkmark & 超収束；FD 比 $4.9 \times 10^7$ 倍 \\
PPE（Neumann + ゲージ）     & $\Ord{h^5}$       & $\Ord{h^{5.0}}$                & \checkmark & 境界スキーム律速 \\
分相 PPE + DC $k{=}3$      & $\Ord{h^7}$       & $\Ord{h^{7.0}}$                & \checkmark & 全密度比で不変（$\rho{=}1$--$1000$） \\
...
圧力フィルタ禁止           & $\bnabla\cdot\bu \neq 0$ & 発散誤差 $\Ord{h^{2.0}}$ & \checkmark & §8c の禁止規則を実証 \\
PPE 条件数                 & $\kappa \propto \rho/h^2$ & $\kappa \propto N^3 \cdot \rho$ & \checkmark & $\rho{=}1000$, $N{=}64$: $\kappa{=}2.2 \times 10^7$ \\
NS 毎ステップ格子リビルド  & 安定・質量保存             & $|\Delta M|/M_0 = 7.7 \times 10^{-5}$ & \checkmark & $\alpha{=}2$, $N{=}32$, 寄生電流 1.2 倍 \\
...
CCD $D^{(2)}$ スペクトル半径 & $\rho h^2 \approx 3.43$ & $3.43$                     & \checkmark & FD 比 $4.0$；2D Kronecker 成立 \\
\bottomrule
\multicolumn{5}{l}{\footnotesize \textsuperscript{†}HFE（§~\ref{sec:extension_pde_disc}）採用時の設計精度；ベースライン風上差分は $\Ord{h^1}$．} \\
\multicolumn{5}{l}{\footnotesize \textsuperscript{‡}$\Ord{h^{3.0}}$：設計精度 $\Ord{h^6}$ 未達だが $N{=}256$ で $L_\infty{=}7.5 \times 10^{-9}$；実用上許容．} \\
...
\multicolumn{5}{l}{\footnotesize \textsuperscript{§}§~\ref{sec:time_integration}：$\mu_l/\mu_g \gg 1$ では陽的交差微分項の打切り誤差 $\Ord{\mu_{\max}\,\Delta t}$ が支配する．} \\
\multicolumn{5}{l}{\footnotesize \checkmark: 設計精度達成，$\triangle$: 条件付き合格，$\times$: 未達（既知の制限，対策計画あり）．} \\
\end{tabular}
\end{table}
...
交差微分粘性項 $\Ord{\Delta t^{1.0}}$（高粘性比界面近傍；§~\ref{sec:time_integration} の予測と一致）．
CCD $D^{(2)}$ のスペクトル半径 $\rho h^2 \approx 3.43$（反復 PPE 放棄の根拠）．
CLS 質量保存 $\Ord{10^{-15}}$（機械精度），面積精度 $\Ord{10^{-4}}$（$N{=}256$），
DGR 厚み制御 $\varepsilon_{\mathrm{eff}}/\varepsilon \approx 1.03$，HFE 2D $\Ord{h^{3.0}}$．
分相 PPE + DC $k{=}3$ は全密度比（$\rho_l/\rho_g = 1$--$1000$）で
...
\begin{itemize}[noitemsep]
  ...
  \item 密度比依存性 → 変密度 PPE の破綻限界（§~\ref{sec:interface_crossing}）
\end{itemize}
