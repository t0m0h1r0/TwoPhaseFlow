...
\begin{enumerate}
  ...
  \item \textbf{界面処理：}
        ...
        ただし風上差分ベースラインは現行データで NaN のため比較対象から除外し，
...
CCD 微分（非一様格子）    & $\Ord{h^6}$        & 前漸近振動（$p=3.4$--$7.9$）  & $\triangle$ & 誤差は単調減少；GCL 合格（$2.4 \times 10^{-14}$） \\
...
HFE 1D                     & $\Ord{h^6}$        & $\Ord{h^{7.0}}$                & \checkmark & 風上ベースラインは NaN のため比較対象外 \\
...
\bottomrule
\multicolumn{5}{l}{\footnotesize \textsuperscript{†}HFE（§~\ref{sec:extension_pde_disc}）採用時の設計精度；風上差分の現行実行結果は NaN のため比較から除外．} \\
