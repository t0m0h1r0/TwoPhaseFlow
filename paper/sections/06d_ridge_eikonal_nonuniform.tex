...
%
% §6.5  Ridge–Eikonal の非一様格子実装（SP-E §3–§7）
...
% -------------------------------------------------------------------------------
\subsubsection{D1 実装：$\sigma_\text{eff}(\bm{x})$ のキャッシュ}
...
$\sigma_\text{eff} \to \sigma_0$ となり SP-B §3 の既定と一致する．
...
定理~\ref{thm:xi_hessian_forbidden}（$\xi$ 空間 Hessian 不可能性）により，
...
Approach A（SP-E 本命実装）は以下の 1 行で実装できる：
\begin{equation}
...
\]
は 2 次精度に留まるが，Morse 非退化リッジでは符号判定に影響しない．
GPU 実装で非一様 CCD ソルバが未整備のデバッグビルド
（WIKI-L-017 参照）ではこの fallback を一時許容する（CHK-160 検証済）．
...
一軸あたり 1.0--1.2$\times$（$h_x[i]$ 参照の分，WIKI-L-017）．

% -------------------------------------------------------------------------------
\subsubsection{D4 実装：$\varepsilon_\text{local}(\bm{x})$ の空間場}
...
\end{itemize}
合計は一様版と同じ漸近オーダー．
一軸あたり実行時間は一様版の 1.1--1.3$\times$（WIKI-L-017）．
