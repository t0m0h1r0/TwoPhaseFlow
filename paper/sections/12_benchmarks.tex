...
§\ref{sec:component_verification}（コンポーネント検証）で各部品の正確性を確認したのち，
本章では複数の物理機構が連成する多相流問題に対するフル NS ソルバーの総合的な検証を行う．
各節ではベンチマークの設定・検証指標・目標値を定義しており，フル NS ソルバーによる数値実験の実施は今後の課題である（§\ref{sec:future_work}）．
ベンチマークは難易度の低い順に配置されており，各ステップで合格してから次のテストに進むことを推奨する．

% ═══════════════════════════════════════════════════════════════════════════════
\subsection{GFM と寄生流れの抑制：静止液滴テスト}
\label{sec:val_static_drop}
\label{sec:bench_droplet}
% ═══════════════════════════════════════════════════════════════════════════════

\textbf{目的：}
Ghost Fluid Method（GFM，§\ref{sec:gfm}）の下で，表面張力ジャンプ条件 $[p]_\Gamma=\kappa/We$ が
PPE 右辺に直接組み込まれ，数値起源の寄生流れ（parasitic currents）が
CSF $O(\varepsilon^2)$ モデル誤差レベル以下に抑制されることを定量的に示す．
...
\begin{enumerate}
  ...
  \item \textbf{寄生流れ収束：}
        GFM により CSF モデル誤差が除去されるため，$\|\bm{u}_{\mathrm{para}}\|_\infty$ は
        格子収束とともに急速に減少することが期待される．
        $N = 64$，$\varepsilon \approx 1.5h$ での目標：
        $\|\bm{u}\|_\infty / (\sigma/\mu_l) \lesssim 10^{-5}$（Hysing et al.~\cite{Hysing2009} 水準）．
\end{enumerate}

\begin{tcolorbox}[resultbox, title={寄生流れ振幅の理論的予測（GFM 下）}]
GFM 下（§\ref{sec:gfm}）では，CSF $O(\varepsilon^2)$ モデル誤差が本質的に除去される．
寄生流れの残差は界面近傍の数値拡張誤差と CCD 境界精度に依存し，
$N = 64$，$\varepsilon \approx 1.5h$ では目標値：
\[
  \|\bm{u}_{\mathrm{para}}\|/(\sigma/\mu_l) \lesssim 10^{-5}
\]
GFM + CCD の組み合わせによって寄生流れの格子収束が CSF 比で大幅に改善される
ことが期待される（Fedkiw et al.~\cite{Fedkiw1999}，Kang et al.~\cite{Kang2000}）．
\end{tcolorbox}
...
理論値との相対誤差 $< 5\%$ を確認する
（線形安定解析の概略は Chandrasekhar \cite{Chandrasekhar1961} 参照）．
