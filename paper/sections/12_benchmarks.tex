...
HFE は $p$ が界面で真のジャンプ $[p] = \sigma\kappa$ を持つ場合に有効である．
現行ソルバのうち，\textbf{増分投影法（IPC）}では
$p^n$ が Young--Laplace ジャンプを持つため HFE による延長が必須となる
（§~\ref{sec:coupling_limitations}）．
...
IPC の実装（§~\ref{sec:conclusion}）により
...
$\sigma > 0$ の RT 不安定など，より挑戦的な問題への拡張については
