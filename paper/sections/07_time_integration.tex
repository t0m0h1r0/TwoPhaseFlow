...
界面張力 & affine jump face cochain + 値域射影 CSF & §\ref{sec:time_csf} \\
...
% -------------------------------------------------------------------------------
\subsection{界面張力項：affine jump face cochain と値域射影 CSF}
\label{sec:time_csf}
% -------------------------------------------------------------------------------
...
したがって本稿の時間積分設計では，
affine jump face cochain + 値域射影 CSF を表面張力項の標準構成とする．
...
表面張力 & affine jump face cochain + 値域射影 CSF & 設計上は同時刻結合；界面帯で実効律速あり \\
...
§\ref{sec:imex_bdf2_twophase_time} の V7 診断では，
...
\noindent\textbf{IPC + ジャンプ分解 CSF が閉じる対象}：
