...
\paragraph{本章の数式の読み方．}
時間積分の式は，時間方向に高次化するためだけの公式集ではなく，
どの物理項を「今すぐ陽的に進めてよいか」と
どの物理項を「同じ時刻の拘束として解くべきか」を仕分ける表である．
TVD-RK3 は界面フラックスの段階台帳を作り，
IMEX-BDF2 は滑らかな履歴を使って非剛性の移流を予測し，
粘性・圧力・毛管閉包は陰的または射影的に同時刻の整合を回収する．
したがって各式では，更新された値そのものよりも，
その値が次にどの拘束へ渡されるかを読むことが重要である．
$\bu^*$ は完成速度ではなく圧力へ渡す候補，
$\delta p$ は単なる圧力差ではなく発散とジャンプ条件を同時に回収する反力，
$h_{\sigma,f}$ は非平衡界面だけを動かす面上の毛管仕事として読む．
