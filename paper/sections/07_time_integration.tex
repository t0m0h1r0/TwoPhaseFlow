...
界面張力 & PPE 内蔵ジャンプ分解 CSF & §\ref{sec:time_csf} \\
...
$\bm{f}_\sigma$（界面張力） & 界面張力 CFL $\Delta t_\sigma$ & PPE 内蔵ジャンプ分解 CSF \\
...
% -------------------------------------------------------------------------------
\subsection{界面張力項：PPE 内蔵ジャンプ分解 CSF}
\label{sec:time_csf}
% -------------------------------------------------------------------------------
...
直接組み込むジャンプ分解 CSF} として扱う．本節は時間積分の
...
確定値として PPE 右辺に与えられる．本節のジャンプ分解 CSF は
...
圧力増分はジャンプ分解 CSF と非圧縮拘束を同時に満たす解へ収束する
...
表面張力 & PPE 内蔵ジャンプ分解 CSF & $\Ord{\Delta t^2}$ \\
...
\noindent\textbf{IPC + ジャンプ分解 CSF が $\Ord{\Delta t^2}$ を閉じる理由}：
IPC 法（§\ref{sec:ipc_derivation}）は予測ステップに $p^n$ を組み込み，
圧力投影の分離誤差を BDF2 の時間幅と整合させる．さらに
ジャンプ分解 CSF（§\ref{sec:time_csf}）が表面張力ジャンプを
