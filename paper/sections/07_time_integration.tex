...
界面張力 & surface-energy face cochain + component saddle & §\ref{sec:time_csf} \\
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{界面張力項}：$J_p=j_{gl}=-\sigma\kappa_{lg}$ は
        Young--Laplace 代表として PPE の affine jump face slot に置く．
        本番毛管力は pre-reinit endpoint 上の surface-energy virtual work から
        face-space Riesz cochain として構成し，pressure-adjoint component saddle で
...
% -------------------------------------------------------------------------------
\subsection{界面張力項：surface-energy face cochain と component saddle}
\label{sec:time_csf}
% -------------------------------------------------------------------------------
...
PPE/corrector と同じ face slot へ写した毛管 cochain} として扱う．
Young--Laplace 圧力ジャンプ $j_{gl}=-\sigma\kappa_{lg}$ はその cut-face 代表であるが，
本番力の受理条件は形状名や raw 曲率値ではなく，
pre-reinit endpoint 上の surface-energy Riesz cochain が
同じ pressure-adjoint face complex で定義されることである．
さらに成分ごとの体積制約反力を component saddle で取り除き，
...
保存形移流が実際に運んだ
$q_c=\psi_{\mathrm{after\ transport\ before\ reinit}}$ である．
...
\paragraph{surface-energy Riesz cochain と component saddle}
式~\eqref{eq:ppe_jump_decomposed} は連続式の所属を示すが，
離散補正で使う毛管 cochain は raw jump の文字通りの差分ではなく，
式~\eqref{eq:capillary_surface_energy_riesz_cochain} の
surface-energy Riesz 代表 $s_f$ である．
圧力反力が到達できる active range を $G_Ap$，成分体積反力を $B_h\mu$ とすると，
本番 cochain は
\[
...
の pressure-adjoint saddle で決める．
この処理は発散右辺を変えずに非圧縮・体積保存の仮想速度空間へ整合させるものであり，
非平衡界面の divergence-free Hodge 成分
$h_{\sigma,f}$ を削除しない．
式~\eqref{eq:capillary_range_projection} の $Z_A$ または $L_A$ は，
静的平衡 gate と原因診断の代表であって，
$s_f$ を本番前に圧力勾配値域へ置換する操作ではない．
...
一方，pressure-jump / component-saddle Hodge projection を含む二相結合系では，
...
圧力ジャンプとして PPE に内蔵し，さらに surface-energy Riesz cochain を
pressure-adjoint component saddle で閉じることが，
静的寄生流抑制と非平衡毛管駆動の保持の双方に必要である．
したがって本稿の時間積分設計では，
surface-energy face cochain + component saddle を表面張力項の標準構成とする．
...
pressure-jump / component-saddle Hodge projection を含む二相結合系で観測される
\textbf{実効次数}を分けて読む．
...
表面張力 & surface-energy face cochain + component saddle & 設計上は同時刻結合；界面帯で実効律速あり \\
...
\textbf{二相毛管結合域}& pressure-jump + component saddle & §\ref{sec:imex_bdf2_twophase_time} で実効 slope $1.59$ \\
\hline
\end{tabular}\\
...
表面張力ジャンプ・component saddle・FCCD projection が作る界面帯誤差を
