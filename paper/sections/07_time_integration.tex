...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{界面張力項}：$J_p=j_{gl}=-\sigma\kappa_{lg}$ は
        Young--Laplace 代表として PPE のアフィンジャンプ面補正枠に置く．
        本番毛管力は再初期化前の移流後状態における表面エネルギーの仮想仕事から
...
PPE 内蔵の圧力ジャンプ面共鎖 / 浮力が陽的残差評価の混合構成である．
...
$\bm{f}_\sigma$（界面張力） & 界面張力 CFL $\Delta t_\sigma$ & PPE 内蔵の圧力ジャンプ面共鎖 \\
...
PPE/補正段と同じ面補正枠へ写した毛管面共鎖} として扱う．
Young--Laplace 圧力ジャンプ $j_{gl}=-\sigma\kappa_{lg}$ はその切断面代表であるが，
本番力の受理条件は形状名や未補正曲率値ではなく，
再初期化前の移流後状態における表面エネルギーの Riesz 面共鎖が
...
離散補正で使う毛管面共鎖は未補正ジャンプの文字通りの差分ではなく，
...
一方，圧力ジャンプと成分体積制約付き Hodge 投影を含む二相結合系では，
...
圧力ジャンプとして PPE に内蔵し，さらに表面エネルギーの Riesz 面共鎖を
...
圧力増分は圧力ジャンプ面共鎖と非圧縮拘束を同時に満たす解へ収束する
...
圧力ジャンプと成分体積制約付き Hodge 投影を含む二相結合系で観測される
\textbf{実効次数}を分けて読む．
...
\textbf{二相毛管結合域}& 圧力ジャンプ + 成分体積制約付き鞍点系 & §\ref{sec:imex_bdf2_twophase_time} で実効傾き $1.59$ \\
\hline
\end{tabular}\\
...
Young--Laplace ジャンプを面共鎖として扱うため，
界面帯の低正則性と射影値域閉包が結合系の実効次数を律速する．
§\ref{sec:imex_bdf2_twophase_time} の二相時間精度診断では，
この結合構成を「BDF2 単体の次数検証」ではなく
毛管結合系診断として測り，最終局所傾き $1.59$ を
