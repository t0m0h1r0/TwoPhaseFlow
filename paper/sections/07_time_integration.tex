...
\textbf{直前の PPE 投影済み} $\bm{u}^n$ である．
一方，運動量移流（\S\ref{sec:fccd_advection}）の EXT2 外挿は
PPE 投影済みの履歴 $\{\bm{u}^n,\bm{u}^{n-1}\}$ から構成し，
未投影 $\bm{u}^*$（運動量予測子出力）は移流速度として用いない．
未投影 $\bm{u}^*$ を使うと：
\begin{itemize}
...
本稿が採用する時間ステップ順序：
\begin{equation*}
  (\text{PPE}_{n-1},\text{PPE}_{n})\to\{\bm{u}^{n-1},\bm{u}^{n}\}
