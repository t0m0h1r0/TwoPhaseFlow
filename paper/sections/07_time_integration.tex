...
界面張力 & 表面エネルギー面共鎖 + 成分体積制約付き鞍点系 & §\ref{sec:time_csf} \\
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{界面張力項}：$J_p=j_{gl}=-\sigma\kappa_{lg}$ は
        ...
        面上の Riesz 表現として構成し，圧力仕事と随伴な成分体積制約付き鞍点系で
...
% -------------------------------------------------------------------------------
\subsection{界面張力項：表面エネルギー面共鎖と成分体積制約付き鞍点系}
\label{sec:time_csf}
% -------------------------------------------------------------------------------
...
\paragraph{表面エネルギーの Riesz 面共鎖と成分体積制約付き鞍点系}
...
一方，pressure-jump と成分体積制約付き Hodge 投影を含む二相結合系では，
...
したがって本稿の時間積分設計では，
表面エネルギー面共鎖 + 成分体積制約付き鞍点系を表面張力項の標準構成とする．
...
pressure-jump と成分体積制約付き Hodge 投影を含む二相結合系で観測される
\textbf{実効次数}を分けて読む．
...
表面張力 & 表面エネルギー面共鎖 + 成分体積制約付き鞍点系 & 設計上は同時刻結合；界面帯で実効律速あり \\
...
\textbf{二相毛管結合域}& pressure-jump + 成分体積制約付き鞍点系 & §\ref{sec:imex_bdf2_twophase_time} で実効 slope $1.59$ \\
\hline
\end{tabular}\\
