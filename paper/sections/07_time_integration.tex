...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{粘性項}：implicit-BDF2 + 欠陥補正（DC）で full-stress Helmholtz 系を解き，
  ...
  \item \textbf{浮力項}：hydrostatic 成分を圧力側へ吸収し，
...
この非対称性を持つ full-stress Helmholtz 系に対し，
...
一方，低次補正 $A_L$ は補正方程式を分離可能にするため速度成分ごとの対角 stress--diffusion に分け，
クロス結合は DC 残差で補正する．
このとき粘性項の時間精度は BDF2 の $\Ord{\Delta t^2}$ に一致する．
界面近傍で $\mu$ が急変しても，クロス項を時間的に外挿して別扱いするのではなく，
同じ高次残差の中で評価するため，粘性由来の 1 次 splitting 誤差を導入しない．
...
この分解により，界面ジャンプを含む PPE でも同じ高次残差に対して
圧力増分を収束させることができる（PPE 求解節：§\ref{sec:ppe_solve}）．
...
NS 粘性（full stress） & implicit-BDF2（DC）& $\Ord{\Delta t^2}$ \\
...
完全陽解法として再初期化方程式を仮想時間積分する場合の参考安定条件は
