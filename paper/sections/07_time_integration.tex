...
界面張力 & affine pressure-jump face cochain + 値域射影 & §\ref{sec:time_csf} \\
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{界面張力項}：$J_p=j_{gl}=-\sigma\kappa_{lg}$ を向き付き圧力ジャンプとして
        affine jump face cochain に写し，PPE/corrector の値域へ閉じてから
        圧力補正と同じ時刻で結合する（§\ref{sec:time_csf}）．
...
PPE 内蔵 pressure-jump face cochain / 浮力が陽的残差評価の混合構成である．
...
$\bm{f}_\sigma$（界面張力） & 界面張力 CFL $\Delta t_\sigma$ & PPE 内蔵 pressure-jump face cochain \\
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{界面張力 $\bm{f}_\sigma$}：
        ...
        本稿は表面張力を PPE 内蔵 pressure-jump face cochain として
...
% -------------------------------------------------------------------------------
\subsection{界面張力項：affine pressure-jump face cochain と値域射影}
\label{sec:time_csf}
% -------------------------------------------------------------------------------

本稿の表面張力は，Continuum Surface Force
（CSF; Brackbill--Kothe--Zemach \cite{Brackbill1992}）型の運動量右辺体積力
として陽的に加えるのではなく，\textbf{圧力ジャンプ条件を PPE 右辺に
直接組み込む pressure-jump face cochain} として扱う．離散的には
§\ref{sec:split_ppe_capillary_range_projection} の affine jump face cochain を
PPE/corrector が到達できる圧力勾配値域へ射影し，
発散右辺と補正 face acceleration を同じ複体で閉じる．本節は時間積分の
...
確定値として PPE 右辺に与えられる．本節の pressure-jump face cochain は
$J_p^{n+1}=j_{gl}^{n+1}$ を向き付き圧力境界条件として
face cochain に注入する陰的処理となる．
...
\paragraph{face cochain 値域射影}
式~\eqref{eq:ppe_jump_decomposed} は連続式の所属を示すが，
離散補正では面上の既知ジャンプ勾配が
PPE/corrector の圧力勾配値域に入っている必要がある．
本稿では §\ref{sec:split_ppe_capillary_range_projection} に従い，
毛管ジャンプ face cochain
$c_f(j_{gl})=A_fB_f(j_{gl})$ を同じ $D_fA_fG_f$ 複体の値域へ射影し，
\[
  \Pi_{\mathcal{R}_h}c_f = A_fG_f\pi_h,\qquad
  D_fA_fG_f\pi_h = D_fc_f
\]
で得た $\Pi_{\mathcal{R}_h}c_f$ を速度補正に渡す．
この操作は発散右辺を変えないため新しい物理モデルではなく，
静止 Young--Laplace 平衡で $A_fG_fp-\Pi_{\mathcal{R}_h}c_f=0$ を
面ごとに満たすための Hodge 閉包である．
したがって時間積分上の役割は，表面張力を別の陽的体積力に戻すことではなく，
PPE 投影と同じ $\Delta t_{\mathrm{proj}}$ の補正 cochain として扱える形へ
代数的に閉じることにある．

\paragraph{同時刻結合による分離誤差の抑制}
本稿では予測子に表面張力体積力を先入れせず，PPE 段で
$J_p^{n+1}$，$\Pi_{\mathcal{R}_h}c_f^{\,n+1}$，$\delta p^{n+1}$ を
同時刻で結合する．これにより
$\kappa_{lg}$，圧力増分，速度補正が同じ界面位置 $\psi^{n+1}$ を参照し，
滑らかな界面・均質流域では圧力補正の設計次数と整合する．
一方，pressure-jump / range-projected projection を含む二相結合系では，
界面帯の低正則性が実効時間次数を律速しうるため，
この段落は「全結合 stack が常に純粋 2 次」とは主張しない．
Hysing らの 2 次元上昇気泡ベンチマーク \cite{Hysing2009} は，
寄生流速（parasitic currents）と界面応力閉包を同一 $\Delta t$・同一空間離散で
評価するための基準問題として用いる．

\paragraph{採用構成の対象範囲}
水--空気級の密度比（$\rho_l/\rho_g = 10^3$）では，界面張力を
圧力ジャンプとして PPE に内蔵し，さらに face cochain の値域射影で
補正加速度を閉じることが寄生流抑制と長時間安定性の双方に必要である．
したがって本稿の時間積分設計では，
affine pressure-jump face cochain + 値域射影を表面張力項の標準構成とする．

% -------------------------------------------------------------------------------
% 毛管波時間刻み制約は物理解像条件であるためベンチマーク章で扱う．
% -------------------------------------------------------------------------------
...
圧力増分は pressure-jump face cochain と非圧縮拘束を同時に満たす解へ収束する
...
顕在化しうる．具体的な数値検証はベンチマーク章（\S\ref{sec:val_rising_bubble}）
に委ねる．
...
pressure-jump / range-projected projection を含む二相結合系で観測される
\textbf{実効次数}を分けて読む．
...
表面張力 & affine pressure-jump face cochain + 値域射影 & 設計上は同時刻結合；界面帯で実効律速あり \\
...
\textbf{二相毛管結合域}& pressure-jump + range projection & §\ref{sec:imex_bdf2_twophase_time} で実効 slope $1.59$ \\
\hline
\end{tabular}\\
...
Young--Laplace ジャンプを face cochain として扱うため，
界面帯の低正則性と projection 値域閉包が結合 stack の実効次数を律速する．
§\ref{sec:imex_bdf2_twophase_time} の二相時間精度診断では，
この結合構成を「BDF2 単体の次数検証」ではなく
capillary coupled-stack 診断として測り，最終局所 slope $1.59$ を
Type-D（界面帯律速）として記録する．
...
\noindent\textbf{IPC + pressure-jump face cochain が閉じる対象}：
IPC 法（§\ref{sec:ipc_derivation}）は予測ステップに $p^n$ を組み込み，
圧力投影の分離誤差を BDF2 の時間幅と整合させる．さらに
pressure-jump face cochain（§\ref{sec:time_csf}）が表面張力ジャンプを
...
圧力投影（IPC）・表面張力（pressure-jump face cochain）の各段で
...
表面張力ジャンプ・range projection・FCCD projection が作る界面帯誤差を
別の実効次数として第~\ref{sec:verification}章 V7 に記録し，本章ではその制約を
全体 2 次主張で覆い隠さない．
...
ベンチマーク別の合成結果はベンチマーク章（§\ref{sec:val_summary}）に委ねる．
