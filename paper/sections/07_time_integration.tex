...
仮想時間 $\tau$ で積分する場合にも同じ SSPRK3 形式で書けるが，
...
$\psi$ は CLS フェーズ場）．affine-jump 形式は，予測子の右辺には
体積力を加えず，補正子で解く PPE
\begin{equation}
...
速度補正式 $\bu^{n+1}=\bu^*-\Delta t_{\mathrm{proj}}\bnabla(\delta p)/\rho^{n+1}$ と
正確な Navier--Stokes 圧力場 $p_e^{n+1}$ を照合する．
予測子段階の誤差は AB2 + CCD で $\bu^*-\bu_e^{n+1}=\Ord{\Delta t^2}$．
...
予測子段階の $\Ord{\Delta t^2}$ 誤差と合わせて全体
...
（PPE 形式圧力投影の安定性・収束性）に依拠する．これらにより
変密度 IPC の予測子--投影構造で
...
UCCD6 の非線形フラックス化は本稿の範囲外であり，\S~\ref{sec:future_work} で
将来課題として整理する．

以上で本章の項別時間積分設計が閉じた．
本章で採用しない高次完全陽解法や高次完全陰解法は本稿のスコープ外であり，
