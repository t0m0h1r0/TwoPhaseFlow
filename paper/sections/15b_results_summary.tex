...
% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{統合検証 V1--V12 の総括}
% ─────────────────────────────────────────────────────────────────────────────

第\ref{sec:verification}章の統合検証は 12 実験から構成され，
...
\begin{description}
  ...
  \item[V12（閉界面体積 admission gate；§\ref{sec:closed_interface_volume_gate}）：]
        §\ref{sec:val_oscillating_droplet} の現行 \texttt{active\_geometry\_capillary}
        YAML を監査し，旧 sharp-volume/Ridge--Eikonal 補修を fail-close へ移した．
        current-config prefix は 2 step 目の face-history 再射影 blocker に到達し，
        1 周期 production は閉界面体積保存の受理結果ではないことを固定した．
\end{description}
