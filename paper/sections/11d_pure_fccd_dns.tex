...
本節では，境界的な一括 smoothed-Heaviside 解法や
...
数値的な寄生流れ~\cite{Popinet2009,Renardy2002}を物理的根拠に基づいて最小化する．
...
\end{tabular}
\end{center}
Phase 1 の軽量版は本文経路の界面追跡層（一括 PPE 経路・分相 PPE 経路の双方）でも採用可能；
...
\end{equation}
で縫合される（全圧力 solve では $J_p^\chi=j_{gl}=-\sigma\kappa_{lg}$，
IPC 増分 solve では $J_p^\chi=\delta j_{gl}$；非相変化非圧縮では $J_n=0$）．
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{主たる勝利}：圧力・表面張力・HFE・界面ジャンプ評価を
        ...
        寄生流れの大幅削減．
  \item \textbf{主たるコスト}：離散化と界面条件の複雑度，
        困難な界面行 assembly，
        高価な楕円解．
...
純 FCCD は本文経路（一括 smoothed-Heaviside 経路および分相 PPE 経路）と
競合せず\textbf{補完}する．本文経路は汎用検証・標準計算を担い，
...
同じ面基準点へ集約する研究構成である．
したがって，本節で主張できる範囲は
「どの既検証 primitive に依存するか」を明示することに限られる．

\begin{center}
\small
\begin{tabular}{@{}p{0.26\linewidth}p{0.43\linewidth}p{0.24\linewidth}@{}}
\hline
研究構成の要素 & 対応する検証済み primitive & 本稿での扱い \\
\hline
分相圧力投影 & 分相 PPE + DC + HFE（U6, V6） & 高密度比でのロバスト性を検証 \\
界面越し延長 & HFE 単体精度（U6-c） & 1D 6 次，2D full-tensor 低次化なし \\
面基準点整合 & FCCD face value/grad（U1-c, U3-b） & face 評価は 4 次律速を明示 \\
...
% -------------------------------------------------------------------------------
\subsubsection{本文経路との関係}
\label{sec:pure_fccd_roadmap}
% -------------------------------------------------------------------------------

本文経路は，低・中密度比の比較経路として一括 smoothed-Heaviside PPE を残しつつ，
高密度比・低 We では §9 の production pressure stack
（分相 PPE + affine jump + HFE + DC + 大域ゲージ）へ切り替える構成である．
純 FCCD DNS はその上位互換ではなく，
すべての界面演算を FCCD 面ジェットへ寄せることで
シャープ界面極限を狙う研究構成である．
本稿の検証結果から直ちに採用できるのは，
分相 PPE + HFE と Balanced--Force 整合を組み合わせる部分であり，
完全な Phase 1--4 の同時採用は本稿の検証範囲外である．
