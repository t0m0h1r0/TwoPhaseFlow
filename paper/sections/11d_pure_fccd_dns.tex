...
%
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{純 FCCD DNS 構成：鋭い界面極限への発展的閉包}
...
どの追加条件が必要になるかを整理する．
\textbf{純 FCCD 鋭界面 DNS}は本稿の標準経路そのものではなく，
高密度比，高粘性比，低 We が同時に成立する鋭い界面極限へ向けた
発展的閉包である．
...
第4層 & 分相 FCCD PPE & 分相 + GFM + DC 残差受理 & 純 FCCD DNS の中核 \\
\hline
\end{tabular}
\end{center}
...
% -------------------------------------------------------------------------------
\subsubsection{CFD 上の位置付け：発展的 DNS / 鋭い界面極限}
\label{sec:pure_fccd_positioning}
% -------------------------------------------------------------------------------

純 FCCD DNS は\textbf{発展的 DNS / 鋭い界面極限}に位置づけられる：
...
単相・低密度比の一流体対照経路と競合せず\textbf{補完}する．
標準経路は汎用検証・標準計算を担い，純 FCCD は極限領域を見据えた発展的構成である．
...
一括 smoothed-Heaviside PPE は単相・低密度比一流体対照または制約診断の対照経路として扱う．
