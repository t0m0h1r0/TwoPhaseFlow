...
\paragraph{AO-Fast 幾何毛管分解の受理境界．}
標準圧力ジャンプ経路とは別に，表面エネルギー仮想仕事を
アクティブ界面幾何から直接構成する AO-Fast 経路を考える場合，
実行時の選択は明示的な実験モードとして扱う．
受理条件は
\begin{enumerate}[noitemsep]
  \item flat/static 問題で $a_{\sigma,\mathrm{bal}}=0$ を丸め誤差まで確認すること，
  \item 毛管波のような非定曲率問題で
        $r_\sigma-\Pi^{M_f}_{R_p}r_\sigma\ne0$ を面仕事ノルムで確認すること，
  \item 近似 Schur/PCG/DC が式~\eqref{eq:ao_fast_error_budget}--\eqref{eq:ao_fast_acceleration_error_bound} の
        誤差境界を満たすこと，
  \item GPU 実装でアクティブ集合，Krylov ベクトル，圧力反力 probe が
        host/device 往復なしに同じ \texttt{backend.xp} 配列上で評価されること，
\end{enumerate}
である．いずれかが不成立なら，その実験は fail-close とし，
標準圧力ジャンプ経路への暗黙 fallback，PCG/DC の自動置換，
またはゼロ駆動のままの時間発展継続を標準アルゴリズムの成功とは数えない．
この境界により，AO-Fast は高速化候補であっても，
第~\ref{sec:numerical_integrity}章の U12 と第~\ref{sec:verification}章の V11 を通過するまで
第~\ref{sec:benchmarks}章の物理ベンチマーク経路へ混入しない．
状態所有者，互換射影，アクティブ表，PCG/DC，GPU 契約，YAML/fail-close の
詳細は，第~\ref{sec:ao_fast_state_space}節で標準アルゴリズムとは別の
受理境界としてまとめる．
...
第6段 & PPE 評価       & 分相 PPE + HFE + 残差低下 DC + IPC 増分（圧力仕事と随伴な面上補正；表面エネルギー毛管平衡判定） & §~\ref{sec:ppe_solve} \\
