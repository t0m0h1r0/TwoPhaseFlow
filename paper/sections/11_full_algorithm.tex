...
FCCD/face-adjoint 面フラックスと同一ロカスの補正段階で抑制する．
...
\PaperCaptionNote{対流項は IMEX-BDF2 の EXT2 外挿で陽的に評価し，運動量 bulk は UCCD6，CLS $\psi$ は FCCD 保存形面フラックスで扱う．粘性項は陰的 BDF2 の全応力 Helmholtz 系を欠陥補正（DC）で閉じる．圧力項は欠陥補正法 PPE（DC 残差契約~\eqref{eq:ppe_dc_residual_contract}）+ 勾配演算子 $\mathcal{G}$（均一格子: CCD；非一様格子+壁面境界条件: $\mathcal{G}^\text{adj}$，§~\ref{sec:fvm_ccd_corrector}）による投影法~\cite{Chorin1968,vanKan1986}で扱い，圧力場には DCCD post-filter を適用しない．重力は釣合い浮力分解の面残差として予測段階に入れる．表面張力は一括 PPE 経路では CSF 体積力として予測段階に含め，高密度比・高粘性比・低 We の分相 PPE 経路では第6段の圧力ジャンプ条件へ移す．分相 affine-jump IPC では，既知圧力履歴を節点勾配ではなく式~\eqref{eq:affine_pressure_history_faces} の正準面加速度として持ち越し，毛管ジャンプ cochain は式~\eqref{eq:capillary_range_projection} の $D_f A_f G_f$ 値域へ射影してから PPE/corrector へ渡す．投影後の正準面速度を式~\eqref{eq:projection_native_psi_transport} の CLS フラックスへ直接渡す．}
\end{figure}

本章の標準経路では，DCCD を章全体の安定化ノブとして扱わない．
DCCD は §~\ref{sec:dccd_derivation} で定義した節点中心 CCD の外殻 post-filter であり，
CLS 移流は §~\ref{sec:advection_motivation} の FCCD 面フラックス，
運動量 bulk は §~\ref{sec:uccd6_def} の UCCD6，
...
\begin{itemize}[noitemsep]
  ...
  \item $\mathcal{C}_{\text{CCD}}(\bu,\, q)$：CCD 中心差分移流オペレータ（比較用の記法．
...
Stage A の保存形輸送で得た $\psi^*$ に対し，
§~\ref{sec:cls_stages} の Stage B--F を適用する．
体積モニタ $M = \int\psi(1-\psi)\,\mathrm{d}V$ が閾値を超えた場合のみ
補助距離再構成を実行し，軽微な乱れでは輸送結果をそのまま受け渡す：
\[
  \psi^{n+1} =
  \begin{cases}
    H_\varepsilon\!\left(-\mathrm{RidgeEikonal}(\psi^*)\right)
    \text{ followed by Stage F closure},
      & M/M_{\mathrm{ref}} > \theta_{\mathrm{reinit}},\\
    \psi^* \text{ followed by Stage B/F mass closure if needed},
      & M/M_{\mathrm{ref}} \le \theta_{\mathrm{reinit}} .
  \end{cases}
\]
反復型 Godunov/WENO--HJ Eikonal solver を比較経路として選ぶ場合だけ，
仮想時間 $\tau$ の CFL と停止条件を §~\ref{sec:cls_compression} の実装ガイドに従って別管理する．
標準経路の本質は反復回数や固定頻度ではなく，
物理輸送 $\delta\psi_T$，幾何射影 $\delta\psi_\Pi$，
および Stage F の $\phi$ 空間一様シフトを別々の離散 identity として閉じる点にある．
狭いスロットやトポロジー変化を含む検証では，
適応トリガーだけで局所角劣化を判定しないため，§~\ref{sec:xi_sdf_reinit} の
適用範囲に従って固定頻度または FMM/ξ-SDF 比較経路を明示する．
...
\end{equation}
右辺は圧力と同じ離散発散ロカスで評価した中間速度の
発散から構成される．以下，この右辺ベクトルを $b_p$ と書く．
圧力場および圧力勾配には DCCD post-filter を適用しない．
