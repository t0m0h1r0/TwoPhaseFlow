...
第6段 & PPE 評価       & 分相 PPE + HFE + DC + IPC 増分（pressure-adjoint face closure；変分毛管 Hodge gate） & §~\ref{sec:ppe_solve} \\
第7段 & 補正段階      & IPC 増分更新（同一 $\gamma\Delta t$ 投影；component-corrected 毛管面加速度を保存） & §~\ref{sec:ipc_derivation} \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}[noitemsep,leftmargin=2em]
  \item \textbf{二相標準の分相 PPE 経路：}
        ...
        同じ pressure-adjoint face complex 上の
...
式~\eqref{eq:capillary_surface_energy_riesz_cochain} の
surface-energy cochain と同じ face slot に置かれる cut-face 代表である．
診断用の圧力勾配代表だけは
\begin{equation}
  L_A(s_f)
  =
...
\end{equation}
を満たす．この代表は Hodge gate と圧力代表表示に用い，
補正段階で差し引く本番 cochain は
同一界面面データ・同一 pressure-adjoint metric から定めた
$\widehat c_{\sigma,f}=s_f-B_h\mu$ である．
