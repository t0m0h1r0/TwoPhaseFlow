...
第6段 & PPE 評価       & 分相 PPE + HFE + DC + IPC 増分（圧力仕事と随伴な面閉包；変分毛管 Hodge 平衡判定） & §~\ref{sec:ppe_solve} \\
...
表面エネルギー仮想仕事に基づく pressure-jump/Hodge 閉包の coupled-stack 実効次数を分けて読む．
...
射影後の面速度を保存する affine-jump IPC では式~\eqref{eq:affine_pressure_history_faces} の
...
\end{equation}
を満たす．この代表は Hodge 平衡判定と圧力代表表示に用い，
補正段階で差し引く本番 cochain は
同一界面面データ・同一の圧力仕事と随伴な面計量から定めた
$\widehat c_{\sigma,f}=s_f-B_h\mu$ である．
