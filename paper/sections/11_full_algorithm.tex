...
\PaperCaptionNote{対流項は IMEX-BDF2 の EXT2 外挿で陽的に評価し，粘性項は陰的 BDF2 の全応力 Helmholtz 系を欠陥補正（DC）で閉じる．圧力項は欠陥補正法 PPE（DC，$k{=}3$）+ 勾配演算子 $\mathcal{G}$（均一格子: CCD；非一様格子+壁面境界条件: $\mathcal{G}^\text{adj}$，§~\ref{sec:fvm_ccd_corrector}）による投影法~\cite{Chorin1968,vanKan1986}で扱う．重力は釣合い浮力分解の面残差として予測段階に入れる．表面張力は一括 PPE 経路では CSF 体積力として予測段階に含め，高密度比・高粘性比・低 We の分相 PPE 経路では第6段の圧力ジャンプ条件へ移す．分相 affine-jump IPC では，既知圧力履歴を節点勾配ではなく式~\eqref{eq:affine_pressure_history_faces} の正準面加速度として持ち越し，投影後の正準面速度を式~\eqref{eq:projection_native_psi_transport} の CLS フラックスへ直接渡す．}
\end{figure}
...
第6段 & PPE 評価       & 一括 PPE または分相 PPE + IPC 増分（同一 $D_fA_fG_f$ face closure） & §~\ref{sec:ppe_solve} \\
第7段 & 補正段階      & IPC 増分更新（同一 $\gamma\Delta t$ 投影；affine 履歴面加速度を保存） & §~\ref{sec:ipc_derivation} \\
\hline
\end{tabular}
\end{table}
