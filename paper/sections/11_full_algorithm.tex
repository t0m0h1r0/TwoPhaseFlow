...
\PaperCaptionNote{速度主変数経路の対流項は IMEX-BDF2 の EXT2 外挿で評価し，領域内部は UCCD6（BF 面整合を要する一流体対照では FCCD Option B/C）で扱う．保存形共通フラックス経路では，CLS $\psi$ の FCCD 面フラックス台帳から密度と運動量も同時に更新する．粘性項は陰的 BDF2 の全応力 Helmholtz 系を欠陥補正（DC）で閉じる．二相標準圧力項は分相 PPE + HFE + DC + 勾配演算子 $\mathcal{G}_\Gamma$ による投影法~\cite{Chorin1968,vanKan1986}で扱い，圧力場には DCCD 後置フィルタを適用しない．一括 smoothed-Heaviside PPE と CSF 体積力は一流体対照経路であり，圧力ジャンプ型二相閉包の代替ではない．重力は釣合い浮力分解の面残差として予測段階に入れる．分相圧力ジャンプ IPC では，既知圧力履歴を履歴圧力面加速度として持ち越し，表面エネルギー仮想仕事から作る毛管補正量を PPE と補正段へ渡す．圧力で表せる範囲への射影は毛管平衡判定と静的平衡診断に用い，標準毛管力を置換しない．壁付き計算では $F_w=\ker C_w$ を face-state 空間とし，post-pressure 壁修復は標準経路にしない．射影後の admissible 面速度を CLS フラックスへ直接渡す．}
\end{figure}
...
第2段 & 再初期化        & Ridge--Eikonal 補助距離再構成 + 鋭い界面体積シフト + 拡散質量プロファイル閉鎖（共通フラックスでは保存的 $q,m,\bm{p}_m$ 再写像がない場合は受理しない） & §~\ref{sec:cls_compression} \\
第3段 & 界面幾何・物性更新 & $\psi\!\to\!\phi$ ロジット変換 + 線形補間              & §~\ref{sec:levelset_mapping} \\
第4段 & 曲率計算        & $\psi$ 直接曲率 + 界面帯制限フィルタ             & §~\ref{sec:curvature_direct_psi_cls} \\
第5段 & 運動量 $(u,v)$ 移流 & 速度主変数経路は UCCD6 + EXT2；保存形共通フラックス経路では第1段で運動量移流済み & §~\ref{sec:time_level1} \\
...
\subsection{壁付き face-state 契約と診断量の解釈}
\label{sec:wall_face_state_algorithm_contract}

壁付き計算では，速度更新の最後に nodal wall velocity をゼロへ上書きするだけでは
保存形共通フラックスの状態にはならない．
本アルゴリズムの主状態は，輸送にも圧力仕事にも使う face state であり，
これは
\[
  f^{n+1}\in F_w=\ker C_w,\qquad D_ff^{n+1}=0,
  \qquad u^{n+1}=R_hf^{n+1},\qquad m^{n+1}=\rho^{n+1}u^{n+1}
\]
を同時に満たす必要がある．したがって，post-pressure wall-only projection，
節点壁速度の後置 clamp，または full-space KKT を未検証のまま
production solve として用いる経路は本稿の標準経路から外す．
採用するのは，式~\eqref{eq:wall_metric_retraction} の $P_w^{(M)}$ と
式~\eqref{eq:restricted_pressure_operator} の $G_w=P_w^{(A)}G_A$ を使い，
制約付き pressure complex が Green 恒等式・rank gate・製造解 gate を通るかを
診断する state-space 契約である．
実装上は，この契約を次の順で検査する．
\[
\begin{gathered}
  C_w=B_h^wR_h,\quad
...
すなわち，periodic image 行を full nodal array の独立行として数えず，
接線 face の image 面を source 面へ同期し，圧力 control volume を
source 行へ畳み込んでから Green 恒等式を測る．
これらの gate を通らない境界トポロジでは，
節点 clamp，壁面だけの後置投影，dense KKT，または圧力 gauge の暗黙変更で
計算を進めず，fail-close として扱う．

同じ理由で，デバッグ量は active operator に結び付けて解釈する．
圧力ジャンプ型経路で作用する毛管量は
切断面 Young--Laplace face cochain または
式~\eqref{eq:capillary_surface_energy_riesz_cochain} の Riesz cochain であり，
節点 $\psi$ 直接曲率の最大値ではない．
PPE 右辺のノルムは，closed-interface 源，履歴圧力面加速度，
および該当する補正項を全て加えた後の
solver 直前 RHS として記録する．
また pressure-jump 経路では CSF 体積力配列が存在しないため，
節点圧力勾配と節点 CSF 力との差を balanced-force residual と呼んでも
active 毛管 cochain の残差にはならない．
本稿で物理判断へ用いる残差は，
$F_w$ と active pressure metric $M_A=Q_f/\alpha_f$ 上で定義した
...
生成してしまうからである．
時刻列 checkpoint は，要求時刻を跨いだ実ステップの直前に
実際に用いられた pre-step 状態を保存するか，
可変ステップ BDF2 と履歴係数を明示的に定式化した場合に限り
厳密時刻状態を保存する．
本実装では前者を採用し，manifest の実時刻と
ファイル名の要求時刻を区別する．
