...
\PaperCaptionNote{対流項は IMEX-BDF2 の EXT2 外挿で陽的に評価し，粘性項は陰的 BDF2 の全応力 Helmholtz 系を欠陥補正（DC）で閉じる．圧力項は欠陥補正法 PPE（DC 残差契約~\eqref{eq:ppe_dc_residual_contract}）+ 勾配演算子 $\mathcal{G}$（均一格子: CCD；非一様格子+壁面境界条件: $\mathcal{G}^\text{adj}$，§~\ref{sec:fvm_ccd_corrector}）による投影法~\cite{Chorin1968,vanKan1986}で扱う．重力は釣合い浮力分解の面残差として予測段階に入れる．表面張力は一括 PPE 経路では CSF 体積力として予測段階に含め，高密度比・高粘性比・低 We の分相 PPE 経路では第6段の圧力ジャンプ条件へ移す．分相 affine-jump IPC では，既知圧力履歴を節点勾配ではなく式~\eqref{eq:affine_pressure_history_faces} の正準面加速度として持ち越し，毛管ジャンプ cochain は式~\eqref{eq:capillary_range_projection} の $D_f A_f G_f$ 値域へ射影してから PPE/corrector へ渡す．投影後の正準面速度を式~\eqref{eq:projection_native_psi_transport} の CLS フラックスへ直接渡す．}
\end{figure}
...
\textbf{段} & 役割 & 手法 & 参照 \\
\hline
第1段 & CLS $\psi$ 移流 & TVD-RK3 + FCCD 面フラックス発散（projection-native 正準面速度；式~\eqref{eq:projection_native_psi_transport}） & §~\ref{sec:time_level1} \\
...
第6段 & PPE 評価       & 一括 PPE または分相 PPE + IPC 増分（同一 $D_f A_f G_f$ face closure；毛管 cochain 値域射影） & §~\ref{sec:ppe_solve} \\
第7段 & 補正段階      & IPC 増分更新（同一 $\gamma\Delta t$ 投影；range-projected affine 履歴面加速度を保存） & §~\ref{sec:ipc_derivation} \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}[noitemsep]
  ...
  \item $\mathrm{Div}_h^{\psi}((P_f\psi)\bu_f)$：FCCD 保存形面フラックス発散（CLS $\psi$ 移流専用．
        projection-native 経路では式~\eqref{eq:projection_native_psi_transport} の正準面速度 $\bu_f$ を使う；§~\ref{sec:advection_motivation}）
...
$p^n_{\mathrm{ext}}$ の節点勾配を再び作るのではなく，
前ステップで保存した式~\eqref{eq:affine_pressure_history_faces} の
正準面加速度を predictor の面速度へ入れる
...
\begin{itemize}[noitemsep,leftmargin=2em]
  ...
  \item \textbf{高密度比・高粘性比・低 We の分相 PPE 経路（水--空気級の密度比）：}
        ...
        表面張力を pressure-jump として扱う場合，raw な
        $c_f=A_f B_f(J_p^\chi)$ は式~\eqref{eq:capillary_range_projection}
        により同じ $D_f A_f G_f$ 値域へ射影し，
        $\widehat c_f=\Pi_{\mathcal{R}_h}c_f$ を PPE 右辺と第7段の補正へ渡す．
        これにより $D_f\widehat c_f=D_f c_f$ を保ったまま，
        divergence-free だが圧力ジャンプとして表現できない face cochain を
        速度補正から除く．
...
式~\eqref{eq:ppe_dc_residual_contract} の高次残差契約で行う．
...
を表す．range-projected 経路では，この記法の
$B(J_p^\chi)$ は raw jump ではなく
\begin{equation}
  A_f B_f^{\mathrm{range}}(J_p^\chi)
  =
  \widehat c_f
  =
  \Pi_{\mathcal{R}_h}\!\left(A_f B_f(J_p^\chi)\right)
  \label{eq:algorithm_range_projected_jump}
\end{equation}
を満たす面 cochain の代表である．PPE 右辺に入れた既知ジャンプ面データと
補正段階で差し引く $B^{\mathrm{range}}(J_p^\chi)$ は
同一の界面面データから定める．
