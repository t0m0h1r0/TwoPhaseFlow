...
\PaperCaptionNote{対流項は CCD + AB2（陽的），粘性項は CCD + Crank--Nicolson（半陰的）， 圧力項は欠陥補正法 PPE（DC，$k{=}3$）+ 勾配演算子 $\mathcal{G}$（均一格子: CCD；非一様格子+壁面境界条件: $\mathcal{G}^\text{adj}$，§~\ref{sec:fvm_ccd_corrector}）による投影法~\cite{Chorin1968,vanKan1986}で扱う． 重力は体積力として時間陽的に評価する． 表面張力は一括 PPE 経路では CSF 体積力として予測段階に含め，高密度比・低 We の分相 PPE 経路では第6段の圧力ジャンプ条件へ移す． 第5a段は既知圧力 $p^n$ を界面越しに延長し，予測段階の圧力勾配がジャンプを直接横断しないようにする補助手順である．}
\end{figure}
...
第2段 & 再初期化        & 仮想時間ステッピング                            & §~\ref{sec:cls_compression} \\
第3段 & 界面幾何・物性更新 & $\psi\!\to\!\phi$ logit + 線形補間              & §~\ref{sec:levelset_mapping} \\
第4段 & 曲率計算        & $\psi$ 直接曲率 + 界面帯制限フィルタ             & §~\ref{sec:curvature_direct_psi_cls} \\
第5段 & 運動量 $(u,v)$ 移流 & UCCD6 + 外挿（AB2 または IMEX--BDF2） & §~\ref{sec:time_level1} \\
       & 予測段階      & 粘性 Helmholtz + 釣合い浮力分解；表面張力は一括 CSF または分相圧力ジャンプ & §~\ref{sec:time_level1} \\
第6段 & PPE 求解       & 一括 PPE または分相 PPE + IPC 増分                  & §~\ref{sec:ppe_solve} \\
...
相内曲率式を no-slip 壁の接触点へそのまま外挿すると，
...
PPE affine jump と補正段階は式~\eqref{eq:cut_face_young_laplace_jump} の
切断面補間値を同じ物理面上で用いる．
...
IMEX--BDF2 予測子~\cite{AscherRuuthSpiteri1997,AscherRuuthWetton1995}を用い，粘性ブロックを陰的 BDF2~\cite{HairerWanner1996} で解く．
いずれの場合も静水圧的な浮力勾配は圧力空間へ残し，
...
低次演算子は収束経路だけを変え，固定点は高次系 $A_H\bm{u}=b_\nu^{n,n-1}$ に保たれる．
...
\paragraph{第5a段：HFE 延長による $p^n_{\mathrm{ext}}$ 構築（$\sigma > 0$ の場合）}
...
% ── 第6段 ──────────────────────────────────────────────────────────────
\paragraph{第6段：圧力 Poisson 方程式の求解（IPC 増分法）}
...
\noindent\textbf{DC $k{=}3$ 反復概要（第6.0--6.5小段；構造は両経路共通，
...
\begin{enumerate}[leftmargin=2em,noitemsep]
  ...
  \item 第6.2小段：補正方程式 $L_L\,\eta^{(k)} = d^{(k)}$ を因子再利用付き疎 LU で解く（§~\ref{sec:dc_sweep}）．
  \item 第6.3小段：緩和更新 $\delta p^{(k)} = \delta p^{(k-1)} + \omega\,\eta^{(k)}$（$\omega \in \{0.3,0.5,0.7\}$）．
  \item 第6.4小段：収束判定 $\|d^{(k)}\|_\infty < \varepsilon_\mathrm{tol}$（許容値 $10^{-10}$）．
  \item 第6.5小段：最終値 $\delta p \leftarrow \delta p^{(3)}$．
\end{enumerate}
