...
\PaperCaptionNote{対流項は IMEX-BDF2 の EXT2 外挿で陽的に評価し，運動量の領域内部は UCCD6（BF 面整合を要する一流体対照では FCCD Option B/C），CLS $\psi$ は FCCD 保存形面フラックスで扱う．粘性項は陰的 BDF2 の全応力 Helmholtz 系を欠陥補正（DC）で閉じる．二相標準圧力項は分相 PPE + HFE + DC + 勾配演算子 $\mathcal{G}_\Gamma$（均一格子: FCCD/CCD 面勾配；非一様格子+壁面境界条件: $\mathcal{G}^{\text{adj}}_\Gamma$，§~\ref{sec:fvm_ccd_corrector}）による投影法~\cite{Chorin1968,vanKan1986}で扱い，圧力場には DCCD 後置フィルタを適用しない．一括 smoothed-Heaviside PPE と CSF 体積力は単相・低密度比の一流体対照経路または BF 原理を説明する対照経路であり，圧力ジャンプ型二相閉包の代替ではない．重力は釣合い浮力分解の面残差として予測段階に入れる．分相圧力ジャンプ IPC では，既知圧力履歴を節点勾配ではなく式~\eqref{eq:affine_pressure_history_faces} の履歴圧力面加速度として持ち越し，毛管補正量は式~\eqref{eq:capillary_surface_energy_riesz_cochain} の表面エネルギー仮想仕事から構成し，式~\eqref{eq:capillary_component_saddle} の成分体積制約付き鞍点系で得る $\widehat c_{\sigma,f}$ を PPE と補正段へ渡す．式~\eqref{eq:capillary_range_projection} の圧力で表せる範囲への射影は毛管平衡判定と静的平衡診断に用い，標準毛管力を置換しない．射影後の面速度を式~\eqref{eq:projection_native_psi_transport} の CLS フラックスへ直接渡す．}
\end{figure}
...
$q,m,\bm{p}_m$ を同時に更新し，運動量予測子では primitive 対流を再評価しない．
したがって標準 UCCD6 対流は滑らかな primitive 速度経路の部品であり，
...
\textbf{段} & 役割 & 手法 & 参照 \\
\hline
第1段 & CLS $\psi$ 移流 & TVD-RK3 + FCCD 面フラックス発散（保存形共通フラックスでは同じ台帳で $q,m,\bm{p}_m$ を更新） & §~\ref{sec:cls_advection} \\
第2段 & 再初期化        & Ridge--Eikonal 補助距離再構成 + sharp 体積シフト + 拡散質量プロファイル閉鎖（共通フラックスでは保存的 $q,m,\bm{p}_m$ remap がない場合 fail-close） & §~\ref{sec:cls_compression} \\
第3段 & 界面幾何・物性更新 & $\psi\!\to\!\phi$ ロジット変換 + 線形補間              & §~\ref{sec:levelset_mapping} \\
第4段 & 曲率計算        & $\psi$ 直接曲率 + 界面帯制限フィルタ             & §~\ref{sec:curvature_direct_psi_cls} \\
第5段 & 運動量 $(u,v)$ 移流 & primitive 経路は UCCD6 + EXT2；保存形共通フラックス経路では第1段で運動量移流済み & §~\ref{sec:time_level1} \\
       & 予測段階      & 粘性 Helmholtz + 釣合い浮力分解；二相標準では表面張力を圧力ジャンプへ渡す & §~\ref{sec:time_level1} \\
第6段 & PPE 評価       & 分相 PPE + HFE + DC + IPC 増分（圧力仕事と随伴な面上補正；表面エネルギー毛管平衡判定） & §~\ref{sec:ppe_solve} \\
...
\begin{itemize}[noitemsep]
  ...
  \item $\mathcal{A}_{\bu}(\bu)$：運動量移流オペレータ．
          ...
          BF 面整合を検証する一流体対照系では FCCD Option B/C（§~\ref{sec:fccd_advection}）を用いる．
          CCD 中心差分の非保存形
...
primitive 速度経路では，各段階完了後に
$\psi \leftarrow \max(0,\min(1,\psi))$ の値域制限を適用し，
その影響は段階 B/F の質量閉鎖残差として記録する．
保存形共通フラックス経路では，端点 clipping は $m,\bm{p}_m$ へ同じ写像を与えないため用いない．
必要な有界性は，段階フラックスを
$F_q=F_{\mathrm{low}}+\alpha(F_{\mathrm{high}}-F_{\mathrm{low}})$
へ置換する flux-corrected transport 型制限で担い，制限後の $F_q$ から
$F_m$ と $F_{\bm{p}}$ を構成する．
この経路は donor 低次フラックスが現在の CFL と境界条件で
$0\le q\le1$ を保つことを受理条件とし，破れた場合はステップを棄却または fail-close する．
...
$m$ と $\bm{p}_m$ も同時に保存的再マップされなければならない．
現在の再初期化が $q$ 単独の幾何射影である設定では，
この段階を生産経路として実行せず fail-close とする．
...
\begin{itemize}[noitemsep,leftmargin=2em]
  \item \textbf{二相標準の分相 PPE 経路：}
        ...
        補正段で使う圧力面加速度は，scalar PPE の発散代表だけではなく，
        式~\eqref{eq:pressure_reaction_green_identity} の変分随伴条件を満たす
        $G_A$ とし，scalar operator も同じ組から
        $L_A=D_fG_A$ として作る．
        これは高次 compact 勾配を捨てるための低次 fallback ではなく，
...
\subsection{保存形チェックポイントと再開同値性}
\label{sec:conservative_checkpoint_contract}

再開可能性は単なる実装上の便利機能ではなく，
離散時間発展写像がどの状態を主変数としているかを検査する条件である．
primitive 速度経路では，次ステップに必要な状態は
...
および次段で使う時間履歴も同じ面複体に属する cochain として保存する．
もし checkpoint が $q$ と $\bu$ だけを保存して
$m,\bm{p}_m$ を再開時に作り直すなら，
再開前後で運動量が属する質量計量が変わり，
数値的には見えない衝撃を入れたのと同じである．
したがって保存形共通フラックス経路の checkpoint は
pre-step の $q,m,\bm{p}_m$ と面履歴 cochain を保存することを受理条件とする．
この条件により，ゼロスタート計算と checkpoint からの再開計算を
同じ離散写像の反復として比較できる．
