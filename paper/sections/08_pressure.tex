...
\begin{tcolorbox}[mybox, title={FVM 変密度 PPE（展開形，実装可能な形式，IPC 増分形）}]
式 \eqref{eq:af_exact} の $a_f$ を用いた FVM 離散化の完全な形（$x$方向のみ表示，未知変数は $\delta p \equiv p^{n+1}-p^n$）：
\begin{equation}
  \underbrace{\frac{a_{i+1/2,j}}{h_x^2}}_{\text{右係数}} (\delta p)_{i+1,j}
  - \underbrace{\frac{a_{i+1/2,j}+a_{i-1/2,j}}{h_x^2}}_{\text{中央係数}} (\delta p)_{i,j}
  + \underbrace{\frac{a_{i-1/2,j}}{h_x^2}}_{\text{左係数}} (\delta p)_{i-1,j}
  + (\text{$y$方向同様})
...
\begin{tcolorbox}[mybox, title={PPE 仮想時間陰解法の反復アルゴリズム（IPC 増分形）}]
\begin{enumerate}
  \item \textbf{初期値：} $(\delta p)^0 = 0$（IPC 増分の初期推定値）を設定する．
        初期残差は $\|\mathcal{L}_h\cdot 0 - q_h\|_2 = \|q_h\|_2$ であり，
        収束には典型的に数回〜数十回の反復を要する．
  \item \textbf{RHS 構築：} $q_h = (\bnabla_h^{\mathrm{RC}}\cdot\bu^*)/\Delta t$ を計算する．
  \item \textbf{反復ループ（$m = 0, 1, 2, \dots$）：}
    \begin{enumerate}
      \item \textbf{残差評価：} $x$・$y$ 方向 CCD スウィープで $\mathcal{L}_h(\delta p)^m$ を計算し，
            残差 $\varepsilon^m = \|\mathcal{L}_h(\delta p)^m - q_h\|_2$ を求める．
            $\varepsilon^m < \varepsilon_{\mathrm{tol}}$ なら収束として終了．
      \item \textbf{$x$ 方向スウィープ（逐次第1段）：} 各行 $j$ に対して
            $(\mathbf{I}+\Delta\tau\mathcal{L}_x)\widetilde{(\delta p)}^{m+1}_{:,j} = (\delta p)^m_{:,j} + \Delta\tau\, q_{h,:,j}$
            をブロック Thomas 法で解き，中間解 $\widetilde{(\delta p)}^{m+1}$ を得る．
      \item \textbf{$y$ 方向スウィープ（逐次第2段）：} 各列 $i$ に対して
            $(\mathbf{I}+\Delta\tau\mathcal{L}_y)(\delta p)^{m+1}_{i,:} = \widetilde{(\delta p)}^{m+1}_{i,:}$
            をブロック Thomas 法で解き，$(\delta p)^{m+1}$ を得る．
    \end{enumerate}
  \item \textbf{出力：} 収束した $(\delta p)^{m+1} = \delta p$；$p^{n+1} = p^n + \delta p$ を Corrector に渡す．
\end{enumerate}
\end{tcolorbox}
