...
\begin{enumerate}
  ...
  \item \textbf{§~\ref{sec:gfm}（界面越し場延長）}：
        Young--Laplace 不連続による CCD の Gibbs 振動を，
        最近接点 Hermite 補間（$\Ord{h^6}$）による界面越し場延長で PPE 求解前に除去する．
        加えて，界面型密度場で変密度 PPE が破綻する問題（§~\ref{sec:varrho_ppe_limitation}）に対し，
        各相で定数密度 PPE を独立に解く GFM 分相解法の理論的基盤を提供する．
...
\begin{itemize}
  ...
  \item \textbf{$\bnabla p^n$ の評価前} に界面越し Hermite 延長（§~\ref{sec:gfm}，Step 5c）で
        $p^n_{\mathrm{ext}}$ を構築する．
  \item \textbf{$\bnabla(\delta p)$ の評価前} に
        界面越し Hermite 延長（§~\ref{sec:gfm}，Step 7 前処理）で $\delta p_{\mathrm{ext}}$ を構築する．
