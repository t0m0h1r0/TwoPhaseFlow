...
（詳細は§\ref{sec:advection}，§\ref{sec:algorithm}）．
本節の導出では以下 Backward Euler 形式で統一し，Projection 法の構造（圧力--速度分離）を示す．
全体時間精度および Crank--Nicolson の実装詳細は第\ref{sec:advection}章（式~\eqref{eq:crank_nicolson}）を参照のこと．
\end{tcolorbox}
...
\begin{itemize}
  \item \textbf{静止液滴（検証問題）：} Young-Laplace 解 $p^0 = \sigma\kappa_0$（液相内），$p^0 = 0$（気相）を解析的に与える．
...
（詳細は §\ref{sec:ccd_balanced_force} 参照）．
実用上，$\varepsilon \sim h$ の設定では界面近傍での有効精度は $\Ord{h^4}$ 以上に保たれることが確認されている．
\end{tcolorbox}
...
\begin{tcolorbox}[mybox, title={CCD-Poisson Neumann 境界実装の単体テスト（実装後に必ず確認）}]
