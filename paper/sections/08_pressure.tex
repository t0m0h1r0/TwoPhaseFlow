...
\begin{enumerate}
  ...
  \item \textbf{§~\ref{sec:field_extension}（界面越し場延長）}：
...
\begin{itemize}
  ...
  \item \textbf{$\bnabla p^n$ の評価前} に界面越し Hermite 延長（§~\ref{sec:field_extension}，Step 5c）で
        $p^n_{\mathrm{ext}}$ を構築する．
  \item \textbf{$\bnabla(\delta p)$ の評価前} に
        界面越し Hermite 延長（§~\ref{sec:field_extension}，Step 7 前処理）で $\delta p_{\mathrm{ext}}$ を構築する．
