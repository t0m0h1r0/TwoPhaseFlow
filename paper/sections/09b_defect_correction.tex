...
§~\ref{sec:ppe_discretization_choice} の収束解析に基づき，現行実装では
k=1（FD 直接解に等価）を使用する；k$\geq$3 では $\Ord{h^6}$ 精度に漸近する
（§~\ref{sec:dc_accuracy}；分相 PPE 基盤としての CCD 演算子は §~\ref{sec:ccd_poisson_matrix}）．
...
これにより壁面での $\bnabla\rho\cdot\bnabla p$ 交差項が消滅し，
...
方向分離（ADI 型）スイープとの比較解析（理論・数値実験）は §~\ref{sec:verify_dc_spectral} および付録~\ref{app:dc_convergence_theory} を参照．
...
CCD-Poisson（§~\ref{sec:ccd_poisson_matrix}，将来の分相 PPE 向け）を解く標準ソルバとして欠陥補正法を示す；
LGMRES/直接 LU はデバッグ・検証用途に限定する．
\textbf{現行実装}（変密度界面では DC 反復が収束不能；§~\ref{sec:ppe_discretization_choice}）は $k=1$ の FD 直接法 + CCD 勾配を採用する（§~\ref{sec:ppe_bc_stability}）．
...
\textbf{欠陥補正法（DC）（本稿採用，現行 k=1）} & $\boldsymbol{\Ord{h^6}}$ & \textbf{不要} & \textbf{—} & $\boldsymbol{\mathcal{O}(N^2)}\times k$\textbf{反復} \\
\bottomrule
\end{tabular}
