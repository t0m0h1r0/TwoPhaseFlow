...
左境界では対応する結合ブロックが標準内点と同じ $\mathbf{A}_L$（変化なし）となるため，
...
$N_y$ 本のスウィープを並列実行できる．マルチコアCPU/GPU では行方向・列方向のスウィープをそれぞれバッチ並列化し，実効性能を向上させることが可能である（実装詳細：付録~\ref{app:ccd_ghost} 参照）．
