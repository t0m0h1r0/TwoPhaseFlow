...
\begin{itemize}
  ...
  \item \textbf{Equation-II（$\fpp{0}$）}：最小閉包は $\Ord{h^2}$ 精度の片側スキーム
        （式~\eqref{eq:bcII_left}）として導出できるが，実装経路としては採用しない．
        本稿のライブラリ実装は付録~\ref{app:ccd_bc_derivation_II_h4} の
        6 点 $\Ord{h^4}$ 昇格式を必須条件とし，$n_\mathrm{pts}<6$ は
        paper-exact 条件未満として明示的に拒否する．
...
漸近次数の主張とは区別する．
具体的には，まず境界点 $i=0$ での微分値 $\fp{0}, \fpp{0}$ を，境界近傍の関数値 $f_0, \dots, f_3$ と，内側の微分値 $\fp{1}, \fpp{1}$ を用いて表現する最小閉包を導出し，実装既定の $\Ord{h^4}$ 昇格式は付録~\ref{app:ccd_bc_derivation_II_h4} に分離する．
