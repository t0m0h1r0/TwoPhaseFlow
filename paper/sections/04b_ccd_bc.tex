...
この「閉包問題」を解決するため，境界点専用の片側 Hermite 閉包が必要となる．
...
\begin{itemize}
  ...
  \item \textbf{Equation-II（$\fpp{0}$）}：最小閉包は $\Ord{h^2}$ 精度の片側スキーム
        （式~\eqref{eq:bcII_left}）として導出できるが，標準閉区間の閉包としては採用しない．
        本稿の標準閉区間演算子は付録~\ref{app:ccd_bc_derivation_II_h4} の
        6 点 $\Ord{h^4}$ 昇格式を必須閉包とする．
        以下のノルム重み付け機構は，$\Ord{h^2}$ 最小閉包を採用した場合の理論比較であり，
        標準閉包の代替経路ではない．
\end{itemize}
...
境界点（$i=0,N-1$）の Equation-II 最小閉包による誤差が
$|e_{\mathrm{bc}}| = \Ord{h^2}$ であるとき：
\begin{equation}
  \|e\|_{L^2}^2
...
したがって，最小閉包を用いた閉区間全体の保守的な漸近次数は $\Ord{h^{5/2}}$ であり，
...
漸近次数の主張とは区別する．
具体的には，まず境界点 $i=0$ での微分値 $\fp{0}, \fpp{0}$ を，境界近傍の関数値 $f_0, \dots, f_3$ と，内側の微分値 $\fp{1}, \fpp{1}$ を用いて表現する最小閉包を導出し，標準閉区間で用いる $\Ord{h^4}$ 昇格式は付録~\ref{app:ccd_bc_derivation_II_h4} に分離する．
...
したがって Equation-I と対になる境界関係を明示し，本節で最小閉包の精度と限界を導出する．
\end{tcolorbox}
...
標準閉区間演算子では，この最小閉包ではなく
4次精度昇格スキーム（付録~\ref{app:ccd_bc_derivation_II_h4}）を用いる．
ゴーストセル法（付録~\ref{app:ccd_ghost}）は境界値を外挿形式で与える問題設定の
別表現として扱う．
...
\noindent\textbf{演算子構造の注意：}
式~\eqref{eq:ccd_global} は各座標線ごとのブロック三重対角問題であり，
2 次元では $x$ 方向に $N_y$ 本，$y$ 方向に $N_x$ 本の独立した 1 次元閉包を解く．
各座標線の前進消去は隣接未知量に依存するが，
異なる座標線は同じ係数ブロックを用いる独立問題である．
したがって本章で用いるべき事実は，特定の計算機での性能ではなく，
境界閉包を含めても全体が線形計算量の座標線問題として分離されることである．
...
式~\eqref{eq:bc_left} + 6 点 $\Ord{h^4}$ Equation-II 昇格閉包（付録~\ref{app:ccd_bc_derivation_II_h4}） &
$\Ord{h^5}$（Eq.-I），$\Ord{h^4}$（Eq.-II） \\[4pt]
Neumann BC \\（$f'_0$ 既知，例：PPE 壁面圧力） &
$f'_0$ を既知値として Eq.-I の左辺に移項し，Equation-II は同じ昇格閉包で閉じる &
$\Ord{h^5}$（Eq.-I の逆利用），$\Ord{h^4}$（Eq.-II） \\[4pt]
周期 BC \\（$f_0 = f_N$，例：一様流検証） &
ブロック循環 CCD（付録~\ref{app:ccd_periodic}） &
$\Ord{h^6}$（全節点，境界スキーム不要） \\[4pt]
解析下限の確認 \\（標準経路外） &
式~\eqref{eq:bcII_left} の最小閉包 &
$\Ord{h^5}$（Eq.-I），$\Ord{h^2}$（Eq.-II） \\
\hline
\end{tabular}
...
標準的な閉容器流れ（静止液滴，ライジングバブル）では
Equation-I 片側スキームと Equation-II 昇格閉包を組み合わせる．
