...
\begin{itemize}
  ...
  \item \textbf{Equation-II（$\fpp{0}$）}：最小閉包は $\Ord{h^2}$ 精度の片側スキーム
        （式~\eqref{eq:bcII_left}）．実装既定では $n_\mathrm{pts}\ge6$ のとき
        付録~\ref{app:ccd_bc_derivation_II_h4} の 6 点 $\Ord{h^4}$ 昇格式を用い，
        小格子のみ式~\eqref{eq:bcII_left}へフォールバックする．
        以下のノルム重み付け機構は，$\Ord{h^2}$ 最小閉包を用いた場合の保守的な上界である．
\end{itemize}
