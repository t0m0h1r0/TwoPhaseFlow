...
PPE（離散化＋求解） & CCD 微分形式 + GFM ジャンプ補正 & $\Ord{h^6}$ & — \\
（参考：FVM + BiCGSTAB） & （FVM + 調和平均係数） & （$\Ord{h^2}$） & — \\
...
GFM 界面処理（表面張力） & Young--Laplace ジャンプ直接組み込み & $\Ord{h^2}$（界面近傍†） & — \\
\midrule
\textbf{全体（空間律速：GFM 界面処理†）} & & $\boldsymbol{\Ord{h^2}}$（界面近傍） & — \\
\textbf{全体（時間律速：AB2+IPC 整合）} & & — & $\boldsymbol{\Ord{\Delta t^2}}$ \\
\bottomrule
\end{tabular}

\smallskip
\noindent†\ GFM は CSF の $\Ord{\varepsilon^2}$ 物理誤差床を除去するが，界面直近（1–2格子幅以内）では
Extension PDE なしに $\Ord{h^6}$ 精度を完全に維持することは困難である（§\ref{sec:gfm}，今後の課題）．
平滑領域では全コンポーネント $\Ord{h^5}$ 以上が確保される．
\end{table}
...
\begin{enumerate}
  \item \textbf{空間精度：}
        ...
        GFM により旧 CSF モデルの誤差床 $\Ord{\varepsilon^2} \approx \Ord{h^2}$ は除去されたが，
        界面直近では GFM 処理精度（$\Ord{h^2}$ 以上）が律速段階となる．
        FVM + BiCGSTAB では $\Ord{h^2}$ に留まる（表~\ref{tab:ccd_vs_fvm}）．
