% paper/sections/appendix_e2_ppe_pseudotime.tex
% 付録 E.2：対象 PPE 演算子と仮想時間ステップ反復
% ※ 旧: §8.4（本文）→ 付録 E.2 に移動（Modification IV: ソルバーアルゴリズムを付録へ）
%
\subsection{対象 PPE 演算子と仮想時間ステップ反復}
\label{sec:ppe_pseudotime}

\subsubsection{\texorpdfstring{初期圧力場 $p^0$ の設定}{初期圧力場 p0 の設定}}
\label{sec:ppe_p0_init}
%% 旧所在: §8.1（08_pressure.tex）→ 付録 E.2 へ移動

IPC 法の Predictor は前時刻圧力 $p^n$ を用いるため，$n=0$ における $p^0$ が必要となる．
問題に応じて以下から選択する：
\begin{itemize}
  \item \textbf{静止液滴（検証問題）：} Young--Laplace 解 $p^0 = \sigma\kappa_{lg,0} = \sigma/R$（液相内），$p^0 = 0$（気相）を解析的に与える（$\kappa_{lg,0} = 1/R > 0$）．
  \item \textbf{一般問題（高密度比を含む）：} $n=0$ でのみ経路別 PPE（低・中密度比では一括 PPE，高密度比・低 Weber 数では分相 PPE）を $p^0 = 0$ 初期値で解き，得られた圧力場を $p^0$ として設定する（1回追加求解のコスト）．
  \item \textbf{簡易設定：} $p^0 = 0$ のまま発進する．$\Ord{\Delta t^2}$ 精度は定常後に達成され，初期数ステップでは $\Ord{\Delta t}$ 過渡誤差が生じうるが，多くの実用問題では無視できる．
\end{itemize}

\medskip
本節は，仮想時間（pseudo-time）陰解法と欠陥補正法の導出を整理する補足である．
本稿の標準 PPE 契約は，低次の安定な base solver で補正方程式を解き，
対象演算子の残差を外側で再評価する \texttt{PPESolverDefectCorrection} である．
付録~\ref{app:ccd_lu_direct} の \texttt{PPESolverCCDLU} は CCD Kronecker 演算子の
直接 LU 基準解であり，smooth-RHS 単体検証とデバッグ用途に限定する．
以下の仮想時間・スウィープ型の解析は，欠陥補正の前処理設計と
分割誤差評価を説明するための理論的背景として用いる．

\phantomsection\label{warn:ppe_splitting}
\begin{tcolorbox}[warnbox, title={2 次元求解における分割誤差について}]
...
\textbf{アプローチ 2（明示行列の直接解法）：} 組み立てた連立系を直接解く．
この場合，式~\eqref{eq:pseudo_linear} の左辺をそのまま解くため分割誤差は生じないが，
ブロック Thomas 法の低メモリ・低コスト優位性は失われる．

\textbf{スウィープ型を用いる場合の実用的な解釈：}
スウィープ型を用いる場合，「分割誤差ゼロ」は成立しないが，
...
\begin{equation}
  ...
  \qquad
  q_h \equiv \frac{\mathcal{D}_{\mathrm{path}}\bu^*}{\Delta t}
  \label{eq:PPE_discrete}
\end{equation}
と書く．ここで $\mathcal{D}_{\mathrm{path}}$ は一括 PPE または分相 PPE の
経路別発散，$\mathcal{L}_h$ は対象経路の PPE 演算子である．一括 PPE では
式~\eqref{eq:varrho_product_rule} の product-rule 展開を CCD で離散化した
可変係数演算子となり，分相 PPE では相内 Poisson と界面ジャンプ行を合わせた
演算子となる．
...
\textbf{安定性の利点：}式~\eqref{eq:pseudo_implicit_raw} の陰的離散化は，線形・正定値モデルに対して
CFL 型の上限制約を課さない（A 安定）．
これは $\Delta\tau$ を無制限に大きくしてよいという意味ではなく，スウィープ分割誤差，
条件数，残差フロアを併せて評価しながら式~\eqref{eq:dtau_opt} 近傍を探索する，という選択指針である．
...
スウィープ型前処理を用いる場合は，$x$ 方向 Thomas 法 $\to$ $y$ 方向 Thomas 法を逐次適用する：
...
（分割誤差 $\Ord{\Delta\tau^2}$ が伴うため，残差収束判定と分割誤差評価を併記する；
\ref{warn:ppe_splitting} および表~\ref{tab:ppe_methods} 参照）．
ADI 法は本稿の標準 PPE 経路ではなく，スウィープ型前処理を理解するための
比較モデルである．収束後の採否は，ADI かどうかではなく，
対象演算子 $\mathcal{L}_h$ による外側残差
$\|\mathcal{L}_h\delta p-q_h\|$ で判定する．
...
\noindent\textbf{本稿の標準 PPE 契約は，}\texttt{PPESolverDefectCorrection}\textbf{である．}
本節のスウィープ型導出は，欠陥補正法の前処理設計を説明する解析モデルとして位置づける．
...
収束先は対象演算子 $\mathcal{L}_h$ の離散方程式として評価できる
（表~\ref{tab:ppe_methods} 参照）．
ただし実際の到達精度は $\varepsilon_\mathrm{tol}$，base solver 誤差，境界・界面での係数評価に律速される．
したがって低次 FD を補正方程式の左辺に用いる場合でも，最終判定は
対象演算子による外側残差で行う，というのが欠陥補正法の契約である．
高密度比の一括 smoothed-Heaviside PPE では残差床により停滞し得るため，
これは全密度比での収束保証を意味しない
（付録~\ref{app:dc_convergence_theory}，§\ref{sec:verify_dc_spectral} 参照）．

\textbf{\texttt{PPESolverDefectCorrection} との対応：}
欠陥補正法（式~\eqref{eq:defect_correction_linear}）は
\texttt{PPESolverDefectCorrection} として与えられており（表~\ref{tab:ppe_methods} 5行目），
本稿の標準 PPE ソルバ契約である．
base solver が補正方程式を解き，operator 側が対象離散化の残差
$R_h = q_h-\mathcal{L}_h p^{(m)}$ を評価するため，
契約は特定のスウィープ専用クラスに依存しない．
典型構成では安定な直接解法を base solver とし，CCD/FCCD/FVM の対象演算子を
operator として外側残差で検査する．
スウィープ型を前処理として用いる場合の三重対角係数は
付録 \ref{app:sweep_thomas_coeff} に示すが，その場合は
\ref{warn:ppe_splitting} の分割誤差評価を併記する必要がある．
...
\begin{tabular}{lcccc}
\hline
手法 & 空間離散化次数 & 分割誤差 & 収束判定 & 役割 \\
\hline
FVM 直接解（\texttt{spsolve}） & $\Ord{h^2}$ & なし & 直接解 & 低次基準 \\
ADI + CCD（軸方向分割） & $\Ord{h^6}$ & $\Ord{\Delta\tau^2}$ & 固定スウィープ数 & 比較モデル \\
仮想時間陰解法 + CCD（スウィープ型）& $\Ord{h^6}^{\dagger}$ & $\Ord{\Delta\tau^2}$ & 残差駆動 & 前処理モデル \\
CCD Kronecker + 直接 LU（\texttt{PPESolverCCDLU}） & $\Ord{h^6}$ & なし & 直接解 & smooth-RHS 単体検証 \\
欠陥補正（base + target 残差）$\ddagger$ \textbf{[標準契約]} & 対象演算子依存 & base 依存 & 残差駆動 & 標準契約 \\
\hline
...
\multicolumn{5}{l}{$^{\ddagger}$ 標準契約は \texttt{PPESolverDefectCorrection} の base solver/operator 契約で与える．}\\
\multicolumn{5}{l}{\phantom{$^{\ddagger}$} FD 左辺 + CCD 残差は一構成であり，対象演算子と外側残差で精度を判定する（§\ref{sec:defect_correction}）．}\\
\multicolumn{5}{l}{\phantom{$^{\ddagger}$} スウィープ型を使う場合は \ref{warn:ppe_splitting} の分割誤差を別途評価する．}
\end{tabular}
\end{table}

本稿では\textbf{欠陥補正法}（表~\ref{tab:ppe_methods} 5 行目）を標準 PPE ソルバ契約として用いる：
base solver で補正解を求め，右辺残差に CCD/FCCD/FVM の対象演算子を用いることで，
標準経路の精度判定を外側残差に集約する
（精度条件は §\ref{sec:defect_correction}，スウィープ型前処理の係数導出は付録 \ref{app:sweep_thomas_coeff} 参照）．
...
スウィープ型では線形系は $\Ord{\Delta\tau^2}$ の誤差を残すため，
...
\begin{equation}
  ...
  \label{eq:dtau_opt}
\end{equation}
スウィープ型の理想化モデルでは任意の $\Delta\tau > 0$ で誤差を増幅しない一方，
$\Delta\tau \gg \Delta\tau_{\mathrm{opt}}$ でも $\Delta\tau \ll \Delta\tau_{\mathrm{opt}}$ でも
$\gamma \to 1$ となり収束は遅くなる．
\end{tcolorbox}
...
\end{equation*}
となり，理想化した局所線形モデルでは密度依存性が打ち消される．
この均等化により，気体相・液体相を問わず各セルを同一の「規格化仮想時間刻み」で更新することを狙う．
実際の減衰速度は係数勾配，境界条件，分割誤差にも依存する．
...
残差が十分に減少した後に段階的に増大させると定常解への到達を早める場合がある．
$\Delta\tau_{i,j}$ が陰的 Euler（式~\eqref{eq:pseudo_implicit_raw}）に基づく限り，
線形モデル上の安定性は保たれやすいが，$C_\tau$ を大きくすると分割誤差フロアや
係数非一様性による停滞が生じ得る（\ref{warn:ppe_splitting} 参照）．

\subsubsection{全周 Neumann 境界の可解性条件とゲージ固定点の対称性}
