...
変密度 PPE の導出（§~\ref{sec:projection_derivation}）は §~\ref{sec:collocate}章で行った．
...
HFE による界面越し場延長（$\Ord{h^6}$）は付録~D（§~\ref{sec:field_extension}）を参照．
...
\begin{enumerate}
  \item \textbf{空間精度：}
        ...
        HFE（付録~D，§~\ref{sec:extension_pde_disc}）は $\Ord{h^6}$ を達成しており，
