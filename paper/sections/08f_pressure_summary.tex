...
PPE（離散化＋求解） & CCD 微分形式 + 欠陥補正法 + HFE & $\Ord{h^6}$ & — \\
...
HFE & 最近接点 Hermite 補間（CCD データ） & $\Ord{h^6}$ & — \\
\midrule
...
全体の空間精度を律速する．HFE は CCD Hermite 補間（§~\ref{sec:extension_pde_disc}）
...
\begin{enumerate}
  \item \textbf{空間精度：}
        ...
        HFE は最近接点 Hermite 補間（§~\ref{sec:extension_pde_disc}）により $\Ord{h^6}$ を達成し，
