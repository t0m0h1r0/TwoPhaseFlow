...
\label{sec:grid_convergence}% backward-compat: rename to HFE in final draft (referenced from §4, curvature convergence → now in §10.1)
...
DCCD 非侵襲性 & $E_k^{\text{CCD}} \approx E_k^{\text{DCCD}}$ & 相対差 & $< 10^{-3}$\\
静止液滴安定性 & 寄生流れ有界 & 200 ステップ & 成長比 $< 5$\\
HFE 適合性 & smoothed $p$: HFE 不要 & ON/OFF 比較 & 適用範囲の特定\\
Galilean 不変性 & $\|\bu - \bu_0\|_\infty = 0$ & 1 周期後 & $< 10^{-10}$\\
PPE 破綻限界 & DC 発散の閾値 & 残差履歴 & 閾値の特定\\
分相 PPE 精度回復 & 密度比非依存 & 格子収束 & DC $k{=}3$: $\Ord{h^7}$（Dirichlet）確認\\
\bottomrule
\end{tabular}
\end{table}
