...
\midrule
静水圧バランス & $p = \rho g z$，$\bu = \bm{0}$ & $\|\bu\|_\infty$ & $< 10^{-5}$\textsuperscript{\dag} & §\ref{sec:force_balance}\\
...
\end{tabular}
\\[2pt]
{\footnotesize \textsuperscript{\dag}$N{=}32$ は CCD 壁面境界スキーム（$\Ord{h^5}$）の影響で
$\Ord{10^{-6}}$ に留まる；$N{\geq}64$ では $\Ord{10^{-10}}$ 以下（§~\ref{sec:force_balance}）．}
\end{table}
