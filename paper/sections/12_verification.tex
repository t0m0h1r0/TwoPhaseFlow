...
\bottomrule
\multicolumn{5}{l}{\footnotesize \textsuperscript{$\dagger$}$N{=}32$ は CCD 壁面境界スキーム（$\Ord{h^5}$）の影響で $\Ord{10^{-5}}$ に留まる；$N{\geq}64$ では $\Ord{10^{-10}}$ 以下（§~\ref{sec:force_balance}）．} \\
\multicolumn{5}{l}{\footnotesize \textsuperscript{$\ddagger$}FFT 直接法 PPE（単相・周期境界）での基準．二相流では CSF ソース項・変密度 PPE の打切り誤差により $\|\bnabla\cdot\bu\|_\infty \sim \Ord{10^{-2}}$ となる場合がある（§~\ref{sec:advection_pressure_coupling}）．} \\
\multicolumn{5}{l}{\footnotesize \textsuperscript{$\S$}第~\ref{sec:numerical_integrity}章のコンポーネント検証（§~\ref{sec:verify_split_ppe_dc}）で実施．§~\ref{sec:interface_crossing} の診断結果を受けた解決策として本章ロードマップに含める．} \\
\end{tabular}
\end{table}
