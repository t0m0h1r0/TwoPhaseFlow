...
OUT = pathlib.Path(__file__).resolve().parent / "results" / "dccd_dissipation"
...
def main():
    ...
    np.savez(OUT / "dissipation_data.npz",
             xi=xi, H_results=H_results, checker=checker_results, advection=adv_results)
...
if __name__ == "__main__":
    ...
    if _args.plot_only:
        _d = np.load(OUT / "dissipation_data.npz", allow_pickle=True)
        plot_all(_d["xi"], _d["H_results"].item(), list(_d["checker"]), _d["advection"].item())
    else:
        main()
