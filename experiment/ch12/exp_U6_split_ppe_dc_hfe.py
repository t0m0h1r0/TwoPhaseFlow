...
Paper ref: §12.5 (sec:U6_split_ppe_dc_hfe); 12e_interface_field.tex.
...
---------
  (a) DC iteration / stall study with the lumped (phase_density) FCCD-PPE.
      ...
      runs to completion without hitting tolerance ("stall"). Paper §12e:
  ...
  (b) Lumped MMS at N=64, ρ_l/ρ_g ∈ {1, 10, 100, 1000}.
      ...
      measured inside the liquid bulk (φ > 3h). Paper §12e Tab.5: 一括 DC
  ...
  (c) HFE 1-D + 2-D extension. 1-D: _hermite5_xp polynomial extrapolated one
      ...
      reported on the 6-cell extension band. Paper §12e: 1-D ~7, 2-D
...
N_CONV_SIZES = [32, 64, 128]  # per-N study at ρ=1000 (paper Tab.6)
RHO_CONV = 1000.0
R_CIRCLE = 0.25
CENTER = (0.5, 0.5)
PI = float(np.pi)
...
def _u6a_run_one(N: int, rho_l: float, omega: float, k_dc: int,
    ...
    # high-ρ struggles (matches paper §12e's "stall for all ω at ρ ≥ 5"
...
def make_figures(results: dict) -> None:
    ...
    save_figure(fig, OUT / "U6_split_ppe_dc_hfe")
...
def print_summary(results: dict) -> None:
    a = results["U6a"]
    print("U6-a Lumped DC stall study (paper §12e: ρ_l/ρ_g ≥ 5 stalls all ω):")
    ...
    b = results["U6b"]
    print(f"U6-b Lumped + DC k=3 MMS (paper Tab.5 一括 DC: "
    ...
    print(f"  N-sweep slope (paper Tab.6 negative @ N≥64): "
          f"{n_slope:.2f}  {flag}")

    rows_1d = results["U6c"]["hfe_1d"]
    rows_2d = results["U6c"]["hfe_2d"]
    print("U6-c HFE convergence (paper: 1D ≈ 7.0; 2D ≈ 6.33 / 5.79):")
