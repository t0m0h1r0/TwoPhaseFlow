...
| 2026-04-24 | Post-merge rigorous review (CHK-191): 3 並列 Explore agents が 21 件修正点を特定 → R-1..R-8 で全解消. **R-1** SP-B/SP-J `\cite{}` in §2 tcolorboxes; **R-2** 13 subsection 見出しを `\texorpdfstring` でラップ (hyperref PDF bookmark 保護, 11 warnings → 0); **R-3** §5 Level 3 ↔ §10.5 物語整合性 (forward-ref + footnote); **R-4** raw `\xi_\text{idx}` → `\xiidx` 置換 (§3.4); **R-5** 孤立 fig/tab ラベル 3 件 (`fig:eikonal_violation`, `fig:grid_transform`, `tab:reinit_comparison_chk138`) を本文から `\ref` 追加; **R-6** Overfull (10_3:76-91, 36.7pt) + Underfull (09_6:172-173) hbox 解消; **R-7** 6 孤立 bib を本文 `\cite` 化 (Zalesak1979, Chandrasekhar1961, JiangPeng2000, Fedkiw1999, Sussman1999, VanderVorst1992); **R-8** `\mathrm{BF}_\text{res}` → `\BF_\text{res}` 置換 (math mode のみ). **品質指標**: 246 pp unchanged, Token not allowed 11→0, Overfull 1→0, Underfull 1→0, undefined 0→0, multiply-defined 0→0, SP coverage 14/15 (SPO 自己参照). 24 .tex files modified, ~50 LOC net diff, 0 regressions. **出版品質 B → A**. L-3 (269 件孤立 label 棚卸し) は CHK-192 キュー化 (軽微, 別セッション). | 1a045ef |
| 2026-04-24 | Narrative grade A→A+ rewrite (CHK-194): §1-§10 + 付録 A-H 第2パス厳正再レビュー. **Phase A** WIKI-T/E/L/X + CHK-xxx 内部 ID 全廃 (body/section title/algbox title/table caption/bullet から 80 件 removed; high-density 6 files: 07b 31件/03d 11件/04c 7件/06c 6件/04e 5件/06d 5件; medium 6 files + low 14 files; 本文 H-01 診断 → `\S\ref{sec:fccd_motivation_h01}` / CHK-xxx → `SP-J \S N \cite{SPJ}` 形の学術 cross-ref に無害化; grep WIKI-[TELX]-[0-9]+|CHK-[0-9]+ on paper/sections/ + main.tex = **0 件**). **Phase B** 付録 sub-file header 注釈を A-H 実態 (A=interface/B=ccd/C=advection/D=pressure/E=verification/F=nondim/G=bootstrap/H=gpu_fvm) に整合 (8 files: appendix_numerics_solver_s5/s4/s1, appD_dc_ppe_iteration, appD_predictor_adi, appendix_ppe_pseudotime, appendix_ccd_impl_s3, 02c_nondim_curvature). **Phase D** CHK-193 post-processing: (D-1) 付録 H に 3 alias label 付与 (`app:gpu_fvm_ops` / `app:gpu_fvm_pcr` / `app:gpu_fvm_d2h`) + §9f 橋渡し段落を演算子 calculus / 可変バッチ PCR / D2H 規律の 3 granular refs に差し替え; (D-2) 付録 H 末尾に §8.5 圧力フィルタ禁止論 (P-1/P-2) への reverse link 段落追加. **Phase E** `\phantomsection` 第2パス監査 — 16 箇所すべて live ref 1-11 件保有を確認 (zero-ref なし, CHK-193 徹底済み保険確認成功). **Phase F** 検証 — clean xelatex build 247 pp 維持, 0 Token/Overfull/Underfull/Undefined/Multiply warnings. **退避アーカイブ**: `paper/archive/v1_pre_sp_core/snippets/wiki_chk_references_snapshot.md` — 除去前の 80 件 grep 出力を `{file}:{line}: {context}` 形式で保存, 将来「どの文章が何番と紐付いていたか」の検索可能性を確保. **品質指標**: 内部 ID 露出 0 件で SP-I/SP-J 投稿稿下準備完了, 付録 letter 整合性 8 箇所 fix で履歴追跡者の混乱除去, CHK-193 narrative loop forward/reverse ref 両方向完備, archive 除去前情報は git 履歴 + snapshot 二重化で永続保存. 32 .tex files modified, 1 snapshot 新規, 0 regressions. **narrative grade A → A+** (外部公開 ready). | 3404747 + af88b3f + 6a4f840 + fa145cb |
| 2026-04-24 | Narrative grade B→A rewrite (CHK-193): 3 並列 Explore agents が §1-§10 の 9 カテゴリ narrative 問題を特定 → 7 Phase で全退避・圧縮. **Phase A+B** archive 基盤整備 + 3 旧ファイル退避 (`04d_dissipative_ccd.tex` 314 LOC CHK-185 吸収済 / `13_legacy_benchmarks.tex` 7 LOC stub / `appendix_grid_impl.tex` zero-ref) → `paper/archive/v1_pre_sp_core/` へ `git mv`. **Phase C** §9.6 GPU-Native FVM (220 LOC) を付録 H へ昇格 — PPE 数学定式化と GPU 実装詳細を narrative 層で分離 (`09_6_gpu_native_fvm.tex` → `appendix_gpu_fvm.tex`, top label `sec:gpu_native_fvm_projection` → `app:gpu_fvm`, subsection 階層 1 段昇格, §9f に橋渡し段落). **Phase D** 完全重複 3 件の cross-ref 化 (保守的): (D-1) §8.4.1 H-01 再掲 25 LOC → 3 行 + phantomsection 保持 (外部参照 4 箇所); (D-2) §7c FCCD 行列インライン再掲 (L13/L152) → `eq:fccd_composite_matrix` 参照; (D-3) §8b 圧力フィルタ warnbox 37 LOC → 4 行 + phantomsection 保持 (外部参照 3 箇所). **Phase E** 陳腐化記述修正: (E-1) §7 冒頭「今後の課題」→ §7.3/§8.4/§7.6 forward-ref; (E-2) §5 Level 3 タイトル 3 箇所を「\textbf{時間積分}のみ設計フック」限定で統一; (E-3) §5 内部チケット「別 CHK にキュー化」削除 (管理は ledger 側). **Phase F** Zero-ref phantomsection 6 件削除: `sec:tvd_rk3`, `eq:Heps_def_preview`, `eq:gfm_jump`, `eq:extension_pde`, `eq:gfm_rhs_correction`, `eq:gfm_ccd_div` (grep 0 件確認済). **退避アーカイブ**: `paper/archive/v1_pre_sp_core/` に README + 3 旧 .tex + 4 snippets (`h01_recap_08_2.tex`, `fccd_matrix_redef_07c.tex`, `pressure_filter_warnbox_08b.tex`, `level3_activation_note.tex`) を保存, `%!TEX root` ヘッダ付きで再 `\input` による復活可能. **品質指標**: 247 pp (246 + 1 §9f 橋渡し段落), 0 undefined/multiply/token/overfull/underfull, phantomsection 件数 16 → 10, single-source-of-truth 強化 (H-01 → §4.5 / FCCD 行列 → §4e / 圧力フィルタ禁止 → §8c). 11 .tex files modified, 3 git mv, 4 snippets 新規, 0 regressions. **narrative grade B → A**. | a00c09e + 191f65a + 49b030f |
