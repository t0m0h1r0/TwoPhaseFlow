...
| branch | main |
| last_decision | CHK-079 CLOSED 2026-03-31: PaperCorrector full-narrative sweep — 23 audit issues resolved across 5 files. (1) 00_abstract: table RC→DCCD, CSF→GFM, accuracy row updated; abstract text BF/RC→GFM+DCCD. (2) 13_conclusion: summary table RC→DCCD, §7 description RC/BF→GFM+DCCD, §2 description CSF→GFM, accuracy balance CSF律速→GFM除去. (3) 07_collocate: §7.1 line 28 RC→DCCD note; §7.4 CSF-era mybox framing added. (4) 09_full_algorithm: operator map RC→DCCD; fig caption CSF body force→GFM PPE; Step 3 note RC→GFM; Predictor Steps 5 x/y: removed κ∇ψ/We; Step 6 PPE RHS: RC→DCCD-filtered+b_GFM; Step 6.1 algbox: RC face vel→DCCD+GFM; Step 7 RC divergence→CCD divergence; input RC→GFM. (5) 12_benchmarks §11.1: title/purpose/resultbox BF/CSF→GFM. 165pp, 0 errors, 0 undefined refs. |
