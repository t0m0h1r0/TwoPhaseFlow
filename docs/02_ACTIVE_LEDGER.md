...
| last_CHK | CHK-159 DONE 2026-04-21 — Ridge-Eikonal hybrid 非一様格子拡張 理論確立 + ライブラリ実装. **新規 SP-E** ([docs/memo/short_paper/SP-E_ridge_eikonal_nonuniform_grid.md](memo/short_paper/SP-E_ridge_eikonal_nonuniform_grid.md), 16 節): SP-B Gaussian-ξ ridge + FMM Eikonal hybrid を非一様 interface-fitted 格子に拡張. 四 derivation: **D1** σ_eff(x)=σ_0·h(x)/h_ref (resolution-consistency lemma; SP-B §3 Gaussian PDE は物理空間ネイティブ, パラメータ解釈のみ変更), **D2** 物理空間 Hessian via chain rule $\nabla^2_x = J^T\nabla^2_\xi J + (\nabla_\xi J^T)\nabla_\xi$ — **Theorem 2** (ξ-space Hessian 禁止, WIKI-T-039 不可能定理継承): Approach A (Direct Non-Uniform CCD, paper-exact) は CHK-160 deferred, **Approach B** (2nd-order physical-space FD, O(h²)) を CHK-159 pragmatic fallback として採用 (sign-flip error は Morse-degenerate 点の O(h) 近傍のみ, 標準ベンチマーク外), **D3** 非一様 FMM Eikonal 二次方程式 $(d-a_x)^2/h_x^2 + (d-a_y)^2/h_y^2 = 1$ + 閉形式解 + caustic fallback $d=\min(a_x+h_x, a_y+h_y)$ + 物理座標 seeding $d_\text{seed}=|\phi_i|\cdot h_\text{fwd}/(|\phi_i|+|\phi_{i+1}|)$, **D4** ε_local(x)=ε_scale·ε_ξ·h(x) (CHK-138/139 calibrated ε_scale=1.4 保持 + WIKI-T-038 bandwidth 制約と両立). 新規 **[WIKI-T-057](wiki/theory/WIKI-T-057.md)** (σ_eff + ε_local spatial scaling), **[WIKI-T-058](wiki/theory/WIKI-T-058.md)** (physical-space Hessian + Theorem 2 + Approach A), **[WIKI-T-059](wiki/theory/WIKI-T-059.md)** (non-uniform FMM quadratic), **[WIKI-L-025](wiki/code/WIKI-L-025.md)** (library). ライブラリ: [src/twophase/levelset/ridge_eikonal.py](../src/twophase/levelset/ridge_eikonal.py) `NonUniformFMM` + `RidgeExtractor` + `RidgeEikonalReinitializer` (499 LOC, 2 `@_fuse` kernels: `_sigma_eff_kernel` / `_eps_local_kernel` + plain xp-parameterised `_sigmoid_xp`, Hessian FD に `# CHK-160: upgrade to Approach A` marker). Config: `reinit_method: str = "split"` (default, bit-exact), `ridge_sigma_0: float = 3.0` 追加 + validator; 'ridge_eikonal' branch を `Reinitializer` facade に登録 ([src/twophase/levelset/reinitialize.py](../src/twophase/levelset/reinitialize.py)). Builder は `method=` + `sigma_0=` forward ([src/twophase/simulation/builder.py:189](../src/twophase/simulation/builder.py#L189)) — **FCCDConvectionTerm 自動注入 (CHK-158) と共存** (cherry-pick conflict resolved, 両変更維持). 検証 V1-V6 all PASS: 2-disk ridge topology (V1), σ_eff convergence on α=2 (V2), FMM residual p99<0.35 mean<0.1 parameterised α=1/2/3 (V3), volume conservation single-step α=1/2 (V4), CPU/GPU parity rtol 1e-12 ridge kernels (V5), backward-compat default→'split' bit-exact (V6). 実験 probe: [experiment/ch13/config/ch13_04_capwave_ridge_alpha2.yaml](../experiment/ch13/config/ch13_04_capwave_ridge_alpha2.yaml) (α=2 stretched, 水-空気 T=8). テスト: 14 passed ridge-specific, 225 passed full suite (regression-free). **Topology-freedom / metric-rigidity 分離 (WIKI-X-019)** を非一様格子上でも保持. INDEX 164→168 (Theory 56→59, Code 24→25). Approach A Hessian upgrade は CHK-160 deferred; V4 capillary-wave α=2 end-to-end 実機実行は follow-on. Branch: worktree-fccd-advection-theory-lib |
| next_action | [NEXT] V4 capillary-wave α=2 end-to-end 実行 (`make cycle EXP=experiment/ch13/run.py ARGS='ch13_04_capwave_ridge_alpha2'`) で |ΔV/V|<5% 確認; >5% なら CHK-160 で adaptive ε_scale(x)=1.4·h_ref/h_min fix. 並行して CHK-160 候補: (a) **Approach A** Direct Non-Uniform CCD `DirectNonUniformCCDSolver` ($\theta$-weighted block system, WIKI-T-058 §5) で ξ-space Hessian 不可能定理回避 + paper-exact Hessian, (b) 3D 検証, (c) FCCD-curvature (face-centred 4th-order κ), (d) FCCD-reinitialization 統合. V11 BF residual on WIKI-E-030 は依然 later CHK. |
...
- Wiki: **168 entries** (docs/wiki/INDEX.md). T-057/T-058/T-059 + L-025 added 2026-04-21 (CHK-159); T-055/T-056 + L-024 added 2026-04-21 (CHK-158); T-054 added 2026-04-21 (CHK-156); T-053 added 2026-04-20 (CHK-155); T-050/T-051/T-052 + L-023 added 2026-04-20 (CHK-154). SP-E short paper added 2026-04-21 (CHK-159); SP-D added 2026-04-21 (CHK-158); SP-C added 2026-04-21 (CHK-156).
