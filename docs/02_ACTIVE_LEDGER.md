...
| branch | worktree-researcharchitect-fccd-equations |
| last_CHK | CHK-155 DONE 2026-04-20 — ResearchArchitect FCCD 計算方程式レビュー: 論文 CCD 導出は $(u',u'')$ 連立であり $u'''$ は未知数ではないため, [WIKI-T-053](wiki/theory/WIKI-T-053.md) を新規作成し, FCCD 補正を $q_i=(D_{\mathrm{CCD}}^{(2)}u)_i$ の face 差分で閉じる実装可能式に整理. Uniform: $D^{FCCD}u_f=(u_i-u_{i-1})/H-H(q_i-q_{i-1})/24$. Nonuniform: $D^{FCCD,nu}u_f=(u_i-u_{i-1})/H-\mu H[\theta q_{i-1}+(1-\theta)q_i]-\lambda H(q_i-q_{i-1})$. [WIKI-T-046](wiki/theory/WIKI-T-046.md) と [WIKI-T-050](wiki/theory/WIKI-T-050.md) に WIKI-T-053 cross-link 追加; [WIKI-T-001](wiki/theory/WIKI-T-001.md) の CCD 要約式の $2h$ 正規化を論文・実装と整合修正. INDEX 159→160. コード変更ゼロ. Branch: worktree-researcharchitect-fccd-equations |
| next_action | [NEXT] CHK-156: R-1.5 Phase 1 実装 ([WIKI-L-023](wiki/code/WIKI-L-023.md)) と並行して, R-1 FCCD PoC-1 を WIKI-T-053 の $q=D^{(2)}_{\mathrm{CCD}}u$ closure で実装検証 (uniform $O(h^4)$, stretched-grid $O(h^3)$, BF residual 比較). |
...
- Wiki: **160 entries** (docs/wiki/INDEX.md). T-053 added 2026-04-20 (CHK-155); T-050/T-051/T-052 + L-023 added 2026-04-20 (CHK-154).
