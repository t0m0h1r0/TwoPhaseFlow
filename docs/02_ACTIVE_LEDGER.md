...
| next_action | worktree-perf+cupy-gpu-efficiency MERGED to main 2026-04-12 with 6 rounds of CuPy GPU perf tuning. Authoritative numbers from CHK-120: exp11_18 Test B 900 s → 41.25 s (−95.4%, 21.8× vs CHK-106). Caveat: CHK-115 through CHK-119 originally published numbers were inflated 30-40% due to parallel-worktree rsync race (now mitigated with --checksum in remote.sh; fundamental per-worktree REMOTE_DIR fix deferred). Awaiting next instruction. Known out-of-scope bugs: FieldExtender upwind nan (CPU+GPU). |
