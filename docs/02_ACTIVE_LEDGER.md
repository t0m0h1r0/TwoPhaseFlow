...
| last_CHK | CHK-158 DONE 2026-04-21 — FCCD 移流項 理論確立 + ライブラリ実装. **新規 SP-D** ([docs/memo/short_paper/SP-D_fccd_advection.md](memo/short_paper/SP-D_fccd_advection.md), 16 節): CCD-closure FCCD を対流項/ψ 移流に拡張. **Option B** (保存型 face-flux divergence, BF-preservation theorem 証明: $\mathbf{u}=0$ で恒等的に残差=0 ⇒ H-01 残り半分解消), **Option C** (Hermite R_4 reconstructor で node-centred 出力, AB2 バッファ互換, $c=-1/16$), 新primitive $\mathbf{P}_f=\mathbf{P}_1-\mathbf{P}_2\mathbf{S}_{\mathrm{CCD}}$ ($H^2/16$ 係数), 壁 **Option IV** (Dirichlet $u$ odd-mirror $u_{-1}=-u_1$ + IV-clean face-zero). 新規 **[WIKI-T-055](wiki/theory/WIKI-T-055.md)** (FCCD advection operator), **[WIKI-T-056](wiki/theory/WIKI-T-056.md)** (Option IV wall), **[WIKI-L-024](wiki/code/WIKI-L-024.md)** (library). ライブラリ: [src/twophase/ccd/fccd.py](../src/twophase/ccd/fccd.py) `FCCDSolver` (face_gradient/face_value/node_gradient/face_divergence/advection_rhs, 3 @_fuse kernels, CCD LU 共有 ≈1.1× overhead), [src/twophase/ns_terms/fccd_convection.py](../src/twophase/ns_terms/fccd_convection.py) `FCCDConvectionTerm`, [src/twophase/levelset/fccd_advection.py](../src/twophase/levelset/fccd_advection.py) `FCCDLevelSetAdvection`. Config: `convection_scheme` + `advection_scheme` 両フラグに `fccd_nodal|fccd_flux` 追加, デフォルト `ccd|dissipative_ccd` 維持 (既存 242 tests PASS). [SimulationBuilder](../src/twophase/simulation/builder.py) で単一 `FCCDSolver` を momentum + ψ 間で共有. GPU/CPU `backend.xp` 統一, V7 bit-close (rtol 1e-12). INDEX 161→164. V11 (BF residual on WIKI-E-030) は後続 R-1 PoC CHK に deferred. Branch: worktree-fccd-advection-theory-lib |
| next_action | [NEXT] 界面処理 (interface handling) 新方式の理論整理 + ライブラリ実装. 対象候補: (a) FCCD-curvature (face-centred 4th-order κ 評価), (b) FCCD-reinitialization, (c) FCCD-field-extension, (d) 新規な界面表現. ユーザ確認後にスコープ確定; 既存可換性 + GPU/CPU 統一 + SP + wiki 方針. 並行して V11 BF residual end-to-end は WIKI-E-030 setup で later CHK. |
...
- Wiki: **164 entries** (docs/wiki/INDEX.md). T-055/T-056 + L-024 added 2026-04-21 (CHK-158); T-054 added 2026-04-21 (CHK-156); T-053 added 2026-04-20 (CHK-155); T-050/T-051/T-052 + L-023 added 2026-04-20 (CHK-154). SP-D short paper added 2026-04-21 (CHK-158); SP-C added 2026-04-21 (CHK-156).
