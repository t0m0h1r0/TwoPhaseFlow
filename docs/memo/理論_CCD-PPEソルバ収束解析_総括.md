...
### 2.5 exp10_18 実験結果（2026-04-05）

ω vs 密度比 収束マップ（tol=1e-8 L2ノルム，maxiter=150，stagnation検出付き）：

| N | ρ_l/ρ_g | ω=0.30 | ω=0.50 | ω=0.70 | ω=0.83 | 観察 |
|---|---------|--------|--------|--------|--------|------|
| 32 | 1 | 72✓ | 38✓ | 22✓ | 109~ | 理論と一致 |
| 64 | 1 | 72✓ | 37✓ | 22✓ | 140~ | 理論と一致 |
| 64 | 2 | 74✓ | 39✓ | 23✓ | 142~ | 薄い界面で収束 |
| 32 | ≥2 | ~ | ~ | ~ | ~ | 界面プラトーで停滞 |
| 64 | ≥5 | ~ | ~ | ~ | ~ | 界面プラトーで停滞 |

**ω=0.1**: 均一密度でも収滞（滑らかモード収束率 0.9/反復 → ~230 回必要，tol 前に CCD 離散化誤差床 ~3×10⁻⁸ に到達）。

**停滞の原因（可変密度）**: 界面近傍で CCD（O(h⁶)）と FD（O(h²)）の ∇ρ 離散化が異なるため，残差床 ∝ (ρ_l/ρ_g − 1) × O(h²) が生じる。N=64 では界面幅 3h が細くなり床が低下 → ρ=2 で収束。

**実用的 ω 選択**: ω ∈ {0.3, 0.5, 0.7} が最適（均一密度で確実収束，可変密度では低密度比かつ高解像度で収束）。
