...
**date**: 2026-04-19  
**status**: VERIFIED (b83837r0w 確認済み 2026-04-20)  
...
**実験 b83837r0w**（G^adj 修正適用後、ch13_02_waterair_bubble、α=1.5、64×128）:

| 指標 | 修正前（G_CCD） | 修正後（G^adj） |
|------|--------------|--------------|
| ブローアップ発生 | ステップ 51、t≈0.023 | ステップ 28,122、t≈12.60 |
| KE @ t=0.023 | 1.141 × 10⁶ | 1.41 × 10⁻² ✓ |
| KE @ t=10 | — | 1.29 × 10⁻¹（正常） |
| 延命倍率 | — | **×550**（51 → 28,122 ステップ） |
| 体積保存 | — | 4 × 10⁻¹⁵（機械精度） |

**考察**：G^adj 修正により当初の FVM-CCD メトリクス不整合起因のブローアップは完全に解消された。
t≈12.6 で発生する後期ブローアップは別機構（G^adj 修正の対象外）であり、今後の調査課題。
気泡重心は t=12.6 でも yc=0.54（LY=2.0 のドメイン内、初期位置 0.5 から 0.04 しか上昇していない）。
