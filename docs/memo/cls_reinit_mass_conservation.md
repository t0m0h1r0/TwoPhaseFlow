...
L₂ 収束次数は ~0.4 と低い。これは **渦の引き延ばしで CLS 場のフィラメントが
格子幅以下に薄くなる** ためであり、質量補正では解決できない解像度の限界。
N=512 でも L∞=0.70 が示すように、局所的な誤差は大きいまま。

### 偶然の誤差相殺の発見

N=128 Zalesak での累積質量変化:
- 移流: **-38.48** (DCCD フィルタ + clip が質量を失う)
- 再初期化 (補正なし): **+21.57** (偶然、質量を追加していた)
- 差し引き: -16.91 → mass_err = 1.08e-3 (見かけ上良好)

reinit のみに補正を入れると +21.57 の「恩恵」が消え mass_err = 2.36e-3 に悪化。
移流にも補正を入れて初めて機械精度に到達。

**教訓:** 誤差の偶然相殺は偽りの精度を与える。移流・再初期化の両方を個別に
質量保存する設計が正しい。
