...
## 7. 幾何量フィルターの実装（追記: 2026-04-04）

§2–§4 の保存則ベースの議論に加え、**界面の幾何量（法線ベクトル n および曲率 κ）に直接作用するフィルター** を実装した。これらは保存則ではなく「ノイズ源の制御」を目的とする。

### 7.1 重要な設計原則：h² スケーリング

CCDは物理的微分（単位: 1/L）を計算するため、ラプラシアン $\nabla^2 q$ は $O(q/h^2)$ のオーダーとなる。フィルター強度を格子不変（メッシュ独立）にするには、必ず $h^2$ スケーリングが必要：

$$q^* = q + C h^2 \, w(\psi) \, \nabla^2 q$$

ここで $h$ は最小格子幅、$C$ は無次元パラメータである。**符号は必ず正（拡散）**。負号は逆拡散（反拡散）となり高波数を増幅する。

**安定条件（2次元）:** $C < 1/8 = 0.125$。推奨値: $C = 0.03 \sim 0.08$。

### 7.2 法線ベクトル拡散フィルター（優先度1）

$$\mathbf{n}^* = \mathbf{n} + C h^2 \nabla \cdot (|\nabla\phi| \, \nabla \mathbf{n})$$

- **重み** $w = |\nabla\phi|$: SDF で $\approx 1$（$O(1)$、格子不変）
- **界面制限マスク:** $\delta_\varepsilon(\phi) > f_{\rm thr} \cdot \max(\delta_\varepsilon)$
- **CDD コスト:** 各成分で $2 \times \mathrm{ndim}$ 回（2次元で8回/成分）
- **再正規化必須:** フィルタ後 $\mathbf{n}^* \leftarrow \mathbf{n}^*/|\mathbf{n}^*|$

実装: `src/twophase/levelset/normal_filter.py::NormalVectorFilter`

### 7.3 界面限定高波数除去フィルター（優先度2）

$$\kappa^* = \kappa + C h^2 \, w(\psi) \, \nabla^2 \kappa$$

- **重み** $w(\psi) = 4\psi(1-\psi)$: $O(1)$（$\delta_\varepsilon$ と異なり $h$ 依存性なし）
  - $\delta_\varepsilon(\phi) = O(1/h)$ を使うと $h^2 \cdot \delta_\varepsilon \cdot \nabla^2\kappa \sim q/h$（格子依存）
  - $4\psi(1-\psi) = O(1)$ なので $h^2 \cdot w \cdot \nabla^2\kappa \sim O(q)$（格子不変）✓
- **CCD コスト:** `differentiate(κ, ax)` は $(d_1, d_2)$ を同時返却。$d_2$ を事前計算で渡せばCCDコスト0。

実装: `src/twophase/levelset/curvature_filter.py::InterfaceLimitedFilter`

### 7.4 CurvatureCalculator への統合
...
curv = CurvatureCalculator(backend, ccd, eps,
                            normal_filter=nf, kappa_filter=hfe)
```

パイプライン順序: φ→∇φ(CCD)→n→**[nフィルタ]**→κ=−∇·n*→**[κフィルタ]**→CSF

**Keywords:** Compact Finite Difference, Low-pass Filter, Conservation Laws, Multiphase Flow, Implicit Filtering, Normal Vector Diffusion, Interface Curvature Filtering