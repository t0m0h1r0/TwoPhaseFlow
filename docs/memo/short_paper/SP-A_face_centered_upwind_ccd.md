...
Compact finite-difference schemes provide high-order accuracy with small stencils. Among them, the Combined Compact Difference (CCD) method of Chu and Fan (1998) achieves higher-order accuracy by combining multiple compact derivative relations without enlarging the stencil or sacrificing upwind structure. In this short paper we present a **face-centered, upwind-limited reformulation of CCD (FCCD)** that serves as a **common face-jet primitive** $\mathcal{J}_f(u) = (u_f, u'_f, u''_f)$ on a shared face locus for four co-located operators of an interface-resolved two-phase solver: (R1) HFE upwind advective transport, (R2) GFM ghost-pressure jet conditions, (R3) a phase-separated PPE elliptic operator $D_h^{bf} = -(G_h^{bf})^*$, and (R4) the variable-coefficient viscous stress-divergence interface band. Fourth-order spatial accuracy is obtained by introducing higher derivatives as auxiliary unknowns and combining them algebraically to cancel leading truncation-error terms. A minimal formulation is derived using only immediate upwind cell values, avoiding stencil enlargement while preserving strict upwind causality. Historically, the FCCD concept was first proposed as a candidate remediation for the FVM–CCD metric inconsistency (hypothesis H-01, [WIKI-E-030](../../wiki/experiment/WIKI-E-030.md)); under the pure-FCCD design of the present paper, the FVM auxiliary operator $\mathcal{G}^{\text{adj}}$ is retired entirely and the four roles above replace that discussion as the primary motivation.
...
The primary motivation is operator unification. An interface-resolved two-phase solver contains four distinct face-locus operators that, in conventional designs, use independently chosen discretisations:

1. **(R1) HFE upwind advection** — one-sided face values $u_f$ for monotone transport of $\psi$ (and, on cubic-logit coordinates, $\phi$).
2. **(R2) GFM ghost-pressure jet conditions** — face-averaged $\llbracket p \rrbracket$ and $\llbracket \partial_n p \rrbracket$ rows enforcing interface jumps.
3. **(R3) Phase-separated PPE** — the compatible elliptic operator $D_h^{bf} = -(G_h^{bf})^*$, with $G_h^{bf}$ the face-locus gradient used in the momentum corrector.
4. **(R4) Viscous stress-divergence interface band** — face-locus $\nabla \cdot (\mu [\nabla \bm u + (\nabla \bm u)^{\!\top}])$ entries within the three-layer Hermite band around the interface.

FCCD provides the **common face-jet primitive** $\mathcal{J}_f(u) = (u_f, u'_f, u''_f)$ at $\mathcal{O}(\Delta x^4)$ that supplies all four operators from the same two-cell upwind footprint. This "one primitive, four operators" contract is the main design statement of this short paper.

*Historical note.* The FCCD concept was first proposed as a candidate remediation for the FVM–CCD metric inconsistency ([WIKI-T-044](../../wiki/theory/WIKI-T-044.md)) that drove the late-time blow-up documented in [WIKI-E-030](../../wiki/experiment/WIKI-E-030.md) and [WIKI-T-045](../../wiki/theory/WIKI-T-045.md) (hypothesis H-01). Under the pure-FCCD design of the present paper, the FVM pressure-gradient auxiliary $\mathcal{G}^{\text{adj}}$ is retired entirely, so the metric inconsistency cannot arise structurally. H-01 is preserved here as a historical case study; the present motivation is carried by the (R1)–(R4) common-primitive contract above.
